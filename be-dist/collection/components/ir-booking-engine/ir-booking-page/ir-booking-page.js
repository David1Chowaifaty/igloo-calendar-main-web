...
export class IrBookingPage {
    ...
    render() {
        ...
        return (h(Host, null, h("div", { class: "space-y-5 " }, !isInjected && (h("div", { ref: el => (this.propertyGalleryRef = el) }, h("ir-property-gallery", null))), h("div", null, h("ir-availability-header", { ages: this.ages, ref: el => (this.availabilityHeaderRef = el), fromDate: this.fromDate, toDate: this.toDate, adultCount: this.adultCount, childrenCount: this.childrenCount })), h("section", { class: app_store.app_data.displayMode === 'default' ? 'relative justify-between gap-4 rounded-md ' : '', ref: el => (this.roomTypeSectionRef = el) }, h("div", { class: app_store.app_data.displayMode === 'default' ? ' flex-1 py-2' : 'grid-container' }, (_a = booking_store.roomTypes) === null || _a === void 0 ? void 0 : _a.map(roomType => {
