...
const utils$2 = require('./utils-40203e5f.js');
const localization_store = require('./localization.store-9178217b.js');
...
class PickupService {
    calculateTotalPersons() {
        let count = 0;
        Object.keys(utils$2.booking_store.ratePlanSelections).map(roomTypeId => {
            return Object.keys(utils$2.booking_store.ratePlanSelections[roomTypeId]).map(ratePlanId => {
                const r = utils$2.booking_store.ratePlanSelections[roomTypeId][ratePlanId];
                if (r.reserved !== 0) {
    ...
    async savePickup(params, booking_nbr, is_remove) {
        try {
            ...
            await axios$1.post(`/Do_Pickup?Ticket=${utils$2.app_store.app_data.token}`, {
                booking_nbr,
                is_remove,
    ...
    transformDefaultPickupData(data) {
        const arrival_time = utils$2.renderTime(data.hour) + ':' + utils$2.renderTime(data.minute);
        return {
    ...
    getAvailableLocations(message) {
        ...
        (_a = utils$2.app_store.property.pickup_service.allowed_options) === null || _a === void 0 ? void 0 : _a.forEach(option => {
    ...
    getPickUpPersonStatus(code) {
        const getCodeDescription = utils$2.app_store.property.pickup_service.allowed_pricing_models.find(model => model.code === code);
...
class PropertyHelpers {
    async convertPickup(pickup, hasDifferentCurrency) {
        let selected_option = pickup.selected_option;
        let total = Number(pickup.due_upon_booking);
        let currency = utils$2.app_store.property.currency;
        if (hasDifferentCurrency) {
            ...
            const { pickup_service } = await propertyService.getExposedProperty({
                sync: false,
                id: utils$2.app_store.property.id,
                currency_id: utils$2.app_store.property.currency.code,
                aname: '',
                language: 'en',
                perma_link: '',
            });
            selected_option = pickup_service.allowed_options.find(o => o.id === pickup.selected_option.id);
            total = Number(pickupService
                .updateDue({
...
class PropertyService {
    constructor() {
        ...
        this.getMostEffectiveCurrency = (paymentMethod) => {
            var _a, _b, _c;
            const allowed = utils$2.normalize((paymentMethod === null || paymentMethod === void 0 ? void 0 : paymentMethod.allowed_currencies) || '');
            if (!allowed)
                return null;
            const allowedList = allowed
                .split(',')
                .map(c => utils$2.normalize(c))
                .filter(Boolean);
            if (allowedList.length === 0)
                return null;
            // Resolve user + hotel currency objects (fall back to 'usd' only if needed)
            const userCode = utils$2.normalize((_a = utils$2.app_store.userPreferences) === null || _a === void 0 ? void 0 : _a.currency_id) || 'usd';
            const hotelCode = utils$2.normalize((_c = (_b = utils$2.app_store.property) === null || _b === void 0 ? void 0 : _b.currency) === null || _c === void 0 ? void 0 : _c.code);
            const hotelCurrency = hotelCode ? utils$2.getCurrencyByCode(hotelCode) : null;
    ...
    async getExposedProperties(props) {
        const { data } = await axios$1.post('/Get_Exposed_Properties', props);
        const result = data;
        if (result.ExceptionMsg !== '') {
            throw new Error(result.ExceptionMsg);
        }
    ...
    async fetchPropertiesByLevel2(props) {
        const { data } = await axios$1.post('/Fetch_Properties_By_Level2', props);
        const result = data;
        if (result.ExceptionMsg !== '') {
            throw new Error(result.ExceptionMsg);
        }
    ...
    async getExposedProperty(_a, initTheme) {
        var _b, _c, _d, _e;
        var { sync = true, currency_id = utils$2.app_store.userPreferences.currency_id } = _a, params = __rest(_a, ["sync", "currency_id"]);
        if (initTheme === void 0) { initTheme = true; }
        const { data } = await axios$1.post(`/Get_Exposed_Property`, Object.assign(Object.assign({}, params), { currency: currency_id, include_sales_rate_plans: true }));
        const result = data;
        if (result.ExceptionMsg !== '') {
            throw new Error(result.ExceptionMsg);
        }
        ...
        if (!utils$2.app_store.fetchedBooking) {
            utils$2.booking_store.roomTypes = [...((_c = (_b = result.My_Result) === null || _b === void 0 ? void 0 : _b.roomtypes) !== null && _c !== void 0 ? _c : [])];
        }
        // } else {
        ...
        // }
        if (!utils$2.app_store.fetchedBooking) {
            utils$2.booking_store.roomTypes = [...((_e = (_d = result.My_Result) === null || _d === void 0 ? void 0 : _d.roomtypes) !== null && _e !== void 0 ? _e : [])];
        }
    ...
    async getExposedNonBookableNights(params) {
        ...
        utils$2.app_store.nonBookableNights = Object.assign({}, nights);
    ...
    filterRooms(roomTypes) {
        ...
        const hasDifferentCurrency = utils$2.normalize(utils$2.app_store.userPreferences.currency_id) !== utils$2.normalize((_b = (_a = utils$2.app_store.property) === null || _a === void 0 ? void 0 : _a.currency) === null || _b === void 0 ? void 0 : _b.code);
        const effectiveRoomTypes = (_c = roomTypes !== null && roomTypes !== void 0 ? roomTypes : utils$2.booking_store.roomTypes) !== null && _c !== void 0 ? _c : [];
        const roomTypeLookup = new Map();
        effectiveRoomTypes.forEach(rt => {
            if (rt)
                roomTypeLookup.set(rt.id, rt);
        });
        Object.values(utils$2.booking_store.ratePlanSelections).forEach(ratePlanSelections => {
    ...
    collectBookedRoomsId() {
        ...
        for (const roomtypeId in utils$2.booking_store.ratePlanSelections) {
            for (const rateplanId in utils$2.booking_store.ratePlanSelections[roomtypeId]) {
                const rateplan = utils$2.booking_store.ratePlanSelections[roomtypeId][rateplanId];
    ...
    recalculatePrepaymentAmount(roomTypes) {
        ...
        for (const roomType of roomTypes) {
            const selectedRoomType = utils$2.booking_store.ratePlanSelections[roomType.id];
            if (!selectedRoomType)
                continue;
    ...
    async bookUser(paymentMethod) {
        ...
        let checkAvailabilityPayload = {
            propertyid: utils$2.app_store.app_data.property_id,
            from_date: utils$2.booking_store.bookingAvailabilityParams.from_date,
            to_date: utils$2.booking_store.bookingAvailabilityParams.to_date,
            room_type_ids: roomTypeIds,
            rate_plan_ids: ratePlanIds,
            adult_nbr: utils$2.booking_store.bookingAvailabilityParams.adult_nbr,
            child_nbr: utils$2.booking_store.bookingAvailabilityParams.child_nbr,
            language: utils$2.app_store.userPreferences.language_id,
            is_in_loyalty_mode: utils$2.booking_store.bookingAvailabilityParams.loyalty ? true : !!utils$2.booking_store.bookingAvailabilityParams.coupon,
            promo_key: utils$2.booking_store.bookingAvailabilityParams.coupon || '',
            is_in_agent_mode: !!utils$2.booking_store.bookingAvailabilityParams.agent || false,
            agent_id: ((_a = utils$2.booking_store.bookingAvailabilityParams.agent) === null || _a === void 0 ? void 0 : _a.id) || 0,
            is_in_affiliate_mode: !!utils$2.app_store.app_data.affiliate,
            affiliate_id: utils$2.app_store.app_data.affiliate ? utils$2.app_store.app_data.affiliate.id : null,
            update_store: false,
        };
        let roomTypes = null;
        const normalizedUserCurrency = utils$2.normalize((_b = utils$2.app_store.userPreferences) === null || _b === void 0 ? void 0 : _b.currency_id) || 'usd';
        const normalizedMethodCurrency = mostEffectiveCurrency ? utils$2.normalize(mostEffectiveCurrency.code) : null;
        const normalizedPropertyCurrency = utils$2.normalize((_d = (_c = utils$2.app_store.property) === null || _c === void 0 ? void 0 : _c.currency) === null || _d === void 0 ? void 0 : _d.code);
        ...
        if (needsPropertyAvailability) {
            availabilityCalls.push(this.getExposedBookingAvailability(Object.assign(Object.assign({}, checkAvailabilityPayload), { currency_ref: utils$2.app_store.property.currency.code })).then(data => {
        ...
        try {
            let guest = {
                ...
                cci: utils$2.booking_store.bookingAvailabilityParams.agent && ((_f = (_e = utils$2.booking_store.bookingAvailabilityParams.agent) === null || _e === void 0 ? void 0 : _e.payment_mode) === null || _f === void 0 ? void 0 : _f.code) === '001'
                    ? null
            ...
            const body = {
                ...
                is_pms: false,
                is_direct: true,
                language: (_p = (_o = utils$2.app_store === null || utils$2.app_store === void 0 ? void 0 : utils$2.app_store.userPreferences) === null || _o === void 0 ? void 0 : _o.language_id) !== null && _p !== void 0 ? _p : 'en',
                agent: utils$2.booking_store.bookingAvailabilityParams.agent ? { id: (_q = utils$2.booking_store.bookingAvailabilityParams.agent) === null || _q === void 0 ? void 0 : _q.id } : null,
                is_in_loyalty_mode: utils$2.booking_store.bookingAvailabilityParams.loyalty,
                promo_key: (_r = utils$2.booking_store.bookingAvailabilityParams.coupon) !== null && _r !== void 0 ? _r : null,
                booking: {
                    booking_nbr: '',
                    from_date: utils$2.booking_store.bookingAvailabilityParams.from_date.locale('en').format('YYYY-MM-DD'),
                    to_date: utils$2.booking_store.bookingAvailabilityParams.to_date.locale('en').format('YYYY-MM-DD'),
                ...
                extras: [
                        ...
                        : null,
                    {
                        ...
                        value: utils$2.app_store.property.currency.code,
                    },
                ].filter(f => f !== null),
                pickup_info,
            };
...
//# sourceMappingURL=Token-2248a01e.js.map