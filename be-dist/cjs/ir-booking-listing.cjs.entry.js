...
const common_service = require('./common.service-90893a61.js');
const bookingListing_service = require('./booking-listing.service-846e6780.js');
...
class BookingListingService extends property_service.Token {
    async getExposedGuestBookings(params) {
        const token = this.getToken();
        if (!token) {
            throw new property_service.MissingTokenError();
        }
        const { data } = await property_service.axios.post(`/Get_Exposed_Guest_Bookings?Ticket=${token}`, params);
...
const IrBookingListing = class {
    ...
    render() {
        var _a, _b, _c, _d, _e;
        if (!this.token) {
            return (index.h(index.Host, null, index.h("main", { class: "flex h-screen flex-col  justify-center" }, index.h("div", { class: "mx-auto w-full max-w-md px-4" }, index.h("ir-auth", { enableSignUp: false })))));
        }
        if (this.isLoading) {
            return (index.h("div", { class: "grid h-screen w-full place-content-center" }, index.h("div", { class: "page-loader" })));
        }
        const totalPages = Math.round(this.total_count / this.maxPages);
        return (index.h(index.Host, null, index.h("main", { class: 'flex min-h-screen flex-col gap-4 md:px-4 lg:px-6 xl:px-0' }, this.headerShown && (index.h("ir-nav", { isBookingListing: true, showBookingCode: false, showCurrency: false, website: (_a = app_store.app_store.property) === null || _a === void 0 ? void 0 : _a.space_theme.website, logo: (_c = (_b = app_store.app_store.property) === null || _b === void 0 ? void 0 : _b.space_theme) === null || _c === void 0 ? void 0 : _c.logo })), index.h("div", { class: "ir-table-container mx-auto hidden max-w-6xl flex-1 overflow-x-hidden px-4  shadow-md md:block" }, index.h("ir-booking-header", { bookingNumber: this.bookingNumber, activeLink: this.activeLink, mode: this.bookingNumber ? 'multi' : 'single' }), index.h("div", { class: "max-w-full overflow-x-auto" }, index.h("table", { class: "ir-table" }, index.h("thead", null, index.h("tr", { class: "ir-table-header" }, index.h("th", { class: "ir-table-head" }, "Status"), index.h("th", { class: "ir-table-head" }, "Booking reference"), index.h("th", { class: "ir-table-head md:hidden lg:table-cell" }, "Booking date"), index.h("th", { class: "ir-table-head" }, "Check-in"), index.h("th", { class: "ir-table-head" }, "Duration"), index.h("th", { class: "ir-table-head" }, "Total price"), index.h("th", { class: "ir-table-head sr-only" }, "pay now"))), index.h("tbody", null, (_d = this.bookings) === null || _d === void 0 ? void 0 : _d.map(booking => {
            ...
            return (index.h("tr", { class: "ir-table-row", key: booking.booking_nbr }, index.h("td", { class: "ir-table-cell" }, index.h("ir-badge", { label: booking.status.description, variant: this.getBadgeVariant(booking.status.code) })), index.h("td", { class: "ir-table-cell" }, booking.booking_nbr), index.h("td", { class: "ir-table-cell md:hidden lg:table-cell" }, utils.dateFns.format(new Date(booking.booked_on.date), 'dd-MMM-yyyy')), index.h("td", { class: "ir-table-cell" }, utils.dateFns.format(new Date(booking.from_date), 'dd-MMM-yyyy')), index.h("td", { class: "ir-table-cell" }, totalNights, " ", totalNights > 1 ? 'nights' : 'night'), index.h("td", { class: "ir-table-cell" }, utils.formatAmount(booking.total, booking.currency.code)), index.h("td", { class: "ir-table-cell" }, (payment || cancel) && (index.h("div", { class: "flex items-center justify-end gap-2.5" }, cancel && index.h("ir-button", { class: "w-full", variants: "outline", label: "Cancel" }), payment && index.h("ir-button", { label: `Pay ${utils.formatAmount(booking.financial.due_amount || 0, booking.currency.code)} to guarentee` }))))));
        })))), this.page_mode === 'multi' && (index.h("div", { class: "px-[20px] py-[16px] " }, index.h("ir-pagination", { total: totalPages, current: this.currentPage })))), index.h("section", { class: 'flex-1 space-y-4 px-4 md:hidden' }, index.h("ir-booking-header", { bookingNumber: this.bookingNumber, mode: this.bookingNumber ? 'multi' : 'single' }), (_e = this.bookings) === null || _e === void 0 ? void 0 :
            _e.map(booking => index.h("ir-booking-card", { booking: booking, key: booking.booking_nbr })), this.page_mode === 'multi' && index.h("ir-pagination", { total: totalPages, current: this.currentPage })), this.footerShown && index.h("ir-footer", null))));
    }
};
