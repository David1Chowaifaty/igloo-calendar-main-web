...
const index$2 = require('./index-ad166e1c.js');
const index = require('./index-e20105af.js');
const index$1 = require('./index-08156e03.js');
...
const initialState = {
    ...
    bookings: [],
    userSelection: {
        from: index.dateFns.format(index.dateFns.addDays(new Date(), -7), 'yyyy-MM-dd'),
        to: index.dateFns.format(new Date(), 'yyyy-MM-dd'),
        channel: '',
        property_id: null,
        start_row: 0,
        end_row: 20,
...
function initializeUserSelection() {
    ...
    booking_listing.userSelection = Object.assign(Object.assign({}, booking_listing.userSelection), { channel: '', booking_status: booking_listing.statuses[0].code, filter_type: booking_listing.types[0].id, book_nbr: '', name: '', from: index.dateFns.format(index.dateFns.addDays(new Date(), -7), 'yyyy-MM-dd'), to: index.dateFns.format(new Date(), 'yyyy-MM-dd'), start_row: 0, end_row: booking_listing.rowCount });
}

class BookingListingService extends index.Token {
    async getExposedBookingsCriteria(property_id) {
        const token = this.getToken();
        if (!token) {
            throw new Error('Invalid token');
        }
        const { data } = await axios.axios.post(`/Get_Exposed_Bookings_Criteria?Ticket=${token}`, {
            property_id,
        });
    ...
    async getExposedBookings(params) {
        ...
        const { data } = await axios.axios.post(`/Get_Exposed_Bookings?Ticket=${token}`, params);
        const result = data.My_Result;
    ...
    async removeExposedBooking(booking_nbr, is_to_revover) {
        ...
        await axios.axios.post(`/Remove_Exposed_Booking?Ticket=${token}`, {
            booking_nbr,
            is_to_revover,
        });
    }
...
const IrBookingListing = class {
    ...
    render() {
        if (!this.token) {
            return (index$2.h("main", { class: "flex h-screen flex-col  justify-center" }, index$2.h("div", { class: "mx-auto w-full max-w-md px-4" }, index$2.h("ir-auth", { enableSignUp: false }))));
        }
        return (index$2.h(index$2.Host, null, index$2.h("div", { class: 'p-4' }, index$2.h("div", { class: "table-container overflow-x-hidden shadow-md" }, index$2.h("div", { class: "flex flex-col gap-2.5 px-[20px] py-[16px] md:flex-row md:items-center md:justify-between " }, index$2.h("h3", null, "Bookings"), index$2.h("input", { type: "text", placeholder: "Search...", class: "w-full max-w-sm rounded-md border border-solid border-gray-300 px-4 py-1.5 " })), index$2.h("div", { class: "max-w-full overflow-x-auto " }, index$2.h("table", { class: "table" }, index$2.h("thead", null, index$2.h("tr", { class: "table-header" }, index$2.h("th", { class: "table-head" }, "Status"), index$2.h("th", { class: "table-head" }, "Booking reference"), index$2.h("th", { class: "table-head" }, "Booking date"), index$2.h("th", { class: "table-head" }, "Check-in"), index$2.h("th", { class: "table-head" }, "Duration"), index$2.h("th", { class: "table-head" }, "Total price"), index$2.h("th", { class: "table-head sr-only" }, "pay now"))), index$2.h("tbody", null, [...new Array(10)].map((_, i) => (index$2.h("tr", { class: "table-row" }, index$2.h("td", { class: "table-cell" }, index$2.h("ir-badge", { label: "Pending", variant: "pending" })), index$2.h("td", { class: "table-cell" }, i), index$2.h("td", { class: "table-cell" }, "Name ", i), index$2.h("td", { class: "table-cell" }, "email", i, "@example.com"), index$2.h("td", { class: "table-cell" }, "City ", i), index$2.h("td", { class: "table-cell" }, "Country ", i), index$2.h("td", { class: "table-cell" }, "Zip", i))))))), index$2.h("div", { class: "flex items-center justify-between px-[20px] py-[16px] " }, index$2.h("ir-button", { variants: "outline", label: "Previous", haveLeftIcon: true }, index$2.h("ir-icons", { name: "arrow_left", slot: "left-icon", svgClassName: "size-3" })), index$2.h("ir-button", { variants: "outline", label: "Next", haveRightIcon: true }, index$2.h("ir-icons", { name: "arrow_right", slot: "right-icon", svgClassName: "size-3" })))))));
    }
};
