...
class PropertyHelpers {
    ...
    async fetchAvailabilityData(props, roomtypeIds, rateplanIds) {
        const response = await axios.axios.post(`/Get_Exposed_Booking_Availability`, Object.assign(Object.assign({}, props.params), { identifier: props.identifier, room_type_ids: roomtypeIds, rate_plan_ids: rateplanIds, skip_getting_assignable_units: true, is_specific_variation: true, is_backend: false }));
    ...
    async validateFreeCancelationZone(booking_nbr) {
        ...
        const result = await this.paymentService.GetExposedApplicablePolicies({
            ...
            params: {
                booking_nbr,
                currency_id: localization_store.app_store.currencies.find(c => c.code.toLowerCase() === (localization_store.app_store.userPreferences.currency_id.toLowerCase() || 'usd')).id,
                language: localization_store.app_store.userPreferences.language_id,
                property_id: localization_store.app_store.app_data.property_id,
                rate_plan_id: 0,
                room_type_id: 0,
            },
        });
    ...
    updateRatePlan(ratePlans, newRoomtype) {
        const agentExists = !!utils.booking_store.bookingAvailabilityParams.agent;
        return ratePlans.reduce((updatedRatePlans, rp) => {
            var _a, _b;
...
class PropertyService {
    ...
    async getExposedProperty(params, initTheme = true) {
        var _a, _b;
        const { data } = await axios.axios.post(`/Get_Exposed_Property`, Object.assign(Object.assign({}, params), { currency: localization_store.app_store.userPreferences.currency_id, include_sales_rate_plans: true }));
        const result = data;
        if (result.ExceptionMsg !== '') {
            throw new Error(result.ExceptionMsg);
        }
        ...
        if (!localization_store.app_store.fetchedBooking) {
            utils.booking_store.roomTypes = [...((_b = (_a = result.My_Result) === null || _a === void 0 ? void 0 : _a.roomtypes) !== null && _b !== void 0 ? _b : [])];
        }
        // } else {
        ...
        if (params.aname || params.perma_link) {
            localization_store.app_store.app_data = Object.assign(Object.assign({}, localization_store.app_store.app_data), { property_id: result.My_Result.id });
        }
        localization_store.app_store.app_data.displayMode = result.My_Result.be_listing_mode === 'grid' ? 'grid' : 'default';
        localization_store.app_store.property = Object.assign({}, result.My_Result);
        localization_store.app_store.app_data.property_id = result.My_Result.id;
        if (initTheme) {
    ...
    async getExposedNonBookableNights(params) {
        var _a;
        const { data } = await axios.axios.post(`/Get_Exposed_Non_Bookable_Nights`, params);
        ...
        localization_store.app_store.nonBookableNights = nights;
    ...
    async getExposedBooking(params, withExtras = true) {
        const { data } = await axios.axios.post(`/Get_Exposed_Booking`, Object.assign(Object.assign({}, params), { extras: withExtras
                ? [
    ...
    async fetchSetupEntries() {
        if (localization_store.app_store.setup_entries) {
            return localization_store.app_store.setup_entries;
        }
        ...
        localization_store.app_store.setup_entries = setupEntries;
        checkout_store.updateUserFormData('arrival_time', setupEntries.arrivalTime[0].CODE_NAME);
        return setupEntries;
    }
    filterRooms() {
        let rooms = [];
    ...
    async bookUser() {
        var _a, _b, _c, _d, _e, _f;
        const prePaymentAmount = checkout_store.checkout_store.prepaymentAmount;
        try {
            console.log('payment', checkout_store.checkout_store.payment);
            let guest = {
                ...
                address: '',
                country_phone_prefix: checkout_store.checkout_store.userFormData.country_phone_prefix,
                dob: null,
                subscribe_to_news_letter: true,
                cci: ((_a = checkout_store.checkout_store.payment) === null || _a === void 0 ? void 0 : _a.code) === '001'
                    ? {
                        nbr: (_b = checkout_store.checkout_store.payment) === null || _b === void 0 ? void 0 : _b.cardNumber.replace(/ /g, ''),
                        holder_name: (_c = checkout_store.checkout_store.payment) === null || _c === void 0 ? void 0 : _c.cardHolderName,
                        expiry_month: (_d = checkout_store.checkout_store.payment) === null || _d === void 0 ? void 0 : _d.expiry_month.split('/')[0],
                        expiry_year: (_e = checkout_store.checkout_store.payment) === null || _e === void 0 ? void 0 : _e.expiry_year.split('/')[1],
                        cvc: checkout_store.checkout_store.payment.cvc,
                    }
                    : null,
            };
            const body = {
                ...
                promo_key: (_f = utils.booking_store.bookingAvailabilityParams.coupon) !== null && _f !== void 0 ? _f : null,
                booking: {
                    booking_nbr: '',
                    from_date: index.dateFns.format(utils.booking_store.bookingAvailabilityParams.from_date, 'yyyy-MM-dd'),
                    to_date: index.dateFns.format(utils.booking_store.bookingAvailabilityParams.to_date, 'yyyy-MM-dd'),
                    remark: checkout_store.checkout_store.userFormData.message || null,
                    property: {
                        id: localization_store.app_store.app_data.property_id,
                    },
                    source: { code: localization_store.app_store.app_data.isFromGhs ? 'ghs' : window.location.href, tag: localization_store.app_store.app_data.stag, description: '' },
                    referrer_site: localization_store.app_store.app_data.affiliate ? `https://${localization_store.app_store.app_data.affiliate.sites[0].url}` : 'www.igloorooms.com',
                    currency: localization_store.app_store.currencies.find(currency => currency.code.toString().toLowerCase() === localization_store.app_store.userPreferences.currency_id.toLowerCase()),
                    arrival: { code: checkout_store.checkout_store.userFormData.arrival_time },
                    guest,
                ...
                extras: [
                    prePaymentAmount > 0
                        ? {
                            key: 'payment_code',
                            value: checkout_store.checkout_store.payment.code,
                        }
                        : null,
                    prePaymentAmount > 0
                        ? {
                        ...
                        : null,
                    {
                        ...
                        value: localization_store.app_store.userPreferences.currency_id,
                    },
                ].filter(f => f !== null),
                pickup_info: checkout_store.checkout_store.pickup.location ? this.propertyHelpers.convertPickup(checkout_store.checkout_store.pickup) : null,
            };
            console.log('body');
    ...
    async getExposedGuest() {
        ...
        checkout_store.checkout_store.userFormData = Object.assign(Object.assign({}, checkout_store.checkout_store.userFormData), { country_id: res.country_id, email: res.email, firstName: res.first_name, lastName: res.last_name, mobile_number: res.mobile, country_phone_prefix: res.country_phone_prefix, id: res.id });
    }
}

exports.PropertyService = PropertyService;

//# sourceMappingURL=property.service-128e6af6.js.map