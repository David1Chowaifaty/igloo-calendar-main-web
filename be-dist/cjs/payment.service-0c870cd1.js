...
class PaymentService {
    async getExposedCancelationDueAmount(params) {
        const { data } = await axios.axios.post(`/Get_Exposed_Cancelation_Due_Amount`, Object.assign({}, params));
    ...
    async GeneratePaymentCaller({ token, params, onRedirect, onScriptRun, }) {
        ...
        const { data } = await axios.axios.post('/Generate_Payment_Caller', Object.assign(Object.assign({}, params), { callback_url: utils.generateCheckoutUrl(localization_store.app_store.property.perma_link) }), { headers: { Authorization: token } });
        if (data['ExceptionMsg'] !== '') {
            throw new Error(data.ExceptionMsg);
        }
        const res = data['My_Result'];
    ...
    async RequestBookingCancellation(booking_nbr) {
        const { data } = await axios.axios.post(`/Request_Booking_Cancelation`, { BOOK_NBR: booking_nbr });
        if (data['ExceptionMsg'] !== '') {
            throw new Error(data.ExceptionMsg);
        }
    ...
    async GetExposedApplicablePolicies({ params, book_date, }) {
        const { data } = await axios.axios.post(`/Get_Exposed_Applicable_Policies`, params);
        if (data['ExceptionMsg'] !== '') {
            throw new Error(data.ExceptionMsg);
        }
    ...
    processAlicablePolicies(policies, book_date) {
        ...
        let cancelation = policies.find(po => { var _a; return po.type === 'cancelation' && ((_a = po === null || po === void 0 ? void 0 : po.brackets) === null || _a === void 0 ? void 0 : _a.some(b => index.dateFns.isBefore(new Date(b.due_on), book_date) || index.dateFns.isSameDay(new Date(b.due_on), book_date))); });
        if (cancelation) {
            const cancelationAmount = (_d = (_c = cancelation.brackets.find(b => index.dateFns.isBefore(new Date(b.due_on), book_date) || index.dateFns.isSameDay(new Date(b.due_on), book_date))) === null || _c === void 0 ? void 0 : _c.gross_amount) !== null && _d !== void 0 ? _d : null;
    ...
    checkFreeCancelationZone(policies) {
        ...
        let cancelation = policies === null || policies === void 0 ? void 0 : policies.find(po => { var _a; return po.type === 'cancelation' && ((_a = po === null || po === void 0 ? void 0 : po.brackets) === null || _a === void 0 ? void 0 : _a.some(b => index.dateFns.isBefore(new Date(b.due_on), now) || index.dateFns.isSameDay(new Date(b.due_on), now))); });
        if (!cancelation) {
    ...
    getCancelationMessage(applicablePolicies, showCancelation = false, includeGuarentee = true) {
        ...
        let message = cancelationMessage ? `${showCancelation ? `<b><u>${localization_store.localizedWords.entries.Lcz_Cancelation}: </u></b>` : ''}${cancelationMessage}<br/>` : '<span></span>';
        if (includeGuarentee) {
            const guarenteeMessage = (_b = applicablePolicies.find(t => t.type === 'guarantee')) === null || _b === void 0 ? void 0 : _b.combined_statement;
            if (guarenteeMessage) {
                message += `${showCancelation ? `<b><u>${localization_store.localizedWords.entries.Lcz_Guarantee}: </u></b>` : ''}${guarenteeMessage}<br/>`;
            }
        }
        return {
            message,
    ...
    async getBookingPrepaymentAmount(booking) {
        ...
        let requests = await Promise.all(list.map(l => this.GetExposedApplicablePolicies({
            ...
            params: {
                ...
                property_id: localization_store.app_store.property.id,
            },
        })));
    ...
    findClosestDate(data) {
        ...
        for (const item of data) {
            const currentDueDate = index.dateFns.parseISO(item.due_on);
            if (!closestDateObj || index.dateFns.isBefore(currentDueDate, index.dateFns.parseISO(closestDateObj.due_on))) {
                closestDateObj = item;
            }
        }
...
//# sourceMappingURL=payment.service-0c870cd1.js.map