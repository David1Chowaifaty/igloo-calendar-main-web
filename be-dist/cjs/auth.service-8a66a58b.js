...
class AuthService extends property_service.Token {
    constructor() {
        super(...arguments);
        this.subscribers = [];
    }
    ...
    async login(params, signIn = true) {
        ...
        const { data } = await property_service.axios.post(`/Exposed_Guest_SignIn?Ticket=${token}`, option === 'direct' ? rest.params : Object.assign({}, rest));
    ...
    async signUp(params) {
        ...
        const { data } = await property_service.axios.post(`/Exposed_Guest_SignUp?Ticket=${token}`, params);
        if (data['ExceptionMsg'] !== '') {
            throw new Error(data['ExceptionMsg']);
        }
...
//# sourceMappingURL=auth.service-8a66a58b.js.map