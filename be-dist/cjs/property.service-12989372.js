...
const utils$3 = require('./utils-383cd8d2.js');
...
class PropertyService extends Token {
    ...
    async getExposedProperty(params, initTheme = true) {
        ...
        const { data } = await axios$1.post(`/Get_Exposed_Property?Ticket=${token}`, Object.assign(Object.assign({}, params), { currency: app_store.app_store.userPreferences.currency_id }));
        const result = data;
    ...
    async getExposedBookingAvailability(params) {
        ...
        let roomtypeIds = [];
        const { roomtype_id } = app_store.app_store.app_data;
        if (roomtype_id) {
    ...
    async getExposedBooking(params, withExtras = true) {
        ...
        const { data } = await axios$1.post(`/Get_Exposed_Booking?Ticket=${token}`, Object.assign(Object.assign({}, params), { extras: withExtras ? ['payment_code'] : null }));
        const result = data;
        if (result.ExceptionMsg !== '') {
            throw new Error(result.ExceptionMsg);
        }
                ...
                if (rp.reserved > 0) {
                    [...new Array(rp.reserved)].map((_, index) => {
                        ...
                        rooms.push({
                            ...
                            notes: null,
                            days: this.generateDays(booking_store.bookingAvailabilityParams.from_date, booking_store.bookingAvailabilityParams.to_date, +rp.checkoutVariations[index].amount / utils$3.getDateDifference(booking_store.bookingAvailabilityParams.from_date, booking_store.bookingAvailabilityParams.to_date)),
                            guest: {
                                email: null,
                                first_name,
                                last_name,
...
//# sourceMappingURL=property.service-12989372.js.map