...
class CommonService extends payment_service.Token {
    async getCurrencies() {
        const token = this.getToken();
        if (!token) {
            throw new payment_service.MissingTokenError();
        }
        const { data } = await payment_service.axios.post(`/Get_Exposed_Currencies?Ticket=${token}`);
    ...
    async getExposedLanguages() {
        ...
        const { data } = await payment_service.axios.post(`/Get_Exposed_Languages?Ticket=${token}`);
    ...
    async getCountries(language) {
        try {
            const token = this.getToken();
            if (token) {
                const { data } = await payment_service.axios.post(`/Get_Exposed_Countries?Ticket=${token}`, {
                    language,
                });
    ...
    async getUserDefaultCountry() {
        try {
            ...
            const { data } = await payment_service.axios.post(`/Get_Country_By_IP?Ticket=${token}`, {
                IP: '',
            });
            if (data.ExceptionMsg !== '') {
                throw new Error(data.ExceptionMsg);
    ...
    async getExposedCountryByIp() {
        try {
            ...
            const { data } = await payment_service.axios.post(`/Get_Exposed_Country_By_IP?Ticket=${token}`, {
                IP: '',
                lang: 'en',
            });
            if (data.ExceptionMsg !== '') {
    ...
    async getBEToken() {
        try {
            const { data } = await payment_service.axios.post(`/Get_BE_Token`, {});
            if (data.ExceptionMsg !== '') {
                throw new Error(data.ExceptionMsg);
            }
            return data['My_Result'];
    ...
    async getExposedLanguage() {
        try {
            ...
            const { data } = await payment_service.axios.post(`/Get_Exposed_Language?Ticket=${token}`, { code: app_store.app_store.userPreferences.language_id, sections: ['_BE_FRONT'] });
            if (data.ExceptionMsg !== '') {
                throw new Error(data.ExceptionMsg);
            }
...
//# sourceMappingURL=common.service-e0453fee.js.map