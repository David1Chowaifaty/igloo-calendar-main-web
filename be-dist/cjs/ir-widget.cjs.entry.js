...
const IrBookingWidget = class {
    ...
    handleBooknow() {
        ...
        const fromDate = from_date ? `checkin=${utils.dateFns.format(from_date, 'yyyy-MM-dd')}` : '';
        const toDate = from_date ? `checkout=${utils.dateFns.format(to_date, 'yyyy-MM-dd')}` : '';
    ...
    renderDateTrigger() {
        return (index.h("div", { class: "date-trigger", slot: "trigger" }, index.h("ir-icons", { name: "calendar", svgClassName: "size-4" }), this.dates && this.dates.from_date && this.dates.to_date ? (index.h("div", null, index.h("p", null, index.h("span", null, utils.dateFns.format(this.dates.from_date, 'MMM dd')), index.h("span", null, " - "), index.h("span", null, utils.dateFns.format(this.dates.to_date, 'MMM dd'))))) : (index.h("div", null, index.h("p", null, "Your dates")))));
    }
    renderAdultChildTrigger() {
        const { adultCount, childrenCount } = this.guests;
        return (index.h("div", { class: "guests-trigger", slot: "trigger" }, index.h("ir-icons", { name: "user", svgClassName: "size-4" }), index.h("p", { class: 'guests' }, adultCount > 0 && (index.h("span", null, adultCount, " ", adultCount === 1 ? utils.localizedWords.entries.Lcz_Adult : utils.localizedWords.entries.Lcz_Adults)), childrenCount > 0 && (index.h("span", null, ", ", childrenCount, " ", childrenCount === 1 ? utils.localizedWords.entries.Lcz_Child : utils.localizedWords.entries.Lcz_Children)), adultCount === 0 && index.h("span", null, "Guests"))));
    }
    render() {
        var _a, _b, _c, _d, _e;
        if (this.isLoading) {
        ...
        return (index.h(index.Fragment, null, index.h("div", { class: "booking-widget-container", style: this.contentContainerStyle }, index.h("div", { class: 'hovered-container' }), index.h("ir-popover", { class: 'ir-popover', showCloseButton: false, placement: "auto", ref: el => (this.popover = el), onOpenChange: e => {
                this.isPopoverOpen = e.detail;
                if (!this.isPopoverOpen) {
                    if (!this.dates.to_date && this.dates.from_date) {
                        this.dates = Object.assign(Object.assign({}, this.dates), { to_date: utils.dateFns.addDays(this.dates.from_date, 1) });
                    }
                }
            } }, this.renderDateTrigger(), index.h("div", { slot: "popover-content", class: "popup-container w-full border-0 bg-white p-4 pb-6 shadow-none sm:w-auto sm:border sm:p-4 sm:shadow-sm md:p-6 " }, index.h("ir-date-range", { minDate: utils.dateFns.addDays(new Date(), -1), style: { '--radius': 'var(--ir-widget-radius)' }, fromDate: (_a = this.dates) === null || _a === void 0 ? void 0 : _a.from_date, toDate: (_b = this.dates) === null || _b === void 0 ? void 0 : _b.to_date, locale: utils.app_store.selectedLocale, maxSpanDays: 5, onDateChange: e => {
            ...
            } }))), index.h("ir-popover", { ref: el => (this.guestPopover = el), class: 'ir-popover', showCloseButton: false, placement: "auto" }, this.renderAdultChildTrigger(), index.h("ir-guest-counter", { slot: "popover-content", minAdultCount: 0, maxAdultCount: (_c = utils.app_store === null || utils.app_store === void 0 ? void 0 : utils.app_store.property) === null || _c === void 0 ? void 0 : _c.adult_child_constraints.adult_max_nbr, maxChildrenCount: (_d = utils.app_store === null || utils.app_store === void 0 ? void 0 : utils.app_store.property) === null || _d === void 0 ? void 0 : _d.adult_child_constraints.child_max_nbr, childMaxAge: (_e = utils.app_store.property) === null || _e === void 0 ? void 0 : _e.adult_child_constraints.child_max_age, onUpdateCounts: e => (this.guests = e.detail), class: 'h-full', onCloseGuestCounter: () => this.guestPopover.toggleVisibility() })), index.h("button", { class: "btn-flip", onClick: this.handleBooknow.bind(this) }, "Book now"))));
    }
...
exports.ir_widget = IrBookingWidget;

//# sourceMappingURL=ir-widget.cjs.entry.js.map