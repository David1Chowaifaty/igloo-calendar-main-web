...
const IrTranslationsEntriesTable = class {
    ...
    handleRowAction(action, entry) {
        switch (action) {
            ...
            case 'copy-row':
                ...
                utils$1.setCopiedEntry(entry);
                navigator.clipboard?.writeText(entry.key);
                utils.showToast({ type: 'success', title: 'Row copied — paste its key into a new entry to reuse its translations.' });
                break;
            case 'move':
    ...
    renderValueCell(entry, column) {
        ...
        if (entry.meta?.isUpdateable === false) {
            return (index.h("span", { class: "entries-table__cell-display --readonly" }, utils$1.hasValue(value) ? (index.h("span", { class: "entries-table__cell-text", "data-tooltip": value }, value)) : (index.h("span", { class: "entries-table__cell-missing" }, "Missing")), index.h("wa-icon", { name: "lock", class: "entries-table__cell-lock", "aria-hidden": "true" })));
        }
        if (isEditing) {
        ...
        return (index.h("span", { class: `entries-table__cell-display ${utils$1.hasValue(value) ? '' : '--empty'}` }, utils$1.hasValue(value) ? (index.h("span", { class: "entries-table__cell-text", "data-tooltip": value }, value)) : (index.h("span", { class: "entries-table__cell-missing" }, "Missing"))));
    }
