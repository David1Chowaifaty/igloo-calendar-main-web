...
const IrTranslationsEntriesTable = class {
    ...
    renderValueCell(entry, language) {
        ...
        if (entry.meta?.isUpdateable === false) {
            return (index.h("span", { class: "entries-table__cell-display --readonly", "aria-label": `${ariaLabel} (read-only)` }, utils.hasValue(value) ? (index.h("span", { class: "entries-table__cell-text", "data-tooltip": value }, value)) : (index.h("span", { class: "entries-table__cell-missing" }, "Missing")), index.h("wa-icon", { name: "lock", class: "entries-table__cell-lock", "aria-hidden": "true" })));
        }
        if (isEditing) {
            return (index.h("wa-input", { size: "s", value: value, class: "entries-table__cell-input", label: ariaLabel, autocomplete: "off", spellcheck: false, ref: el => (this.cellInputRef = el), oninput: (e) => (this.draft = e.target.value), onKeyDown: (e) => this.handleCellKeyDown(e, entry, language.code, value), onblur: () => this.handleCellBlur(entry, language.code), onchange: () => {
                    this.commitDraft(entry, language.code);
                } }));
        }
        return (index.h("button", { type: "button", class: `entries-table__cell-display ${utils.hasValue(value) ? '' : '--empty'}`, "aria-label": utils.hasValue(value) ? `Edit ${ariaLabel}` : `Add ${ariaLabel}`, onClick: () => this.startEditing(entry, language.code) }, utils.hasValue(value) ? (index.h("span", { class: "entries-table__cell-text", "data-tooltip": value }, value)) : (index.h("span", { class: "entries-table__cell-missing" }, "Missing"))));
    }
    ...
    buildColumns() {
        ...
        return [
            ...
            helper.display({
                id: 'actions',
                header: () => index.h("span", { class: "entries-table__sr-only" }, "Actions"),
    ...
    renderCell(cell) {
        ...
        return (index.h("td", { key: cell.id, class: {
            ...
            }, onClick: columnId === 'key' ? () => this.editEntry.emit(cell.row.original) : undefined }, useTable.flexRender(cell.column.columnDef.cell, cell.getContext())));
    }
    renderRow(row) {
    ...
    renderGroupHeader(name, count) {
        ...
        return (index.h("tr", { key: `group:${name}`, class: "entries-table__group-row" }, index.h("td", { class: "entries-table__group-cell", colSpan: 3 + this.languages.length }, index.h("button", { type: "button", class: "entries-table__group-toggle", "aria-expanded": collapsed ? 'false' : 'true', onClick: () => this.toggleGroup(name) }, index.h("wa-icon", { class: "entries-table__group-chevron", name: "chevron-down", "aria-hidden": "true" }), index.h("span", { class: "entries-table__group-name" }, name), index.h("span", { class: "entries-table__group-count" }, count, " key", count === 1 ? '' : 's')))));
    }
    /**
