...
const FetchUnBookableRoomsSchema = index.libExports.z.object({
    property_ids: index.libExports.z.array(commonSchemas.PropertyIdSchema),
...
const TaxCategorySchema = index.libExports.z.object({
    ...
    pct: index.libExports.z.number(),
    default_price: index.libExports.z.number().nullable().optional(),
...
const HandleExposedPropertyTaxCategoriesParamsSchema = index.libExports.z.object({
    ...
    BABY_COT_PRICING_MODEL: index.libExports.z.string(),
    SERVICE_CHARGE_PCT: index.libExports.z.number().optional(),
    tax_categories: index.libExports.z.array(TaxCategorySchema),
    TAXATION_STRATEGY: index.libExports.z.string(),
    DAY_USE_BLOCK: index.libExports.z
        .union([index.libExports.z.literal('0'), index.libExports.z.literal('1')])
        .nullable()
        .optional()
        .default(null),
});
...
const UnifiedFolioRecordSchema = index.libExports.z.object({
    ...
    DOC_HOUR: index.libExports.z.number().nullable().optional(),
    DOC_MINUTE: index.libExports.z.number().nullable().optional(),
...
const GetExposedBookingsByInvoicedStatusParamsSchema = index.libExports.z.object({
    property_id: commonSchemas.PropertyIdSchema,
    booking_nbr: commonSchemas.BookingNumberSchema,
    from_date: commonSchemas.DateSchema,
    to_date: commonSchemas.DateSchema,
    source: index.libExports.z.string().optional(),
    is_totally_invoiced: index.libExports.z.boolean().optional().default(false),
    start_row: index.libExports.z.number().default(0),
    end_row: index.libExports.z.number(),
});
const GetDayUseBookingsForCalendarParamsSchema = index.libExports.z.object({
    property_id: commonSchemas.PropertyIdSchema,
    from_date: commonSchemas.DateSchema,
    to_date: commonSchemas.DateSchema,
});
const CalculateNetAmountParamsSchema = index.libExports.z.object({
    property_id: commonSchemas.PropertyIdSchema,
    amount: index.libExports.z.number(),
