...
const IglBookingEvent = class {
    ...
    render() {
        ...
        return (index.h(index.Host, { key: '7dcc4c0cb50bf69f456af3f2879f04eeddf57c4c', class: `bookingEvent  ${this.isNewEvent() || this.isHighlightEventType() ? 'newEvent' : ''} ${legend.clsName} `, style: this.getPosition(), id: 'event_' + this.getBookingId() }, index.h("div", { key: 'debd25cfc192f21253fd458d34de44c3c320f36e', class: `bookingEventBase  ${!this.isNewEvent() && moment.hooks(new Date(this.bookingEvent.defaultDates.from_date)).isBefore(new Date(this.bookingEvent.FROM_DATE)) ? 'skewedLeft' : ''}
          ...
          ${!this.bookingEvent.is_direct &&
                ...
                'border border-dark ota-booking-event'}  ${this.isSplitBooking() ? 'splitBooking' : ''}`, style: { 'backgroundColor': legend.color, '--ir-event-bg': legend.color }, onTouchStart: event => this.startDragging(event, 'move'), onMouseDown: event => this.startDragging(event, 'move') }), noteNode ? index.h("div", { class: "legend_circle noteIcon", style: { backgroundColor: noteNode.color } }) : null, balanceNode ? index.h("div", { class: "legend_circle balanceIcon", style: { backgroundColor: balanceNode.color } }) : null, index.h("div", { key: '013854bdfd6acdd2394e65891bea94766ce097ea', class: "bookingEventTitle", onTouchStart: event => this.startDragging(event, 'move'), onMouseDown: event => this.startDragging(event, 'move') }, this.getBookedBy(), this.renderEventBookingNumber()), (this.bookingEvent.is_direct || utils.isBlockUnit(this.bookingEvent.STATUS_CODE)) && (index.h(index.Fragment, { key: 'f64f79d88efa06f7b9405dcfc26a162cd3c9e2f4' }, index.h("div", { key: 'a951f75538d67bf3de088a402b4e9ac1b2c806ee', class: `bookingEventDragHandle leftSide ${!this.isNewEvent() && moment.hooks(new Date(this.bookingEvent.defaultDates.from_date)).isBefore(new Date(this.bookingEvent.FROM_DATE)) ? 'skewedLeft' : ''}
            ${!this.isNewEvent() && moment.hooks(new Date(this.bookingEvent.defaultDates.to_date)).isAfter(new Date(this.bookingEvent.TO_DATE)) ? 'skewedRight' : ''}`, onTouchStart: event => this.startDragging(event, 'leftSide'), onMouseDown: event => this.startDragging(event, 'leftSide') }), index.h("div", { key: '75cf736aba7f0e59db4c82461a72c39c14ffe7cc', class: `bookingEventDragHandle rightSide ${!this.isNewEvent() && moment.hooks(new Date(this.bookingEvent.defaultDates.from_date)).isBefore(new Date(this.bookingEvent.FROM_DATE)) ? 'skewedLeft' : ''}
...
const IglCalBody = class {
    ...
    componentWillLoad() {
        ...
        calendarDates_store.calendar_dates.days.forEach(day => {
    ...
    getGeneralRoomDayColumns(roomId, roomCategory, roomName, index$1) {
        ...
        return this.calendarData.days.map(dayInfo => {
            ...
            const isDisabled = (_a = calendarDates_store.calendar_dates.days.find(e => e.day === formattedDate)) === null || _a === void 0 ? void 0 : _a.rate[index$1].exposed_inventory.rts;
    ...
    render() {
        ...
        return (index.h(index.Host, { key: 'bcb645bbf672183c28af7928744e03c07f7c2494' }, index.h("div", { key: '2aec178e66f9220534a103c658b8321ae15fe5bf', class: "bodyContainer" }, this.getRoomRows(), index.h("div", { key: 'ff5850b223326c8e709ff553398f9f25c311a5f5', class: "bookingEventsContainer preventPageScroll" }, (_a = this.getBookingData()) === null || _a === void 0 ? void 0 : _a.map(bookingEvent => {
            var _a, _b, _c;
...
const IglCalFooter = class {
    ...
    render() {
        return (index.h(index.Host, { key: 'c1e80a82c616f424fa4915a3d443af8db7eba435', class: "footerContainer" }, index.h("div", { key: '6ad24e188ecc0240bc652db90c4b7a7c26a7738e', class: "footerCell bottomLeftCell align-items-center preventPageScroll" }, index.h("div", { key: 'e7eb92c0a57f58f54ab349d8dc75642f8ac50841', class: "legendBtn", onClick: () => this.handleOptionEvent('showLegend') }, index.h("i", { key: 'ec7d93258618295e740ba65b90f10e28ecd4e1f0', class: "la la-square" }), index.h("u", { key: 'f29eca8c46aac405c4eb72e03884b9effbe3138d' }, locales_store.locales.entries.Lcz_Legend), index.h("span", { key: '2bc43c1bd49287626fa96f41da0301d994b018e7' }, " - v1.007"))), this.calendarData.days.map(dayInfo => (index.h("div", { class: "footerCell align-items-center" }, index.h("div", { class: `dayTitle full-height align-items-center ${dayInfo.day === this.today || this.highlightedDate === dayInfo.day ? 'currentDay' : ''}` }, dayInfo.dayDisplayName))))));
    }
};
...
const IglCalHeader = class {
    ...
    render() {
        return (index.h(index.Host, { key: '78b143f0b1196395199c22db661148b38359fa81' }, index.h("div", { key: 'ea49bf7d76e4a96f234f5e08a1fc15da7a1d8043', class: "stickyCell align-items-center topLeftCell preventPageScroll" }, index.h("div", { key: 'e22a5478247bf28aa3f35765aea57afeb1903e8b', class: "row justify-content-around no-gutters" }, !this.calendarData.is_vacation_rental && (index.h("ir-button", { key: '97edb4907640ac6ce7cc828dd3ae01e0f077c9b7', variant: "icon", icon_name: "server", style: Object.assign(Object.assign({}, icons.colorVariants.secondary), { '--icon-size': '1.5rem' }), "data-toggle": "tooltip", "data-placement": "bottom", title: locales_store.locales.entries.Lcz_UnassignedUnitsTooltip, onClickHandler: () => this.handleOptionEvent('showAssigned'), btn_styles: "caledarBtns", visibleBackgroundOnHover: true })), index.h("ir-date-picker", { key: '2f9b89721b31768ee67023c2426b540529970fbe', minDate: moment.hooks().add(-2, 'months').startOf('month').format('YYYY-MM-DD'),
            // autoApply
            // singleDatePicker
            onDateChanged: evt => {
                console.log('evt', evt);
...
const IglTbaCategoryView = class {
    ...
    render() {
        var _a;
        return (index.h(index.Host, { key: 'd5f7fa759f4dce4af8b8e5d8fce137d19c50b05e' }, index.h("div", { key: '17937baaae3e4237b40a75013c90d110fe59530f', class: "sectionContainer" }, index.h("div", { key: 'e090baa9fb3607c50b50befa93203640455941d6', class: "font-weight-bold mt-1 font-small-3" }, (_a = this.categoriesData[this.categoryId]) === null || _a === void 0 ? void 0 : _a.name), this.getEventView(this.categoryId, this.eventDatas))));
    }
};
...
const IrInteractiveTitle = class {
    ...
    render() {
        return (index.h(index.Host, { key: '87cf1e66f3f9a632348dfd3729232243003db7e6', style: { '--ir-popover-left': this.irPopoverLeft } }, index.h("p", { key: '34b783ac5cef31c24adc86ff582d24f85e764fd4', class: "popover-title", style: {
                whiteSpace: 'nowrap',
                overflow: 'hidden',
                textOverflow: 'ellipsis',
            } }, index.h("span", { key: 'abedb3b96605beb241355f5c2f7f32c6f40a678d', ref: el => (this.croppedTitleEl = el), class: "croppedTitle" }, this.croppedTitle), this.hkStatus && (index.h("div", { key: '230ad0cf2c6a3f1cbc4ac982f9b8d073bdfdc381', title: "This unit is dirty", class: `hk-dot` }, index.h("svg", { key: '90984fefa9d98ee9b4a7f262ccc0641bca5df6f2', xmlns: "http://www.w3.org/2000/svg", height: "12", width: "13.5", viewBox: "0 0 576 512" }, index.h("path", { key: 'a45400d167e5993361d34943ca28f28b1e8e48db', fill: "currentColor", d: "M566.6 54.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0l-192 192-34.7-34.7c-4.2-4.2-10-6.6-16-6.6c-12.5 0-22.6 10.1-22.6 22.6l0 29.1L364.3 320l29.1 0c12.5 0 22.6-10.1 22.6-22.6c0-6-2.4-11.8-6.6-16l-34.7-34.7 192-192zM341.1 353.4L222.6 234.9c-42.7-3.7-85.2 11.7-115.8 42.3l-8 8C76.5 307.5 64 337.7 64 369.2c0 6.8 7.1 11.2 13.2 8.2l51.1-25.5c5-2.5 9.5 4.1 5.4 7.9L7.3 473.4C2.7 477.6 0 483.6 0 489.9C0 502.1 9.9 512 22.1 512l173.3 0c38.8 0 75.9-15.4 103.4-42.8c30.6-30.6 45.9-73.1 42.3-115.8z" })))))));
    }
