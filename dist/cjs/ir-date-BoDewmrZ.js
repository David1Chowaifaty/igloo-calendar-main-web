...
function resolveCalendar(options) {
    return options?.calendar ?? index.calendarPreference.resolved;
}
...
function getWeekdayLabels(options) {
    const formatter = buildFormatter(options?.locale ?? LanguageObserver.getLang(), options?.calendar ?? index.calendarPreference.resolved, 'latn', {
...
function getMonthLabel(value, options) {
    ...
    if (!date)
        return '';
    return buildFormatter(options?.locale ?? LanguageObserver.getLang(), options?.calendar ?? index.calendarPreference.resolved, 'latn', {
