...
const ymdDate = types.stringType().regex(/^\d{4}-\d{2}-\d{2}$/, 'Expected date in YYYY-MM-DD format');
types.objectType({
    channel: types.stringType(),
    // These are null in your initialState, so allow nulls
    property_id: types.numberType().int().nullable(),
    ...
    property_ids: types.arrayType(types.numberType().int()).optional(),
});
const initialState = {
    channels: [],
    settlement_methods: [],
