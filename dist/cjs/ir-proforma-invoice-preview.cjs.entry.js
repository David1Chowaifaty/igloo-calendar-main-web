...
const IrProformaInvoicePreview = class {
    ...
    get bookingNumber() {
        if (!this.booking.is_direct) {
            return `${number.formatBookingNumber(this.booking.booking_nbr)} | ${number.formatBookingNumber(this.booking.channel_booking_nbr)}`;
        }
        return number.formatBookingNumber(this.booking.booking_nbr);
    }
    get CompanyLocation() {
    ...
    formatDisplayDate(value) {
        ...
        return irDate.formatDate(parsedDate, 'MMMM DD, YYYY');
    }
    get issueDate() {
    ...
    renderPropertyCompanyHeader() {
        ...
        return (index.h("div", { class: "invoice-company", "aria-label": t.t('Lcz_IssuingCompanyDetailsAria', { fallback: 'Issuing company details' }) }, company.name && index.h("p", { class: "invoice-company__name" }, company.name), company.address && index.h("p", { class: "invoice-company__address" }, company.address), this.CompanyLocation, company.phone && (index.h("ir-printing-label", { class: "proforma-invoice__company-details", label: `${t.t('Lcz_Phone', { fallback: 'Phone' })}:`, content: `${company.country?.phone_prefix ?? ''} ${company.phone}`.trim() })), company.tax_nbr && index.h("ir-printing-label", { class: "proforma-invoice__company-details", label: `${t.t('Lcz_TaxId', { fallback: 'Tax ID' })}:`, content: company.tax_nbr })));
    }
    renderPropertyInfo() {
    ...
    renderBillToSection() {
        ...
        if (target?.code === '002') {
            return (index.h("div", { class: "bill-to", "aria-label": t.t('Lcz_BillToCompanyAria', { fallback: 'Bill to company' }) }, company_name && index.h("p", { class: "bill-to__name" }, company_name), company_tax_nbr && index.h("p", { class: "bill-to__id" }, company_tax_nbr)));
        }
        return (index.h("div", { class: "bill-to", "aria-label": t.t('Lcz_BillToGuestAria', { fallback: 'Bill to guest' }) }, billed_to_name && index.h("p", null, billed_to_name), index.h("p", { class: "bill-to__name" }, ' ', billed_to_name && index.h("b", { class: "bill-to__connector" }, t.t('Lcz_For', { fallback: 'For' })), " ", [guest.first_name ?? '', guest.last_name ?? ''].join(' ').trim()), guest.email && index.h("p", { class: "bill-to__contact" }, guest.email), this.guestPhoneNumber && index.h("p", { class: "bill-to__contact" }, this.guestPhoneNumber)));
    }
    renderCancellationPenalty() {
        ...
        return (index.h("section", { class: "proforma-payment__section" }, index.h("div", { class: "ir-proforma-invoice__service" }, index.h("div", { class: 'ir-proforma-invoice__cancellation-info' }, index.h("p", null, t.t('Lcz_CancellationPenalty', { fallback: 'Cancellation penalty' })), index.h("p", { class: 'ir-proforma-invoice__cancellation-date' }, "( ", this.formatDisplayDate(cancellationPenalty.date), " )")), index.h("span", { class: "ir-proforma-invoice__checkbox-price" }, number.formatAmount(this.booking.currency.symbol, cancellationPenalty.amount)))));
    }
    render() {
        ...
        return (index.h(index.Host, null, index.h("article", { class: "invoice", "aria-label": t.t('Lcz_ProformaInvoice', { fallback: 'Proforma invoice' }) }, index.h("header", { class: "invoice__header" }, index.h("h3", { class: "invoice__title" }, t.t('Lcz_ProformaInvoice', { fallback: 'Proforma Invoice' })), index.h("section", { class: "invoice__layout", "aria-label": t.t('Lcz_InvoiceSummaryAria', { fallback: 'Invoice summary' }) }, index.h("div", { class: "invoice__column invoice__column--details" }, index.h("div", { class: "invoice__details" }, index.h("ir-printing-label", { label: t.t('Lcz_DateOfIssueLabel', { fallback: 'Date of issue:' }), content: this.issueDate }), index.h("ir-printing-label", { label: t.t('Lcz_BookingNoLabel', { fallback: 'Booking no:' }), content: this.bookingNumber })), billToContent && (index.h("section", { class: "bill-to-section", "aria-label": t.t('Lcz_BillTo', { fallback: 'Bill to' }) }, index.h("h4", { class: "section-heading" }, t.t('Lcz_BillTo', { fallback: 'Bill To' })), billToContent))), index.h("div", { class: "invoice__column invoice__column--property" }, companyDetails && (index.h("section", { class: "issuer-section", "aria-label": t.t('Lcz_IssuerAria', { fallback: 'Issuer' }) }, companyDetails)), propertyOverview))), index.h("main", null, existInvocableRoom && (index.h("section", { style: { marginTop: '2.5rem' } }, index.h("div", { class: "proforma__accommodation-container" }, index.h("p", { class: "proforma__accommodation-title" }, "ACCOMMODATION"), index.h("p", { class: "booking-dates" }, this.formatBookingDates(this.booking?.from_date)), index.h("p", { class: "booking-dates" }, this.formatBookingDates(this.booking?.to_date)), index.h("p", { class: "number-of-nights" }, totalNights, " ", totalNights === 1 ? t.t('Lcz_Night', { fallback: 'night' }) : t.t('Lcz_Nights', { fallback: 'nights' })), index.h("p", { class: "vat-exclusion" }, index.h("i", null, this.property?.tax_statement))), index.h("div", null, invocableRoom.map((room, idx) => {
        ...
        })))), existInvocablePickup && index.h("ir-printing-pickup", { pickup: this.booking.pickup_info }), existInvocableExtraService && (index.h("ir-printing-extra-service", { invocableKeys: this.invocableKeys, extraServices: this.booking.extra_services, currency: this.booking.currency })), this.renderCancellationPenalty(), index.h("section", { class: "proforma-payment__section" }, index.h("ir-printing-label", { label: t.t('Lcz_Balance', { fallback: 'Balance' }) + ":", content: number.formatAmount(this.booking.currency.symbol, this.booking.financial.due_amount) }), index.h("ir-printing-label", { label: t.t('Lcz_CollectedLabel', { fallback: 'Collected (payments - refunds):' }), content: number.formatAmount(this.booking.currency.symbol, this.booking.financial.collected + this.booking.financial.refunds) }))), this.footerNote && (index.h("footer", { class: "invoice__footer" }, index.h("p", { class: "invoice__footer-note" }, this.footerNote))))));
    }
    static get watchers() { return {
        "invoice": [{
                "handleInvoiceChange": 0
            }]
    }; }
};
IrProformaInvoicePreview.style = irProformaInvoicePreviewCss();
