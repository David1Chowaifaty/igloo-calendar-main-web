...
function canCheckIn({ from_date, to_date, isCheckedIn }) {
    ...
    if ((moment.hooks().isSameOrAfter(new Date(from_date), 'days') && moment.hooks().isBefore(new Date(to_date), 'days')) ||
        (moment.hooks().isSame(new Date(to_date), 'days') &&
            !compareTime(now.toDate(), createDateWithOffsetAndHour(calendarData.calendar_data.checkin_checkout_hours?.offset, calendarData.calendar_data.checkin_checkout_hours?.hour)))) {
        return true;
    }
    return false;
}
...
//# sourceMappingURL=utils-c46c34dc.js.map