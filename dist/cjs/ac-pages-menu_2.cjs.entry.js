...
const IrNotifications = class {
    ...
    render() {
        return (index.h(index.Host, { key: '3884972d96349b94bbb3227ecea916fad41ad243' }, index.h("div", { key: 'f51e9d1a6d1d4b282303e443c5eb64e0a1e33335', class: `dropdown notifications-dropdown ${this.isOpen ? 'show' : ''}` }, index.h("ir-button", { key: 'e00f2d3d8d697343696311467c563f91e38c2074', ref: el => (this.buttonRef = el), variant: "icon", icon_name: "bell", "data-notifications": this.notifications.length.toString(), class: "notification-trigger", btn_type: "button", "data-reference": "parent", "aria-expanded": String(this.isOpen), onClickHandler: () => (this.isOpen = !this.isOpen) }), index.h("div", { key: 'a3d6a8e079fb3e8a983badad4fec6d10c288235e', class: `dropdown-menu dropdown-menu-right ` }, this.notifications.length === 0 ? (index.h("p", { class: "m-0 dropdown-header" }, "All caught up.")) : (this.notifications.map(notification => (index.h("div", { class: `notification-item dropdown-item ${notification.type}`, key: notification.id }, index.h("div", { class: "notification-content" }, index.h("strong", null, notification.title), index.h("p", null, notification.message), notification.link && (index.h("a", { href: notification.link.href, target: notification.link.target || '_self' }, notification.link.text || 'View more'))), notification.dismissible && (index.h("ir-button", { onClickHandler: () => this.dismissNotification(notification), variant: "icon", btn_color: "light", icon_name: "xmark" }))))))))));
    }
