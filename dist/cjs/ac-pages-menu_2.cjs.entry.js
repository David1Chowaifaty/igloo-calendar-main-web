...
const IrNotifications = class {
    ...
    render() {
        return (index.h(index.Host, { key: '1c9b2c45d07e38e58fbfe8e8a6eb52672170487e' }, index.h("div", { key: 'a554ad28fcf98e3dd823037728672448ec7e005e', class: `dropdown notifications-dropdown ${this.isOpen ? 'show' : ''}` }, index.h("ir-button", { key: '7bdee12d7ba77cedd45f1d22d4f8b3e70284987c', ref: el => (this.buttonRef = el), variant: "icon", icon_name: "bell", "data-notifications": this.notifications.length.toString(), class: "notification-trigger", btn_type: "button", "data-reference": "parent", "aria-expanded": String(this.isOpen), onClickHandler: () => (this.isOpen = !this.isOpen) }), index.h("div", { key: 'ca90a588afae1fb024b3d80ed2cb11aa65717281', class: `dropdown-menu dropdown-menu-right ` }, this.notifications.length === 0 ? (index.h("p", { class: "m-0 dropdown-header" }, "All caught up.")) : (this.notifications.map(notification => (index.h("div", { class: `notification-item dropdown-item ${notification.type}`, key: notification.id }, index.h("div", { class: "notification-content" }, index.h("strong", null, notification.title), index.h("p", null, notification.message), notification.link && (index.h("a", { href: notification.link.href, target: notification.link.target || '_self' }, notification.link.text || 'View more'))), notification.dismissible && (index.h("ir-button", { onClickHandler: () => this.dismissNotification(notification), variant: "icon", btn_color: "light", icon_name: "xmark" }))))))))));
    }
