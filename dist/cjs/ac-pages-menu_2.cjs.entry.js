...
const IrNotifications = class {
    ...
    render() {
        return (index.h(index.Host, { key: '07ceee897240e4606f1d13e67f7f07d2971d1fbe' }, index.h("div", { key: 'aca1b142a78bd166d7a842759624d0f38002e66c', class: `dropdown notifications-dropdown ${this.isOpen ? 'show' : ''}` }, index.h("ir-button", { key: 'f24229953cb9123e2a9895fea814054cc7939f5b', ref: el => (this.buttonRef = el), variant: "icon", icon_name: "bell", "data-notifications": this.notifications.length.toString(), class: "notification-trigger", btn_type: "button", "data-reference": "parent", "aria-expanded": String(this.isOpen), onClickHandler: () => (this.isOpen = !this.isOpen) }), index.h("div", { key: 'aa9d5bf4d3085a5ddb02a26189dd1454bb020c42', class: `dropdown-menu dropdown-menu-right ` }, this.notifications.length === 0 ? (index.h("p", { class: "m-0 dropdown-header" }, "All caught up.")) : (this.notifications.map(notification => (index.h("div", { class: `notification-item dropdown-item ${notification.type}`, key: notification.id }, index.h("div", { class: "notification-content" }, index.h("strong", null, notification.title), index.h("p", null, notification.message), notification.link && (index.h("a", { href: notification.link.href, target: notification.link.target || '_self' }, notification.link.text || 'View more'))), notification.dismissible && (index.h("ir-button", { onClickHandler: () => this.dismissNotification(notification), variant: "icon", btn_color: "light", icon_name: "xmark" }))))))))));
    }
