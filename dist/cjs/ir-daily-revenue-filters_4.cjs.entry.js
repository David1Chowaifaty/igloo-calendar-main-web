...
const IrDailyRevenueFilters = class {
    ...
    render() {
        return (index.h("ir-filter-card", { key: 'f7f7ee0d02cdaa3bafa34149cffde1d66a6dc83f' }, index.h("wa-select", { key: 'f99bd132839bf90af0f81eb3ca257a692fb4c1cd', label: "Selected period", size: "s", value: this.filters?.date?.toString(), defaultValue: this.filters?.date?.toString(), onchange: (e) => {
                const value = e.target.value;
                this.updateFilter({ date: value, to_date: value, from_date: value });
            } }, this.getLast30Days().map(({ text, value }) => (index.h("wa-option", { key: value, value: value }, text)))), index.h("div", { key: '9899fa9e7af60fd55bc65096f7a07643a4f0d287', class: "or-divider" }, index.h("span", { key: '8e1556b04b580ac7b45e2b8390510666ca77f5db', class: "or-divider__line" }), index.h("span", { key: '1da0eaaec29cc37b94f37ed1da1c4c8e89366dd5', class: "or-divider__text" }, "Or"), index.h("span", { key: '0f5dcc2340f83636c3e9439b73563ac9fbf6534a', class: "or-divider__line" })), index.h("ir-date-range-filter", { key: '683ef308d8151e0eea6d0716b492ff5f2fcdf34a', showQuickActions: false, label: "Date range", fromDate: this.filters?.from_date, toDate: this.filters?.to_date, selectionMode: "auto", withClear: false, maxDate: moment.hooks().format('YYYY-MM-DD'), onDatesChanged: e => {
                e.stopImmediatePropagation();
                e.stopPropagation();
                const { from, to } = e.detail;
                this.updateFilter({ from_date: from, to_date: to, date: null });
            } }), index.h("div", { key: 'd1aa1554242d938dfe492bb7b7ae2236d5461537', slot: "footer" }, index.h("ir-custom-button", { key: 'b72124c5dbd06d37a0015d341bda659ac8c5e2f7', variant: "neutral", appearance: "outlined", onClickHandler: e => this.resetFilters(e) }, locales_store.locales.entries?.Lcz_Reset ?? 'Reset'), index.h("ir-custom-button", { key: '3185161d5991eace702f778d69307b7ea265438a', variant: "brand", loading: this.isLoading, onClickHandler: e => this.applyFiltersEvt(e) }, locales_store.locales.entries?.Lcz_Apply ?? 'Apply'))));
    }
...
const IrRevenueRow = class {
    ...
    render() {
        ...
        return (index.h(index.Host, { key: 'd1cb104eab79f754617b4f85758d29827a0ad42d' }, index.h("ir-accordion", { key: '880b55ad98fde1687acbee3720c0c395e4e09270', class: "ir-revenue-row__accordion" }, index.h("div", { key: 'daab2013b0f8cb7019f710bd5fc412c0c8797e5c', slot: "trigger", class: "ir-revenue-row__title" }, index.h("div", { key: 'a35dd9a81ede77ed664c4f9c00197a91bc90a780', class: "ir-revenue-row__header-left" }, index.h("p", { key: 'cfc31a9dc5ea38c03ed541905f70fb33326ffa3f', class: "ir-revenue-row__group" }, this.groupName, ' ', index.h("wa-badge", { key: 'e7106767edb7f2d3153797f8770aa45c5c16dbb9', variant: "brand", "aria-label": `${this.payments.length} transactions` }, this.payments.length))), index.h("p", { key: 'a9b2713fcc1dbdadf451f7da0178304ee82ac577', class: "ir-revenue-row__total" }, utils.formatAmount(calendarData.calendar_data.currency.symbol, total))), index.h("div", { key: '83bb91ea4c114fd7f369af476748b4ad4b646a34', class: "ir-revenue-row__details", id: this.contentId }, index.h("div", { key: 'b1e664982df3336d188b8a27ffdb74f8b34c5eb2', class: "ir-revenue-row__details-inner" }, this.payments.map(payment => (index.h("ir-revenue-row-details", { class: "ir-revenue-row__detail", id: payment.id, payment: payment, key: payment.id }))))))));
    }
};
...
const IrRevenueSummary = class {
    ...
    render() {
        ...
        return (index.h(index.Host, { key: '337856fc08ecc1e773d24fafb74c223333fe869c' }, index.h("div", { key: 'db954e929ea04e2bf481efe5467438be0761528c', class: "revenue-summary__row" }, index.h("ir-metric-card", { key: 'f8ddca41e79dede0ee1e04645867a7a470df6666', class: "revenue-summary__metric", icon: "arrow-trend-up", label: "Payments", value: utils.formatAmount(calendarData.calendar_data.currency.symbol, paymentsTotal), trend: hasPrevious ? paymentsTotal - previousDatePaymentsTotal : undefined, trendLabel: "from previous day", caption: hasPrevious ? `Previous day: ${utils.formatAmount(calendarData.calendar_data.currency.symbol, previousDatePaymentsTotal)}` : undefined }), index.h("ir-metric-card", { key: '15b34520f1a98936b3bec6350e9118d810f66e66', class: "revenue-summary__metric", icon: "arrow-trend-down", label: "Refunds", value: utils.formatAmount(calendarData.calendar_data.currency.symbol, refundAmount), trend: hasPrevious ? refundAmount - previousDateRefundAmount : undefined, trendLabel: "from previous day", invertTrend: true, caption: hasPrevious ? `Previous day: ${utils.formatAmount(calendarData.calendar_data.currency.symbol, previousDateRefundAmount)}` : undefined }), index.h("ir-metric-card", { key: '36d9e6876fc594e2e7e623db084c2b00ba12de06', class: "revenue-summary__metric", icon: this.getTrendIcon(totalAmount, previousDateTotalAmount) ?? 'money-bill', label: "Net Total", value: utils.formatAmount(calendarData.calendar_data.currency.symbol, totalAmount), trend: hasPrevious ? totalAmount - previousDateTotalAmount : undefined, trendLabel: "from previous day", caption: hasPrevious ? `Previous day: ${utils.formatAmount(calendarData.calendar_data.currency.symbol, previousDateTotalAmount)}` : undefined }))));
    }
};
...
const IrRevenueTable = class {
    ...
    render() {
        ...
        return (index.h("wa-card", { key: '7e456d3fe4342e4d084510a7089c5cc7c86273d4', class: "revenue-table__table" }, hasPayments ? (index.h(index.Fragment, null, index.h("div", { class: "revenue-table__header" }, index.h("p", null, "Method"), index.h("p", null, "Amount")), this.groupType === 'type' &&
