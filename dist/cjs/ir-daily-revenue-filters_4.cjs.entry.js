...
const IrDailyRevenueFilters = class {
    ...
    render() {
        return (index.h("ir-filter-card", { key: '1a880d996ab23a10b3899a7d6e1a668cefdb9a91' }, index.h("wa-select", { key: 'c452ba28a96e6edbc5af4bf63b700e41266f4516', label: "Selected period", size: "s", value: this.filters?.date?.toString(), defaultValue: this.filters?.date?.toString(), onchange: (e) => {
                const value = e.target.value;
                this.updateFilter({ date: value, to_date: value, from_date: value });
            } }, this.getLast30Days().map(({ text, value }) => (index.h("wa-option", { key: value, value: value }, text)))), index.h("div", { key: '0effcaae3a51b36f5243449dadb978720a80a256', class: "or-divider" }, index.h("span", { key: 'f5a272f0f8a4c042b0d85e854721fed55ee0e41a', class: "or-divider__line" }), index.h("span", { key: 'df641fb50a1f617e1da2a591309add3735b0e9cf', class: "or-divider__text" }, "Or"), index.h("span", { key: 'b0e4ebda5cb596aedcdcc99c5184f636db0d93e3', class: "or-divider__line" })), index.h("ir-date-range-filter", { key: 'ed47dc3d56193cc582ac970181740c796df63b1b', showQuickActions: false, label: "Date range", fromDate: this.filters?.from_date, toDate: this.filters?.to_date, selectionMode: "auto", withClear: false, maxDate: moment.hooks().format('YYYY-MM-DD'), onDatesChanged: e => {
                e.stopImmediatePropagation();
                e.stopPropagation();
                const { from, to } = e.detail;
                this.updateFilter({ from_date: from, to_date: to, date: null });
            } }), index.h("div", { key: 'ba36f66a2ca83a999460d076f7eea431c9d0d9da', slot: "footer" }, index.h("ir-custom-button", { key: '62c47ddd3fe9d6b0151f9056d0bad0679e081611', variant: "neutral", appearance: "outlined", onClickHandler: e => this.resetFilters(e) }, locales_store.locales.entries?.Lcz_Reset ?? 'Reset'), index.h("ir-custom-button", { key: '03e5307d3ee8545fa5e75b965fa8289976f3553b', variant: "brand", loading: this.isLoading, onClickHandler: e => this.applyFiltersEvt(e) }, locales_store.locales.entries?.Lcz_Apply ?? 'Apply'))));
    }
...
const IrRevenueRow = class {
    ...
    render() {
        ...
        return (index.h(index.Host, { key: 'f10e2d19f4adc5c424c78dd4928bc388f17658c2' }, index.h("ir-accordion", { key: '0991c5443b793876de46e20aefeb7e2a224ff535', class: "ir-revenue-row__accordion" }, index.h("div", { key: '7d3b524ad37f2436a887311cc0a41a67a5123ccc', slot: "trigger", class: "ir-revenue-row__title" }, index.h("div", { key: 'c7959823a53f05cb458e64fbcab503c76b45466f', class: "ir-revenue-row__header-left" }, index.h("p", { key: '11730241131c4f9ea24acadb6834383609837cd8', class: "ir-revenue-row__group" }, this.groupName, ' ', index.h("wa-badge", { key: 'ee099a7adc322744c074ee213b98ff81d72b084a', variant: "brand", "aria-label": `${this.payments.length} transactions` }, this.payments.length))), index.h("p", { key: '2485e6c147aedc45a62ce00fc6454a49025ca69b', class: "ir-revenue-row__total" }, utils.formatAmount(calendarData.calendar_data.currency.symbol, total))), index.h("div", { key: '87ce28c0d5d75e37eb1e6ac941a7ae4fc26273bc', class: "ir-revenue-row__details", id: this.contentId }, index.h("div", { key: 'fe2f6f1e676e7a05500c1a2dd4fff51d9aef4850', class: "ir-revenue-row__details-inner" }, this.payments.map(payment => (index.h("ir-revenue-row-details", { class: "ir-revenue-row__detail", id: payment.id, payment: payment, key: payment.id }))))))));
    }
};
...
const IrRevenueSummary = class {
    ...
    render() {
        ...
        return (index.h(index.Host, { key: 'c8f3f4fd897b6924ce9d2addd5a5e5b66dda4a47' }, index.h("div", { key: '9266fa2c262d99c42eef3904584feeca1d16c699', class: "revenue-summary__row" }, index.h("ir-metric-card", { key: '9569707e005b0f980419c8bc0f6c4d4ba40d08c3', class: "revenue-summary__metric", icon: "arrow-trend-up", label: "Payments", value: utils.formatAmount(calendarData.calendar_data.currency.symbol, paymentsTotal), trend: hasPrevious ? utils.calculateTrend(paymentsTotal, previousDatePaymentsTotal) : undefined, trendLabel: "from previous day", caption: hasPrevious ? `Previous day: ${utils.formatAmount(calendarData.calendar_data.currency.symbol, previousDatePaymentsTotal)}` : undefined }), index.h("ir-metric-card", { key: 'a62f7a1384d7244301c952595ed554026c754b13', class: "revenue-summary__metric", icon: "arrow-trend-down", label: "Refunds", value: utils.formatAmount(calendarData.calendar_data.currency.symbol, refundAmount), trend: hasPrevious ? utils.calculateTrend(refundAmount, previousDateRefundAmount) : undefined, trendLabel: "from previous day", invertTrend: true, caption: hasPrevious ? `Previous day: ${utils.formatAmount(calendarData.calendar_data.currency.symbol, previousDateRefundAmount)}` : undefined }), index.h("ir-metric-card", { key: 'e501ad868d0518a4fd835774c1578b1203d79470', class: "revenue-summary__metric", icon: this.getTrendIcon(totalAmount, previousDateTotalAmount) ?? 'money-bill', label: "Net Total", value: utils.formatAmount(calendarData.calendar_data.currency.symbol, totalAmount), trend: hasPrevious ? utils.calculateTrend(totalAmount, previousDateTotalAmount) : undefined, trendLabel: "from previous day", caption: hasPrevious ? `Previous day: ${utils.formatAmount(calendarData.calendar_data.currency.symbol, previousDateTotalAmount)}` : undefined }))));
    }
};
...
const IrRevenueTable = class {
    ...
    render() {
        ...
        return (index.h("wa-card", { key: 'a7e0ff87a179119037ad5c767dc7ae5db03d9c41', class: "revenue-table__table" }, hasPayments ? (index.h(index.Fragment, null, index.h("div", { class: "revenue-table__header" }, index.h("p", null, "Method"), index.h("p", null, "Amount")), this.groupType === 'type' &&
