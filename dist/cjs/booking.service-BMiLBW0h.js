...
class BookingService {
    ...
    async getCalendarData(propertyid, from_date, to_date) {
        try {
            ...
            const customMonths = [];
            const myBooking = await booking_store.getMyBookings(months);
            const days = months
                .map(month => {
                customMonths.push({
    ...
    async ackExposedRevision(props) {
        const payload = booking_store.AckExposedRevisionPropsSchema.parse(props);
    ...
    async getBookingAvailability(props) {
        try {
            ...
            if (!skip_store) {
                booking_store.booking_store.roomTypes = [...results];
                booking_store.booking_store.tax_statement = { message: data.My_Result.tax_statement };
            }
            return results;
        }
        catch (error) {
    ...
    async calculateExclusiveTax(props) {
        const payload = booking_store.CalculateExclusiveTaxPropsSchema.parse(props);
    ...
    async setDepartureTime(props) {
        const payload = booking_store.SetDepartureTimePropsSchema.parse(props);
    ...
    async getRoomsToCheckIn(props) {
        const payload = booking_store.GetRoomsToCheckInPropsSchema.parse(props);
    ...
    async getRoomsToCheckout(props) {
        const payload = booking_store.GetRoomsToCheckOutPropsSchema.parse(props);
    ...
    async getBookingInvoiceInfo(props) {
        const payload = booking_store.GetBookingInvoiceInfoPropsSchema.parse(props);
        const { data } = await axios.axios.post('/Get_Booking_Invoice_Info', payload);
        return booking_store.BookingInvoiceInfoSchema.parse(data.My_Result);
    }
    async issueInvoice(props) {
        const p = booking_store.IssueInvoicePropsSchema.parse(props);
        const { data } = await axios.axios.post('/Issue_Invoice', p);
        return data;
    }
    async voidInvoice(props) {
        const payload = booking_store.VoidInvoicePropsSchema.parse(props);
        const { data } = await axios.axios.post('/Void_Invoice', payload);
        return data;
    }
    async printInvoice(props) {
        const payload = booking_store.PrintInvoicePropsSchema.parse(props);
