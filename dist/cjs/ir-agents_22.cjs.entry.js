...
var svcCategory_utils = require('./svc-category.utils-Dhqv3eGu.js');
var enums = require('./enums-BSCnMYlE.js');
var types = require('./types-BlCoz3jZ.js');
var index$3 = require('./index-m9Y7cDOF.js');
...
var index$4 = require('./index-Cpu6aeBh.js');
...
const IrExtraServicesSettings = class {
    ...
    formatAccChargeRule(rule) {
        ...
        return `${rule.value ?? 0}% ${rule.mode === index$1.taxationModes.INCLUSIVE ? t.t('Lcz_Inclusive', { fallback: 'Inclusive' }) : t.t('Lcz_Exclusive', { fallback: 'Exclusive' })}`;
    }
...
const IrHkTasks = class {
    ...
    hkNameCache = {};
    roomService = new room_service.RoomService();
    houseKeepingService = new index$3.HouseKeepingService();
    ApiClient = new ApiClient.ApiClient();
    ...
    async init() {
        try {
            ...
            const tasksResult = await this.houseKeepingService.getHkTasks({
                ...
                housekeepers: [],
                cleaning_frequency: (calendarData.calendar_data.cleaning_frequency ?? index$3.housekeeping_store?.hk_criteria?.cleaning_frequencies[0])?.code,
                dusty_window: index$3.housekeeping_store?.hk_criteria?.dusty_periods[0]?.code,
                highlight_window: index$3.housekeeping_store?.hk_criteria?.highlight_checkin_options[0]?.code,
            });
            // updateTaskList();
    ...
    buildHousekeeperNameCache() {
        this.hkNameCache = {};
        index$3.housekeeping_store.hk_criteria?.housekeepers?.forEach(hk => {
    ...
    updateTasks(tasks) {
        ...
        const mapped = tasks.map(t => ({
            ...
            housekeeper: (() => {
                ...
                const hkName = index$3.housekeeping_store.hk_criteria?.housekeepers?.find(hk => hk.id === t.hkm_id)?.name;
...
const IrHousekeeping = class {
    ...
    async initializeApp() {
        try {
            ...
            index$3.updateHKStore('default_properties', { ApiClient: this.ticket, property_id: propertyId, language: locale_controller.LocaleController.language });
