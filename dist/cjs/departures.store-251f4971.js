...
const { state: departuresStore, onChange: onDeparturesStoreChange } = index.createStore(initialState);
...
function setDeparturesReferenceDate(date) {
    departuresStore.today = moment.hooks(date, 'YYYY-MM-DD').format('YYYY-MM-DD');
    runDeparturesPipeline();
}
...
function isNeedsCheckOut(room) {
    return utils.canCheckout({ to_date: room.to_date, inOutCode: room.in_out.code, skipAutoCheckout: true });
}
function isOut(room) {
    return room.in_out?.code === '002';
}
function isFuture(room) {
    return moment.hooks().isBefore(moment.hooks(room.to_date, 'YYYY-MM-DD'), 'date');
}
...
exports.departuresStore = departuresStore;
exports.initializeDeparturesStore = initializeDeparturesStore;
exports.onDeparturesStoreChange = onDeparturesStoreChange;
exports.setDepartureTotal = setDepartureTotal;
exports.setDeparturesPage = setDeparturesPage;
exports.setDeparturesPageSize = setDeparturesPageSize;
exports.setDeparturesReferenceDate = setDeparturesReferenceDate;
exports.setDeparturesSearchTerm = setDeparturesSearchTerm;

//# sourceMappingURL=departures.store-251f4971.js.map