...
const events_service = require('./events.service-cf8c5631.js');
...
const IglooCalendar = class {
    ...
    async initializeApp() {
        try {
            ...
            booking_store.calendar_dates.days = bookingResp.days;
            booking_store.calendar_dates.months = bookingResp.months;
            ...
            booking_store.calendar_dates.fromDate = this.calendarData.from_date;
            booking_store.calendar_dates.toDate = this.calendarData.to_date;
            setTimeout(() => {
                this.scrollToElement(this.today);
            }, 200);
    ...
    async getHousekeepingTasks({ from_date, to_date }) {
        ...
        booking_store.addCleaningTasks(tasks);
    }
            ...
            bookingEvents: [
                ...this.calendarData.bookingEvents.map(e => {
                    if (e.IDENTIFIER === result.identifier) {
                        ...
                        return { ...e, NAME: booking_store.formatName(mainGuest.first_name, mainGuest.last_name), ROOM_INFO: { ...e.ROOM_INFO, sharing_persons: result.guests } };
                    }
                    return e;
                }),
            ],
    ...
    handleRoomStatusChanged(result) {
        this.calendarData = {
            ...
            bookingEvents: [
                ...this.calendarData.bookingEvents.map(e => {
                    if (e.IDENTIFIER === result.room_identifier) {
                        const STATUS = booking_store.getRoomStatus({
    ...
    handleHkSkip(result) {
        booking_store.cleanRoom({ date: result.DATE, unitId: result.PR_ID });
    }
    handleUnitHKStatusChanged(result) {
        ...
        if (result.HKS_CODE === '002') {
            booking_store.addRoomForCleaning(roomPayload);
        }
        else {
            booking_store.cleanRoom(roomPayload);
        }
    }
    async handleDoReservation(result) {
        const transformedBooking = booking_store.transformNewBooking(result);
        this.AddOrUpdateRoomBookings(transformedBooking);
    }
    async handleBlockExposedUnit(result) {
        const transformedBooking = [await booking_store.transformNewBLockedRooms(result)];
        this.AddOrUpdateRoomBookings(transformedBooking);
    }
        ...
        this.calendarData = {
            ...
            bookingEvents: this.calendarData.bookingEvents.map(event => {
                if (result.pools.includes(event.ID)) {
                    return {
                        ...event,
                        STATUS: event.STATUS !== 'IN-HOUSE' ? booking_store.bookingStatus[result.status_code] : result.status_code === '001' ? booking_store.bookingStatus[result.status_code] : 'IN-HOUSE',
                    };
                }
                return event;
            }),
        ...
        this.calendarData = {
            ...
            bookingEvents: this.calendarData.bookingEvents.map(event => {
                if (event.BOOKING_NUMBER === result.booking_nbr) {
                    return { ...event, PRIVATE_NOTE: booking_store.getPrivateNote(result.extras) };
                }
                return event;
            }),
        };
    ...
    updateBookingEventsDateRange(eventData) {
        eventData.forEach(bookingEvent => {
            ...
            if (!utils.isBlockUnit(bookingEvent.STATUS_CODE)) {
                ...
                bookingEvent.STATUS = booking_store.getRoomStatus({
    ...
    processSalesBatch(batch) {
        const days = [...booking_store.calendar_dates.days];
        const disabled_cells = new Map(booking_store.calendar_dates.disabled_cells);
        ...
        booking_store.calendar_dates['disabled_cells'] = new Map(disabled_cells);
        booking_store.calendar_dates.days = days;
    }
    processAvailabilityBatch(batch) {
        let days = [...booking_store.calendar_dates.days];
        const disabled_cells = new Map(booking_store.calendar_dates.disabled_cells);
        ...
        if (new Date(fromDate).getTime() < new Date(this.calendarData.startingDate).getTime()) {
            ...
            bookings = bookings.filter(newBooking => {
                ...
                if (existingBookingIndex !== -1) {
                    ...
                    this.calendarData.bookingEvents[existingBookingIndex].NO_OF_DAYS = booking_store.calculateDaysBetweenDates(newBooking.FROM_DATE, this.calendarData.bookingEvents[existingBookingIndex].TO_DATE);
                    return false;
                }
                return true;
            });
            booking_store.calendar_dates.days = this.days;
            this.calendarData = {
                ...this.calendarData,
                days: this.days,
            ...
            bookings = bookings.filter(newBooking => {
                ...
                if (existingBookingIndex !== -1) {
                    ...
                    this.calendarData.bookingEvents[existingBookingIndex].NO_OF_DAYS = booking_store.calculateDaysBetweenDates(this.calendarData.bookingEvents[existingBookingIndex].FROM_DATE, newBooking.TO_DATE);
                    return false;
                }
                return true;
            });
    ...
    render() {
        ...
        return (index.h(index.Host, { key: '4aa311bcca11da01b06a2dfcd7a53c2ce9616929' }, index.h("ir-toast", { key: '8100a4b65c49815464f025780f763ab011034e02' }), index.h("ir-interceptor", { key: '09d5187b271241975b193fdf32b2ea2a400782ae' }), index.h("div", { key: '026c7e7e3e91adf5e7498d549179d72bbe3bb45d', id: "iglooCalendar", class: { 'igl-calendar': true, 'showToBeAssigned': this.showToBeAssigned, 'showLegend': this.showLegend } }, this.shouldRenderCalendarView() ? (index.h(index.Fragment, { "data-testid": "ir-calendar" }, this.showToBeAssigned && (index.h("igl-to-be-assigned", { unassignedDatesProp: this.unassignedDates, to_date: this.to_date, from_date: this.from_date, propertyid: this.property_id, class: "tobeAssignedContainer", calendarData: this.calendarData, onOptionEvent: evt => this.onOptionSelect(evt) })), this.showLegend && index.h("igl-legend", { class: "legendContainer", legendData: this.calendarData.legendData, onOptionEvent: evt => this.onOptionSelect(evt) }), index.h("div", { class: "calendarScrollContainer", onMouseDown: event => this.dragScrollContent(event), onScroll: () => this.calendarScrolling() }, index.h("div", { id: "calendarContainer" }, index.h("igl-cal-header", { unassignedDates: this.unassignedDates, to_date: this.to_date, propertyid: this.property_id, today: this.today, calendarData: this.calendarData, highlightedDate: this.highlightedDate, onOptionEvent: evt => this.onOptionSelect(evt) }), index.h("igl-cal-body", { propertyId: this.property_id, language: this.language, countries: this.countries, currency: this.calendarData.currency, today: this.today, highlightedDate: this.highlightedDate, isScrollViewDragging: this.scrollViewDragging, calendarData: this.calendarData }), index.h("igl-cal-footer", { isLegendOpen: this.showLegend, highlightedDate: this.highlightedDate, today: this.today, calendarData: this.calendarData, onOptionEvent: evt => this.onOptionSelect(evt) }))))) : (index.h("ir-loading-screen", { message: "Preparing Calendar Data" }))), index.h("ir-sidebar", { key: '385582092a43f688de8c6630bb9090fe3cd9fc23', onIrSidebarToggle: this.handleSideBarToggle.bind(this), open: this.isSidebarOpen, showCloseButton: false, sidebarStyles: {
            ...
            }, checkOut: this.bookingItem?.TO_DATE }), index.h("igl-bulk-operations-drawer", { key: 'b1259cb658981d7af92c868567084fa7f4d52d98', property_id: this.property_id, onCloseDrawer: () => (this.calendarSidebarState = null), open: this.calendarSidebarState?.type === 'bulk-blocks' }), index.h("igl-blocked-date-drawer", { key: '201688ca983e772e274f32a8c21ebd7c761c2023', onBlockedDateDrawerClosed: e => {
                e.stopImmediatePropagation();
                e.stopPropagation();
                this.bookingItem = null;
...
const IrBookingEmailLogs = class {
    ...
    render() {
        return (index.h(index.Host, { key: '93d4e3dc99ce3227d03757f4e871aba198d65c69', class: "p-1" }, index.h("ir-interceptor", { key: '1f8fec3e1c1422448f5d42693b4dec816a13924e', handledEndpoints: ['/Get_Email_log_By_BOOK_NBR'] }), index.h("ir-toast", { key: '570bfedc3fbb8ae8e6c48fa5853e9ad04ea6ac6e' }), index.h("div", { key: '374f6b606dd1bf99999aaae5e3d77435a3c46d23', class: "d-flex align-items-center mb-1", style: { gap: '0.5rem' } }, index.h("ir-input-text", { key: 'b67678a81f8d8c355a78189051add5e0d79dc70f', class: "m-0", inputContainerStyle: { margin: '0' }, value: this.bookingNumber, onTextChange: e => (this.bookingNumber = e.detail), placeholder: "booking number" }), index.h("ir-button", { key: '9deb30586e03ffc3bc7715d7e6f59c3bf908685a', size: "sm", text: "search", onClickHandler: async () => {
            ...
            } })), index.h("p", { key: '80366e07638cff725769c9aa765870f7830b7114' }, JSON.stringify(this.data, null, 2))));
    }
    static get watchers() { return {
        "ticket": ["handleTicketChange"]
    }; }
 ...
 */
const ChargeRuleSchema = index$2.z.object({
    value: index$2.z.number().nullable(),
    mode: index$2.z.string().min(1),
});
/**
 * Main setup schema
 */
index$2.z.object({
    ...
    taxationStrategy: index$2.z.nativeEnum(TaxationStrategy).nullable(),
});
