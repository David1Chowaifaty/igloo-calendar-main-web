...
const InvoicableItemSchema = index$1.objectType({
    amount: index$1.numberType(),
    booking_nbr: index$1.stringType(),
    currency: CurrencySchema,
    invoice_nbr: index$1.nullType(),
    is_invoiceable: index$1.booleanType(),
    key: index$1.nullType(),
    status: index$1.nullType(),
    system_id: index$1.nullType(),
    type: index$1.enumType(['BSA', 'BSP', 'BSE']),
});
const BookingInvoiceInfoSchema = index$1.objectType({
    invoicable_items: index$1.arrayType(InvoicableItemSchema),
    invoices: index$1.arrayType(index$1.anyType()),
});
...
class BookingService {
    ...
    async getBookingInvoiceInfo(props) {
        const { data } = await axios.axios.post('/Get_Booking_Invoice_Info', props);
...
//# sourceMappingURL=booking.service-f92f5d5a.js.map