...
const IrUnbookableRoomsFilters = class {
    ...
    render() {
        ...
        return (index.h(index.Host, { key: '8727cc283a0304da38551dc9da8ef35254526655' }, index.h("wa-card", { key: 'ea62126e2312e94053dc721e726cf0b1bfcbe800', class: "unbookable-rooms__filters-card", appearance: "outlined" }, index.h("div", { key: 'b410ca328fe78a22cf8822a41b5c9fa2f99e6278', slot: "header", class: "filters-card__header" }, index.h("wa-icon", { key: 'c33e8a643254fc279ff16232d91288c1636e1b93', name: "filter" }), index.h("h4", { key: 'af364e32807166c568c8598b9383227797b9e395', class: "filters-card__title m-0" }, "Filters")), index.h("wa-select", { key: 'd07604659f9cb5e717ecb58061cf54a62da78ec8', label: "Look ahead", size: "small", value: this.filters.period_to_check?.toString(), defaultValue: this.filters.period_to_check?.toString(), onchange: e => {
                this.handlePeriodChange(e.target.value);
            } }, Array.from({ length: 5 }, (_, i) => i + 2).map(value => (index.h("wa-option", { value: value.toString() }, value, " month", value > 1 ? 's' : '')))), index.h("ir-input", { key: 'db73d2e68ddc844f3690c21d032f9491fac677ac', type: "number", label: "Minimum consecutive nights", min: "7", hint: "Period where room types are closed for booking.", max: "60", value: this.filters.consecutive_period?.toString(), "onText-change": e => {
                this.handleConsecutiveChange(e.detail);
            } }), this.mode === 'mpo' && sortedCountries.length > 1 && (index.h("wa-select", { key: '452a2729ddd6c441ecbf46860a73920513ff1e41', label: "Country", size: "small", value: this.filters.country?.toString(), defaultValue: "all", onchange: e => {
                this.handleCountryChange(e.target.value);
            } }, index.h("wa-option", { key: '23dfc9a48b460903c786671e00e5fd7b058e017e', value: "all" }, "Show all"), sortedCountries.map(([id, name]) => (index.h("wa-option", { value: id.toString() }, name))))), index.h("div", { key: '905f84e5a4344df26fbd7345310c3f45debd5616', class: "filter__actions" }, index.h("ir-custom-button", { key: '8bfdf32ca5e062cad5bc1045ecdc6de5f04c0f9d', onClickHandler: () => this.filtersReset.emit(), variant: "neutral", appearance: "filled" }, "Reset"), index.h("ir-custom-button", { key: '2d137bf60b3c6673ab5c03f0183e7af043932f32', loading: this.isLoading, onClickHandler: () => this.filtersSave.emit(), variant: "brand" }, "Save")))));
    }
};
