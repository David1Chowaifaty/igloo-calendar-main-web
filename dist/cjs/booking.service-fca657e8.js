...
class BookingService extends Token.Token {
    ...
    async fetchPMSLogs(booking_nbr) {
        try {
            const token = this.getToken();
            if (token !== null) {
                const { data } = await axios.axios.post(`/Get_Exposed_PMS_Logs?Ticket=${token}`, { booking_nbr });
                if (data.ExceptionMsg !== '') {
                    throw new Error(data.ExceptionMsg);
                }
                return data.My_Result;
    ...
    async getBookingAvailability(props) {
        try {
            ...
            if (token) {
                ...
                const { data } = await axios.axios.post(`/Get_Exposed_Booking_Availability?Ticket=${token}`, Object.assign(Object.assign({}, rest), { adult_nbr: adultChildCount.adult, child_nbr: adultChildCount.child, currency_ref: currency.code, is_backend: true }));
                if (data.ExceptionMsg !== '') {
                    throw new Error(data.ExceptionMsg);
                }
                return data['My_Result'];
    ...
    async doReservation(body) {
        ...
        const { data } = await axios.axios.post(`/DoReservation?Ticket=${token}`, Object.assign(Object.assign({}, body), { extras: body.extras ? body.extras : utils.extras }));
        if (data.ExceptionMsg !== '') {
            throw new Error(data.ExceptionMsg);
        }
...
//# sourceMappingURL=booking.service-fca657e8.js.map