...
var index$1 = require('./index-KxiFTvIk.js');
var moment = require('./moment-CdViwxPQ.js');
var index = require('./index-CLqkDPTC.js');
var utils = require('./utils-Bb-1iFW2.js');
var axios = require('./axios-EresIryl.js');

const ymdDate = index.libExports.z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Expected date in YYYY-MM-DD format');
index.libExports.z.object({
    channel: index.libExports.z.string(),
    // These are null in your initialState, so allow nulls
    property_id: index.libExports.z.number().int().nullable(),
    balance_filter: index.libExports.z.string().nullable(),
    filter_type: index.libExports.z.union([index.libExports.z.number(), index.libExports.z.string()]).nullable(),
    from: ymdDate,
    to: ymdDate,
    name: index.libExports.z.string(),
    book_nbr: index.libExports.z.string(),
    booking_status: index.libExports.z.string(),
    userTypeCode: index.libExports.z.number().optional(),
    // In the interface these were literal 0/false, but you treat them like values.
    affiliate_id: index.libExports.z.number().int().default(0),
    is_mpo_managed: index.libExports.z.boolean().default(false),
    is_mpo_used: index.libExports.z.boolean().default(false),
    is_for_mobile: index.libExports.z.boolean().default(false),
    is_combined_view: index.libExports.z.boolean().default(false),
    start_row: index.libExports.z.number().int(),
    end_row: index.libExports.z.number().int(),
    total_count: index.libExports.z.number().int(),
    is_to_export: index.libExports.z.boolean(),
    property_ids: index.libExports.z.array(index.libExports.z.number().int()).optional(),
});
const initialState = {
    channels: [],
    settlement_methods: [],
