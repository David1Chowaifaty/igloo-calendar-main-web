...
class IglBookPropertyService {
    ...
    calculateAmount({ is_amount_modified, selected_variation, view_mode, rp_amount }) {
        ...
        if (is_amount_modified && total_days) {
                ...
                if (rateplan.reserved > 0) {
                    for (let i = 0; i < rateplan.reserved; i++) {
                        ...
                        rooms.push({
                            ...
                            unit: override_unit ? { id: unit } : guest?.unit ? { id: guest.unit } : null,
                            occupancy: {
...
const IglBookProperty = class {
    ...
    async handleSpiltBookingSelected(e) {
        ...
        if (key === 'select' || (key === 'blur' && data !== '')) {
            const res = await this.bookingService.getExposedBooking({
                booking_nbr: data.booking_nbr || data,
    ...
    handleButtonClicked(event) {
        switch (event.detail.key) {
            ...
            case 'next':
                ...
                this.toast.emit({
                    type: 'error',
                    description: t.t('Lcz_SelectRatePlan'),
                    title: t.t('Lcz_SelectRatePlan'),
                });
                break;
            case 'check':
            ...
            for (const rateplanId in roomtype) {
                ...
                if (rateplan.reserved > 0) {
                    for (const guest of rateplan.guest) {
                        types.RoomGuestSchema.parse({ ...guest, requires_bed_preference: rateplan.roomtype.is_bed_configuration_enabled });
                    }
                }
            }
        }
    ...
    async bookUser(check_in) {
        ...
        try {
            ...
            if (!this.isEventType('EDIT_BOOKING') && !this.isEventType('ADD_ROOM')) {
                types.BookingGuestSchema.parse({ first_name: this.bookedByInfoData?.firstName, last_name: this.bookedByInfoData?.lastName });
            }
