...
const ExposedAgentPropsSchema = index.z.object({
    id: index.z.coerce.number(),
});
...
const AgentBaseSchema = index.z.object({
    ...
    has_opening_balance: index.z.boolean().optional().default(false),
...
const ChargesSchema = index.z.object({
    city_tax_amount: index.z.number(),
    city_tax_percent: index.z.number(),
    net_amount: index.z.number(),
    service_charge_amount: index.z.number(),
    service_charge_percent: index.z.number(),
    tax_amount: index.z.number(),
    total_amount: index.z.number(),
    vat_amount: index.z.number(),
    vat_percent: index.z.number(),
});
const ExtraServiceSchema = index.z.object({
    ...
    description: index.z.string().optional().nullable(),
...
function canCheckIn({ from_date, to_date, isCheckedIn }) {
    if (!calendarData.calendar_data.checkin_enabled || calendarData.calendar_data.is_automatic_check_in_out || !calendarData.calendar_data.property.is_frontdesk_enabled) {
        return false;
    }
    if (isCheckedIn) {
        return false;
...
//# sourceMappingURL=utils-d712e1b8.js.map