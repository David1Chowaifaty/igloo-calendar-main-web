...
const NumberOrStringSchema = utils.unionType([utils.numberType(), utils.stringType().optional()]);
const CurrencySchema$1 = utils.objectType({
    id: utils.numberType(),
});
const CurrencyWithCodeSchema = CurrencySchema$1.extend({
    code: utils.stringType().optional(),
});
const ItemSchema$1 = utils.objectType({
    amount: utils.numberType(),
    type: utils.stringType().optional(),
    key: utils.unionType([utils.numberType(), utils.stringType().optional()]),
    description: utils.stringType().optional().optional().default(''),
});
const TargetSchema = utils.objectType({
    code: utils.stringType().optional(),
    description: utils.stringType().optional(),
});
utils.objectType({
    unit_id: utils.numberType(),
    from_date: utils.stringType().optional(),
    to_date: utils.stringType().optional(),
});
utils.objectType({
    starter: utils.stringType().optional(),
});
utils.objectType({
    booking_nbr: utils.stringType().optional(),
    currency_id: utils.numberType(),
    language: utils.stringType().optional().optional(),
    rate_plan_id: utils.numberType(),
    room_type_id: utils.numberType(),
    property_id: utils.numberType(),
    is_preserve_history: utils.booleanType().optional(),
    room_identifier: utils.stringType().optional().optional(),
});
utils.objectType({
    booking_nbr: utils.stringType().optional(),
    room_identifier: utils.stringType().optional(),
    status: utils.stringType().optional(),
});
utils.objectType({
    booking_nbr: utils.stringType().optional(),
    currency_id: utils.numberType(),
    language: utils.stringType().optional(),
});
const RestrictionSchema = utils.objectType({
    room_type_id: NumberOrStringSchema,
    night: utils.stringType().optional(),
});
utils.objectType({
    is_closed: utils.booleanType(),
    restrictions: utils.arrayType(RestrictionSchema),
    operation_type: utils.stringType().optional().optional(),
});
utils.objectType({
    book_nbr: utils.stringType().optional(),
    status: utils.stringType().optional(),
});
const AdultChildCountSchema = utils.objectType({
    adult: utils.numberType(),
    child: utils.numberType(),
});
utils.objectType({
    from_date: utils.stringType().optional(),
    to_date: utils.stringType().optional(),
    propertyid: utils.numberType(),
    adultChildCount: AdultChildCountSchema,
    language: utils.stringType().optional(),
    room_type_ids: utils.arrayType(utils.numberType()),
    room_type_ids_to_update: utils.arrayType(utils.numberType()).optional(),
    rate_plan_ids: utils.arrayType(utils.numberType()).optional(),
    currency: CurrencyWithCodeSchema,
    is_in_agent_mode: utils.booleanType().optional(),
    agent_id: NumberOrStringSchema.optional(),
});
const AvailabilityBracketSchema = utils.objectType({
    from_date: utils.stringType().optional(),
    to_date: utils.stringType().optional(),
});
utils.objectType({
    unit_id: utils.numberType(),
    block_status_code: utils.enumType(['003', '004', '002']).optional(),
    description: utils.stringType().optional().optional(),
    property_id: utils.numberType(),
    brackets: utils.arrayType(AvailabilityBracketSchema),
});
const SetDepartureTimePropsSchema = utils.objectType({
    property_id: utils.numberType(),
    room_identifier: utils.stringType().optional(),
    code: utils.stringType().optional(),
});
utils.objectType({
    service: utils.custom(),
    booking_nbr: NumberOrStringSchema,
    is_remove: utils.booleanType(),
});
/*Arrivals */
const GetRoomsToCheckInPropsSchema = utils.objectType({
    property_id: utils.stringType(),
    check_in_date: utils.stringType(),
    page_index: utils.numberType().default(1),
    page_size: utils.numberType().default(10),
});
/*Departures */
const GetRoomsToCheckOutPropsSchema = utils.objectType({
    property_id: utils.stringType(),
    check_out_date: utils.stringType(),
    page_index: utils.numberType().default(1),
    page_size: utils.numberType().default(10),
});
/* INVOICE TYPES */
const GetBookingInvoiceInfoPropsSchema = utils.objectType({
    booking_nbr: utils.stringType().optional(),
});
const VoidInvoicePropsSchema = utils.objectType({
    invoice_nbr: utils.stringType().optional(),
    reason: utils.stringType().optional(),
    property_id: utils.numberType(),
});
const InvoiceSchema$1 = utils.objectType({
    booking_nbr: utils.stringType().optional(),
    currency: CurrencySchema$1,
    target: TargetSchema,
    Date: utils.stringType().optional(),
    nbr: utils.stringType().optional(),
    remark: utils.stringType().optional(),
    billed_to_name: utils.stringType().optional(),
    billed_to_tax: utils.stringType().optional(),
    items: utils.arrayType(ItemSchema$1),
});
const IssueInvoicePropsSchema = utils.objectType({
    is_proforma: utils.booleanType().optional().default(false),
    property_id: utils.numberType(),
    invoice: InvoiceSchema$1,
});
const PrintInvoicePropsSchema = utils.objectType({
    invoice_nbr: utils.stringType().optional(),
    property_id: utils.numberType(),
    mode: utils.enumType(['invoice', 'creditnote', 'proforma']),
    invoice: InvoiceSchema$1.optional(),
});
const ExposedGuestSchema = utils.objectType({
    address: utils.nullType(),
    alternative_email: utils.nullType(),
    cci: utils.nullType(),
    city: utils.nullType(),
    country: utils.nullType(),
    country_id: utils.numberType(),
    country_phone_prefix: utils.stringType(),
    dob: utils.nullType(),
    email: utils.stringType(),
    first_name: utils.stringType(),
    id: utils.numberType(),
    id_info: utils.nullType(),
    is_main: utils.booleanType(),
    last_name: utils.stringType(),
    mobile: utils.stringType(),
    mobile_without_prefix: utils.stringType(),
    nbr_confirmed_bookings: utils.numberType(),
    notes: utils.nullType(),
    password: utils.nullType(),
    subscribe_to_news_letter: utils.nullType(),
});
utils.arrayType(ExposedGuestSchema);
const CalculateExclusiveTaxPropsSchema = utils.objectType({
    property_id: utils.numberType().min(1),
    amount: utils.numberType(),
});
const AckExposedRevisionPropsSchema = utils.objectType({
    revision_id: utils.numberType(),
});

const ZIEntrySchema = utils.z.object({
    CODE_NAME: utils.z.string(),
    CODE_VALUE_AR: utils.z.string().nullable(),
    CODE_VALUE_DE: utils.z.string().nullable(),
    CODE_VALUE_EL: utils.z.string().nullable(),
    CODE_VALUE_EN: utils.z.string().nullable(),
    CODE_VALUE_FR: utils.z.string().nullable(),
    CODE_VALUE_HE: utils.z.string().nullable(),
    CODE_VALUE_PL: utils.z.string().nullable(),
    CODE_VALUE_RU: utils.z.string().nullable(),
    CODE_VALUE_UA: utils.z.string().nullable(),
    DISPLAY_ORDER: utils.z.number().nullable(),
    ENTRY_DATE: utils.z.string().nullable(),
    ENTRY_USER_ID: utils.z.number().nullable(),
    INVARIANT_VALUE: utils.z.string().nullable(),
    ISDELETEABLE: utils.z.boolean(),
    ISDELETED: utils.z.boolean(),
    ISSYSTEM: utils.z.boolean(),
    ISUPDATEABLE: utils.z.boolean(),
    ISVISIBLE: utils.z.boolean(),
    NOTES: utils.z.string().nullable(),
    OWNER_ID: utils.z.number().nullable(),
    TBL_NAME: utils.z.string(),
});
var AmenityType;
...
const CurrencySchema = utils.objectType({
    code: utils.stringType(),
    id: utils.numberType(),
    symbol: utils.stringType(),
});
const StatusSchema = utils.objectType({
    code: utils.stringType(),
    description: utils.anyType(),
});
const ItemSchema = utils.objectType({
    amount: utils.numberType(),
    booking_nbr: utils.stringType(),
    currency: CurrencySchema,
    description: utils.anyType(),
    invoice_nbr: utils.stringType(),
    is_invoiceable: utils.booleanType(),
    key: utils.numberType(),
    status: StatusSchema,
    system_id: utils.numberType(),
    type: utils.stringType(),
});
const CreditNoteSchema = utils.objectType({
    date: utils.stringType(),
    nbr: utils.stringType(),
    reason: utils.stringType(),
    system_id: utils.stringType().nullable(),
    user: utils.stringType().nullable(),
});
const InvoiceSchema = utils.objectType({
    billed_to_name: utils.anyType(),
    billed_to_tax: utils.anyType(),
    booking_nbr: utils.stringType(),
    credit_note: CreditNoteSchema.nullable(),
    currency: CurrencySchema,
    date: utils.stringType(),
    items: utils.arrayType(ItemSchema),
    nbr: utils.stringType(),
    pdf_url: utils.anyType(),
    remark: utils.stringType(),
    status: StatusSchema,
    system_id: utils.numberType(),
    target: utils.anyType(),
    user: utils.stringType().nullable(),
    total_amount: utils.anyType(),
});
const InvoiceableItemReasonSchema = utils.objectType({
    code: utils.enumType(['001', '002', '003']),
    description: utils.stringType().nullable(),
});
const InvoiceableItemSchema = utils.objectType({
    amount: utils.numberType(),
    booking_nbr: utils.stringType(),
    currency: CurrencySchema,
    invoice_nbr: utils.stringType().nullable(),
    is_invoiceable: utils.booleanType(),
    key: utils.numberType(),
    status: utils.anyType(),
    system_id: utils.anyType(),
    reason: InvoiceableItemReasonSchema.nullable(),
    type: utils.enumType(['BSA', 'BSP', 'BSE', 'PAYMENT']),
});
const BookingInvoiceInfoSchema = utils.objectType({
    invoiceable_items: utils.arrayType(InvoiceableItemSchema),
    invoices: utils.arrayType(InvoiceSchema).nullable(),
});
...
//# sourceMappingURL=booking.service-3793345f.js.map