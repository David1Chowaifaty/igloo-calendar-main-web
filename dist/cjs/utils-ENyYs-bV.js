...
var booking_dto = require('./booking.dto-kenLHU-o.js');
...
function convertDateToCustomFormat(dayWithWeekday, monthWithYear, format = 'D_M_YYYY') {
    ...
    const date = moment.hooks(dateStr, 'DD MMM YYYY', 'en');
    if (!date.isValid()) {
...
function convertDateToTime(dayWithWeekday, monthWithYear) {
    ...
    const date = moment.hooks(dayWithWeekday + ' ' + monthWithYear, 'ddd DD MMM YYYY', 'en').toDate();
...
function checkMealPlan({ rateplan_id, roomTypes, roomTypeId }) {
    ...
    const nonRefundableLabel = t.t('Lcz_NonRefundable', { fallback: 'Non-Refundable' });
    const seen = new Set();
    const options = [];
...
function convertDatePrice(date) {
    return irDate.formatDate(date, 'DD/MM ddd');
}
...
function validateEmail(email) {
    ...
    const parsedEmailResults = index.libExports.z.string().email().safeParse(email);
...
function canCheckIn({ from_date, to_date, isCheckedIn }) {
    if (!calendarData.calendar_data.checkin_enabled || calendarData.calendar_data.property.is_automatic_check_in_out || !calendarData.calendar_data.property.is_frontdesk_enabled) {
        return false;
    }
    if (isCheckedIn) {
        return false;
    ...
    if ((now.isSameOrAfter(moment.hooks(from_date, 'YYYY-MM-DD'), 'date') && now.isBefore(moment.hooks(to_date, 'YYYY-MM-DD'), 'date')) ||
        now.isSame(moment.hooks(to_date, 'YYYY-MM-DD'), 'date')
...
function canCheckout({ to_date, inOutCode, skipAutoCheckout = false }) {
    ...
    if (inOutCode === booking_dto.ROOM_IN_OUT.CHECKOUT || inOutCode === booking_dto.ROOM_IN_OUT.IDLE) {
        return false;
    }
    if (inOutCode === booking_dto.ROOM_IN_OUT.CHECKIN) {
        return true;
    }
