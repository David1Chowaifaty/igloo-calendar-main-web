...
const IrHkStaffTasks = class {
    ...
    async loadTasks() {
        try {
            ...
            if (tasks) {
                const mapped = tasks.map((task) => ({ ...task, id: v4.v4() }));
    ...
    render() {
        ...
        return (index.h(index.Host, null, index.h("ir-hk-staff-tasks-header", { connectedHK: this.connectedHk, language: this.activeLanguage, onLanguageChanged: e => this.applyLanguage(e.detail) }), index.h("div", { class: "tasks__container" }, this.tasksByDate.map(group => (index.h("section", { key: group.date, class: "tasks__section", "aria-label": `Tasks for ${group.formattedDate}` }, index.h("header", { class: "tasks__header" }, index.h("h3", { class: "tasks__date" }, group.formattedDate), index.h("wa-badge", { pill: true, style: { fontSize: '0.875rem', fontWeight: 'bold' }, variant: group.isFuture ? 'neutral' : 'brand', appearance: group.isFuture ? 'filled' : 'accent' }, group.tasks.length)), group.tasks.length > 0 ? (index.h("div", { class: "tasks-grid", role: "list" }, group.tasks.map(task => (index.h("ir-hk-staff-task", { onTaskClick: e => {
