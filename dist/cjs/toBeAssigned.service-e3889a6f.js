...
class ToBeAssignedService {
    ...
    async assignUnit(props) {
        try {
            const { data } = await axios.axios.post(`/Assign_Exposed_Room`, Object.assign(Object.assign({}, props), { extras: calendarDates_store.extras }));
            if (data.ExceptionMsg !== '') {
                throw new Error(data.ExceptionMsg);
            }
            console.log(data);
    ...
    addDefaultDateRange(roomCategory) {
        ...
        roomCategory.defaultDateRange.fromDateStr = calendarDates_store.dateToFormattedString(roomCategory.defaultDateRange.fromDate);
        ...
        roomCategory.defaultDateRange.toDateStr = calendarDates_store.dateToFormattedString(roomCategory.defaultDateRange.toDate);
    ...
    updateAvailableRooms(calendarFromDates, room, roomCategory, formattedLegendData, roomsInfo) {
        ...
        room.assignable_units.forEach((unit) => {
            if (unit.Is_Fully_Available && !unit.Is_Not_Available) {
                const days = calendarDates_store.dateDifference(unit.from_date, unit.to_date);
...
//# sourceMappingURL=toBeAssigned.service-e3889a6f.js.map