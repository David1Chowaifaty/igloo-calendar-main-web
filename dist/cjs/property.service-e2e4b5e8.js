...
const FetchNotificationsParamsSchema = index.z.object({
    property_id: index.z.coerce.number(),
});
const FetchNotificationsResultSchema = index.z.array(index.z.object({ message: index.z.string(), type: index.z.enum(['financial', 'availability_alert']) }));
const ExposedRectifierParamsSchema = index.z.object({
    property_id: index.z.coerce.number(),
    room_type_ids: index.z.array(index.z.number()).min(1),
    from: index.z.string().refine(date => {
...
const FetchUnBookableRoomsSchema = index.z.object({
    property_ids: index.z.array(index.z.number()),
    period_to_check: index.z.coerce.number(),
    consecutive_period: index.z.coerce.number(),
});
const CategorySchema = index.z.object({
    code: index.z.string(),
...
const TaxCategorySchema = index.z.object({
    category: CategorySchema,
    taxation_mode: CategorySchema,
    pct: index.z.number(),
    property_id: index.z.number().optional(),
});
const HandleExposedPropertyTaxCategoriesParamsSchema = index.z.object({
    property_id: index.z.number(),
    VAT_INCLUDED_CODE: index.z.string(),
    VAT_PC: index.z.number(),
    CITY_TAX_INCLUDED_CODE: index.z.string(),
    CITY_TAX_PCT: index.z.number(),
    SERVICE_CHARGE_INCLUDED_CODE: index.z.string(),
    SERVICE_CHARGE_PCT: index.z.number(),
    tax_categories: index.z.array(TaxCategorySchema),
    TAXATION_STRATEGY: index.z.string(),
});
...
//# sourceMappingURL=property.service-e2e4b5e8.js.map