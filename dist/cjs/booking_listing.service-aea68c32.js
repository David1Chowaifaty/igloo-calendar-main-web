...
const ymdDate = utils.z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Expected date in YYYY-MM-DD format');
utils.z.object({
    channel: utils.z.string(),
    // These are null in your initialState, so allow nulls
    property_id: utils.z.number().int().nullable(),
    balance_filter: utils.z.string().nullable(),
    filter_type: utils.z.union([utils.z.number(), utils.z.string()]).nullable(),
    from: ymdDate,
    to: ymdDate,
    name: utils.z.string(),
    book_nbr: utils.z.string(),
    booking_status: utils.z.string(),
    userTypeCode: utils.z.number().optional(),
    // In the interface these were literal 0/false, but you treat them like values.
    affiliate_id: utils.z.number().int().default(0),
    is_mpo_managed: utils.z.boolean().default(false),
    is_mpo_used: utils.z.boolean().default(false),
    is_for_mobile: utils.z.boolean().default(false),
    is_combined_view: utils.z.boolean().default(false),
    start_row: utils.z.number().int(),
    end_row: utils.z.number().int(),
    total_count: utils.z.number().int(),
    is_to_export: utils.z.boolean(),
    property_ids: utils.z.array(utils.z.number().int()).optional(),
});
const initialState = {
    channels: [],
    settlement_methods: [],
...
//# sourceMappingURL=booking_listing.service-aea68c32.js.map