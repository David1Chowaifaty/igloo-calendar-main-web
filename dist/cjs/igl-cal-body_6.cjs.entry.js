...
const toBeAssigned_service = require('./toBeAssigned.service-ab2685bb.js');
...
const IrRoomNights = class {
    ...
    async init() {
        ...
        try {
            ...
            if (moment.hooks(this.rates[0].date).isAfter(this.fromDate)) {
                this.dates.from_date = new Date(this.fromDate);
            }
            else {
                this.dates.from_date = new Date(this.rates[0].date);
            }
            if (moment.hooks(this.rates[this.rates.length - 1].date).isBefore(this.toDate)) {
