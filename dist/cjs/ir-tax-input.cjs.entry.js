...
const taxSetupSchema = utils.z.string().min(1, 'Select a setup entry');
const IrTaxInput = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.taxChange = index.createEvent(this, "taxChange", 7);
    }
    /**
    ...
    render() {
        return (index.h(index.Host, { key: '26bc6edcc1d0769c76b1bf6ee0edacccf4563262', class: "ir-tax-input" }, index.h("ir-validator", { key: '11162ab656060806f8c4e685a6d52487eb52c704', form: "tax-service-categories__form", class: "ir-tax-input__percentage-wrapper", value: this.tax?.value ?? 40, schema: this.isTaxInputDisabled ? utils.z.number().nullable() : utils.z.coerce.number().min(0).max(30) }, index.h("ir-input", { key: 'a5ebb6d4929ffd8dee448a04b1932f6db172ce0b', disabled: this.isTaxInputDisabled, value: this.tax?.value?.toString() ?? '', mask: {
                min: 0,
                max: 30,
                mask: Number,
            }, onChange: () => {
            ...
            } }, index.h("span", { key: '53db8eeee130fcfeccae3bf98ad84caebd9e0394', slot: "end", class: "ir-tax-input__percentage-symbol" }, "%"))), index.h("ir-validator", { key: '70a6f5e426a9707ef8c61d462e46a76d45c4456e', form: "tax-service-categories__form", class: "ir-tax-input__select-wrapper", schema: taxSetupSchema, value: this.tax?.mode || '' }, index.h("wa-select", { key: 'b6d735704f28cc97cf25811190b62356acbefc1c', part: "select", class: "ir-tax-input__select", size: "small", value: this.tax?.mode, defaultValue: this.tax?.mode, onchange: e => {
                const mode = e.target.value.toString();
                this.updateTaxField({ mode });
                this.taxChange.emit(this.tax);
            }, placeholder: "Select..." }, this.setupEntries.map(entry => (index.h("wa-option", { key: entry.CODE_NAME, value: entry.CODE_NAME }, utils.getEntryValue({ entry, language: this.language }))))))));
    }
