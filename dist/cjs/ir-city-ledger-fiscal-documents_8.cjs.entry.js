...
const types = require('./types-97196cef.js');
...
const IrCityLedgerFiscalDocuments = class {
    ...
    render() {
        return (index.h(index.Host, { key: '1f6f543ae5b78ea686258ff538cf34572191e15f' }, index.h("section", { key: '977f339c3620e3fc194b2af05be46cd60909cfc4', class: "fiscal-documents", "aria-label": "City ledger fiscal documents" }, index.h("ir-city-ledger-fiscal-documents-filters", { key: '44a4d2786a2c341740aced623122d0a1193a5d38', filters: this.filters, onFiltersChange: event => {
...
const IrCityLedgerFiscalDocumentsFilters = class {
    ...
    render() {
        return (index.h("section", { key: '412923609635e4f2841591b7f047cee4aa279bba', class: "fiscal-filters", "aria-label": "Fiscal document filters" }, index.h("div", { key: 'c5fcea9aaf2846f5de9c2a18e7b950fe9e91bf63', class: "fiscal-filters__left" }, index.h("ir-date-range-filter", { key: 'ae015f5f712146241046080d95e0fd765f42bf93', class: "fiscal-filters__dates", fromDate: this.filters.fromDate, toDate: this.filters.toDate, onDatesChanged: e => this.updateFilters({ fromDate: e.detail.from, toDate: e.detail.to }) }), index.h("wa-select", { key: 'd3e93282a5b5a5240465ef23a99ed84890fed818', value: this.filters.type, defaultValue: this.filters.type, onchange: e => this.updateFilters({ type: e.target.value }), size: "small", placeholder: "Status" }, this.typeOptions.map(option => (index.h("wa-option", { value: option.value, key: option.value }, option.label)))), index.h("ir-input", { key: '0ff833f04e689fb3015657408eba5df917b751a8', placeholder: "Search", class: "fiscal-filters__search" }, index.h("wa-icon", { key: '471ccf37650619e945ffd8813de42f4228d0c11b', name: "magnifying-glass", slot: "start" })), index.h("wa-switch", { key: 'ffe438b101cd074e327d90e862f6b92d5fb8c723', id: "tax-switch", checked: this.filters.taxableOnly, onchange: event => this.updateFilters({ taxableOnly: event.target.checked }) }, "Taxes"))));
    }
};
...
const IrCityLedgerFiscalDocumentsTable = class {
    ...
    render() {
        ...
        return (index.h(index.Host, { key: '58b07bf0ca748151bc220da5096066917db60e28' }, index.h("ir-preview-screen-dialog", { key: 'fbd2cc37b1044b284ea4abadd7715eec19ec22af', ref: el => (this.previewDialogRef = el), label: this.previewRow ? `Invoice Preview — ${this.previewRow.DOC_NUMBER}` : 'Invoice Preview', action: "print", onOpenChanged: e => {
                if (!e.detail)
                    this.previewRow = null;
            } }, this.previewRow && (index.h("ir-cl-invoice-preview", { key: '2fd3223a32bc0fb2354ed940ede609fb2bfef5f9', ticket: this.ticket, propertyId: this.propertyId, agentId: this.agentId, agentName: this.previewRow.AGENCY_NAME, documentNumber: this.previewRow.DOC_NUMBER, fromDate: this.fromDate, toDate: this.toDate }))), index.h("div", { key: '1ed7576d35d6ddbfcc09e910c3050700a24a4d29', class: "table--container" }, index.h("table", { key: 'f68f6e1510fa35950a58a2a67ff2ee9d6d57e5d7', class: "table data-table" }, index.h("thead", { key: 'e7c32e85fc785a19bc567a955f78cc084990787d' }, table.getHeaderGroups().map(headerGroup => (index.h("tr", { key: headerGroup.id }, headerGroup.headers.map(header => (index.h("th", { key: header.id, class: {
            ...
            } }, useTable.flexRender(header.column.columnDef.header, header.getContext())))))))), index.h("tbody", { key: '638af07e9f81a0ab243642a8fae26544084035a6' }, table.getRowModel().rows.map(row => (index.h("tr", { key: row.id, class: "ir-table-row" }, row.getVisibleCells().map(cell => (index.h("td", { key: cell.id, class: {
            ...
            } }, useTable.flexRender(cell.column.columnDef.cell, cell.getContext()))))))), table.getRowModel().rows.length === 0 && (index.h("tr", { key: '222b2ca72aae8fe0c69e591961867d9b20a64ab1' }, index.h("td", { key: '8db0c50cc16309e9566fcb0699aa3dbb93731d5f', class: "empty-row", colSpan: this.columns.length }, this.isLoading ? (index.h("ir-spinner", null)) : this.hasDates ? ('No fiscal documents match the current filters.') : ('Please select a From and To date to view fiscal documents.')))))))));
    }
};
...
const IrCityLedgerFolioFilters = class {
    ...
    render() {
        return (index.h(index.Host, { key: 'f0cf9ab6c12662c8b63a1fc1515bb48308881caf' }, index.h("div", { key: '313a87b9f943cb1405e04aabec9da398f474c857', class: "filters-bar" }, index.h("ir-date-range-filter", { key: '16525e71ed178ca1c70e28bc5292da80b1f8982b', fromDate: this.dates.from?.format('YYYY-MM-DD'), toDate: this.dates.to?.format('YYYY-MM-DD'), onDatesChanged: e => {
                let newDates = {};
                let dates = e.detail;
                if (dates.from) {
                ...
                this.emitFilters();
            }, style: { minWidth: '230px' } }), index.h("wa-select", { key: 'b5c7343d9799271095d2c7533018fb0d90aa0898', value: this.statusFilter, onchange: e => {
                this.statusFilter = e.target.value?.toString();
                this.emitFilters();
            }, "onwa-clear": () => {
                this.statusFilter = 'all';
                this.emitFilters();
            }, placeholder: "Status", size: "small", withClear: true }, this.statuses.map(s => (index.h("wa-option", { value: s.value, label: s.label }, s.label)))), index.h("ir-input", { key: '6ba5ce98c3b44bd8bdc729aeb39199a302d88ce3', "onText-change": e => {
                this.searchQuery = e.detail;
                this.emitFiltersDebounced();
            }, value: this.searchQuery, placeholder: "Booking# or doc number", withClear: true, style: { minWidth: '250px' } }, index.h("wa-icon", { key: '74a54e2c1f35c24009d925a97e301b5d4f23cd5d', name: "magnifying-glass", slot: "start", class: "filters-bar__search-icon" })), index.h("wa-dropdown", { key: '332b094b9e431839a30b4320ea0269ea247f44b0' }, index.h("ir-custom-button", { key: 'da9d8884b445ac77f770bfdc53d9568df5f78cfe', slot: "trigger", appearance: "outlined", withCaret: true }, index.h("wa-icon", { key: 'a136f7e52a2a67dfa2a917c00fcf78c6c8f53fda', name: "download", slot: "start" }), index.h("span", { key: 'c3c8548fc91f9038cecdb79c36eab4a96c6e8613' }, "Export")), index.h("wa-dropdown-item", { key: '177f870f63006b13b3b9fdd7846c32b92d01e55d', value: "csv" }, index.h("wa-icon", { key: 'eaed8edf26b190280a46b4d15be82f2b6b0b3923', slot: "icon", name: "file-csv" }), "Export as CSV"), index.h("wa-dropdown-item", { key: 'b24788ddb3734ef24fd838dd3c91baa5bfbad9d4', value: "pdf" }, index.h("wa-icon", { key: '3f6fe5262711cbd9a3c3f814b576137b35257362', slot: "icon", name: "file-pdf" }), "Export as PDF"), index.h("wa-divider", { key: '16e7d19ff36824a5db0313e85cca5fc5bee09ce8' }), index.h("wa-dropdown-item", { key: '05ac88ce0f3b7a1728bb4c799cede5ab0fdded06', value: "print" }, index.h("wa-icon", { key: '18ea8af30f05c17a3259ff0f63406933f0775cb9', slot: "icon", name: "print" }), "Print Folio")), index.h("ir-custom-button", { key: 'b7c266e82265b7f7d4df196309210652320d8205', variant: "brand", appearance: "outlined", onClickHandler: () => this.addEntry.emit() }, "Add Entry"))));
    }
};
__decorate([
...
const IrPage = class {
    ...
    description;
    render() {
        return (index.h(index.Host, { key: 'd5a782116df73ffc03a0791a94b93b9d68d78945' }, index.h("ir-interceptor", { key: 'd1f944bbaae3d2cba3ad79b7965dbfdce24ed881' }), index.h("ir-toast", { key: '354d221c045b4b51de76945241fa7e3bd0f9da7e' }), index.h("main", { key: '60726f9190dfccd06d8a706d5783e519dcdac233', class: "ir-page__container" }, index.h("header", { key: 'd58152ec6b68d6fd48ed706fb362fffe9de9b957', class: "tax-page__header" }, index.h("slot", { key: '22195521f46f00c7864a228be2364f9a5bb60763', name: "heading" }, index.h("div", { key: 'ada0cdd0553428b2eb9ed8da0afca4f30cac205a', class: "tax-page__heading" }, index.h("h3", { key: '3f4674e402f4d1259f62d8c304167f1e320d9d66', class: "page-title" }, this.label), this.description && index.h("p", { key: '813b257055a9e12e43c35842a454173a7b1edb3c', class: "tax-page__subtitle" }, this.description))), index.h("slot", { key: '5fc2d9e643feadf90322d6c4e87d25607fd54d21', name: "page-header" })), index.h("slot", { key: '6ff052c6a727087b2ebcd2ee20a8f798462f542a' }))));
    }
};
...
//# sourceMappingURL=ir-city-ledger-fiscal-documents_8.cjs.entry.js.map