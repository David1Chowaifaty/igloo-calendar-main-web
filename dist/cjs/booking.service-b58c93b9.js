...
function transformNewBooking(data) {
    ...
    rooms.forEach(room => {
        ...
        const bookingToDate = moment.hooks(room.to_date, 'YYYY-MM-DD').isAfter(moment.hooks(calendarDates_store.calendar_dates.toDate, 'YYYY-MM-DD')) ? room.to_date : calendarDates_store.calendar_dates.toDate;
...
class BookingService extends Token.Token {
    ...
    async doReservation(body) {
        ...
        const { data } = await Token.axios.post(`/DoReservation?Ticket=${token}`, Object.assign(Object.assign({}, body), { extras: body.extras ? body.extras : extras }));
        if (data.ExceptionMsg !== '') {
            throw new Error(data.ExceptionMsg);
        }
...
//# sourceMappingURL=booking.service-b58c93b9.js.map