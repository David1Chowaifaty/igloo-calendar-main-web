...
const { state: calendar_dates, onChange: onCalendarDatesChange } = locales_store.createStore(initialState);
...
function renderBlock003Date(date, hour, minute) {
    ...
    return `${locales_store.locales.entries.Lcz_BlockedTill} ${utils.hooks(dt).format('MMM DD, HH:mm')}`;
}
function getDefaultData(cell, stayStatus) {
    var _a, _b, _c, _d, _e, _f, _g;
    if (utils.isBlockUnit(cell.STAY_STATUS_CODE)) {
        const blockedFromDate = utils.hooks(cell.My_Block_Info.from_date, 'YYYY-MM-DD').isAfter(cell.DATE) ? cell.My_Block_Info.from_date : cell.DATE;
        const blockedToDate = utils.hooks(cell.My_Block_Info.to_date, 'YYYY-MM-DD').isAfter(cell.DATE) ? cell.My_Block_Info.to_date : cell.DATE;
        return {
            ID: cell.POOL,
            NOTES: '',
            BALANCE: '',
    ...
    //   console.log(cell);
    // }
    const bookingFromDate = utils.hooks(cell.room.from_date, 'YYYY-MM-DD').isAfter(cell.DATE) ? cell.room.from_date : cell.DATE;
    const bookingToDate = utils.hooks(cell.room.to_date, 'YYYY-MM-DD').isAfter(cell.DATE) ? cell.room.to_date : cell.DATE;
...
function transformNewBooking(data) {
    ...
    const renderStatus = room => {
        ...
        if (calendarData.calendar_data.checkin_enabled) {
            ...
            const toDate = utils.hooks(room.to_date, 'YYYY-MM-DD');
            const fromDate = utils.hooks(room.from_date, 'YYYY-MM-DD');
    ...
    rooms.forEach(room => {
        var _a, _b, _c, _d;
        const bookingFromDate = utils.hooks(room.from_date, 'YYYY-MM-DD').isAfter(utils.hooks(calendar_dates.fromDate, 'YYYY-MM-DD')) ? room.from_date : calendar_dates.fromDate;
  ...
  roomToDate:${bookingToDate}\n
      `);
        if (utils.hooks(room.to_date, 'YYYY-MM-DD').isBefore(utils.hooks(calendar_dates.fromDate, 'YYYY-MM-DD'))) {
            return;
        }
        ...
        bookings.push({
            ...
            NAME: formatName(mainGuest === null || mainGuest === void 0 ? void 0 : mainGuest.first_name, mainGuest.last_name),
            PHONE: (_c = data.guest.mobile_without_prefix) !== null && _c !== void 0 ? _c : '',
            ...
            TOTAL_PRICE: (_d = data.financial) === null || _d === void 0 ? void 0 : _d.gross_total,
...
function calculateDaysBetweenDates(from_date, to_date) {
    const startDate = utils.hooks(from_date, 'YYYY-MM-DD');
    const endDate = utils.hooks(to_date, 'YYYY-MM-DD');
...
//# sourceMappingURL=booking-f0d987ca.js.map