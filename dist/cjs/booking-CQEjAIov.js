...
const { state: calendar_dates} = locales_store.createStore(initialState);
...
function renderBlock003Date(date, hour, minute) {
    ...
    return `${t.t('Lcz_BlockedTill')} ${irDate.formatDate(dt, 'MMM DD, HH:mm')}`;
}
 ...
 */
function isEarlyCheckout(room) {
    if (!room?.to_date) {
        return false;
    }
    return moment.hooks().startOf('day').isBefore(moment.hooks(room.to_date, 'YYYY-MM-DD'), 'day');
}
 ...
 */
function getDayUseUnitAvailability(calendarCell) {
    ...
    if (dayStatus === 'checkout' || dayStatus === 'turnover') {
        ...
        checkoutTime = formatClockTime(requested ?? calendarData.calendar_data.property?.time_constraints?.check_out_till);
    }
    ...
    if (dayStatus === 'checkin' || dayStatus === 'turnover') {
        ...
        checkinTime = formatClockTime(requested ?? calendarData.calendar_data.property?.time_constraints?.check_in_from);
    }
    return {
...
function formatDayUseStatusText(dayStatus, checkoutTime, checkinTime) {
 ...
 */
function createDateWithOffsetAndHour(offset, hour, minute = 0) {
    const now = new Date();
...
exports.DAY_USE_STATUS_ICON = DAY_USE_STATUS_ICON;
...
exports.formatDayUseStatusText = formatDayUseStatusText;
exports.formatName = formatName;
exports.getDayUseUnitAvailability = getDayUseUnitAvailability;
exports.getMyBookings = getMyBookings;
exports.getPrivateNote = getPrivateNote;
exports.getRoomStatus = getRoomStatus;
exports.getSplitRole = getSplitRole;
exports.isEarlyCheckout = isEarlyCheckout;
