...
var index$1 = require('./index-BJltewV-.js');
var calendarData$1 = require('./calendar-data-7gSqKDrz.js');
...
function getDefaultData(cell, stayStatusLookup) {
    if (booking.isBlockUnit(cell.STAY_STATUS_CODE)) {
        ...
        return {
            ...
            NO_OF_DAYS: booking.dateDifference(blockedFromDate, blockedToDate),
    ...
    try {
        ...
        return {
            ...
            NO_OF_DAYS: booking.dateDifference(bookingFromDate, bookingToDate),
...
class BookingService {
        ...
        try {
            ...
            const days = months
                ...
                return month.days.map(day => {
                    ...
                    return {
                        day: booking.convertDateToCustomFormat(day.description, month.description),
                        value: day.value,
                        currentDate: booking.convertDateToTime(day.description, month.description),
    ...
    async getExposedBooking(booking_nbr, language, withExtras = true) {
        try {
            const { data } = await axios.axios.post(`/Get_Exposed_Booking`, {
                booking_nbr,
                language,
                extras: withExtras ? booking.extras : null,
    ...
    async doReservation(body) {
        const { data } = await axios.axios.post(`/DoReservation`, { ...body, extras: body.extras ? body.extras : booking.extras });
        if (data.ExceptionMsg !== '') {
            throw new Error(data.ExceptionMsg);
        }
    ...
    async bookUser({ bookedByInfoData, check_in, currency, extras = null, fromDate, guestData, pickup_info, propertyid, rooms, source, toDate, totalNights, arrivalTime, bookingNumber, defaultGuest, identifier, pr_id, }) {
        try {
            const fromDateStr = booking.dateToFormattedString(fromDate);
            const toDateStr = booking.dateToFormattedString(toDate);
            let guest = {
...
function resolveAgentFromBookingSource(source) {
    const matchedSource = calendarData$1.calendar_data.property?.allowed_booking_sources?.find(s => s.id.toString() === source.id.toString());
    if (matchedSource?.type === 'TRAVEL_AGENCY') {
        const agent = calendarData$1.calendar_data.property.agents?.find(a => a.id.toString() === matchedSource.tag);
        return agent ?? null;
    }
    return null;
}
...
async function getRatePlanDisplayAmount({ rateplanSelection, totalNights, index, bookingService, variationService, }) {
    if (rateplanSelection.is_amount_modified) {
        ...
        const tax = await bookingService.calculateExclusiveTax({
            property_id: calendarData$1.calendar_data.property.id,
            amount: net,
        });
        return net + tax;
    }
...
exports.AckExposedRevisionPropsSchema = AckExposedRevisionPropsSchema;
exports.BookingInvoiceInfoSchema = BookingInvoiceInfoSchema;
exports.BookingService = BookingService;
exports.CalculateExclusiveTaxPropsSchema = CalculateExclusiveTaxPropsSchema;
exports.GetBookingInvoiceInfoPropsSchema = GetBookingInvoiceInfoPropsSchema;
exports.GetRoomsToCheckInPropsSchema = GetRoomsToCheckInPropsSchema;
exports.GetRoomsToCheckOutPropsSchema = GetRoomsToCheckOutPropsSchema;
exports.IssueInvoicePropsSchema = IssueInvoicePropsSchema;
exports.PrintInvoicePropsSchema = PrintInvoicePropsSchema;
exports.SetDepartureTimePropsSchema = SetDepartureTimePropsSchema;
exports.VariationService = VariationService;
exports.VoidInvoicePropsSchema = VoidInvoicePropsSchema;
