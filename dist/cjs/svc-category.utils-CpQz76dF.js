...
function getAccTaxPayloadFields() {
    ...
    const taxationStrategy = calendarData.calendar_data.property?.taxation_strategy?.code ?? '000';
    return {
...
const KNOWN_GROUP_LABELS = { [enums.SvcCategory.Accommodation]: 'Accommodation' };
...
function groupSvcCategoriesByParent(categories, language = 'en') {
    ...
    categories.forEach(entry => {
        ...
        groups.set(groupCode, {
            code: groupCode,
            label: groupEntry ? utils.getEntryValue({ entry: groupEntry, language }) : (KNOWN_GROUP_LABELS[groupCode] ?? groupCode),
            categories: [entry],
        });
    });
    return groups;
...
exports.findAccTax = findAccTax;
exports.getAccTaxPayloadFields = getAccTaxPayloadFields;
exports.getTopLevelSvcCategories = getTopLevelSvcCategories;
exports.groupSvcCategoriesByParent = groupSvcCategoriesByParent;
exports.toAccChargeRule = toAccChargeRule;
