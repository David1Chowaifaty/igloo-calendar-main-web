...
const IrTranslationsEntryDrawer = class {
    ...
    render() {
        ...
        return (index.h("ir-drawer", { key: '2a1996aa2c45b387b577a7002bf43aa30185ed36', label: isEditing ? 'Edit key' : 'New key', open: this.open, onDrawerHide: () => this.closeDrawer.emit() }, this.open && (index.h("ir-translations-entry-form", { key: 'c460a816c5175d25b86b0bf38f892cf57999fba3', formId: this.formId, languages: this.languages, entry: this.entry, existingKeys: this.existingKeys, nextDisplayOrder: this.nextDisplayOrder, tableName: this.tableName, ownerId: this.ownerId, entryUserId: this.entryUserId, duplicateSiblings: this.duplicateSiblings, onSubmitDisabledChange: (e) => (this.saveDisabled = e.detail), onIsSubmittingChange: (e) => (this.isSubmitting = e.detail), onEntrySaved: (e) => {
                e.stopImmediatePropagation();
                e.stopPropagation();
                this.entrySaved.emit(e.detail);
                this.closeDrawer.emit();
            } })), index.h("div", { key: 'fd11563fbf85cf30ea2fae94bd11866f5ddfb4f0', slot: "footer", class: "ir__drawer-footer" }, index.h("ir-custom-button", { key: '4f9976aafe9251e2cd8cb4be8d11836d2bee4247', size: "m", appearance: "outlined", variant: "neutral", disabled: this.isSubmitting, onClickHandler: () => this.closeDrawer.emit() }, t.t('Lcz_Cancel', { fallback: 'Cancel' })), index.h("ir-custom-button", { key: 'a53ac8077d93996e1e9a16f5c75d80f2e2787d86', size: "m", appearance: "accent", variant: "brand", form: this.formId, type: "submit", disabled: this.saveDisabled || this.isSubmitting, loading: this.isSubmitting }, t.t('Lcz_Save', { fallback: 'Save' })))));
    }
};
...
const IrTranslationsTableDialog = class {
    ...
    render() {
        ...
        return (index.h("ir-dialog", { key: 'c7430b3c65b24164d221f8e1a9252f675e43dad4', label: isEditing ? 'Table details' : 'New table', ref: el => (this.dialogRef = el), onIrDialogHide: () => this.closeDialog.emit() }, this.open && (index.h("ir-translations-table-form", { key: '2a74b6f3284277fa6e0a461d64a0e5c4c1f3ceef', formId: this.formId, mode: this.mode, table: this.table, existingNames: this.existingNames, ownerId: this.ownerId, entryUserId: this.entryUserId, onSubmitDisabledChange: (e) => (this.saveDisabled = e.detail), onIsSubmittingChange: (e) => (this.isSubmitting = e.detail), onTableSaved: (e) => {
                e.stopImmediatePropagation();
                e.stopPropagation();
                ...
                this.dialogRef?.closeModal();
            } })), index.h("div", { key: '46c48466dde2a9999c437bd981e40cb6792ec722', slot: "footer", class: "ir-dialog__footer" }, index.h("ir-custom-button", { key: '773e4c14e7ed4c6470bb91ce060fa60c1c36e73b', size: "m", appearance: "outlined", variant: "neutral", disabled: this.isSubmitting, onClickHandler: () => this.closeDialog.emit() }, t.t('Lcz_Cancel', { fallback: 'Cancel' })), index.h("ir-custom-button", { key: 'f78f704a1448aa962addb835d3f2dfb77f66f6d7', size: "m", appearance: "accent", variant: "brand", form: this.formId, type: "submit", disabled: this.saveDisabled || this.isSubmitting, loading: this.isSubmitting }, t.t('Lcz_Save', { fallback: 'Save' })))));
    }
    static get watchers() { return {
        "open": [{
                "handleOpenChange": 0
