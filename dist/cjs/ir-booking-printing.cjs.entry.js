...
const IrBookingPrinting = class {
    ...
    async initializeRequests() {
        try {
            ...
            const [property, , booking$1, fetchedCountries] = await Promise.all([
                ...
                locale_controller.LocaleController.load({ language: this.language, tables: locale_controller.SCREEN_TABLES.bookingPrinting }),
    ...
    getTaxAmount(room) {
        ...
        return filtered_data?.map((d, index$1) => {
            ...
            return (index.h(index.Fragment, null, index.h("p", { class: "label-title" }, d.is_exlusive ? 'Excluding' : 'Including', " ", d.name), index.h("p", null, d.pct, "%: ", number.formatAmount(this.currency, amount)), index$1 < filtered_data.length - 1 && index.h("span", null, "-")));
        });
    }
    render() {
