...
async function planDuplicateSync(service, { siblings, changedValues, ownerId, entryUserId, touch, }) {
    ...
    const entries = rows
        .filter((row) => row !== null)
        .map(setupMapping.setupEntryToTranslationEntry)
    ...
    const params = entries.map(sibling => setupMapping.buildEditSetupParams({
...
exports.planDuplicateSync = planDuplicateSync;
