'use strict';

Object.defineProperty(exports, '__esModule', { value: true });

const index = require('./index-07a1d3e6.js');

const IrGeneralSettings = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.sendToParent = index.createEvent(this, "sendToParent", 7);
        this.connectionOff = index.createEvent(this, "connectionOff", 7);
        this.testLoader = false;
        this.mode = undefined;
        this.allowed_channels = [];
        this.allowed_properties = [];
        this.allowed_MinStayTypes = [];
        this.connectionStatus = 'Not connected';
        this.data = {
            id: '123456',
            channel: 'Channel Name',
            status: 'Active',
            //group: 'Group',
            title: 'Title',
            property: 'Property',
            minimumStay: 'Arrival',
            hotelId: 'hotelId',
            RoomsMapping: null,
        };
        this.selectedChannel = '';
        this.connected = false;
    }
    watchHandler(newValue) {
        this.selectedChannel = newValue.channel;
    }
    modewatchHandler(newValue) {
        if (newValue === 'edit') {
            this.connected = true;
            this.connectionStatus = 'Connected';
            this.sendToParent.emit(this.data);
        }
    }
    componentDidLoad() {
        const channelSelect = document.querySelector('ir-select.channel-select');
        channelSelect.addEventListener('selectChange', (event) => {
            this.connected = false;
            this.selectedChannel = event.detail;
            this.data = Object.assign(Object.assign({}, this.data), { channel: event.detail });
            this.sendToParent.emit(this.data);
        });
        const titleInput = document.querySelector('ir-input-text#title-input');
        titleInput.addEventListener('textChange', (event) => {
            this.connected = false;
            this.connectionOff.emit();
            this.data = Object.assign(Object.assign({}, this.data), { title: event.detail });
            this.sendToParent.emit(this.data);
        });
        const propertySelect = document.querySelector('ir-select#property-select');
        propertySelect.addEventListener('selectChange', (event) => {
            this.connected = false;
            this.connectionOff.emit();
            this.data = Object.assign(Object.assign({}, this.data), { property: event.detail });
            this.sendToParent.emit(this.data);
        });
    }
    componentDidUpdate() {
        // const hotelID = document.querySelector('ir-input-text#hotel-id');
        // hotelID.addEventListener('textChange', (event: CustomEvent) => {
        //   this.connected = false;
        //   this.connectionOff.emit();
        //   this.connectionStatus = 'Not connected';
        //   this.data = {
        //     ...this.data,
        //     hotelId: event.detail.trim(),
        //   };
        // });
        // const minimumStay = document.querySelector('ir-select#minimum-stay-select');
        // minimumStay.addEventListener('selectChange', (event: CustomEvent) => {
        //   this.connected = false;
        //   this.connectionOff.emit();
        //   this.data = {
        //     ...this.data,
        //     minimumStay: event.detail.trim(),
        //   };
        // });
        const channelSelect = document.querySelector('ir-select.channel-select');
        channelSelect.addEventListener('selectChange', (event) => {
            this.connected = false;
            this.connectionOff.emit();
            this.selectedChannel = event.detail;
            this.data = Object.assign(Object.assign({}, this.data), { channel: event.detail });
            this.sendToParent.emit(this.data);
        });
        // const groupSelect = document.querySelector('ir-select#group-select');
        // groupSelect.addEventListener('selectChange', (event: CustomEvent) => {
        //   this.connected = false;
        //   this.connectionOff.emit();
        //   this.data = { ...this.data, group: event.detail };
        // });
        const titleInput = document.querySelector('ir-input-text#title-input');
        titleInput.addEventListener('textChange', (event) => {
            this.connected = false;
            this.connectionOff.emit();
            this.data = Object.assign(Object.assign({}, this.data), { title: event.detail });
            this.sendToParent.emit(this.data);
        });
        const propertySelect = document.querySelector('ir-select#property-select');
        propertySelect.addEventListener('selectChange', (event) => {
            this.connected = false;
            this.connectionOff.emit();
            this.data = Object.assign(Object.assign({}, this.data), { property: event.detail });
            this.sendToParent.emit(this.data);
        });
    }
    testConnection() {
        // check if all data is filled
        if (!this.data.hotelId) {
            const alertModal = document.querySelector('ir-modal.alertFields');
            alertModal.openModal();
        }
        else {
            this.testLoader = true;
            setTimeout(() => {
                this.testLoader = false;
                if (this.data.hotelId == '123456') {
                    this.connected = true;
                    this.connectionStatus = ' Connected';
                    this.sendToParent.emit(this.data);
                }
                else {
                    this.connected = false;
                }
            }, 1000);
        }
    }
    render() {
        return [
            index.h("div", { key: 'bd4dcda31c4b712cbebb5c500b282fd895598756', class: "General Settings font-size-small" }, index.h("div", { key: '5a42ef41c270bb39283db6b369c71a853b45fbca', class: "container-fluid" }, this.mode == 'edit' && index.h("div", { class: "text-light border-bottom-light mb-2" }, `ID ${this.data.id}`), index.h("div", { key: 'f142b02d85d4d1331db8bdc1ae6ff58c46a00ece', class: "column" }, index.h("ir-select", { key: 'b99b458b4f1d705a5f048b2b75b468f744f72725', class: "channel-select", label: "Channel", data: this.allowed_channels, "label-background": "white", "label-position": "right", "label-border": "none", size: "sm", textSize: "sm", labelWidth: 4, selectedValue: this.data !== null ? this.data.channel : null }), index.h("ir-input-text", { key: '4bccd00775f77fcc5300a6d17be0ec33b2a30df8', id: "title-input", label: "Title", placeholder: "Title", value: this.data !== null ? this.data.title : null, "label-background": "white", "label-position": "right", "label-border": "none", size: "sm", labelWidth: 4 }), index.h("ir-select", { key: '9e6feafbbf06f6071869083c2aa437f25d0c8b2b', id: "property-select", label: "Propery",
                // placeholder="Propery"
                data: this.allowed_properties, "label-background": "white", "label-position": "right", "label-border": "none", size: "sm", textSize: "sm", labelWidth: 4, selectedValue: this.data !== null ? this.data.property : null })))),
            index.h("ir-modal", { key: '11ce5e06692a69f1aef69ff579c553707ffe5262', class: "alertFields", modalTitle: "Please fill all the fields!", modalBody: "There are fields that are not filled yet.", iconAvailable: true, icon: "ft-alert-circle warning h1", leftBtnActive: false, btnPosition: "center", rightBtnText: "Close", rightBtnColor: "primary" }),
        ];
    }
    static get watchers() { return {
        "data": ["watchHandler"],
        "mode": ["modewatchHandler"]
    }; }
};

const emptyState = 'data:image/png;base64,';

const IrListItem = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.sendDelete = index.createEvent(this, "sendDelete", 7);
        this.openSidebar = index.createEvent(this, "openSidebar", 7);
        this.createNew = index.createEvent(this, "createNew", 7);
        this.changeStatus = index.createEvent(this, "changeStatus", 7);
        this.type = '';
        this.dropdownData = {
            name: 'Action',
            icon: '',
            children: [
                {
                    name: 'Edit',
                    icon: 'ft-edit',
                },
                {
                    name: 'Delete',
                    icon: 'ft-trash',
                },
                {
                    name: 'Disable',
                    icon: 'ft-alert-triangle',
                },
            ],
        };
        this.dropdownDataDisable = {
            name: 'Action',
            icon: '',
            children: [
                {
                    name: 'Edit',
                    icon: 'ft-edit',
                },
                {
                    name: 'Delete',
                    icon: 'ft-trash',
                },
                {
                    name: 'Enable',
                    icon: 'ft-check',
                },
            ],
        };
        this.listData = null;
    }
    addEventListenerToDropdown(item) {
        const dropdown = document.querySelector(`ir-dropdown.dropdown-action-${item.id}`);
        if (dropdown) {
            const eventHandler = (e) => {
                if (e.detail.name === 'Edit') {
                    this.handleCreate('edit', item);
                }
                else if (e.detail.name === 'Delete') {
                    this.onPressDelete(item);
                }
                else if (e.detail.name === 'Disable') {
                    this.onPressDisable(item);
                }
                else if (e.detail.name === 'Enable') {
                    this.onPressDisable(item);
                }
            };
            dropdown.addEventListener('dropdownItemCLicked', eventHandler);
        }
    }
    handleCreate(mode, item) {
        this.openSidebar.emit({ mode: mode, item: item });
    }
    onPressDelete(item) {
        this.type = 'delete';
        const modal = document.querySelector(`ir-modal`);
        if (modal) {
            modal.item = item;
            modal.openModal();
        }
    }
    doAction(event) {
        const item = event.detail;
        if (this.type === 'delete') {
            this.listData = this.listData.filter(data => data.id !== item.id);
            this.sendDelete.emit(this.listData);
        }
        else if (this.type === 'disable') {
            this.listData = this.listData.map(data => {
                if (data.id === item.id) {
                    data.status = 'Disabled';
                    this.changeStatus.emit(this.listData);
                }
                return data;
            });
        }
        else if (this.type === 'enable') {
            this.listData = this.listData.map(data => {
                if (data.id === item.id) {
                    data.status = 'Active';
                    this.changeStatus.emit(this.listData);
                }
                return data;
            });
        }
        const modal = document.querySelector(`ir-modal`);
        if (modal) {
            modal.closeModal();
        }
    }
    onPressDisable(item) {
        this.type = item.status === 'Active' ? 'disable' : 'enable';
        const modal = document.querySelector(`ir-modal`);
        if (modal) {
            modal.openModal();
            modal.item = item;
        }
    }
    componentDidLoad() {
        if (this.listData !== null) {
            this.listData.forEach(item => {
                this.addEventListenerToDropdown(item);
            });
        }
    }
    componentDidUpdate() {
        if (this.listData !== null) {
            this.listData.forEach(item => {
                this.addEventListenerToDropdown(item);
            });
        }
    }
    // disconnectedCallback() {
    //   this.listData.forEach(item => {
    //     const dropdown = document.querySelector(`ir-dropdown.dropdown-action-${item.id}`);
    //     if (dropdown) {
    //       dropdown.removeEventListener('dropdownItemCLicked', this.handleCreate);
    //     }
    //   });
    // }
    _renderEmptyState() {
        return (index.h("div", { class: "cardBody" }, index.h("div", { class: "emptyBody" }, index.h("img", { src: emptyState, alt: "empty", class: "img-fluid emptyImg" }), index.h("p", { class: "font-size-small" }, "You don't have any channels yet.", index.h("br", null), "It's a good time to create", index.h("a", { class: "text-primary openSidebar", onClick: () => this.createNew.emit({ mode: 'create', item: null }) }, ' ', "new")))));
    }
    _renderItem() {
        return (index.h("div", null, index.h("div", { class: "container-fluid" }, this.listData.map(item => {
            return (index.h("div", { class: "row" }, index.h("div", { class: "col-12 item-info" }, index.h("div", { class: "row" }, index.h("div", { class: "col-3 p-1" }, item.title), index.h("div", { class: "col-3 p-1" }, item.channel), index.h("div", { class: "col-3 p-1" }, item.status), index.h("div", { class: "col-3 " }, index.h("ir-dropdown", { class: `dropdown-action-${item.id}`, data: item.status === 'Active' ? this.dropdownData : this.dropdownDataDisable, object: item }))))));
        }))));
    }
    _confirmDelete() {
        return (index.h("div", { class: "row" }, index.h("div", { class: "col-2 d-flex justify-content-center align-items-center" }, index.h("ir-icon", { icon: "ft-trash danger h1" })), index.h("div", { class: "col-10" }, index.h("div", { class: "font-weight-bold" }, "Are you sure you want to delete?"), index.h("br", null), index.h("div", { class: "font-size-small" }, "What you delete here will be permanently deleted."))));
    }
    _enable_disable() {
        return (index.h("div", { class: "row" }, index.h("div", { class: "col-2 d-flex justify-content-center align-items-center" }, index.h("ir-icon", { icon: "ft-alert-circle warning h1" })), index.h("div", { class: "col-10" }, index.h("div", { class: "font-weight-bold" }, "Would you like to ", this.type, " this channel?"), index.h("br", null))));
    }
    render() {
        return [
            this.listData !== null && this.listData.length > 0 ? this._renderItem() : this._renderEmptyState(),
            index.h("ir-modal", { key: 'f867c2fdf55e40084ec5b12648f861e8b66c20fc', modalTitle: this.type === 'delete' ? 'Are you sure you want to delete?' : `Would you like to ${this.type} this channel?`, modalBody: this.type === 'delete' ? 'What you delete here will be permanently deleted.' : `This channel will be ${this.type}d.`, icon: "ft-trash warning h1", iconAvailable: true }),
        ];
    }
};

const irLoaderCss = ".lds-default.xs{scale:0.25}.lds-default.sm{scale:0.5}.lds-default.md{scale:1}.lds-default.lg{scale:1.5}.lds-default{display:inline-block;position:relative;width:80px;height:80px}.lds-default div{position:absolute;width:6px;height:6px;background:#000;border-radius:50%;animation:lds-default 1.2s linear infinite}.lds-default div:nth-child(1){animation-delay:0s;top:37px;left:66px}.lds-default div:nth-child(2){animation-delay:-0.1s;top:22px;left:62px}.lds-default div:nth-child(3){animation-delay:-0.2s;top:11px;left:52px}.lds-default div:nth-child(4){animation-delay:-0.3s;top:7px;left:37px}.lds-default div:nth-child(5){animation-delay:-0.4s;top:11px;left:22px}.lds-default div:nth-child(6){animation-delay:-0.5s;top:22px;left:11px}.lds-default div:nth-child(7){animation-delay:-0.6s;top:37px;left:7px}.lds-default div:nth-child(8){animation-delay:-0.7s;top:52px;left:11px}.lds-default div:nth-child(9){animation-delay:-0.8s;top:62px;left:22px}.lds-default div:nth-child(10){animation-delay:-0.9s;top:66px;left:37px}.lds-default div:nth-child(11){animation-delay:-1s;top:62px;left:52px}.lds-default div:nth-child(12){animation-delay:-1.1s;top:52px;left:62px}@keyframes lds-default{0%,20%,80%,100%{transform:scale(1)}50%{transform:scale(1.5)}}";
const IrLoaderStyle0 = irLoaderCss;

const IrLoader = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.size = 'md';
    }
    render() {
        return (index.h("div", { key: '10cffa1fb9d14269cf4ac499c7c6b45f21c6c419', class: `lds-default ${this.size}` }, index.h("div", { key: 'fdb8c95f00d6ee0093c30afd914445d65602902d' }), index.h("div", { key: 'cfce1a11bd6abd051a85d59f36863332d7747562' }), index.h("div", { key: 'a7d75a08a9beb3ac2ee2d41d1f0f0d3a49bab256' }), index.h("div", { key: 'a8b3b1d8b6790905bb6b9bc6ff57e6b36823fe6e' }), index.h("div", { key: '9ab98fa0b3ebb6b24fc4a3716cdbc6fabd7fd188' }), index.h("div", { key: 'cd4864a1bf985a1aaed5b94d9ee4bdf895e604a6' }), index.h("div", { key: '74a9e24e4757deaabc0de96ad8de5f7f1373ce8b' }), index.h("div", { key: '5568efba1da13864829b2a948c05a288229a1bb8' }), index.h("div", { key: '29ac9453d76f525b376085eeb587830d54571306' }), index.h("div", { key: 'fa769949fc88fcef2548883993bf4901943e9480' }), index.h("div", { key: '038cd91bf74686ed9772071b266139b54acf742c' }), index.h("div", { key: '0645f3a79ca4e4b655a54a1cf100574903b997a4' })));
    }
};
IrLoader.style = IrLoaderStyle0;

const IrMapping = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.sendMappingToParent = index.createEvent(this, "sendMappingToParent", 7);
        this.mapReference = undefined;
        this.hostRoom = [];
        this.map = null;
    }
    async _onSaveMapping() {
        this.sendMappingToParent.emit(this.hostRoom);
    }
    componentWillLoad() {
        if (this.map !== null) {
            this.hostRoom = this.map;
            this.hostRoom.forEach(room => {
                if (room.value) {
                    this.mapReference = this.mapReference.filter(map => map.id !== room.value.id);
                    room.value.ratePlans.forEach(ratePlan => {
                        if (ratePlan.value)
                            this.mapReference = this.mapReference.filter(map => map.id !== ratePlan.id);
                    });
                }
            });
        }
    }
    _onSelectMap(room, value, index) {
        room = Object.assign(Object.assign({}, room), { value: value, mapped: 'mapped', mappedName: value.name, mappedId: value.id });
        this.hostRoom = [...this.hostRoom.slice(0, index), room, ...this.hostRoom.slice(index + 1)];
        this.mapReference = this.mapReference.filter(map => map.id !== value.id);
    }
    _onSelectRatePlan(ratePlan, _index, value, room, index) {
        ratePlan = Object.assign(Object.assign({}, ratePlan), { value: value, mapped: 'mapped', mappedName: value.name, mappedId: value.id });
        this.hostRoom = [
            ...this.hostRoom.slice(0, index),
            Object.assign(Object.assign({}, room), { ratePlans: [...room.ratePlans.slice(0, _index), ratePlan, ...room.ratePlans.slice(_index + 1)] }),
            ...this.hostRoom.slice(index + 1),
        ];
        let __index = room.value.ratePlans.findIndex(rate => rate.id === value.id);
        room.value.ratePlans.splice(__index, 1);
    }
    _deselectRoom(room, index) {
        this.mapReference = [...this.mapReference, room.value];
        this.mapReference = this.mapReference.sort((a, b) => (a.id > b.id ? 1 : -1));
        this.hostRoom = [
            ...this.hostRoom.slice(0, index),
            Object.assign(Object.assign({}, room), { value: null, mapped: 'notMapped', mappedName: '', mappedId: '', ratePlans: room.ratePlans.map(ratePlan => (Object.assign(Object.assign({}, ratePlan), { mapped: 'notMapped', mappedName: '', mappedId: '' }))) }),
            ...this.hostRoom.slice(index + 1),
        ];
    }
    _deselectRatePlan(ratePlan, _index, room, index) {
        console.log(ratePlan);
        room.value.ratePlans = [...room.value.ratePlans, ratePlan.value];
        room.value.ratePlans = room.value.ratePlans.sort((a, b) => (a.id > b.id ? 1 : -1));
        ratePlan = Object.assign(Object.assign({}, ratePlan), { value: null, mapped: 'notMapped', mappedName: '', mappedId: '' });
        this.hostRoom = [
            ...this.hostRoom.slice(0, index),
            Object.assign(Object.assign({}, room), { ratePlans: [...room.ratePlans.slice(0, _index), ratePlan, ...room.ratePlans.slice(_index + 1)] }),
            ...this.hostRoom.slice(index + 1),
        ];
    }
    render() {
        return [
            index.h("div", { key: '41bc1932f483b39d91ebdd00e31cc5f2d42c9d3a', class: "Mapping font-size-small" }, index.h("div", { key: '84bffd69201b9002531d364aa3396264b223a9e2', class: "d-flex justify-content-end align-items-center" }, index.h("a", { key: '312b98e8077238091c983e7563ed4942db7b3d16', class: "text-primary" }, "Refresh")), index.h("div", { key: 'fde6c24fccdaa4bc76c175a8fa012f6eced878ff', class: "container-fluid" }, index.h("div", { key: '3085e3a106aef2efab166d762afc34bdb8eae787', class: "row" }, index.h("div", { key: 'd701a636f551a5acae0e8eb5ae50e610b10ae8fb', class: "col-12 mb-1" }, index.h("div", { key: 'd23428dbfabb35430bf2f0f25babacad19eb9162', class: "row " }, index.h("div", { key: 'f82626efc05d0796f1aa3545dae1ada0ded689ba', class: "col-6 d-flex justify-content-between align-items-center font-weight-bold" }, "Iglooroom", index.h("ir-icon", { key: '0ceaa1c07ef144e9855ca3a24cfc16a83b740eb0', icon: "la la-long-arrow-right" })), index.h("div", { key: '88f0c656447d1398f4d5dfcefdec6e9fb79d3512', class: "col-6 font-weight-bold" }, "Channel Manager"))), this.hostRoom.map((room, index$1) => {
                return (index.h("div", { class: "col-12 mb-1" }, index.h("div", { class: "row mb-1" }, index.h("div", { class: "col-6 d-flex justify-content-between align-items-center" }, room.name, index.h("ir-icon", { icon: "la la-long-arrow-right" })), index.h("div", { class: `col-6 ` }, room.mapped === 'notMapped' ? (index.h("div", { class: "text-danger", onClick: () => {
                        this.hostRoom = [
                            ...this.hostRoom.slice(0, index$1),
                            Object.assign(Object.assign({}, room), { value: '', mapped: 'mapping', mappedName: '', mappedId: '' }),
                            ...this.hostRoom.slice(index$1 + 1),
                        ];
                    } }, "Not mapped")) : room.mapped === 'mapping' ? (index.h("select", { id: `${index$1}`, class: "form-control form-control-sm", onChange: (event) => {
                        let value = JSON.parse(event.target.value);
                        this._onSelectMap(room, value, index$1);
                    } }, index.h("option", { value: "" }, "Select Room"), this.mapReference.length > 0 &&
                    this.mapReference.map(ref => {
                        return index.h("option", { value: JSON.stringify(ref) }, ref.name);
                    }))) : (index.h("div", { class: "d-flex flex-grow-1 justify-content-between" }, index.h("div", { class: "text-primary" }, room.mappedName, index.h("ir-icon", { icon: "ft-user" }), " ", room.roomCapacity), index.h("ir-icon", { icon: "text-primary ft-trash", onClick: () => {
                        this._deselectRoom(room, index$1);
                    } }))))), index.h("div", { class: "col-12 mb-1" }, room.ratePlans &&
                    room.ratePlans.length &&
                    room.ratePlans.map((ratePlan, _index) => (index.h("div", { class: "row mb-1" }, index.h("div", { class: "col-6 d-flex justify-content-between align-items-center" }, index.h("div", null, ratePlan.name, index.h("ir-icon", { icon: "ft-user" }), " ", room.roomCapacity), room.mapped === 'mapped' && index.h("ir-icon", { icon: "la la-long-arrow-right" })), index.h("div", { class: "col-6 pr-0" }, room.mapped === 'mapped' &&
                        (ratePlan.mapped === 'notMapped' ? (index.h("div", { class: "text-danger", onClick: () => {
                                ratePlan = Object.assign(Object.assign({}, ratePlan), { value: '', mapped: 'mapping', mappedId: '', mappedName: '' });
                                this.hostRoom = [
                                    ...this.hostRoom.slice(0, index$1),
                                    Object.assign(Object.assign({}, room), { ratePlans: [...room.ratePlans.slice(0, _index), ratePlan, ...room.ratePlans.slice(_index + 1)] }),
                                    ...this.hostRoom.slice(index$1 + 1),
                                ];
                            } }, "Not mapped")) : ratePlan.mapped === 'mapping' ? (index.h("select", { class: "form-control form-control-sm", onChange: (event) => {
                                // mapped.selectedPlan = event.target.value;
                                let value = JSON.parse(event.target.value);
                                this._onSelectRatePlan(ratePlan, _index, value, room, index$1);
                            } }, index.h("option", { value: "" }, "Select Plan"), room.value.ratePlans.length > 0 ? room.value.ratePlans.map(ratePlan => index.h("option", { value: JSON.stringify(ratePlan) }, ratePlan.name)) : null)) : (index.h("div", { class: "d-flex flex-grow-1 justify-content-between" }, index.h("div", { class: "text-primary" }, ratePlan.mappedName, index.h("ir-icon", { icon: "ft-user" }), " ", room.roomCapacity), index.h("ir-icon", { icon: "text-primary ft-trash", onClick: () => {
                                this._deselectRatePlan(ratePlan, _index, room, index$1);
                            } })))))))))));
            })))),
        ];
    }
};

const IrChannelManager = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.openSidebar = index.createEvent(this, "openSidebar", 7);
    }
    handleCreate() {
        this.openSidebar.emit();
    }
    render() {
        return (index.h("div", { key: '91672199f1c0545fe791d6c775590ec4c5ee5ea1', class: "card-head" }, index.h("div", { key: '2a96ab3a496ba7effca320d42cdeacb5c78521bd', class: "input-group input-group-sm" }, index.h("input", { key: '334a2a484de9c90b7c1217f5bffb6657bc5a73d8', type: "text", class: "form-control border-light", placeholder: "Search" }), index.h("div", { key: 'd37238b2316824b9114f1bb738ef89dbc900ee08', class: "input-group-append" }), index.h("button", { key: '767802ab5b7e1d0b22b39bc9a7e33d1970e5f97b', class: "ml-1 btn btn-primary btn-sm openSidebar", onClick: () => this.handleCreate() }, "Create")), index.h("div", { key: 'ba5156a15bd0127d8a849f8015d5e5ba3aa9cbed', class: "container-fluid" }, index.h("div", { key: '0a4b95f601d25427a32ea2257f70b4487403efec', class: "row" }, index.h("div", { key: '34f895176fc501a16d0c31c254a8eab856b24394', class: "col-3 p-1 section-title" }, "Title ", index.h("ir-icon", { key: '4e8eefd0ad1ac18f557ed15374d730d3276cc6b7', icon: "la la-unsorted" })), index.h("div", { key: '665626773f4b107ce5cbd3a7ae158506e2f2d529', class: "col-3 p-1 section-title" }, "Channel ", index.h("ir-icon", { key: '10188db03bd2787d7f8837f8c40ace9248f3c84b', icon: "la la-unsorted" })), index.h("div", { key: '7a49ba65a008bcba559872824f2e707aea31a3ff', class: "col-3 p-1 section-title" }, "Status ", index.h("ir-icon", { key: '428a7687be1b25add42791a0a05c293540dbf8c3', icon: "la la-unsorted" }))))));
    }
};

exports.ir_general_settings = IrGeneralSettings;
exports.ir_list_item = IrListItem;
exports.ir_loader = IrLoader;
exports.ir_mapping = IrMapping;
exports.ir_topbar = IrChannelManager;

//# sourceMappingURL=ir-general-settings_5.cjs.entry.js.map