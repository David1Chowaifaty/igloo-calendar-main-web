...
index$1.z.object({
    AC_ID: index$1.z.number(),
    NAME: index$1.z.string(),
    aname: index$1.z.string(),
    level2: index$1.z.string().nullable().optional(),
    COUNTRY_ID: index$1.z.number(),
});
const Params_Get_GHS_Candidate_Properties_Schema = index$1.z.object({
    COUNTRY_ID: index$1.z.number().nullable().optional(),
});
const Params_Generate_GHS_Listing_For_Selection_Schema = index$1.z.object({
    Selected_AC_IDs: index$1.z.array(index$1.z.number()),
});
const Params_Update_GHS_Enablement_Schema = index$1.z.object({
    AC_ID: index$1.z.number(),
    IS_ENABLED: index$1.z.boolean(),
});
...
const IrGhsOnboarding = class {
    ...
    render() {
        ...
        return (index.h(index.Host, null, index.h("ir-toast", null), index.h("ir-interceptor", null), index.h("ir-dialog", { ref: el => (this.activateModal = el), label: "Activation Confirmation", onIrDialogHide: () => {
            ...
            } }, "Confirm"))), index.h("section", { class: "ir-ghs-onboarding__container" }, index.h("div", { class: "ir-ghs-onboarding__header" }, index.h("h3", { class: "ir-ghs-onboarding__title" }, "Google hotels request")), index.h("div", { class: "ir-ghs-onboarding__layout" }, index.h("ir-ghs-filters", { countries: this.countries, selectedCountryId: this.selectedCountryId, isLoading: this.isDataLoading, onCountryChange: (e) => {
                this.selectedCountryId = e.detail;
                this.fetchProperties();
            }, onFilterApply: () => this.fetchProperties(), onFilterReset: () => {
                this.selectedCountryId = null;
                this.fetchProperties();
            } }), index.h("ir-ghs-candidate-table", { properties: this.properties, countries: this.countries, selectedCountryId: this.selectedCountryId, selectedProperties: this.selectedProperties, propertyToActivate: this.propertyToActivate, isLoading: this.isDataLoading, onToggleSelection: (e) => this.togglePropertySelection(e.detail), onToggleAll: (e) => this.handleToggleAll(e.detail), onActivateProperty: (e) => this.handleActivateProperty(e.detail) }), index.h("ir-ghs-selection-bucket", { selectedProperties: this.selectedProperties, isGenerating: this.isGenerating, onGenerateRequest: () => this.handleGenerateRequest(), onRemoveAll: () => this.handleRemoveAll(), onRemoveProperty: (e) => this.removePropertySelection(e.detail) })))));
    }
    static get watchers() { return {
        "ticket": ["ticketChanged"]
    }; }
...
const IrMealReport = class {
    ...
    render() {
        ...
        return (index.h(index.Host, null, index.h("ir-toast", null), index.h("ir-interceptor", null), index.h("section", { class: "ir-meal-report__container" }, index.h("div", { class: "ir-meal-report__header" }, index.h("h3", { class: "ir-meal-report__title" }, "Meal report"), index.h("ir-custom-button", { type: "button", size: "small", appearance: "outlined", loading: this.isExporting, onClickHandler: (e) => {
                const ev = e.detail;
                if (ev && typeof ev.preventDefault === 'function') {
                    ev.preventDefault();
                    ev.stopPropagation();
                ...
                this.mealCountSummary = [];
                await this.applyFilters();
            }, onFilterApply: () => this.applyFilters(), onFilterReset: () => this.resetFilters(), onPresetDate: e => this.setPresetDate(e.detail) }), index.h("div", { class: "ir-meal-report__results-card" }, index.h("div", { class: "ir-meal-report__results-header" }, index.h("h3", { class: "ir-meal-report__results-title" }, headerTitle, index.h("span", { class: "ir-meal-report__results-subtitle" }, "(", formattedFrom, this.localReportType === 'MEAL_COUNT' ? ` - ${formattedTo}` : '', ")", this.localReportType === 'GUEST_LIST' && mealTypeLabel && ` - ${mealTypeLabel}`)), this.localReportType === 'GUEST_LIST' && this.guestList?.length > 0 && (index.h("wa-tag", null, this.guestList.length, " Units"))), index.h("div", { class: "ir-meal-report__results-body" }, this.isDataLoading && (index.h("div", { class: "ir-meal-report__loading-overlay" }, index.h("ir-spinner", null))), this.localReportType === 'GUEST_LIST' ? (index.h("ir-meal-guest-list", { guestList: this.guestList })) : (index.h("ir-meal-count-summary", { mealCountSummary: this.mealCountSummary }))))))));
    }
    static get watchers() { return {
        "ticket": ["ticketChanged"],
 ...
 */
const ChargeRuleSchema = index$1.z.object({
    value: index$1.z.number().nullable(),
    mode: index$1.z.string().min(1),
});
/**
 * Main setup schema
 */
index$1.z.object({
    ...
    taxationStrategy: index$1.z.nativeEnum(TaxationStrategy).nullable(),
});
...
//# sourceMappingURL=ir-agents_18.cjs.entry.js.map