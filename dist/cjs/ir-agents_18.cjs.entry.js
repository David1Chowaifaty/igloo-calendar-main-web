...
var paymentOption_store = require('./payment-option.store-D8OB3t15.js');
...
const IrBookingEmailLogs = class {
    ...
    render() {
        return (index.h(index.Host, { key: '94bf269d37c281caef10c815e993e94962c01401', class: "p-1" }, index.h("ir-interceptor", { key: '97cb4de4e75dc40ae8ca3207f6a6b7bc6cb94d6b', handledEndpoints: ['/Get_Email_log_By_BOOK_NBR'] }), index.h("ir-toast", { key: '0caf3dd5f775923624e26a8ac4a8699f8466cbfb' }), index.h("div", { key: '30e98a1a2bd5bba77ca0f6c620b86555bc14f5c5', class: "d-flex align-items-center mb-1", style: { gap: '0.5rem' } }, index.h("ir-input-text", { key: '749dd91f90668d24ba09f829f4b69af92ae986c4', class: "m-0", inputContainerStyle: { margin: '0' }, value: this.bookingNumber, onTextChange: e => (this.bookingNumber = e.detail), placeholder: "booking number" }), index.h("ir-button", { key: '339cbaebee06f3d0180fb7f188ed963aa7725f7e', size: "sm", text: "search", onClickHandler: async () => {
            ...
            } })), index.h("p", { key: 'f85330bbecc1be41adce9aa90b45da8efdc9f1d4' }, JSON.stringify(this.data, null, 2))));
    }
    static get watchers() { return {
        "ticket": [{
                "handleTicketChange": 0
...
const IrMonthlyBookingsReport = class {
    ...
    render() {
        ...
        return (index.h("ir-page", { label: "Daily Occupancy" }, index.h("ir-custom-button", { variant: "neutral", appearance: "outlined", slot: "page-header", loading: this.isLoading === 'export' }, index.h("wa-icon", { name: "download", slot: "start" }), locales_store.locales.entries?.Lcz_Export), index.h("section", { class: "report-layout" }, index.h("section", null, index.h("div", { class: "report-stats-row" }, index.h("ir-metric-card", { class: "report-metric", icon: this.stats?.Occupancy_Difference_From_Previous_Month < 0 ? 'arrow-trend-down' : 'arrow-trend-up', label: "Average Occupancy", value: this.stats.AverageOccupancy ? this.stats?.AverageOccupancy.toFixed(2) : null, unit: "%", trend: this.stats?.Occupancy_Difference_From_Previous_Month, trendLabel: "from last month", caption: this.stats?.Occupancy_Difference_From_Previous_Month != null && this.stats?.AverageOccupancy != null
                ...
                : undefined }), index.h("ir-metric-card", { class: "report-metric", icon: "hotel", label: "Total Units", value: this.stats?.TotalUnitsBooked ? this.stats?.TotalUnitsBooked.toString() : null, caption: "Booked" }), index.h("ir-metric-card", { class: "report-metric", icon: "user-group", label: "Total Guests", value: this.stats?.Total_Guests ? this.stats?.Total_Guests?.toString() : null, caption: "Stayed" }), index.h("ir-metric-card", { class: "report-metric", icon: "calendar", label: "Peak Days", value: this.stats?.PeakDays.length === 0 ? null : this.stats?.PeakDays?.map(pd => moment.hooks(pd.Date, 'YYYY-MM-DD').format('D').concat('th')).join(' - '), caption: `${Math.max(...(this.stats.PeakDays?.map(pd => pd.OccupancyPercent) || []))}% occupancy` })), index.h("div", { class: "report-content-row" }, index.h("ir-monthly-bookings-report-filter", { isLoading: this.isLoading === 'filter', class: "filters-card", baseFilters: this.baseFilters }), index.h("wa-card", { class: "report-table-card" }, index.h("ir-monthly-bookings-report-table", { reports: this.reports })))))));
    }
    static get watchers() { return {
        "ticket": [{
                "handleTicketChange": 0
...
const IrSalesByChannel = class {
    ...
    render() {
        ...
        return (index.h(index.Host, null, index.h("ir-page", { label: "Sales by Channel" }, index.h("ir-custom-button", { slot: "page-header", variant: "neutral", appearance: "outlined", loading: this.isLoading === 'export', onClickHandler: async (e) => {
            ...
            } }, index.h("wa-icon", { name: "download", slot: "start" }), locales_store.locales.entries?.Lcz_Export), index.h("ir-sales-by-channel-summary", { records: this.salesData }), index.h("div", { class: "channel-content-row" }, index.h("ir-sales-by-channel-filters", { isLoading: this.isLoading === 'filter', onApplyFilters: e => {
                e.stopImmediatePropagation();
                e.stopPropagation();
