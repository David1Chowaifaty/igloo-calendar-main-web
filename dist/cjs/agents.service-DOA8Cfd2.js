...
const AgentSchema = AgentBaseSchema.superRefine((value, ctx) => {
    ...
    if (value.agent_type_code?.code === type.AgentsTypes.TOUR_OPERATOR) {
        if (value.verification_mode?.trim() !== '' && value.verification_mode !== null) {
            ctx.addIssue({
                code: index.libExports.z.ZodIssueCode.custom,
                message: `Verification mode must be empty when agent_type_code is ${type.AgentsTypes.TOUR_OPERATOR}.`,
        ...
        if (trimmedCode !== '') {
            ctx.addIssue({
                code: index.libExports.z.ZodIssueCode.custom,
                message: `Code must be empty when agent_type_code is ${type.AgentsTypes.TOUR_OPERATOR}.`,
                path: ['code'],
            });
        }
        if (trimmedQuestion !== '') {
            ctx.addIssue({
                code: index.libExports.z.ZodIssueCode.custom,
                message: `Question must be empty when agent_type_code is ${type.AgentsTypes.TOUR_OPERATOR}.`,
                path: ['question'],
            });
        }
    }
