...
const IrFiscalDocumentsFilters = class {
    ...
    render() {
        return (index.h("form", { key: 'c8b80600a70c1244247c1166d14596cc363f492a', onSubmit: e => {
                e.preventDefault();
                const submitter = e.submitter;
                this.applyFilters.emit({ ...this.draft, export: submitter?.value === 'export' });
            } }, index.h("div", { key: '1fa602942a22ff41295973494ebca33935af887e', class: "filters-bar" }, index.h("ir-validator", { key: '8d73ea7ff6a1328c4902ea2a735bc2b0506cb995', value: this.draft?.fromDate || this.draft?.toDate, schema: index$1.libExports.z.string().nonempty(), class: "filters-bar__dates" }, index.h("ir-date-range-filter", { key: 'f75324e654ea2fb7b65707a8ef9eb3667dc34ae9', maxDate: today.format('YYYY-MM-DD'), class: "filters-bar__date_picker", fromDate: this.draft.fromDate, toDate: this.draft.toDate, onDatesChanged: e => {
            ...
            } })), index.h("div", { key: '2ee9721b53b6c45969f45550d1e88c7c2e99e487', class: "filters-bar__search-group" }, index.h("div", { key: '4b847b774b81919b557dd7611913541505a2cf05', class: "filters-bar__type-group" }, index.h("wa-select", { key: '6c8c2fe31bcdc2ac006fe17a121a1c3703d221aa', class: "filters-bar__status-select", value: this.draft.type, defaultValue: this.draft.type, onchange: e => this.updateDraft({ type: e.target.value }), size: "s", placeholder: "Document Type" }, this.typeOptions.map(option => (index.h("wa-option", { value: option.value, key: option.value }, option.label)))), index.h("wa-select", { key: '78a9c89e87fc17d68dadeb4bd7608065055d88bd', class: "filters-bar__status-select", value: this.draft.folioType, defaultValue: this.draft.folioType, onchange: e => this.handleFolioTypeChange(e.target.value), size: "s", placeholder: "Folios" }, this.folioOptions.map(option => (index.h("wa-option", { value: option.value, key: option.value }, option.label))))), this.draft.folioType === 'agent' && (index.h("ir-autocomplete", { key: '05d6d53fd46c39a5724ea786cb648197505eac53', class: "filters-bar__folio-select", size: "s", placeholder: "Select agent", withExpandIcon: true, value: this.draft.agentId ? (this.agents.find(a => a.id === this.draft.agentId)?.name ?? '') : ALL_AGENTS_LABEL, "onText-change": (e) => {
                this.agentSearch = e.detail ?? '';
            }, "onCombobox-change": (e) => {
                this.agentSearch = '';
                const value = e.detail;
                this.updateDraft({ agentId: value && value !== ALL_AGENTS_VALUE ? Number(value) : null });
            } }, index.h("ir-autocomplete-option", { key: '6cb4f9596fe03b3064e91077184914e2a8325667', label: ALL_AGENTS_LABEL, value: ALL_AGENTS_VALUE }, ALL_AGENTS_LABEL), this.filteredAgents.map(agent => (index.h("ir-autocomplete-option", { key: agent.id, label: agent.name, value: String(agent.id) }, agent.name))))), this.draft.folioType === 'guest' && (index.h("ir-picker", { key: '592ff0af4140c233e45394cf7826c5a14439bcb0', class: "filters-bar__folio-select", size: "s", placeholder: "Customer email or name", withClear: true, mode: "select-async", debounce: 500, loading: irInterceptor_store.isRequestPending('/Fetch_Exposed_Guests'), "onText-change": event => this.fetchGuests(event.detail), "onCombobox-select": this.handleGuestSelect.bind(this), "onCombobox-clear": () => {
        ...
        }))), index.h("div", { key: '878934a0b4d8ff708863e959c05ab5bbe8f85e66', class: `filters-bar__search-actions${this.draft.folioType === 'guest' ? ' filters-bar__search-actions--wide' : ''}` }, index.h("div", { key: 'ca56851cf0cd9200637c74e3bf6b542781cf7bd1', class: "filters-bar__search-combo" }, index.h("ir-input", { key: 'be4f0514137eb8885b3c0e2d660fb85bc8364af4', class: `filters-bar__search-input${this.draft.folioType === 'guest' ? ' filters-bar__combo-input' : ''}`, placeholder: this.searchPlaceholder, value: this.draft.docNumber, "onText-change": e => {
            ...
            } }, index.h("wa-icon", { key: '600e048b7fb58f6cafe8701db93c39bb55d1a256', name: "magnifying-glass", slot: "start", class: "filters-bar__search-icon" })), this.draft.folioType === 'guest' && (index.h("wa-select", { key: 'c5f47b8dfdb68e22c020fbab4815be0de8e3209d', class: "filters-bar__combo-select", size: "s", value: this.draft.searchBy, defaultValue: this.draft.searchBy, onchange: e => this.updateDraft({ searchBy: e.target.value }) }, index.h("wa-option", { key: 'ad004c70ce7bdf6e860ce4b5c3f815aabc7454d6', value: "doc_nbr" }, "Document number"), index.h("wa-option", { key: '5df9b79ab8045c0df9daab3ad7ebb7d42e32dba3', value: "booking_nbr" }, "Booking number")))), index.h("wa-tooltip", { key: '6cf10759948b1654d20ff4aeeb1e32883c155f77', for: "search-btn" }, "Search"), index.h("ir-custom-button", { key: '56adb2971c007db295f00baad00d0fb6a85857ee', id: "search-btn", loading: this.loading === 'search', class: "filters-bar__search-submit", value: "search", variant: "neutral", appearance: "outlined", type: "submit" }, index.h("wa-icon", { key: 'a46df7ac63295809736bbc6c3ccba6788c534ae3', name: "magnifying-glass" })), index.h("wa-tooltip", { key: '2f0b3c0ff9050fec01cd834335b5e80056735f49', for: "excel-btn" }, 'Export to excel'), index.h("ir-custom-button", { key: '021f83372deb746c791c25baf73ac6a0db200cb9', disabled: !(this.draft?.fromDate || this.draft?.toDate), id: "excel-btn", variant: "neutral", loading: this.loading === 'export', appearance: "outlined", type: "submit", value: "export" }, index.h("wa-icon", { key: '10c1207e88400473565d2757b0793629fde44766', name: "file-excel", variant: "regular" })))))));
    }
