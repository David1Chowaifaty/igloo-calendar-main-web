...
const IrFiscalDocumentsFilters = class {
    ...
    render() {
        return (index.h("form", { key: '98207922dee96984f3f37d045445d28129323dbc', onSubmit: e => {
            ...
            } }, index.h("div", { key: '5a863fca948527e6c6201863f02f492462b16af1', class: "filters-bar" }, index.h("ir-validator", { key: '8f1360d47f233b76e054679046c5e25020e7fec7', value: this.filters?.fromDate || this.filters?.toDate, schema: index$2.libExports.z.string().nonempty(), class: "filters-bar__dates" }, index.h("ir-date-range-filter", { key: 'ef91f1f4324ff77dc7e9246bba16039210c4b387', maxDate: today.format('YYYY-MM-DD'), class: "filters-bar__date_picker", fromDate: this.filters.fromDate, toDate: this.filters.toDate, onDatesChanged: e => this.updateFilters({ fromDate: e.detail.from, toDate: e.detail.to }) })), index.h("div", { key: '76bf96e4f92028b72444419e89300ff76e9d7bec', class: "filters-bar__search-group" }, index.h("div", { key: '2804cea30228139d0e004eba5c3fc54ab321b084', class: "filters-bar__type-group" }, index.h("wa-select", { key: 'ca3134026af0906a36ec4efbc0c07c66a8fa48fc', class: "filters-bar__status-select", value: this.filters.type, defaultValue: this.filters.type, onchange: e => this.updateFilters({ type: e.target.value }), size: "s", placeholder: "Document Type" }, this.typeOptions.map(option => (index.h("wa-option", { value: option.value, key: option.value }, option.label)))), index.h("wa-select", { key: '23a2948930d1106fe1ebbcf8606d15ec0cf48109', class: "filters-bar__status-select", value: this.filters.folioType, defaultValue: this.filters.folioType, onchange: e => this.handleFolioTypeChange(e.target.value), size: "s", placeholder: "Folios" }, this.folioOptions.map(option => (index.h("wa-option", { value: option.value, key: option.value }, option.label)))), index.h("wa-switch", { key: '99851bc7be76aac903399751b5348e7f676a5d3e', class: "filters-bar__tax-switch", checked: this.filters.taxableOnly, onchange: e => this.updateFilters({ taxableOnly: e.target.checked }) }, "Taxes")), this.filters.folioType === 'agent' && (index.h("ir-autocomplete", { key: '36471d10c692695d88702149c710c19d73024817', class: "filters-bar__folio-select", size: "s", placeholder: "Select agent", value: this.filters.agentId ? (this.agents.find(a => a.id === this.filters.agentId)?.name ?? '') : ALL_AGENTS_LABEL, "onText-change": (e) => {
                this.agentSearch = e.detail ?? '';
            }, "onCombobox-change": (e) => {
                this.agentSearch = '';
                this.updateFilters({ agentId: e.detail && e.detail !== ALL_AGENTS_VALUE ? Number(e.detail) : null });
            } }, index.h("ir-autocomplete-option", { key: '0625704c0c6c1bd814d4915dc8da86244f565f14', label: ALL_AGENTS_LABEL, value: ALL_AGENTS_VALUE }, ALL_AGENTS_LABEL), this.filteredAgents.map(agent => (index.h("ir-autocomplete-option", { key: agent.id, label: agent.name, value: String(agent.id) }, agent.name))))), this.filters.folioType === 'guest' && (index.h("ir-picker", { key: '7964778d2d35ddea42531930a3174c1e8b8e4b04', class: "filters-bar__folio-select", size: "s", placeholder: "Search customer by email or name", withClear: true, mode: "select-async", debounce: 500, loading: irInterceptor_store.isRequestPending('/Fetch_Exposed_Guests'), "onText-change": event => this.fetchGuests(event.detail), "onCombobox-select": this.handleGuestSelect.bind(this), "onCombobox-clear": () => this.updateFilters({ guestId: null }) }, this.guests?.map(guest => {
            const label = `${guest.email} - ${guest.first_name} ${guest.last_name}`;
            return (index.h("ir-picker-item", { label: label, value: guest.id?.toString(), key: guest.id }, label));
        }))), index.h("div", { key: '07c9e422dd9b39161ea0bed168d495beae2edc9a', class: "filters-bar__search-actions" }, index.h("ir-input", { key: '79b228d04338ca30fae975ff3d0398049dfb18c3', class: "filters-bar__search-input", placeholder: this.searchPlaceholder, value: this.docNumber, "onText-change": e => {
                this.docNumber = e.detail;
                this.emitSearchDebounced(e.detail);
            }, withClear: true }, index.h("wa-icon", { key: 'bdffa66688fad1c105ee2a1b7ce216e5e359df3e', name: "magnifying-glass", slot: "start", class: "filters-bar__search-icon" })), index.h("ir-custom-button", { key: 'dead55c35e2dc921b52d559108dc1dcccfa593fe', class: "filters-bar__search-submit", variant: "neutral", appearance: "outlined", type: "submit" }, index.h("wa-icon", { key: '314f7d19bd27c271c98324da684a8a43fbc275a1', name: "magnifying-glass" })))))));
    }
