...
const booking_store = require('./booking.store-59ea623c.js');
...
class BookingService {
    ...
    async getCalendarData(propertyid, from_date, to_date) {
        try {
            const { data } = await axios.axios.post(`/Get_Exposed_Calendar`, {
                ...
                from_date,
                to_date,
                extras: calendarDates_store.extras,
            });
            if (data.ExceptionMsg !== '') {
                throw new Error(data.ExceptionMsg);
            }
            ...
            const days = months
                ...
                return month.days.map(day => ({
                    day: calendarDates_store.convertDateToCustomFormat(day.description, month.description),
                    currentDate: calendarDates_store.convertDateToTime(day.description, month.description),
    ...
    async getExposedBooking(booking_nbr, language, withExtras = true) {
        try {
            const { data } = await axios.axios.post(`/Get_Exposed_Booking`, {
                booking_nbr,
                language,
                extras: withExtras ? calendarDates_store.extras : null,
            });
            if (data.ExceptionMsg !== '') {
                throw new Error(data.ExceptionMsg);
            }
    ...
    async doReservation(body) {
        const { data } = await axios.axios.post(`/DoReservation`, Object.assign(Object.assign({}, body), { extras: body.extras ? body.extras : calendarDates_store.extras }));
        if (data.ExceptionMsg !== '') {
            throw new Error(data.ExceptionMsg);
        }
    ...
    async bookUser({ bookedByInfoData, check_in, currency, extras = null, fromDate, guestData, pickup_info, propertyid, rooms, source, toDate, totalNights, arrivalTime, bookingNumber, defaultGuest, identifier, pr_id, }) {
        try {
            const fromDateStr = calendarDates_store.dateToFormattedString(fromDate);
            const toDateStr = calendarDates_store.dateToFormattedString(toDate);
            let guest = {
...
//# sourceMappingURL=booking.service-08b2e7ab2.js.map