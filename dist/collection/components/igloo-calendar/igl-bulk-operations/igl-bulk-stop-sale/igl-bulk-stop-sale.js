...
export class IglBulkStopSale {
    ...
    render() {
        return (h("form", { key: 'bbc7e3193bfb6c98505cd26893fbe5c451ae0a4c', id: this.formId, class: 'bulk-sheet-container', onSubmit: e => {
                e.preventDefault();
                this.addBlockDates();
            } }, h("div", { key: '2c8d4270a74746e2d00edbf56ca6e7e3ab01a897', class: "sheet-body px-1" }, h("div", { key: 'a11d96adee1a8b855d653791700102523e05da8d', class: "text-muted text-left py-0 my-0" }, h("p", { key: '01461d4403e01a03784c4b1027a021d7441017c0' }, "Select the types to stop or open sales for all related rate plans")), h("div", { key: '9d56c042794e609b8cfc8b3ae129ce6289b87228' }, this.errors === 'rooms' && (h("p", { key: 'aca9f52e7c7ce0c769c7385067883b449f2bdb5a', class: 'text-danger text-left smaller p-0 ', style: { 'margin-bottom': '0.5rem' } }, calendar_data.is_vacation_rental ? locales.entries.Lcz_PlzSelectOneListing : locales.entries.Lcz_PlzSelectOneUnit)), h("table", { key: 'e8d85668611c8d43e1fe2b9b0a8549c7b514e110', ref: el => (this.unitSections = el) }, h("thead", { key: '2a305e8e3f5c23eadfd1862d4f8186f1467b9306' }, h("tr", { key: '16feed37432ff9e4806b53b7b9dd6106119f7fcf' }, h("th", { key: '0a0c08550821be06e695d8e7048ebcf98604fce6', class: "sr-only" }, "choice"), h("th", { key: 'fc8b9a2aec7113aaab46f6a8497ed46fd0d1d26f', class: "sr-only" }, "room type"))), h("tbody", { key: '4927521fd65afeccfb96044440e69aea9ae9db4d' }, calendar_data.roomsInfo.map((roomType, i) => {
        ...
        })))), h("p", { key: 'fb79f406b0e5183b5acb5f6c8e3838f4468925fd', class: "text-left mt-2 text-muted" }, "Included days"), this.errors === 'weekdays' && h("p", { key: '968b5a6bcd9853af71d853699f60a6fd32d300a1', class: 'text-danger text-left smaller m-0 p-0' }, "Please select at least one day"), h("ir-weekday-selector", { key: '90eb030ec85d9c22cf15f5f45d7fc62fe62731ab', ref: el => (this.weekdaysSections = el), weekdays: Array.from(this.selectedWeekdays), onWeekdayChange: e => {
                e.stopPropagation();
                e.stopImmediatePropagation();
                this.selectedWeekdays = new Set(e.detail);
            } }), h("table", { key: '2019e10f4a31c9a1ac8aad08829585e4c5a46049', class: "mt-1", ref: el => (this.datesSections = el) }, h("thead", { key: 'b5d490d10de6c02f1ec34f3db864b57c316f7996' }, h("tr", { key: 'f3517d79ed45aaaa37521166ec494f7b14b99211' }, h("td", { key: '9204b6aa01ca57462ce286d898017f50abed19f3', class: "text-left bulk-stop-sale__date-label" }, locales.entries.Lcz_From), h("td", { key: '0bb7a949ac901decd7f45f43dabe3aeb3044b3fe', class: "text-left bulk-stop-sale__date-label" }, locales.entries.Lcz_ToExclusive), h("td", { key: 'a06d8bb5931affdd55a6efeca8d7e9bad48eb5de' }, this.dates.length !== this.maxDatesLength && (h("ir-custom-button", { key: 'b31d4616869baff06163011728f3985873f9d3b2', appearance: "plain", variant: "neutral", onClickHandler: () => {
                this.addDateRow();
            } }, h("wa-icon", { key: '436e3aa1465ac5e6239ea1b3d1880b0a468cdfc0', name: "plus", style: { fontSize: '1.2rem' } })))))), h("tbody", { key: '2dbc0e3c0c0a63d03142e2ee426846803d299396' }, this.dates.map((d, i) => {
            if (!this.dateRefs[i]) {
                this.dateRefs[i] = {};
            }
