...
export class IglBulkStopSale {
    ...
    render() {
        return (h("form", { key: '44d0eba3de75540aa8f00c7f76682e2f497e1cab', id: this.formId, class: 'bulk-sheet-container', onSubmit: e => {
                e.preventDefault();
                this.addBlockDates();
            } }, h("div", { key: 'f039330bab5892ac4835f819c51b93575077038c', class: "sheet-body px-1" }, h("div", { key: '7e7bfc1a1668d64ee154b1aabab136909876fb54', class: "text-muted text-left py-0 my-0" }, h("p", { key: '63dcc1aee4ff58bdfeefdde519266f8e6bf1e599' }, "Select the types to stop or open sales for all related rate plans")), h("div", { key: 'eeb04b1e163dddbb4e0873b52e953bff48d5589c' }, this.errors === 'rooms' && (h("p", { key: '58fa762abc13538081de6b95a24431dfd175959b', class: 'text-danger text-left smaller p-0 ', style: { 'margin-bottom': '0.5rem' } }, calendar_data.is_vacation_rental ? locales.entries.Lcz_PlzSelectOneListing : locales.entries.Lcz_PlzSelectOneUnit)), h("table", { key: 'beaa6bcf6fbb037ba0114d71be9aab6f3d76e666', ref: el => (this.unitSections = el) }, h("thead", { key: '614bc27c1e91f7859a6e0d383c20e285dfda0537' }, h("tr", { key: '1b8850531063dd9887fe1e5823dcc8f6aabcc7e3' }, h("th", { key: '064d6212d2adc8bc3c70d720a8b9d1fce690791f', class: "sr-only" }, "choice"), h("th", { key: 'd7080127e6f04e06573cbdd8bd0b6d80aba753fd', class: "sr-only" }, "room type"))), h("tbody", { key: 'a1be3092de53ac363eb8c2f33dc5b4d00d44299f' }, calendar_data.roomsInfo.map((roomType, i) => {
        ...
        })))), h("p", { key: '7a72f5433e71f1f36933c32e8d47e5fad49843e9', class: "text-left mt-2 text-muted" }, "Included days"), this.errors === 'weekdays' && h("p", { key: '436602375a8a451df69250995899fc123ff88661', class: 'text-danger text-left smaller m-0 p-0' }, "Please select at least one day"), h("ir-weekday-selector", { key: '2f7cdc4d3dea8c57ef22568d9ed182530a251356', ref: el => (this.weekdaysSections = el), weekdays: Array.from(this.selectedWeekdays), onWeekdayChange: e => {
                e.stopPropagation();
                e.stopImmediatePropagation();
                this.selectedWeekdays = new Set(e.detail);
            } }), h("table", { key: '701b34705c1f350afd2c9a987985ce47fe67e335', class: "mt-1", ref: el => (this.datesSections = el) }, h("thead", { key: '7dfbd8ba05219f2ad378cbdf1923c11b3181fa44' }, h("tr", { key: '174856d80969289a1ceb7ba50eda2179949fcb47' }, h("td", { key: '86cd9bd902b53a4af2253a0587b358062d3c3186', class: "text-left bulk-stop-sale__date-label" }, locales.entries.Lcz_From), h("td", { key: '8af19e7bd6e2ac5e5c4b9a53d866e8f34abda4dd', class: "text-left bulk-stop-sale__date-label" }, locales.entries.Lcz_ToExclusive), h("td", { key: 'e0cf4f2a68fb7b70248de580d271237615206a3d' }, this.dates.length !== this.maxDatesLength && (h("ir-custom-button", { key: 'f5bf8cc6b278c224fc9b00245996321ca17c68c5', appearance: "plain", variant: "neutral", onClickHandler: () => {
                this.addDateRow();
            } }, h("wa-icon", { key: '77faa9b01c176606df803f19049fea29799bedb2', name: "plus", style: { fontSize: '1.2rem' } })))))), h("tbody", { key: 'c8e18154ed71f5f213bef12904a9700d974bbab8' }, this.dates.map((d, i) => {
            if (!this.dateRefs[i]) {
                this.dateRefs[i] = {};
            }
