...
export class IrBookingSourceEditorForm {
    ...
    render() {
        ...
        return (h("form", { key: '19df97beae6ae75b554c275a9fbb0b97f7570fe5', id: `change-source-form-${this.booking?.booking_nbr}`, onSubmit: this.handleSubmit.bind(this) }, this.booking.agent === null && this.booking?.financial?.payments?.filter(p => !p.is_city_ledger)?.length > 0 && (h("wa-callout", { key: 'b63baaa3cd3a38b1041a5b3bf1198edeb9a2fe4b', size: "small", variant: "warning", style: { marginBottom: '1rem' } }, h("wa-icon", { key: '2ad6fb084b118e635a4a0d3e106d97f8ce6910d0', slot: "icon", name: "triangle-exclamation" }), "You have guest folio entries that may need to be removed and recreated in the agent folio.")), h("wa-select", { key: '0683c78587139a59de51f28241f07d7dbf349b04', label: "New source", onchange: this.handleSelectChange.bind(this), size: "small", value: this.selectedSource?.id, defaultValue: this.selectedSource?.id }, calendar_data?.property?.allowed_booking_sources?.map(option => option.type === 'LABEL' ? (h("small", { key: option.id }, option.description)) : (h("wa-option", { key: option.id, value: option.id?.toString() }, option.description)))), isAssign && h("ir-booking-assign-items", { key: '971d98f3d1f515cb5f6c368b44bf4082d329747c', items: this.buildAssignableItems(), onBookingSelectionChange: e => (this.checkedItems = e.detail) })));
    }
