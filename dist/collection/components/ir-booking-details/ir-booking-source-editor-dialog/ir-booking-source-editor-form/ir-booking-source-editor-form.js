...
export class IrBookingSourceEditorForm {
    ...
    render() {
        ...
        return (h("form", { key: '6c2896d2daddc474d273bb63aadbec83fa74e581', id: `change-source-form-${this.booking?.booking_nbr}`, onSubmit: this.handleSubmit.bind(this) }, this.booking.agent === null && this.booking?.financial?.payments?.filter(p => !p.is_city_ledger)?.length > 0 && (h("wa-callout", { key: '28246fd86d3a2b516dcfe0f54386481fee09c717', size: "small", variant: "warning", style: { marginBottom: '1rem' } }, h("wa-icon", { key: '8ba9f932af9e9187db85b52df053a39f9c36db4f', slot: "icon", name: "triangle-exclamation" }), "You have guest folio entries that may need to be removed and recreated in the agent folio.")), h("wa-select", { key: 'e6be0da4f1df0c291e31a88610ec38a001d65636', label: "New source", onchange: this.handleSelectChange.bind(this), size: "small", value: this.selectedSource?.id, defaultValue: this.selectedSource?.id }, calendar_data?.property?.allowed_booking_sources?.map(option => option.type === 'LABEL' ? (h("small", { key: option.id }, option.description)) : (h("wa-option", { key: option.id, value: option.id?.toString() }, option.description)))), isAssign && h("ir-booking-assign-items", { key: '3021df01605b63af3cb5072362669cf72038b777', items: this.buildAssignableItems(), onBookingSelectionChange: e => (this.checkedItems = e.detail) })));
    }
