...
export class IrBookingSourceEditorForm {
    ...
    render() {
        ...
        return (h("form", { key: 'd8c1204acc627f7188a90a56f32dc0cf1a1840e3', id: `change-source-form-${this.booking?.booking_nbr}`, onSubmit: this.handleSubmit.bind(this) }, this.booking.agent === null && this.booking?.financial?.payments?.filter(p => !p.is_city_ledger)?.length > 0 && (h("wa-callout", { key: '2e43302e90eae122beb67367e05f8c2a965e3067', size: "s", variant: "warning", style: { marginBottom: '1rem' } }, h("wa-icon", { key: '45470d3bd95858e21f7eb67d831282c6255b6a33', slot: "icon", name: "triangle-exclamation" }), "You have guest folio entries that may need to be removed and recreated in the agent folio.")), h("wa-select", { key: '698d117230e394f8570f2f615b4d38d09ba616f0', label: "New source", onchange: this.handleSelectChange.bind(this), size: "s", value: this.selectedSource?.id, defaultValue: this.selectedSource?.id }, calendar_data?.property?.allowed_booking_sources?.map(option => option.type === 'LABEL' ? (h("small", { key: option.id }, option.description)) : (h("wa-option", { key: option.id, value: option.id?.toString() }, option.description)))), isAssign && h("ir-booking-assign-items", { key: '3a71732906996c7ee5ce3c0efbaece96b86b255c', items: this.buildAssignableItems(), onBookingSelectionChange: e => (this.checkedItems = e.detail) })));
    }
