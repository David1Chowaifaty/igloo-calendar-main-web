...
export class IrBookingSourceEditorForm {
    ...
    render() {
        ...
        return (h("form", { key: '0505532e9096e93505ffa7ddad160c9ff388d9c4', id: `change-source-form-${this.booking?.booking_nbr}`, onSubmit: this.handleSubmit.bind(this) }, this.booking.agent === null && this.booking?.financial?.payments?.filter(p => !p.is_city_ledger)?.length > 0 && (h("wa-callout", { key: 'eaf67847ca90cb61b8ef46fab3225598c3b34386', size: "small", variant: "warning", style: { marginBottom: '1rem' } }, h("wa-icon", { key: '3d58484478ca219dbaf099bcee86ee74debf7510', slot: "icon", name: "triangle-exclamation" }), "You have guest folio entries that may need to be removed and recreated in the agent folio.")), h("wa-select", { key: '591d337dc2d88d940efa23d5b028b46850b1ee63', label: "New source", onchange: this.handleSelectChange.bind(this), size: "small", value: this.selectedSource?.id, defaultValue: this.selectedSource?.id }, calendar_data?.property?.allowed_booking_sources?.map(option => option.type === 'LABEL' ? (h("small", { key: option.id }, option.description)) : (h("wa-option", { key: option.id, value: option.id?.toString() }, option.description)))), isAssign && h("ir-booking-assign-items", { key: '15f68bdc9325069ce78a491c12cd8dd408a6f503', items: this.buildAssignableItems(), onBookingSelectionChange: e => (this.checkedItems = e.detail) })));
    }
