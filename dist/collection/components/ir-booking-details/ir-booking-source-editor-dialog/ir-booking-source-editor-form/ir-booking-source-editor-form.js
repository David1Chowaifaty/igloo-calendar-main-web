...
export class IrBookingSourceEditorForm {
    ...
    render() {
        ...
        return (h("form", { key: '5b049f6978c76ae6850fab67102e7563e57272d9', id: `change-source-form-${this.booking?.booking_nbr}`, onSubmit: this.handleSubmit.bind(this) }, this.booking.agent === null && this.booking?.financial?.payments?.filter(p => !p.is_city_ledger)?.length > 0 && (h("wa-callout", { key: '9cd66baaa2b61fc17e90bc93064a5e7e3d8b5dc1', size: "s", variant: "warning", style: { marginBottom: '1rem' } }, h("wa-icon", { key: 'cbc400ad7f960f1ac3aa7f0cf22186e64796fcf5', slot: "icon", name: "triangle-exclamation" }), "You have guest folio entries that may need to be removed and recreated in the agent folio.")), h("wa-select", { key: '38a4282cecab3f888feec6b9a42aaf34dc33a103', label: "New source", onchange: this.handleSelectChange.bind(this), size: "s", value: this.selectedSource?.id, defaultValue: this.selectedSource?.id }, calendar_data?.property?.allowed_booking_sources?.map(option => option.type === 'LABEL' ? (h("small", { key: option.id }, option.description)) : (h("wa-option", { key: option.id, value: option.id?.toString() }, option.description)))), isAssign && h("ir-booking-assign-items", { key: 'd4aec3f175751706187cdf4ece5db1b61df64bcd', items: this.buildAssignableItems(), onBookingSelectionChange: e => (this.checkedItems = e.detail) })));
    }
