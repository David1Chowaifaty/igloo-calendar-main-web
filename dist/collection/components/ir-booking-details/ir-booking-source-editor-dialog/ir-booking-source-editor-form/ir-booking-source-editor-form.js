...
export class IrBookingSourceEditorForm {
    ...
    render() {
        ...
        return (h("form", { key: 'b5edc27bf3c157e2f28c68e75821598741d39609', id: `change-source-form-${this.booking?.booking_nbr}`, onSubmit: this.handleSubmit.bind(this) }, this.booking.agent === null && this.booking?.financial?.payments?.filter(p => !p.is_city_ledger)?.length > 0 && (h("wa-callout", { key: 'ef118aaf3a17cee3d4978171612b8591f62ec325', size: "small", variant: "warning", style: { marginBottom: '1rem' } }, h("wa-icon", { key: '27ad9d69f4e1f7ffbd62910429ccfa64cca81488', slot: "icon", name: "triangle-exclamation" }), "You have guest folio entries that may need to be removed and recreated in the agent folio.")), h("wa-select", { key: '15191cfc5a5c165e3ddce066b71fd8493702f309', label: "New source", onchange: this.handleSelectChange.bind(this), size: "small", value: this.selectedSource?.id, defaultValue: this.selectedSource?.id }, calendar_data?.property?.allowed_booking_sources?.map(option => option.type === 'LABEL' ? (h("small", { key: option.id }, option.description)) : (h("wa-option", { key: option.id, value: option.id?.toString() }, option.description)))), isAssign && h("ir-booking-assign-items", { key: '2307b141830bcfe26de67741c7149914f7cbc4a1', items: this.buildAssignableItems(), onBookingSelectionChange: e => (this.checkedItems = e.detail) })));
    }
