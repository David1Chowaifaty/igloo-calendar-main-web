...
export class IrBookingSourceEditorForm {
    ...
    render() {
        ...
        return (h("form", { key: '0ca956fa55e14a83c0e4371d101926190f4d7abe', id: `change-source-form-${this.booking?.booking_nbr}`, onSubmit: this.handleSubmit.bind(this) }, this.booking.agent === null && this.booking?.financial?.payments?.filter(p => !p.is_city_ledger)?.length > 0 && (h("wa-callout", { key: 'c2e3f97cc46eabda326aee406dc324ad8631a1e1', size: "small", variant: "warning", style: { marginBottom: '1rem' } }, h("wa-icon", { key: '2188078c7cfb3fbf27d508c6443265c1b4cc3ecf', slot: "icon", name: "triangle-exclamation" }), "You have guest folio entries that may need to be removed and recreated in the agent folio.")), h("wa-select", { key: 'fcb1184df72096d28d33e3610335142619b470d8', label: "New source", onchange: this.handleSelectChange.bind(this), size: "small", value: this.selectedSource?.id, defaultValue: this.selectedSource?.id }, calendar_data?.property?.allowed_booking_sources?.map(option => option.type === 'LABEL' ? (h("small", { key: option.id }, option.description)) : (h("wa-option", { key: option.id, value: option.id?.toString() }, option.description)))), isAssign && h("ir-booking-assign-items", { key: '47ae4d72ebd0d247b6f5f156dca68a60686902f0', items: this.buildAssignableItems(), onBookingSelectionChange: e => (this.checkedItems = e.detail) })));
    }
