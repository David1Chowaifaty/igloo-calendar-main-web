...
export class IrBookingSourceEditorForm {
    ...
    render() {
        ...
        return (h("form", { key: 'b702113fbbc5c247b3904f9be7b329b7e91e9d11', id: `change-source-form-${this.booking?.booking_nbr}`, onSubmit: this.handleSubmit.bind(this) }, this.booking.agent === null && this.booking?.financial?.payments?.filter(p => !p.is_city_ledger)?.length > 0 && (h("wa-callout", { key: '493e480068e78efc63b9d549269543f3a6611246', size: "small", variant: "warning", style: { marginBottom: '1rem' } }, h("wa-icon", { key: '754cc08beaa869cd3e8c12d4b16c3e44d55f7dee', slot: "icon", name: "triangle-exclamation" }), "You have guest folio entries that may need to be removed and recreated in the agent folio.")), h("wa-select", { key: '52aa5ad9d67084838ecbe727f962512502ae99d7', label: "New source", onchange: this.handleSelectChange.bind(this), size: "small", value: this.selectedSource?.id, defaultValue: this.selectedSource?.id }, calendar_data?.property?.allowed_booking_sources?.map(option => option.type === 'LABEL' ? (h("small", { key: option.id }, option.description)) : (h("wa-option", { key: option.id, value: option.id?.toString() }, option.description)))), isAssign && h("ir-booking-assign-items", { key: '5325a3f2bcdb757f0046124c462d937202101292', items: this.buildAssignableItems(), onBookingSelectionChange: e => (this.checkedItems = e.detail) })));
    }
