...
export class IrBookingSourceEditorForm {
    ...
    render() {
        ...
        return (h("form", { key: 'b197eee9e6d6d80b215710885e35174b1c6b1e65', id: `change-source-form-${this.booking?.booking_nbr}`, onSubmit: this.handleSubmit.bind(this) }, this.booking.agent === null && this.booking?.financial?.payments?.filter(p => !p.is_city_ledger)?.length > 0 && (h("wa-callout", { key: 'd2fba74af36fd366e39c51b143a7f6e7d49da298', size: "s", variant: "warning", style: { marginBottom: '1rem' } }, h("wa-icon", { key: 'c1a90846019e4c5d698f8887cd840a36ad566c64', slot: "icon", name: "triangle-exclamation" }), "You have guest folio entries that may need to be removed and recreated in the agent folio.")), h("wa-select", { key: '113992ac722b1d106c0a4da6fb6183274d7c59a9', label: "New source", onchange: this.handleSelectChange.bind(this), size: "s", value: this.selectedSource?.id, defaultValue: this.selectedSource?.id }, calendar_data?.property?.allowed_booking_sources?.map(option => option.type === 'LABEL' ? (h("small", { key: option.id }, option.description)) : (h("wa-option", { key: option.id, value: option.id?.toString() }, option.description)))), isAssign && h("ir-booking-assign-items", { key: 'c37564fc3f3b12e8e0b4ffbb19ee3b337462d371', items: this.buildAssignableItems(), onBookingSelectionChange: e => (this.checkedItems = e.detail) })));
    }
