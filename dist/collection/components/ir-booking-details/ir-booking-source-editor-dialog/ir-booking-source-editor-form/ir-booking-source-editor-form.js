...
export class IrBookingSourceEditorForm {
    ...
    render() {
        ...
        return (h("form", { key: '5026f8b89e1843725bfb76c33ae5cb5d04621549', id: `change-source-form-${this.booking?.booking_nbr}`, onSubmit: this.handleSubmit.bind(this) }, this.booking.agent === null && this.booking?.financial?.payments?.filter(p => !p.is_city_ledger)?.length > 0 && (h("wa-callout", { key: 'ea7abe93713fc7aca1ceb5e1718acfb2a799aa06', size: "s", variant: "warning", style: { marginBottom: '1rem' } }, h("wa-icon", { key: 'ff3d39d61620223d995284d0806930ab594ffd85', slot: "icon", name: "triangle-exclamation" }), "You have guest folio entries that may need to be removed and recreated in the agent folio.")), h("wa-select", { key: '1b322bba07f22dcc24e217e77f2afa72be378ff1', label: "New source", onchange: this.handleSelectChange.bind(this), size: "s", value: this.selectedSource?.id, defaultValue: this.selectedSource?.id }, calendar_data?.property?.allowed_booking_sources?.map(option => option.type === 'LABEL' ? (h("small", { key: option.id }, option.description)) : (h("wa-option", { key: option.id, value: option.id?.toString() }, option.description)))), isAssign && h("ir-booking-assign-items", { key: '82efb69dc9ee5427bf08e0fdfef50b80de8bb530', items: this.buildAssignableItems(), onBookingSelectionChange: e => (this.checkedItems = e.detail) })));
    }
