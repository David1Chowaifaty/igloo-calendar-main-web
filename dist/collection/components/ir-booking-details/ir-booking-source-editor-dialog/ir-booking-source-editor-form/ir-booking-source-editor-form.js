...
export class IrBookingSourceEditorForm {
    ...
    render() {
        ...
        return (h("form", { key: '36dbabda0851d2f49ef8f3f14171ba909ae2515b', id: `change-source-form-${this.booking?.booking_nbr}`, onSubmit: this.handleSubmit.bind(this) }, this.booking.agent === null && this.booking?.financial?.payments?.filter(p => !p.is_city_ledger)?.length > 0 && (h("wa-callout", { key: '27b1ba844ffdb6972d8cdc80b7c80c07bacd90fd', size: "small", variant: "warning", style: { marginBottom: '1rem' } }, h("wa-icon", { key: '7fd513b89e58844716b5c8ae831a9f5a3f762298', slot: "icon", name: "triangle-exclamation" }), "You have guest folio entries that may need to be removed and recreated in the agent folio.")), h("wa-select", { key: '60eb5b5ae60f65e832f3e4b525536519b5481cb0', label: "New source", onchange: this.handleSelectChange.bind(this), size: "small", value: this.selectedSource?.id, defaultValue: this.selectedSource?.id }, calendar_data?.property?.allowed_booking_sources?.map(option => option.type === 'LABEL' ? (h("small", { key: option.id }, option.description)) : (h("wa-option", { key: option.id, value: option.id?.toString() }, option.description)))), isAssign && h("ir-booking-assign-items", { key: 'cf69743c72f1cac5e0652b59a6407c4ca42f9406', items: this.buildAssignableItems(), onBookingSelectionChange: e => (this.checkedItems = e.detail) })));
    }
