...
export class IrBookingSourceEditorForm {
    ...
    render() {
        ...
        return (h("form", { key: '2f092f55de305bfdf8d350b676e0c653de231c8d', id: `change-source-form-${this.booking?.booking_nbr}`, onSubmit: this.handleSubmit.bind(this) }, this.booking.agent === null && this.booking?.financial?.payments?.filter(p => !p.is_city_ledger)?.length > 0 && (h("wa-callout", { key: '70849f088bd8a2903ad52983eced21881d2ccb47', size: "s", variant: "warning", style: { marginBottom: '1rem' } }, h("wa-icon", { key: 'cfbc123a2957d3416a8e1cc071246c1ff75e4864', slot: "icon", name: "triangle-exclamation" }), "You have guest folio entries that may need to be removed and recreated in the agent folio.")), h("wa-select", { key: '61cf8502e5e967427970fca57ca09dc7b02291a5', label: "New source", onchange: this.handleSelectChange.bind(this), size: "s", value: this.selectedSource?.id, defaultValue: this.selectedSource?.id }, calendar_data?.property?.allowed_booking_sources?.map(option => option.type === 'LABEL' ? (h("small", { key: option.id }, option.description)) : (h("wa-option", { key: option.id, value: option.id?.toString() }, option.description)))), isAssign && h("ir-booking-assign-items", { key: 'b2184c49be9b1931b1cc2217b0eda9b38887edc1', items: this.buildAssignableItems(), onBookingSelectionChange: e => (this.checkedItems = e.detail) })));
    }
