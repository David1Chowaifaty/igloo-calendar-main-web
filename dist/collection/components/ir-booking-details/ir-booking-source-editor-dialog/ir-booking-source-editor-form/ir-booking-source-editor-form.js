...
export class IrBookingSourceEditorForm {
    ...
    render() {
        ...
        return (h("form", { key: '39ba1e19a8c03aa79db8647b4c9b3a2b850d99cb', id: `change-source-form-${this.booking?.booking_nbr}`, onSubmit: this.handleSubmit.bind(this) }, this.booking.agent === null && this.booking?.financial?.payments?.filter(p => !p.is_city_ledger)?.length > 0 && (h("wa-callout", { key: '0d62533bab35d5c68d33aa6ecef52b2bbc4028e7', size: "s", variant: "warning", style: { marginBottom: '1rem' } }, h("wa-icon", { key: '02c40164d024f0485f86ce5e5a2959f7fa9be19c', slot: "icon", name: "triangle-exclamation" }), "You have guest folio entries that may need to be removed and recreated in the agent folio.")), h("wa-select", { key: '4d01b86b2bdca06fb050ca64a439d8df5fc4abac', label: "New source", onchange: this.handleSelectChange.bind(this), size: "s", value: this.selectedSource?.id, defaultValue: this.selectedSource?.id }, calendar_data?.property?.allowed_booking_sources?.map(option => option.type === 'LABEL' ? (h("small", { key: option.id }, option.description)) : (h("wa-option", { key: option.id, value: option.id?.toString() }, option.description)))), isAssign && h("ir-booking-assign-items", { key: '802b79c4bedc4a971d61d7eba52c3f21aae47f84', items: this.buildAssignableItems(), onBookingSelectionChange: e => (this.checkedItems = e.detail) })));
    }
