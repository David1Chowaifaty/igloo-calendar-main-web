...
export class IrClFiscalDocumentPreview {
    ...
    render() {
        return (h(Host, { key: '57a9902c78746a55200073f2cbf12b0a428f2aa0' }, h("ir-preview-screen-dialog", { key: '523f949ab50fafa2b85e413eebeb906c319e0d9b', hideDefaultAction: true, open: this.request !== null, label: this.getDialogLabel(), action: "print", onOpenChanged: e => {
                if (!e.detail)
                    this.request = null;
            } }, h("div", { key: '754b643672609c549135ca7fb0a58a24e3ef0ca5', slot: "header-actions", class: "header-actions" }, this.request?.fdTypeCode === FdTypes.Draft && (h("ir-custom-button", { key: 'bd2b0f17f4abc64755a79e11d98d754d710e0714', onClickHandler: () => (this.showConvertDialog = true), variant: "brand", appearance: "accent" }, "Convert to invoice")), this.request?.url && (h("ir-custom-button", { key: '317e94fcc058daf7650448ca93554af44d516c09', size: "medium", variant: "neutral", appearance: "plain", onClickHandler: () => this.handleDownload() }, h("wa-icon", { key: 'f795b349ef7631e4645c4c4cdfc5950ba0e2c9ab', name: "download", style: { fontSize: '1.2rem' }, label: "Download PDF" })))), this.renderPreview()), h("ir-fd-confirm-dialog", { key: 'b650c98cadaa2b13f21d30f355c5c19ed54ac4f5', open: this.showConvertDialog, action: "convert-to-invoice", docNumber: this.request?.documentNumber ?? 'this document', isConfirming: this.isConverting, onConfirmed: () => this.handleConvertConfirm(), onCancelled: () => (this.showConvertDialog = false) })));
    }
