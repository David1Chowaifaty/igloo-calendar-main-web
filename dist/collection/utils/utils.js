 ...
 * ```
 */
export const isPrivilegedUser = (userTypeCode) => {
...
export function canCheckout({ to_date, inOutCode, skipAutoCheckout = false }) {
 ...
 */
export function toFloat(value, decimalPlaces) {
    const factor = Math.pow(10, decimalPlaces);
    return value / factor;
}
export async function sleep(time = 200) {
    return new Promise(r => setTimeout(() => r(null), time));
}
export function handleBodyOverflow(open) {
 ...
 */
export function generateTimeSlotsMilitary(from, to, stepMinutes = 60) {
