...
const IrPaymentFolio = /*@__PURE__*/ proxyCustomElement(class IrPaymentFolio extends HTMLElement {
    ...
    render() {
        ...
        return (h("ir-drawer", { key: '39bb098156f5a14bcc568e02b646101dcee6febf', placement: "start", style: {
                ...
                this.isOpen = false;
            } }, this.isOpen && (h("div", { key: 'cddb7b41f3488f1326b593a7d71507d51faf48c8', class: "payment-folio__form", id: "ir__folio-form" }, h("ir-custom-date-picker", { key: 'b944eb149d200c55a8c3a959f2bda9a675f3471a', label: "Date", "aria-invalid": this.errors?.date && !this.folioData?.date ? 'true' : 'false', "data-testid": "pickup_date", onDateChanged: evt => {
                this.updateFolioData({ date: evt.detail.start?.format(DATE_FORMAT) });
            }, minDate: hooks().add(-2, 'months').format('YYYY-MM-DD'), emitEmptyDate: true, maxDate: this.today, date: this.folioData?.date }), h("ir-validator", { key: '24027cf793c30b47f3c54bc2497c50aba12b5087', value: this.folioData?.payment_type?.code, autovalidate: this.autoValidate, schema: paymentTypeSchema.shape.code, valueEvent: "change wa-change select-change", blurEvent: "wa-hide" }, h("wa-select", { key: '5bd1f4b62adda7c862bad9b073539a040c6a1521', "onwa-hide": event => this.stopEventPropagation(event), "onwa-show": event => this.stopEventPropagation(event), placeholder: "Select...", label: "Transaction type", defaultValue: this.folioData?.payment_type?.code, value: this.folioData?.payment_type?.code, disabled: this.mode === 'payment-action', onchange: event => {
                this.stopEventPropagation(event);
                this.handleDropdownChange(event.target.value);
            } }, h("wa-option", { key: '456a092723ab25bff76afdde0a5030eb4876c76a', value: "" }, "Select..."), this.renderDropdownItems())), this.requiresPaymentMethod(this.folioData?.payment_type?.code) && (h("ir-validator", { key: 'e303725e8ff6f5323586c853f4a240503bb24c4b', value: this.folioData?.payment_method?.code ?? '', autovalidate: this.autoValidate, schema: paymentMethodSchema.shape.code, valueEvent: "change wa-change select-change", blurEvent: "wa-hide" }, h("wa-select", { key: 'e1e13c8ad6f67d483256b14b05f7b288935ff328', label: `${this.folioData.payment_type?.code === '001' ? 'Payment' : 'Refund'} method`, "onwa-show": event => this.stopEventPropagation(event), "onwa-hide": event => this.stopEventPropagation(event), defaultValue: this.folioData?.payment_method?.code, value: this.folioData?.payment_method?.code ?? '', onchange: event => {
                this.stopEventPropagation(event);
                this.handlePaymentMethodDropdownChange(event.target.value);
            } }, h("wa-option", { key: '9964c542bfe0d1230b8a347de1ce4c631e851d93', value: "" }, "Select..."), this.paymentEntries?.methods?.map(pt => {
            return (h("wa-option", { key: pt.CODE_NAME, label: pt.CODE_VALUE_EN, value: pt.CODE_NAME }, pt.CODE_VALUE_EN));
        })))), h("ir-validator", { key: 'e932340183545c6917964fed29b01edc44571cc7', value: this.folioData?.amount?.toString() ?? undefined, autovalidate: this.autoValidate, schema: folioBaseSchema.shape.amount, valueEvent: "text-change input input-change", blurEvent: "input-blur" }, h("ir-custom-input", { key: '0490a964cc2475ade29792e04b56830b007fa2e7', value: this.folioData?.amount?.toString() ?? '', label: "Amount", mask: "price", min: 0, "onText-change": e => this.updateFolioData({ amount: !e.detail ? undefined : Number(e.detail) }) }, h("span", { key: '8d1367daaf043605ee8fe33be8a49701863a8bd8', slot: "start" }, calendar_data.currency.symbol))), h("ir-validator", { key: '75f8ae808ce68d8aaa0707420eb578d5cd28ba30', value: this.folioData?.reference ?? '', autovalidate: this.autoValidate, schema: folioBaseSchema.shape.reference, valueEvent: "text-change input input-change", blurEvent: "input-blur" }, h("ir-custom-input", { key: 'bf2a7284bdd875fc17083c20218250e7b7637753', value: this.folioData?.reference ?? '', label: "Reference", maxlength: 50, "onText-change": e => this.updateFolioData({ reference: e.detail ?? '' }) })))), h("div", { key: 'ad62fb374cb3542d55cb88a14df5ccccd702e9d3', slot: "footer", class: "w-100 d-flex align-items-center", style: { gap: 'var(--wa-space-xs)' } }, h("ir-custom-button", { key: 'de553418794f2d56ee03e5bac88350498f19b790', class: "flex-fill", size: "medium", "data-drawer": "close", appearance: "filled", variant: "neutral", onClickHandler: () => this.closeFolio() }, "Cancel"), h("ir-custom-button", { key: 'bc58794d09e5bc74cf798c8bee66d05447de4962', onClickHandler: () => this.savePayment(), loading: this.isLoading === 'save', class: "flex-fill", size: "medium",
...
function defineCustomElement() {
    ...
    const components = ["ir-payment-folio", "ir-custom-button", "ir-custom-date-picker", "ir-custom-input", "ir-drawer", "ir-validator"];
