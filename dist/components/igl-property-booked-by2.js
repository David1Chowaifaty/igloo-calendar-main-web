...
const IglPropertyBookedBy = /*@__PURE__*/ proxyCustomElement(class IglPropertyBookedBy extends HTMLElement {
    ...
    render() {
        return (h(Host, { key: 'cbf875e474648e7e9d852b82c65a88f480e30e75' }, h("div", { key: '3702c45512891f27e66af050ba663886887bf3c4', class: "text-left mt-3" }, h("div", { key: '884a1e430f9bf6c1d5b5e3603efa30620c897a8d', class: "d-flex", style: { alignItems: 'flex-end', gap: '0.5rem' } }, h("ir-picker", { key: 'f2d447fc8e189eaa876a8305b8c5596223a05674', class: "bookedByEmailContainer m-0 p-0", label: locales.entries.Lcz_BookedBy, value: this.bookedByData.email, ref: el => (this.pickerRef = el), "aria-invalid": String(Boolean(this.isButtonPressed && this.bookedByData.email !== '' && validateEmail(this.bookedByData.email))), withClear: true, "onText-change": event => this.fetchGuests(event.detail), debounce: 300, "onInput-picker-blurred": e => {
                e.stopPropagation();
        ...
        })), h("div", { key: '82f2712cddcd1096a9f794b9dc2930a2b8a1784f', style: { paddingBottom: '0.5rem' } }, h("wa-tooltip", { key: '609f5c47fde0a0e903384bf7cc97f56dff2206ce', for: `main_guest-search-tooltip` }, "Leave empty if email is unavailable"), h("wa-icon", { key: '210d1499843b37a7408502a9592b36df710165d8', name: "circle-info", id: `main_guest-search-tooltip` })))), h("div", { key: '434e222e4a526270df2fe64546d3e16e0f50a097', class: "bookedDetailsForm text-left mt-2 font-small-3 " }, h("div", { key: 'e1f1d0865c2360438c426d605eb0dde9830a0e66', class: "d-flex flex-column flex-md-row  justify-content-md-between ", style: { gap: '1rem' } }, h("div", { key: 'f91cf96de8c729af5ca2d6d93c0fc677abd7eff5', class: "fd-property-booked-by__guest-form " }, h("ir-validator", { key: '5768ff167c8576abdd38b5032a96e8f55303be04', value: this.bookedByData.firstName, schema: BookingGuestSchema.shape.first_name }, h("ir-input", { key: 'a99ed70af972d3a9ba48bce62504f69ad9b5dc2f', "onText-change": event => {
                this.updateGuest({ first_name: event.detail });
                this.handleDataChange('firstName', { target: { value: event.detail.trim() } });
            }, defaultValue: this.bookedByData.firstName, value: this.bookedByData.firstName, label: locales.entries.Lcz_FirstName, placeholder: locales.entries.Lcz_FirstName, required: true, name: "last_name_custom", autocomplete: "family-name" })), h("ir-validator", { key: '9f9bd717c551076d6bd71108d274df250f9c570f', value: this.bookedByData.lastName, schema: BookingGuestSchema.shape.last_name }, h("ir-input", { key: 'e1db8522b6c0cbed0a88b5b0b60ff52d1d8764e6', "onText-change": event => {
                this.updateGuest({ last_name: event.detail });
                this.handleDataChange('lastName', { target: { value: event.detail.trim() } });
            }, name: "first_name_custom", autocomplete: "given-name", defaultValue: this.bookedByData.lastName, value: this.bookedByData.lastName, label: locales.entries.Lcz_LastName, placeholder: locales.entries.Lcz_LastName, required: true })), h("ir-country-picker", { key: '08fecf52b52b7c8466bf886e92ffcb67c54c73da', label: locales.entries.Lcz_Country, variant: "modern", testId: "main_guest_country", class: "flex-grow-1 m-0", onCountryChange: e => this.handleCountryChange(e.detail.id), countries: this.countries, country: this.countries.find(c => c.id === this.bookedByData.countryId) }), h("ir-mobile-input", { key: '9df70a58ada6157c53d967ff905829c796afb595', size: "small", "onMobile-input-change": e => {
            ...
            // required
            countryCode: this.countries.find(c => c.phone_prefix === this.bookedByData.isdCode)?.code, countries: this.countries }), h("wa-select", { key: 'a74c0a9b75d5b874628fb81e137ffc072312f93c', size: "small", label: locales.entries.Lcz_YourArrivalTime, "data-testid": "arrival_time", "aria-disabled": String(Boolean(this.isButtonPressed && this.bookedByData.selectedArrivalTime.code === '')), id: v4(), defaultValue: this.arrivalTimeList[0].CODE_NAME, value: this.bookedByData.selectedArrivalTime.code, onchange: event => this.handleDataChange('selectedArrivalTime', event) }, this.arrivalTimeList.map(time => (h("wa-option", { value: time.CODE_NAME, selected: this.bookedByData.selectedArrivalTime.code === time.CODE_NAME }, time.CODE_VALUE_EN))))), h("div", { key: 'f5d930102b61ab448adf27898187e695587db6b8', class: "p-0 flex-fill  ml-md-3 d-flex flex-column", style: { gap: '0.5rem' } }, h("wa-textarea", { key: '9328e9884c6b772e644aca6d502af16b640dc0be', onchange: event => this.handleDataChange('message', event), size: "small", value: this.bookedByData.message, defaultValue: this.bookedByData.message, label: locales.entries.Lcz_AnyMessageForUs, rows: 4 }), this.paymentMethods.length > 1 && (h("wa-select", { key: '3d36691765765de443d67cd7ace1af398d017ec4', label: 'Payment Method', size: "small", value: booking_store?.selectedPaymentMethod?.code, onchange: e => modifyBookingStore('selectedPaymentMethod', {
                code: e.target.value,
            }) }, this.paymentMethods.map(p => (h("wa-option", { value: p.code }, p.description))))), booking_store.selectedPaymentMethod?.code === '001' && (h(Fragment, { key: 'b6d9085b18c2c351ecc61ceca13df374c4ce1979' }, h("ir-input", { key: '34d13de9ac7e810ac90b87baf98b8acddd156444', value: this.bookedByData.cardNumber, defaultValue: this.bookedByData.cardNumber, "onText-change": e => this.handleCreditCardDataChange('cardNumber', e.detail.trim()), label: locales.entries.Lcz_CardNumber }), h("ir-input", { key: 'e017a35a933a0f0357a50b797f3493c95eadc348', value: this.bookedByData.cardHolderName, defaultValue: this.bookedByData.cardHolderName, "onText-change": e => this.handleCreditCardDataChange('cardHolderName', e.detail.trim()), label: locales.entries.Lcz_CardHolderName }), h("ir-input", { key: 'c59c1b39afcdd8b078a3a7f8d219413c3c9360dc', "onText-change": e => {
            ...
            }, label: locales.entries.Lcz_ExpiryDate }))), booking_store.selectedPaymentMethod?.code === '005' && (h("div", { key: '4527954602acac60043ef337b67305b6a69b7c48', class: "form-group mt-md-1 mt-1 p-0 d-flex flex-column flex-md-row align-items-md-center" }, h("label", { key: '09a71f3f821069df7c0de62d28f3eb51608bc91c', class: "p-0 m-0 margin3" }), h("div", { key: '838b51eb66f0bcbabbea7edd4d2f4876cab68dc8', class: "p-0 m-0  controlContainer flex-fill" }, h("div", { key: 'c18820e0ec9e75a29d4a644c57be400a8053475e', class: "property-booked-by__money-transfer-description", innerHTML: this.paymentMethods.find(p => p.code === '005')?.localizables.find(l => l.language.code.toLowerCase() === 'en')?.description })))), h("wa-checkbox", { key: '2add82726f5708ccee17d15c8a20e1c1ede0c328', checked: this.bookedByData.emailGuest, onchange: event => this.handleDataChange('emailGuest', event) }, locales.entries.Lcz_EmailTheGuest))))));
    }
