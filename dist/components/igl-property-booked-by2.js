...
const IglPropertyBookedBy = /*@__PURE__*/ proxyCustomElement(class IglPropertyBookedBy extends HTMLElement {
    ...
    render() {
        return (h(Host, { key: 'e1480adccc324fa6d5fbefaf3a93190247bb491f' }, h("div", { key: '487478710c47689c7eb21ee81feb3a774258a0e6', class: "text-left mt-3" }, h("div", { key: 'c09adcc82999fa10fe997d873f0515822a47388a', class: "d-flex", style: { alignItems: 'flex-end', gap: '0.5rem' } }, h("ir-picker", { key: '11111d8e7da3c7b8d6888ce51f25d7d2cdaae5d7', class: "bookedByEmailContainer m-0 p-0", label: locales.entries.Lcz_BookedBy, value: this.bookedByData.email, ref: el => (this.pickerRef = el), "aria-invalid": String(Boolean(this.isButtonPressed && this.bookedByData.email !== '' && validateEmail(this.bookedByData.email))), withClear: true, "onText-change": event => this.fetchGuests(event.detail), debounce: 300, "onInput-picker-blurred": e => {
        ...
        })), h("div", { key: 'e49c80e97c63fba09d595bfee3084364023dc353', style: { paddingBottom: '0.5rem' } }, h("wa-tooltip", { key: '9dcda043bb52710aced031fa76e6ae8f19d01e07', for: `main_guest-search-tooltip` }, "Leave empty if email is unavailable"), h("wa-icon", { key: 'aa78020e68ff0979fc07f24286be0a700feb38bb', name: "circle-info", id: `main_guest-search-tooltip` })))), h("div", { key: 'ab363492762810d55da53b024035554927e6d634', class: "bookedDetailsForm text-left mt-2 font-small-3 " }, h("div", { key: 'f3528784811ac7aa2764d11f675b0ba6c0b832c1', class: "d-flex flex-column flex-md-row  justify-content-md-between ", style: { gap: '1rem' } }, h("div", { key: '5d6ef554fc19842f098927eeaa7e9027cf789c8c', class: "fd-property-booked-by__guest-form " }, h("ir-validator", { key: '3cadcec89ae727d78f2b272b108638bcd8e66fd3', value: this.bookedByData.firstName, schema: BookingGuestSchema.shape.first_name }, h("ir-input", { key: '4e4f8ff5a35950077e4349179ab875439d68290e', "onText-change": event => {
                this.updateGuest({ first_name: event.detail });
                this.handleDataChange('firstName', { target: { value: event.detail.trim() } });
            }, defaultValue: this.bookedByData.firstName, value: this.bookedByData.firstName, label: locales.entries.Lcz_FirstName, placeholder: locales.entries.Lcz_FirstName, required: true, name: "last_name_custom", autocomplete: "family-name" })), h("ir-validator", { key: '4a12fb424c4c677bf9e1ae17df2f92f4eb6a3234', value: this.bookedByData.lastName, schema: BookingGuestSchema.shape.last_name }, h("ir-input", { key: '5f073e1d2fc5a0d29609dbd7eb9b4dc0695b5ba5', "onText-change": event => {
                this.updateGuest({ last_name: event.detail });
                this.handleDataChange('lastName', { target: { value: event.detail.trim() } });
            }, name: "first_name_custom", autocomplete: "given-name", defaultValue: this.bookedByData.lastName, value: this.bookedByData.lastName, label: locales.entries.Lcz_LastName, placeholder: locales.entries.Lcz_LastName, required: true })), h("ir-country-picker", { key: 'b1e29df65fae1588dfc469a3b01cbf54bc76297e', label: locales.entries.Lcz_Country, variant: "modern", testId: "main_guest_country", class: "flex-grow-1 m-0", onCountryChange: e => this.handleCountryChange(e.detail.id), countries: this.countries, country: this.countries.find(c => c.id === this.bookedByData.countryId) }), h("ir-mobile-input", { key: 'af4300556776c57cf5dfe5c5d0bfd952cc1e5cdc', size: "small", "onMobile-input-change": e => {
            ...
            // required
            countryCode: this.countries.find(c => c.phone_prefix === this.bookedByData.isdCode)?.code, countries: this.countries }), h("wa-select", { key: '2eb3557bbd7604ea5e951e583b9221311d566307', size: "small", label: locales.entries.Lcz_YourArrivalTime, "data-testid": "arrival_time", "aria-disabled": String(Boolean(this.isButtonPressed && this.bookedByData.selectedArrivalTime.code === '')), id: v4(), defaultValue: this.arrivalTimeList[0].CODE_NAME, value: this.bookedByData.selectedArrivalTime.code, onchange: event => this.handleDataChange('selectedArrivalTime', event) }, this.arrivalTimeList.map(time => (h("wa-option", { value: time.CODE_NAME, selected: this.bookedByData.selectedArrivalTime.code === time.CODE_NAME }, time.CODE_VALUE_EN))))), h("div", { key: '2b52280efebb7b21523b282238c03b7e8afa05eb', class: "p-0 flex-fill  ml-md-3 d-flex flex-column", style: { gap: '0.5rem' } }, h("wa-textarea", { key: '40f88075920270b61930ff184b17356625831072', onchange: event => this.handleDataChange('message', event), size: "small", value: this.bookedByData.message, defaultValue: this.bookedByData.message, label: locales.entries.Lcz_AnyMessageForUs, rows: 4 }), this.paymentMethods.length > 1 && (h("wa-select", { key: 'ae4965bb0473b8ec0bb2e4995982d1e8bcbebb46', label: 'Payment Method', size: "small", value: booking_store?.selectedPaymentMethod?.code, onchange: e => modifyBookingStore('selectedPaymentMethod', {
                code: e.target.value,
            }) }, this.paymentMethods.map(p => (h("wa-option", { value: p.code }, p.description))))), booking_store.selectedPaymentMethod?.code === '001' && (h(Fragment, { key: 'cf3ba5c1b89e48039749cc8515efcf34cef3e180' }, h("ir-input", { key: '81a1edb4c4c76e8176b56d8311bde63c3644fdc3', value: this.bookedByData.cardNumber, defaultValue: this.bookedByData.cardNumber, "onText-change": e => this.handleCreditCardDataChange('cardNumber', e.detail.trim()), label: locales.entries.Lcz_CardNumber }), h("ir-input", { key: 'cf2175fef901433fc996ce85e7b7600ef8b4ab52', value: this.bookedByData.cardHolderName, defaultValue: this.bookedByData.cardHolderName, "onText-change": e => this.handleCreditCardDataChange('cardHolderName', e.detail.trim()), label: locales.entries.Lcz_CardHolderName }), h("ir-input", { key: 'bb194e364f9790dc7ded1fd0404b82822d420486', "onText-change": e => {
            ...
            }, label: locales.entries.Lcz_ExpiryDate }))), booking_store.selectedPaymentMethod?.code === '005' && (h("div", { key: '60669cbebea7fdcf78124e1b84280d743a85ab2b', class: "form-group mt-md-1 mt-1 p-0 d-flex flex-column flex-md-row align-items-md-center" }, h("label", { key: '737d6144de7e45d5dec399137eebe6e937405327', class: "p-0 m-0 margin3" }), h("div", { key: 'f287cfa6619db3fc584ed91ccbfbbf216d554430', class: "p-0 m-0  controlContainer flex-fill" }, h("div", { key: 'e3e72b01de492ca2de57f55e9aa7755fc13b3601', class: "property-booked-by__money-transfer-description", innerHTML: this.paymentMethods.find(p => p.code === '005')?.localizables.find(l => l.language.code.toLowerCase() === 'en')?.description })))), h("wa-checkbox", { key: 'dd573508b38a0329734996545b5e3290e3895a3d', checked: this.bookedByData.emailGuest, onchange: event => this.handleDataChange('emailGuest', event) }, locales.entries.Lcz_EmailTheGuest))))));
    }
