...
const IrChannelEditor = class {
    ...
    render() {
        return (h(Host, { key: '5b4e16c3fddafc935d788e2fcc3632e8f244a6ea', class: " d-flex flex-column h-100" }, h("nav", { key: 'a50a05b6c25e4b2e4e711a8abc6bd9af777ab486', class: "position-sticky sticky-top pb-1 top-0 bg-white " }, h("div", { key: '75a904192acf874719cdfa00379a83f4cffd8c1d', class: "d-flex align-items-center px-1 py-1  justify-content-between" }, h("h3", { key: '32f55aedee97ad2ed277ab303e976929a3309e5d', class: "text-left font-medium-2  py-0 my-0" }, this.channel_status === 'create' ? locales.entries?.Lcz_CreateChannel : locales.entries?.Lcz_EditChannel), h("ir-icon", { key: 'f019bb400db5c33d8695dee27cbc4a91281d4b76', class: 'm-0 p-0 close', onIconClickHandler: () => {
                this.closeSideBar.emit(null);
            } }, h("svg", { key: 'ec03e9c3b5c00c09282880bf4f332b5ae2de4112', slot: "icon", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 384 512", height: 20, width: 20 }, h("path", { key: '707248b11f2a67ea1aa13a91eac5acc78f849ee6', d: "M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z" })))), h("ir-channel-header", { key: 'af0a7a2d533e4add77a7621197764ab34030a676', class: "mt-1 px-0", headerTitles: this.headerTitles })), h("section", { key: 'd1799091ecfe781c1a5f9f10623549c67b3ee54c', class: "flex-fill tab-container px-1" }, this.renderTabScreen()), h("ir-button", { key: '40db3002b0908746c0b842905bd92303e0c15bba', isLoading: this.isLoading, onClickHandler: () => {
...
const IrChannelHeader = class {
    ...
    render() {
        return (h(Host, { key: '5d7db79b1117a2c5028199401326e621bd5c0502' }, h("ul", { key: 'cfe204a8e9e4766aff2d9cfbfbb9afb7367c7990', class: "px-1" }, this.headerTitles.map((title, index) => (h("li", { class: `tab ${title.disabled ? 'text-light' : ''}`, key: title.id, onClick: () => {
                if (!title.disabled)
                    this.handleTabSelection(index);
            }, "data-disabled": title.disabled, "data-state": this.selectedIndex === index ? 'selected' : '' }, title.name)))), h("span", { key: 'ea39cecc343e422b90e71754f5ab972270540e41', class: "active-indicator", ref: el => (this.activeIndicator = el) })));
    }
};
...
const IrChannelMapping = class {
    ...
    render() {
        return (h(Host, { key: '5f11b7451c91b570b0c6cf17324c4f3dea1900b1', class: "py-md-2 px-md-2" }, h("div", { key: '8c0601e3488fb048158c9303cc2f8622ba87b030', class: "d-flex p-0 m-0 w-100 justify-content-end" }, h("button", { key: 'a111faecb3875073c57282d8ab7e7ddf52c59c73', onClick: () => {
                setMappedChannel();
            }, class: "btn refresh-btn" }, locales.entries?.Lcz_Refresh)), h("section", { key: '6da04a48e016612cf4de640a55ca9d6ed7dd334f', class: "w-100" }, h("div", { key: '0b79643e1734adf667d58e68d86c9a73107d7304', class: "pt-1 mapped_row" }, h("p", { key: '1acd59a194631239cad68b0192e9bc91990bfa5d', class: "mapped_item channel_name" }, channels_data.selectedChannel?.name), h("svg", { key: '2a6a24bd0c2a3490ee08d28249f3a2efeb987f60', xmlns: "http://www.w3.org/2000/svg", height: "14", width: "12.25", viewBox: "0 0 448 512" }, h("path", { key: 'f9a142a7a8a230082d15ed44cac60be176bec2b2', d: "M438.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-160-160c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L338.8 224 32 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l306.7 0L233.4 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l160-160z" })), h("p", { key: '570267d8b0de5f7f4ee8975064ec53beca1fbbb3', class: "pl-2 mapped_item channel_name" }, "igloorooms")), h("div", { key: 'e81d82fde83ea1389d3cc79e293edf29418c9f53' }, channels_data.selectedChannel?.property?.room_types?.map(room_type => {
...
const IrSwitch = class {
    ...
    render() {
        return (h(Host, { key: 'df4209e58a6a985b5b3d2248a6dcc1a93393db8b' }, h("button", { key: 'e97e5cca1d53738ec8750f0f4dfa5fbefaf2953d', disabled: this.disabled, ref: el => (this.switchRoot = el), type: "button", id: this.switchId || this._id, onClick: this.handleCheckChange.bind(this), role: "switch", "data-state": this.checked ? 'checked' : 'unchecked', value: 'on', class: "SwitchRoot" }, h("span", { key: '4944b27c4670ae947a7c3978a4c796a13775df2e', class: "SwitchThumb", "data-state": this.checked ? 'checked' : 'unchecked' })), h("input", { key: '5e787666efbf537daa73e5832c0bb497a9d5106f', type: "checkbox", checked: this.checked, "aria-hidden": "true", tabIndex: -1, value: 'on', class: "hidden-input" })));
    }
};
