...
const IrChannelHeader = class {
    ...
    render() {
        return (h(Host, { key: '5f5f141a30ae23c7a1a8959143e91ce61d0008b3' }, h("ul", { key: '6c9c341509603435c862e5baf0b3883d7bf3e5bd', class: "px-1" }, this.headerTitles.map((title, index) => (h("li", { class: `tab ${title.disabled ? 'text-light' : ''}`, key: title.id, onClick: () => {
                if (!title.disabled)
                    this.handleTabSelection(index);
            }, "data-disabled": title.disabled, "data-state": this.selectedIndex === index ? 'selected' : '' }, title.name)))), h("span", { key: '53a593fed28796ac8ca87636fb3c8157aa0e955b', class: "active-indicator", ref: el => (this.activeIndicator = el) })));
    }
};
...
const IrChannelMapping = class {
    ...
    render() {
        return (h(Host, { key: '0f3963ccd9482b150ec302ff798e34fe857711e3', class: "py-md-2 px-md-2" }, h("div", { key: '45a43c3100c49cdda6d3c9a676c0ae58c69a14f4', class: "d-flex p-0 m-0 w-100 justify-content-end" }, h("button", { key: '5cd7eb74db0fe693f073be1cf67b48ace818a141', onClick: () => {
                setMappedChannel();
            }, class: "btn refresh-btn" }, locales.entries?.Lcz_Refresh)), h("section", { key: '7462648dee0025b49fbc6b42478ff186e7d8e764', class: "w-100" }, h("div", { key: '5a868743d53527bcaca7b94a7ce6d9815ea201c7', class: "pt-1 mapped_row" }, h("p", { key: '04b50ecac8c40fc3025acce2d5dda33211902d3a', class: "mapped_item channel_name" }, channels_data.selectedChannel?.name), h("svg", { key: 'a4e541e2dee6f3576cbcb67857bd04c1aa271a77', xmlns: "http://www.w3.org/2000/svg", height: "14", width: "12.25", viewBox: "0 0 448 512" }, h("path", { key: '2dd5d506eb3a44e1211ee7c00eba6c54d38e9693', d: "M438.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-160-160c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L338.8 224 32 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l306.7 0L233.4 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l160-160z" })), h("p", { key: 'bdaf12c7c67b839324c5c2aae9b81c2647b18141', class: "pl-2 mapped_item channel_name" }, "igloorooms")), h("div", { key: 'f517c039814695466ba0161ba3e21e683dc2ef43' }, channels_data.selectedChannel?.property?.room_types?.map(room_type => {
...
const IrSwitch = class {
    ...
    render() {
        return (h(Host, { key: '1b0e207c92341eebc6bdef98a6c35ee937bd03cb' }, h("button", { key: 'b066114610ab2121fed00bfa58a4bc99ec72a208', disabled: this.disabled, ref: el => (this.switchRoot = el), type: "button", id: this.switchId || this._id, onClick: this.handleCheckChange.bind(this), role: "switch", "data-state": this.checked ? 'checked' : 'unchecked', value: 'on', class: "SwitchRoot" }, h("span", { key: '602d51cfc11126b73ef466514ddf024edb7f8d0c', class: "SwitchThumb", "data-state": this.checked ? 'checked' : 'unchecked' })), h("input", { key: '812109dd64f43af85416e5c1136a4ea86f31e670', type: "checkbox", checked: this.checked, "aria-hidden": "true", tabIndex: -1, value: 'on', class: "hidden-input" })));
    }
};
