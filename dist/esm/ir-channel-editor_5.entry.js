...
const IrChannelEditor = class {
    ...
    render() {
        return (h(Host, { key: '037a82ddc98d143d662a64e80172e7cd2092aca1', class: " d-flex flex-column h-100" }, h("nav", { key: '11e8f85ade62a5ad8247bcae1ed7df7ddcbf9984', class: "position-sticky sticky-top pb-1 top-0 bg-white " }, h("div", { key: '42a72f6f9e124b641484ea722e34aeac2b87b6e1', class: "d-flex align-items-center px-1 py-1  justify-content-between" }, h("h3", { key: '68e9e1c7a0ffc637a4c7e97c395bea36a2c3ae14', class: "text-left font-medium-2  py-0 my-0" }, this.channel_status === 'create' ? locales.entries?.Lcz_CreateChannel : locales.entries?.Lcz_EditChannel), h("ir-icon", { key: '0c657e4445d582be8b1df8566c70fa9d7cc056c1', class: 'm-0 p-0 close', onIconClickHandler: () => {
                this.closeSideBar.emit(null);
            } }, h("svg", { key: '7518e436a6c4109159ae6476b64c63607d33ba6d', slot: "icon", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 384 512", height: 20, width: 20 }, h("path", { key: '5c3c2351aca515ed41a05320d67782e6eea533bb', d: "M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z" })))), h("ir-channel-header", { key: 'e026e241bafe7c99043a31c3b7f15d49b399f459', class: "mt-1 px-0", headerTitles: this.headerTitles })), h("section", { key: '9b7e47d6107e693e445993c3f36267df849b4791', class: "flex-fill tab-container px-1" }, this.renderTabScreen()), h("ir-button", { key: 'f4f81e6f9b41f7d29645875107238425e1759dd7', isLoading: this.isLoading, onClickHandler: () => {
...
const IrChannelHeader = class {
    ...
    render() {
        return (h(Host, { key: '3306c9eed45a0db1d3617387781dc0d2aad7a312' }, h("ul", { key: '45ef08a1a7b9be0c2e86c914dece3aeb3426296a', class: "px-1" }, this.headerTitles.map((title, index) => (h("li", { class: `tab ${title.disabled ? 'text-light' : ''}`, key: title.id, onClick: () => {
                if (!title.disabled)
                    this.handleTabSelection(index);
            }, "data-disabled": title.disabled, "data-state": this.selectedIndex === index ? 'selected' : '' }, title.name)))), h("span", { key: 'b86c5005c9c52a9c2803fcad457ca07136bf27d8', class: "active-indicator", ref: el => (this.activeIndicator = el) })));
    }
};
...
const IrChannelMapping = class {
    ...
    render() {
        return (h(Host, { key: 'e0a6012ef2c318f0d59d33a2d20cf101ef1a2f82', class: "py-md-2 px-md-2" }, h("div", { key: '894b68a7f12aa1aed5fe01ccfd5f305815d08ad7', class: "d-flex p-0 m-0 w-100 justify-content-end" }, h("button", { key: 'ef4d6f654e87dd65ddeddcf358563f2e33ffd5d5', onClick: () => {
                setMappedChannel();
            }, class: "btn refresh-btn" }, locales.entries?.Lcz_Refresh)), h("section", { key: 'e1789f30ce3c0b26880c40f5937a863746df74db', class: "w-100" }, h("div", { key: 'd96fa47e9ce3745fd8b814a8dd6c658f55804442', class: "pt-1 mapped_row" }, h("p", { key: '61763bae8f1b318baa563b9e202edaf0fdc4813c', class: "mapped_item channel_name" }, channels_data.selectedChannel?.name), h("svg", { key: 'edb62ef81b107bd20e69a2efd8575ecc8c017039', xmlns: "http://www.w3.org/2000/svg", height: "14", width: "12.25", viewBox: "0 0 448 512" }, h("path", { key: '16220eecf11a25ee16bdd4e4ad827aaa8441a59e', d: "M438.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-160-160c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L338.8 224 32 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l306.7 0L233.4 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l160-160z" })), h("p", { key: '7f4638650271ed8d119322354063a038d5a46c01', class: "pl-2 mapped_item channel_name" }, "igloorooms")), h("div", { key: 'c601135494a1dc16063c3e489b61d51f3894fa66' }, channels_data.selectedChannel?.property?.room_types?.map(room_type => {
...
const IrSwitch = class {
    ...
    render() {
        return (h(Host, { key: '980a151cc9f2e38fe5df92aa1fe9ec40258d20f2' }, h("button", { key: '1170b6f2e6d15372d8af4e3f40e1afaab4e797f5', disabled: this.disabled, ref: el => (this.switchRoot = el), type: "button", id: this.switchId || this._id, onClick: this.handleCheckChange.bind(this), role: "switch", "data-state": this.checked ? 'checked' : 'unchecked', value: 'on', class: "SwitchRoot" }, h("span", { key: '5f705f9b0a5099acae57fded739ebda0791f043d', class: "SwitchThumb", "data-state": this.checked ? 'checked' : 'unchecked' })), h("input", { key: '1e0ade757bc276f1b2cfce65b2c229eadd818b74', type: "checkbox", checked: this.checked, "aria-hidden": "true", tabIndex: -1, value: 'on', class: "hidden-input" })));
    }
};
