...
const IrChannelEditor = class {
    ...
    render() {
        return (h(Host, { key: '72599e336973b161d1c8e45a086bd1d29b0260a4', class: " d-flex flex-column h-100" }, h("nav", { key: '33aa336d71646800bf32c3422b11a1cee838ac65', class: "position-sticky sticky-top pb-1 top-0 bg-white " }, h("div", { key: '47d6166f34bde83d848401b3915b608a9c925cd0', class: "d-flex align-items-center px-1 py-1  justify-content-between" }, h("h3", { key: '06ffa4c65e883e8c28c73a9967fc64b2e2324b64', class: "text-left font-medium-2  py-0 my-0" }, this.channel_status === 'create' ? locales.entries?.Lcz_CreateChannel : locales.entries?.Lcz_EditChannel), h("ir-icon", { key: 'f6eaf94a64b44aff92e05ab3f646f1f54295b16a', class: 'm-0 p-0 close', onIconClickHandler: () => {
                this.closeSideBar.emit(null);
            } }, h("svg", { key: '28edb5ce62fb3cfa3c649a1117572858336f6b9f', slot: "icon", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 384 512", height: 20, width: 20 }, h("path", { key: '1efa8ea3ebcbf94a970b1cbe10389bef2d1cc7fe', d: "M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z" })))), h("ir-channel-header", { key: '4f27259f3b2f52816893261c9af299d245157af3', class: "mt-1 px-0", headerTitles: this.headerTitles })), h("section", { key: '3cfc620b9ecc633b886d41e2c4c1002de8b5c8f4', class: "flex-fill tab-container px-1" }, this.renderTabScreen()), h("ir-button", { key: '3d96128128f9f35318bfc767069103c919eb8ca0', isLoading: this.isLoading, onClickHandler: () => {
...
const IrChannelHeader = class {
    ...
    render() {
        return (h(Host, { key: '6e4003be856c0d6cce3dd999ad5eb5ad2f3892d3' }, h("ul", { key: '803ebcc1b4a369b5fb2d5afb68dff99888faa07b', class: "px-1" }, this.headerTitles.map((title, index) => (h("li", { class: `tab ${title.disabled ? 'text-light' : ''}`, key: title.id, onClick: () => {
                if (!title.disabled)
                    this.handleTabSelection(index);
            }, "data-disabled": title.disabled, "data-state": this.selectedIndex === index ? 'selected' : '' }, title.name)))), h("span", { key: 'd204f4f9216e385d110ff5f4a97aeaf0dd01cc4c', class: "active-indicator", ref: el => (this.activeIndicator = el) })));
    }
};
...
const IrChannelMapping = class {
    ...
    render() {
        return (h(Host, { key: '8b4c36ef3e5175d081ad126c8790a3df9fa1e0cd', class: "py-md-2 px-md-2" }, h("div", { key: 'b4ca24f905183ed8ba1d1a34c86c5f3f013d4065', class: "d-flex p-0 m-0 w-100 justify-content-end" }, h("button", { key: '66cd17e7819937f64f40caf96084e7bf4ee80131', onClick: () => {
                setMappedChannel();
            }, class: "btn refresh-btn" }, locales.entries?.Lcz_Refresh)), h("section", { key: 'a853bb9dc998624a6923b6f6aa218170ced0595f', class: "w-100" }, h("div", { key: '90cc1f24d7bfa8e35b5e3172186e3272cbeadbc4', class: "pt-1 mapped_row" }, h("p", { key: '7bd800e214d7c474cfb7b0e641cd3f033f5532da', class: "mapped_item channel_name" }, channels_data.selectedChannel?.name), h("svg", { key: 'a949525702f975c1613626f318f001ac3da038a3', xmlns: "http://www.w3.org/2000/svg", height: "14", width: "12.25", viewBox: "0 0 448 512" }, h("path", { key: '5f4ed34982b9fd878ca1b912d3fe2c924e4e5b1b', d: "M438.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-160-160c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L338.8 224 32 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l306.7 0L233.4 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l160-160z" })), h("p", { key: 'e2a73d63dc47e0016dfd786b13c51c0aceb09a13', class: "pl-2 mapped_item channel_name" }, "igloorooms")), h("div", { key: 'c9197168ef205eee081ea0388753e370deb7096b' }, channels_data.selectedChannel?.property?.room_types?.map(room_type => {
...
const IrSwitch = class {
    ...
    render() {
        return (h(Host, { key: '89f64119fe75722099b0847c1bc3e8969cbf4647' }, h("button", { key: '0c4b924baee9d5b4bea912ebc420427d7401c5af', disabled: this.disabled, ref: el => (this.switchRoot = el), type: "button", id: this.switchId || this._id, onClick: this.handleCheckChange.bind(this), role: "switch", "data-state": this.checked ? 'checked' : 'unchecked', value: 'on', class: "SwitchRoot" }, h("span", { key: 'd14d427b2fa7ddd66e5bb982070831dc37e771a7', class: "SwitchThumb", "data-state": this.checked ? 'checked' : 'unchecked' })), h("input", { key: 'a69288df2d45f99950cc4be2028e21ed0e4ca06a', type: "checkbox", checked: this.checked, "aria-hidden": "true", tabIndex: -1, value: 'on', class: "hidden-input" })));
    }
};
