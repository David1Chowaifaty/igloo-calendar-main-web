...
const IrChannelEditor = class {
    ...
    render() {
        return (h(Host, { key: '1c7f51f17e5b8e4e3a70271987b5a096e18b9f7e', class: " d-flex flex-column h-100" }, h("nav", { key: 'ad96418052eed787648361b46ece100ec6665379', class: "position-sticky sticky-top pb-1 top-0 bg-white " }, h("div", { key: 'a7be4d2f77531490736113f6111df25435165c5b', class: "d-flex align-items-center px-1 py-1  justify-content-between" }, h("h3", { key: 'e4baa59680c0d9cfc0e2fdea1543c0203d9a609f', class: "text-left font-medium-2  py-0 my-0" }, this.channel_status === 'create' ? locales.entries?.Lcz_CreateChannel : locales.entries?.Lcz_EditChannel), h("ir-icon", { key: '0e722695a280a2abbedb90e4d7235a12d4c8d7a6', class: 'm-0 p-0 close', onIconClickHandler: () => {
                this.closeSideBar.emit(null);
            } }, h("svg", { key: '5e8f29d5196a2c061c6c8abb6077a36193f3e13f', slot: "icon", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 384 512", height: 20, width: 20 }, h("path", { key: 'c24b13aa2fc01ad51b6b695ea181c178468aa76e', d: "M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z" })))), h("ir-channel-header", { key: 'df3f711a0faa329535c2ad9413289084ba95ab67', class: "mt-1 px-0", headerTitles: this.headerTitles })), h("section", { key: '7ba31175dbb3c7923429fb65aba46e5e0aa24964', class: "flex-fill tab-container px-1" }, this.renderTabScreen()), h("ir-button", { key: 'a1d01e5bb6b827cb83d153448eb7903bf63784ac', isLoading: this.isLoading, onClickHandler: () => {
...
const IrChannelHeader = class {
    ...
    render() {
        return (h(Host, { key: 'ea1fd00d8b116c76b2ebd16774ddca2e2b7f8b92' }, h("ul", { key: '7a83b08145d337b208e259274393291c78e4bbea', class: "px-1" }, this.headerTitles.map((title, index) => (h("li", { class: `tab ${title.disabled ? 'text-light' : ''}`, key: title.id, onClick: () => {
                if (!title.disabled)
                    this.handleTabSelection(index);
            }, "data-disabled": title.disabled, "data-state": this.selectedIndex === index ? 'selected' : '' }, title.name)))), h("span", { key: '0e06c3aece6af81efbf3130f0ec3197ab9016822', class: "active-indicator", ref: el => (this.activeIndicator = el) })));
    }
};
...
const IrChannelMapping = class {
    ...
    render() {
        return (h(Host, { key: 'c582505e1315131265052a9c63bd2ab90f80aa05', class: "py-md-2 px-md-2" }, h("div", { key: 'aa80589e1e2fa227981428eb41e993397b70b75a', class: "d-flex p-0 m-0 w-100 justify-content-end" }, h("button", { key: '13c711ed6a9a8e31262f76c32f298e0493583001', onClick: () => {
                setMappedChannel();
            }, class: "btn refresh-btn" }, locales.entries?.Lcz_Refresh)), h("section", { key: 'd2edf7171068622f07695f7e87b9329f6049a91e', class: "w-100" }, h("div", { key: '20e363fef164720a27e22ac035da2f24c84fbb4f', class: "pt-1 mapped_row" }, h("p", { key: '9cce4843bdeae7ce166195a5aa875e1bccd2e207', class: "mapped_item channel_name" }, channels_data.selectedChannel?.name), h("svg", { key: '59bedf3741cc7fc97806eff182c67bc400c22f58', xmlns: "http://www.w3.org/2000/svg", height: "14", width: "12.25", viewBox: "0 0 448 512" }, h("path", { key: '0a7f3dffb5ee08b1019c4ac5c6ca0daf8b48fb6e', d: "M438.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-160-160c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L338.8 224 32 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l306.7 0L233.4 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l160-160z" })), h("p", { key: '8e4ac03e429f96417e0e92d45b9d247436cecb66', class: "pl-2 mapped_item channel_name" }, "igloorooms")), h("div", { key: 'a5e41bcb1b81f37fc049c214c03cf4720322b35d' }, channels_data.selectedChannel?.property?.room_types?.map(room_type => {
...
const IrSwitch = class {
    ...
    render() {
        return (h(Host, { key: 'e32f28562617e1a7f7fa4231a3e35f7734b767a7' }, h("button", { key: '53c0b763202773a004aaef8c64785f65557e8640', disabled: this.disabled, ref: el => (this.switchRoot = el), type: "button", id: this.switchId || this._id, onClick: this.handleCheckChange.bind(this), role: "switch", "data-state": this.checked ? 'checked' : 'unchecked', value: 'on', class: "SwitchRoot" }, h("span", { key: '44c140efa022c2c29e28733d95379a74e0b7e9a6', class: "SwitchThumb", "data-state": this.checked ? 'checked' : 'unchecked' })), h("input", { key: '60388f1e33598eb3a0ce15fe1670f0065bc01dc5', type: "checkbox", checked: this.checked, "aria-hidden": "true", tabIndex: -1, value: 'on', class: "hidden-input" })));
    }
};
