...
import { C as ChannelService, o as onChannelChange, c as channels_data, t as testConnection, u as updateChannelSettings, s as selectChannel, a as addMapping, b as setMappedChannel } from './channel.service-DS6oytML.js';
...
const IrChannelEditor = class {
    ...
    render() {
        return (h(Host, { key: '0b7faffd3a9218eb3c91ad607b362cf9f40b5409', class: " d-flex flex-column h-100" }, h("nav", { key: 'e9d2ebe246bb19707693b4ca25c4356922d4584c', class: "position-sticky sticky-top pb-1 top-0 bg-white " }, h("div", { key: '50950b9b0c7cbf9d053f3e1da1c77859a3092513', class: "d-flex align-items-center px-1 py-1  justify-content-between" }, h("h3", { key: '6051a227cf63ada827b5e6c1229436162c53f657', class: "text-left font-medium-2  py-0 my-0" }, this.channel_status === 'create' ? locales.entries?.Lcz_CreateChannel : locales.entries?.Lcz_EditChannel), h("ir-icon", { key: 'ed6a40610e6069bbfc6098515253e8448d090aa0', class: 'm-0 p-0 close', onIconClickHandler: () => {
                this.closeSideBar.emit(null);
            } }, h("svg", { key: '40d196f56126bc73a277b7026ec61ca05d823e86', slot: "icon", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 384 512", height: 20, width: 20 }, h("path", { key: '3bd333b4c2c490e7047600c198dfb5671eb52b80', d: "M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z" })))), h("ir-channel-header", { key: 'fab9e0b52ce519f91db0b70f18a53f083385a91c', class: "mt-1 px-0", headerTitles: this.headerTitles })), h("section", { key: '50b99650c9315d94956c79e98713e0dced367bdc', class: "flex-fill tab-container px-1" }, this.renderTabScreen()), h("ir-button", { key: '102c73554999bb89f7032709969336e68613db86', isLoading: this.isLoading, onClickHandler: () => {
...
const IrChannelHeader = class {
    ...
    render() {
        return (h(Host, { key: '26078768fce83404d23a4aaf9eb9786b75339fd5' }, h("ul", { key: 'a5cf61ce812f78add59bdf9abedfdf4ffa8ae2b2', class: "px-1" }, this.headerTitles.map((title, index) => (h("li", { class: `tab ${title.disabled ? 'text-light' : ''}`, key: title.id, onClick: () => {
                if (!title.disabled)
                    this.handleTabSelection(index);
            }, "data-disabled": title.disabled, "data-state": this.selectedIndex === index ? 'selected' : '' }, title.name)))), h("span", { key: 'c67025908fae6d59ae7f24fb916b98ad6ed07d85', class: "active-indicator", ref: el => (this.activeIndicator = el) })));
    }
};
...
const IrChannelMapping = class {
    ...
    render() {
        return (h(Host, { key: '932f3f8c9de93b0da9ac5a9e81466609fa1ac99f', class: "py-md-2 px-md-2" }, h("div", { key: '114ae69546f8a528fd805126314b3a83a0bf6461', class: "d-flex p-0 m-0 w-100 justify-content-end" }, h("button", { key: '564eccaafd8e4e8f9352d2a4eb1d20ed388586d2', onClick: () => {
                setMappedChannel();
            }, class: "btn refresh-btn" }, locales.entries?.Lcz_Refresh)), h("section", { key: 'd0c353ccf04ed4d2855e13154d226c8dfc631229', class: "w-100" }, h("div", { key: '14912ff97fd18caba162566461ae526a9c0bb255', class: "pt-1 mapped_row" }, h("p", { key: '05fbc39c20c241dc95511ff30760aecc2d258055', class: "mapped_item channel_name" }, channels_data.selectedChannel?.name), h("svg", { key: '09ed36c9a6d51cd99257e77a0a463412e93d7710', xmlns: "http://www.w3.org/2000/svg", height: "14", width: "12.25", viewBox: "0 0 448 512" }, h("path", { key: 'abc1533bacddf76c5f09f32941b1b518da7fb9e6', d: "M438.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-160-160c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L338.8 224 32 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l306.7 0L233.4 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l160-160z" })), h("p", { key: 'a292aa0f119ea69b78a7aa40c6d7bb1735122440', class: "pl-2 mapped_item channel_name" }, "igloorooms")), h("div", { key: '448bd42931860c7aa9e0b291cfdb42696d2c8681' }, channels_data.selectedChannel?.property?.room_types?.map(room_type => {
...
const IrSwitch = class {
    ...
    render() {
        return (h(Host, { key: '9ad8de9897300038e6117f76c7958728820ebe97' }, h("button", { key: '055c9244f5bd3ca361a9bfe1bb0a66334a31b6c0', disabled: this.disabled, ref: el => (this.switchRoot = el), type: "button", id: this.switchId || this._id, onClick: this.handleCheckChange.bind(this), role: "switch", "data-state": this.checked ? 'checked' : 'unchecked', value: 'on', class: "SwitchRoot" }, h("span", { key: 'ee00361074c7cf2b5e4dd9243686263bc513d7da', class: "SwitchThumb", "data-state": this.checked ? 'checked' : 'unchecked' })), h("input", { key: '2af0e2cbad021120191e20aec179abd2406ae9a1', type: "checkbox", checked: this.checked, "aria-hidden": "true", tabIndex: -1, value: 'on', class: "hidden-input" })));
    }
};
