...
const IrChannelEditor = class {
    ...
    render() {
        return (h(Host, { key: 'b35dadaa0bc0b59d35596b2f3f812f455526e694', class: " d-flex flex-column h-100" }, h("nav", { key: '579a02740a33436a87b5df1b475855677b9c218d', class: "position-sticky sticky-top pb-1 top-0 bg-white " }, h("div", { key: '405a76430e035730283e8236fdcef606d1c12c62', class: "d-flex align-items-center px-1 py-1  justify-content-between" }, h("h3", { key: 'b6c986937e5d911bb0f25b9cb5399fc8aebf879e', class: "text-left font-medium-2  py-0 my-0" }, this.channel_status === 'create' ? locales.entries?.Lcz_CreateChannel : locales.entries?.Lcz_EditChannel), h("ir-icon", { key: '8802e1cf8c3f15b4aa7190ccb62270e0479945dc', class: 'm-0 p-0 close', onIconClickHandler: () => {
                this.closeSideBar.emit(null);
            } }, h("svg", { key: '0cfe19a18f0aca50837d9dd87615b958234ac1ad', slot: "icon", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 384 512", height: 20, width: 20 }, h("path", { key: 'e6b08bce570f6837372c46bc84b5e68eebe6878b', d: "M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z" })))), h("ir-channel-header", { key: 'd4768d9c89f35a683535636d2c15b361fb061fb3', class: "mt-1 px-0", headerTitles: this.headerTitles })), h("section", { key: '80c4799186dc61f0f09d67157ba302775fb5fb63', class: "flex-fill tab-container px-1" }, this.renderTabScreen()), h("ir-button", { key: 'f2e124d95c143416fbd8ef4505c68ce3a0c64a37', isLoading: this.isLoading, onClickHandler: () => {
...
const IrChannelHeader = class {
    ...
    render() {
        return (h(Host, { key: '291df88525363009562b02c25339e07ff08fe320' }, h("ul", { key: 'd1e1f23057e65956b3c1dea6d70d4cce6db86b33', class: "px-1" }, this.headerTitles.map((title, index) => (h("li", { class: `tab ${title.disabled ? 'text-light' : ''}`, key: title.id, onClick: () => {
                if (!title.disabled)
                    this.handleTabSelection(index);
            }, "data-disabled": title.disabled, "data-state": this.selectedIndex === index ? 'selected' : '' }, title.name)))), h("span", { key: 'bb2f21cb9a605b3d672faeb0bbfbd7c4162fdb4f', class: "active-indicator", ref: el => (this.activeIndicator = el) })));
    }
};
...
const IrChannelMapping = class {
    ...
    render() {
        return (h(Host, { key: '1a90203f1c9037ad6a19ce870e4951e55dbf68a3', class: "py-md-2 px-md-2" }, h("div", { key: 'd527c207dabb307595b7c4ff6a42aaded1dc0c4f', class: "d-flex p-0 m-0 w-100 justify-content-end" }, h("button", { key: '22d6911ba589d93f4c501ce1ff739f527111acff', onClick: () => {
                setMappedChannel();
            }, class: "btn refresh-btn" }, locales.entries?.Lcz_Refresh)), h("section", { key: '3e7919998a2d62d4406cf4024882c0cb4314a946', class: "w-100" }, h("div", { key: '49031d5e4a37e1ee97941ea0e2896a0c3abd406a', class: "pt-1 mapped_row" }, h("p", { key: '9d7f2026abbb120dc380b361b9a37c3cbbdde6ee', class: "mapped_item channel_name" }, channels_data.selectedChannel?.name), h("svg", { key: 'a71c4d082f3334d4299f38328000da7393b8fac7', xmlns: "http://www.w3.org/2000/svg", height: "14", width: "12.25", viewBox: "0 0 448 512" }, h("path", { key: '9b51a5eb0e09d4150e1cf27a3fa92eac151d99d5', d: "M438.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-160-160c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L338.8 224 32 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l306.7 0L233.4 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l160-160z" })), h("p", { key: '16ab215e77d0a28101ec59140e4f2d7e9d48ad59', class: "pl-2 mapped_item channel_name" }, "igloorooms")), h("div", { key: '77569471bee7d936ded1781845c367c25602c94a' }, channels_data.selectedChannel?.property?.room_types?.map(room_type => {
...
const IrSwitch = class {
    ...
    render() {
        return (h(Host, { key: '43c3ae2dda99ff3f7116d5515f47dbed02141fb9' }, h("button", { key: 'b38252a8087486d324db097417a1a9552b860566', disabled: this.disabled, ref: el => (this.switchRoot = el), type: "button", id: this.switchId || this._id, onClick: this.handleCheckChange.bind(this), role: "switch", "data-state": this.checked ? 'checked' : 'unchecked', value: 'on', class: "SwitchRoot" }, h("span", { key: 'f51b2d3637f23d8112927ea79e22d5ac49742af4', class: "SwitchThumb", "data-state": this.checked ? 'checked' : 'unchecked' })), h("input", { key: 'e8b83a6322e6d2e14600ef37b97c2312f863825a', type: "checkbox", checked: this.checked, "aria-hidden": "true", tabIndex: -1, value: 'on', class: "hidden-input" })));
    }
};
