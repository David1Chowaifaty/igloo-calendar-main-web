...
const IrChannelEditor = class {
    ...
    render() {
        return (h(Host, { key: '3a157f1b0d62eb9703a0f9ec87ab2a3904d6b07c', class: " d-flex flex-column h-100" }, h("nav", { key: '30b1a72af9959d885a368bd9cc6ea745e28f9ed2', class: "position-sticky sticky-top pb-1 top-0 bg-white " }, h("div", { key: 'fc4be725c5ce3c892db8cab2cd545b40a7df268e', class: "d-flex align-items-center px-1 py-1  justify-content-between" }, h("h3", { key: '3ceb9b7a29f2947bb8fe59d3fca5cc2fea7547bc', class: "text-left font-medium-2  py-0 my-0" }, this.channel_status === 'create' ? locales.entries?.Lcz_CreateChannel : locales.entries?.Lcz_EditChannel), h("ir-icon", { key: 'e357b8380cdd6ca626384b634445700a8e55845f', class: 'm-0 p-0 close', onIconClickHandler: () => {
                this.closeSideBar.emit(null);
            } }, h("svg", { key: 'd4f47b53c52ab822f7c5f763815fd13d4a4a9c75', slot: "icon", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 384 512", height: 20, width: 20 }, h("path", { key: 'e4f0929638dc2313c93ee146e48d37fc5a75d1de', d: "M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z" })))), h("ir-channel-header", { key: 'ae89a6e0d12cc0b978c5eab64f6d5798dd469791', class: "mt-1 px-0", headerTitles: this.headerTitles })), h("section", { key: '46d4eb0419042efa7e0657be7cf3c33f9fde5a71', class: "flex-fill tab-container px-1" }, this.renderTabScreen()), h("ir-button", { key: '0bc1062beafaa85162324be16703125ed8329449', isLoading: this.isLoading, onClickHandler: () => {
...
const IrChannelHeader = class {
    ...
    render() {
        return (h(Host, { key: 'b7b9404d0eae1ae4e2e318b5a8d7c683cd581bfb' }, h("ul", { key: 'b6c200eafa7db7ea38acd666ac874e5738144a1a', class: "px-1" }, this.headerTitles.map((title, index) => (h("li", { class: `tab ${title.disabled ? 'text-light' : ''}`, key: title.id, onClick: () => {
                if (!title.disabled)
                    this.handleTabSelection(index);
            }, "data-disabled": title.disabled, "data-state": this.selectedIndex === index ? 'selected' : '' }, title.name)))), h("span", { key: '6544959c4f1f7a8321877aa49b1510f6468e0953', class: "active-indicator", ref: el => (this.activeIndicator = el) })));
    }
};
...
const IrChannelMapping = class {
    ...
    render() {
        return (h(Host, { key: '1f8e91dcd0c83d6ab73f31fd75fdf0e0523250cf', class: "py-md-2 px-md-2" }, h("div", { key: '43dcb588a98d53f13d2d7d6cb365f3e3e21e3a32', class: "d-flex p-0 m-0 w-100 justify-content-end" }, h("button", { key: 'ecdb3ef50ffe7c93095faf39e442f9c66d193bb5', onClick: () => {
                setMappedChannel();
            }, class: "btn refresh-btn" }, locales.entries?.Lcz_Refresh)), h("section", { key: '9e28cf2296a195cacf543f123b1f9f8e0e75ab0d', class: "w-100" }, h("div", { key: 'a2241be6f3348ca0a9953dd6efcd3d858ad42b3d', class: "pt-1 mapped_row" }, h("p", { key: '1333cd230c90c8d8f6b356fe9c2d4dc6059282d5', class: "mapped_item channel_name" }, channels_data.selectedChannel?.name), h("svg", { key: '7a66dccaae534b861cafd6ecc05e34a7fbff986c', xmlns: "http://www.w3.org/2000/svg", height: "14", width: "12.25", viewBox: "0 0 448 512" }, h("path", { key: '8fcf37d9b731ec60d5426e8bb1d4ef0565fb2b51', d: "M438.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-160-160c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L338.8 224 32 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l306.7 0L233.4 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l160-160z" })), h("p", { key: '9f3ce3b2bc9c1812280a6fb773a6ac35177ecbc5', class: "pl-2 mapped_item channel_name" }, "igloorooms")), h("div", { key: '947335afbb2a855a4dadb7d5fec36da519baf167' }, channels_data.selectedChannel?.property?.room_types?.map(room_type => {
...
const IrSwitch = class {
    ...
    render() {
        return (h(Host, { key: 'c96b003f55070366013830b13c39fda09259c6f5' }, h("button", { key: 'ed7ccc4f78ec3e717147faf535f758c34ea846a3', disabled: this.disabled, ref: el => (this.switchRoot = el), type: "button", id: this.switchId || this._id, onClick: this.handleCheckChange.bind(this), role: "switch", "data-state": this.checked ? 'checked' : 'unchecked', value: 'on', class: "SwitchRoot" }, h("span", { key: 'db27d49d92faf16bcf860e476ba842c323c0f501', class: "SwitchThumb", "data-state": this.checked ? 'checked' : 'unchecked' })), h("input", { key: '6e225faad611224a94419e84eafbd167e09cee84', type: "checkbox", checked: this.checked, "aria-hidden": "true", tabIndex: -1, value: 'on', class: "hidden-input" })));
    }
};
