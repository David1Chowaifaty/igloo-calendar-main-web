...
const IrChannelEditor = class {
    ...
    render() {
        return (h(Host, { key: '6521502c4648abd8ca7267136dfd9928b7ddfa3e', class: " d-flex flex-column h-100" }, h("nav", { key: 'ac895927a80348717edf2c0b44cf9fcca870f4e2', class: "position-sticky sticky-top pb-1 top-0 bg-white " }, h("div", { key: '8003437f15602448dfa32d3c2919166f5363e015', class: "d-flex align-items-center px-1 py-1  justify-content-between" }, h("h3", { key: 'e9552843ac9bb3d16471ed16afac9f5f35691b3c', class: "text-left font-medium-2  py-0 my-0" }, this.channel_status === 'create' ? locales.entries?.Lcz_CreateChannel : locales.entries?.Lcz_EditChannel), h("ir-icon", { key: '4ad02938a368e17bd1a59ac3a55d0e0d96dce5bb', class: 'm-0 p-0 close', onIconClickHandler: () => {
                this.closeSideBar.emit(null);
            } }, h("svg", { key: 'd12580a8bcff028c4731cbfe750fe1a75232a3f9', slot: "icon", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 384 512", height: 20, width: 20 }, h("path", { key: 'c63d9a1e49383ad03e72ed429e9e8f947f3b2b86', d: "M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z" })))), h("ir-channel-header", { key: '8ab9143dfed43cecb606486371c1ac132b315605', class: "mt-1 px-0", headerTitles: this.headerTitles })), h("section", { key: 'ffcf26453716d49201222c42324d1d2b069dcfa1', class: "flex-fill tab-container px-1" }, this.renderTabScreen()), h("ir-button", { key: '18cd76f0e161abc22ee915c21c3e5ee6e25d2098', isLoading: this.isLoading, onClickHandler: () => {
...
const IrChannelHeader = class {
    ...
    render() {
        return (h(Host, { key: 'f5655574024f033c011671b1cd5af5f2f1f11932' }, h("ul", { key: 'd77428aca83aaed7dfa2ea3f326da9d434c796d5', class: "px-1" }, this.headerTitles.map((title, index) => (h("li", { class: `tab ${title.disabled ? 'text-light' : ''}`, key: title.id, onClick: () => {
                if (!title.disabled)
                    this.handleTabSelection(index);
            }, "data-disabled": title.disabled, "data-state": this.selectedIndex === index ? 'selected' : '' }, title.name)))), h("span", { key: 'e114def0ddb0bfbaf9534e7f2f83929fb12539a7', class: "active-indicator", ref: el => (this.activeIndicator = el) })));
    }
};
...
const IrChannelMapping = class {
    ...
    render() {
        return (h(Host, { key: 'a5eadf74acbe0d99243a29bdfe7f1c7e6a7dee74', class: "py-md-2 px-md-2" }, h("div", { key: '754a7cf3d4898fea12177389e8836128cedda16d', class: "d-flex p-0 m-0 w-100 justify-content-end" }, h("button", { key: 'a456c4e2feefcb92dfc4d8a41281435fa90f7086', onClick: () => {
                setMappedChannel();
            }, class: "btn refresh-btn" }, locales.entries?.Lcz_Refresh)), h("section", { key: '791a62f7cf4684a48631791cbb24b03231535ded', class: "w-100" }, h("div", { key: '83cdcc5c6c2fe259822a1c8c1e944acc5859e72e', class: "pt-1 mapped_row" }, h("p", { key: '5bbad61871ddba75cb884d212e77ce874307b0f2', class: "mapped_item channel_name" }, channels_data.selectedChannel?.name), h("svg", { key: '170346605614f851ac474601b45b2211f06693b6', xmlns: "http://www.w3.org/2000/svg", height: "14", width: "12.25", viewBox: "0 0 448 512" }, h("path", { key: '701c10a37684f79f140939e640b250677da029bc', d: "M438.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-160-160c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L338.8 224 32 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l306.7 0L233.4 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l160-160z" })), h("p", { key: 'a3ee1c63ac85696b86be8596b15955c6c3c51cca', class: "pl-2 mapped_item channel_name" }, "igloorooms")), h("div", { key: 'd6c7e04bcb5aaf62ca9e856839e83671921e0889' }, channels_data.selectedChannel?.property?.room_types?.map(room_type => {
...
const IrSwitch = class {
    ...
    render() {
        return (h(Host, { key: 'e8ecaa820b9df00abbc6b9dadde859f8db25a4dd' }, h("button", { key: '9a96929b2e7838f77c68bbe89c714bab160d3ae2', disabled: this.disabled, ref: el => (this.switchRoot = el), type: "button", id: this.switchId || this._id, onClick: this.handleCheckChange.bind(this), role: "switch", "data-state": this.checked ? 'checked' : 'unchecked', value: 'on', class: "SwitchRoot" }, h("span", { key: '39edff7e81ad89cf5042bec8222b737af490f91b', class: "SwitchThumb", "data-state": this.checked ? 'checked' : 'unchecked' })), h("input", { key: 'f422bafb83cc479f8f61d88708530286a7f29f43', type: "checkbox", checked: this.checked, "aria-hidden": "true", tabIndex: -1, value: 'on', class: "hidden-input" })));
    }
};
