...
const IrChannelEditor = class {
    ...
    render() {
        return (h(Host, { key: '7327829878818e80e724661f2b7af9cafc32e534', class: " d-flex flex-column h-100" }, h("nav", { key: '4f738a7129f033248396ce32528b3bb81f96ca2d', class: "position-sticky sticky-top pb-1 top-0 bg-white " }, h("div", { key: '92ea6939c77f3c9764898cec7e53ed792ce4dc2b', class: "d-flex align-items-center px-1 py-1  justify-content-between" }, h("h3", { key: '38001f789900541b3074e0734f19e3123cd8e1d2', class: "text-left font-medium-2  py-0 my-0" }, this.channel_status === 'create' ? locales.entries?.Lcz_CreateChannel : locales.entries?.Lcz_EditChannel), h("ir-icon", { key: '114337a33608ea32e38b0ff375c047f18356943e', class: 'm-0 p-0 close', onIconClickHandler: () => {
                this.closeSideBar.emit(null);
            } }, h("svg", { key: '9f13396f04605febe5366b09b524ec07400055bc', slot: "icon", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 384 512", height: 20, width: 20 }, h("path", { key: 'd89b73417f220fc7c2cd86556a7e764a09b52605', d: "M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z" })))), h("ir-channel-header", { key: '5bff4ee178c153616e52372d7fa80812ee8ee16d', class: "mt-1 px-0", headerTitles: this.headerTitles })), h("section", { key: '04275dd4e106602beab05ddf0692c32d890eddb6', class: "flex-fill tab-container px-1" }, this.renderTabScreen()), h("ir-button", { key: '09b417f59081aa896618b52586be8d6a34deec15', isLoading: this.isLoading, onClickHandler: () => {
...
const IrChannelHeader = class {
    ...
    render() {
        return (h(Host, { key: '8da51b045c99c095cb7479eafd3d17b9337178e5' }, h("ul", { key: 'da505b833b56deb18a06e63d256cd328988b63bb', class: "px-1" }, this.headerTitles.map((title, index) => (h("li", { class: `tab ${title.disabled ? 'text-light' : ''}`, key: title.id, onClick: () => {
                if (!title.disabled)
                    this.handleTabSelection(index);
            }, "data-disabled": title.disabled, "data-state": this.selectedIndex === index ? 'selected' : '' }, title.name)))), h("span", { key: 'd0360b1a33072b6dcdc409e3f1f1418ef44fa5da', class: "active-indicator", ref: el => (this.activeIndicator = el) })));
    }
};
...
const IrChannelMapping = class {
    ...
    render() {
        return (h(Host, { key: '929b0e5ad6b4a5f535bbe2e8f7f93181ed8cf515', class: "py-md-2 px-md-2" }, h("div", { key: '8d4ab5a3a7017777bcc8e79f0223fe7876eac1a6', class: "d-flex p-0 m-0 w-100 justify-content-end" }, h("button", { key: '021ca49c96b2230a0bc10939247c732d0bdbb8e1', onClick: () => {
                setMappedChannel();
            }, class: "btn refresh-btn" }, locales.entries?.Lcz_Refresh)), h("section", { key: '265ddbbe1343f1c9332b6a19d24133f152f3cc7b', class: "w-100" }, h("div", { key: '36b811e20dee99c61a33cca107b9a4fc163b00fa', class: "pt-1 mapped_row" }, h("p", { key: '56afc93b24b54ebd89eae65b809a36f7588c690d', class: "mapped_item channel_name" }, channels_data.selectedChannel?.name), h("svg", { key: '7363d6dbce92040d0a6816de61d6e07472183e57', xmlns: "http://www.w3.org/2000/svg", height: "14", width: "12.25", viewBox: "0 0 448 512" }, h("path", { key: 'ec4c481b9c80eea0d28452807993af767590fb2d', d: "M438.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-160-160c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L338.8 224 32 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l306.7 0L233.4 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l160-160z" })), h("p", { key: '5f7ed0ca75a8d924b1ea9e205eb276b73f96f913', class: "pl-2 mapped_item channel_name" }, "igloorooms")), h("div", { key: '931eddee6894fda45e16643244ce6118638f4baa' }, channels_data.selectedChannel?.property?.room_types?.map(room_type => {
...
const IrSwitch = class {
    ...
    render() {
        return (h(Host, { key: 'da6fdacd606584c8fa9ee5bf005afc1e577e0d9b' }, h("button", { key: 'a8e5e87c26492999d0ddb06259871f0ce6710fa1', disabled: this.disabled, ref: el => (this.switchRoot = el), type: "button", id: this.switchId || this._id, onClick: this.handleCheckChange.bind(this), role: "switch", "data-state": this.checked ? 'checked' : 'unchecked', value: 'on', class: "SwitchRoot" }, h("span", { key: 'cc83cc6dfcab5a1d8024465a7f281e15b796a656', class: "SwitchThumb", "data-state": this.checked ? 'checked' : 'unchecked' })), h("input", { key: 'df00f4bb9b29b722f437b0e92b54a5c96da4e137', type: "checkbox", checked: this.checked, "aria-hidden": "true", tabIndex: -1, value: 'on', class: "hidden-input" })));
    }
};
