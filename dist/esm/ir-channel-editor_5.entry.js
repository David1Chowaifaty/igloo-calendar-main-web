...
const IrChannelEditor = class {
    ...
    render() {
        return (h(Host, { key: 'a4d8dd510e4dec91d0b96068baa0d600bf1c25af', class: " d-flex flex-column h-100" }, h("nav", { key: 'b92450a4aed745831eec216a96aa393e8e931a0d', class: "position-sticky sticky-top pb-1 top-0 bg-white " }, h("div", { key: '9551ca536efc3540e6bec9ca510c22317ae45de3', class: "d-flex align-items-center px-1 py-1  justify-content-between" }, h("h3", { key: '020d79a1b13919163680578bab90df9c85b956ad', class: "text-left font-medium-2  py-0 my-0" }, this.channel_status === 'create' ? locales.entries?.Lcz_CreateChannel : locales.entries?.Lcz_EditChannel), h("ir-icon", { key: 'e9462494171bf257e25470ee5203a6f46080a353', class: 'm-0 p-0 close', onIconClickHandler: () => {
                this.closeSideBar.emit(null);
            } }, h("svg", { key: '535c45db54345b7e399ef3e2d912ceade426460b', slot: "icon", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 384 512", height: 20, width: 20 }, h("path", { key: '07b7b4563125ab8294359a1890dd3ed35973ea8e', d: "M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z" })))), h("ir-channel-header", { key: '443d3717f54056439218786d85fd0298cdebe659', class: "mt-1 px-0", headerTitles: this.headerTitles })), h("section", { key: 'f84826c59ce6ad0e9863c1ffb46c28f5aff287cb', class: "flex-fill tab-container px-1" }, this.renderTabScreen()), h("ir-button", { key: '81553c94213a6a197dade1508556f6f5caf295d3', isLoading: this.isLoading, onClickHandler: () => {
...
const IrChannelHeader = class {
    ...
    render() {
        return (h(Host, { key: '6d8f2c55fd9b277c75a287055e8c9813a95d4e80' }, h("ul", { key: '323557b751bf9a402d4236775d8bca44e4d90ffd', class: "px-1" }, this.headerTitles.map((title, index) => (h("li", { class: `tab ${title.disabled ? 'text-light' : ''}`, key: title.id, onClick: () => {
                if (!title.disabled)
                    this.handleTabSelection(index);
            }, "data-disabled": title.disabled, "data-state": this.selectedIndex === index ? 'selected' : '' }, title.name)))), h("span", { key: '3004fed7886418b814ffc3cc791b24d2f2f196fe', class: "active-indicator", ref: el => (this.activeIndicator = el) })));
    }
};
...
const IrChannelMapping = class {
    ...
    render() {
        return (h(Host, { key: 'a44998dc1484f2789efaf6510295d4eebb467ec6', class: "py-md-2 px-md-2" }, h("div", { key: 'c112e690adcd3f7de1b14be96e2a30c9c81b84e3', class: "d-flex p-0 m-0 w-100 justify-content-end" }, h("button", { key: 'a5599e649ccb33b4a9309cc3b7d5a90184290418', onClick: () => {
                setMappedChannel();
            }, class: "btn refresh-btn" }, locales.entries?.Lcz_Refresh)), h("section", { key: '5f9a86dd9296339cb12a6b3f24ea911a8cd8bc3d', class: "w-100" }, h("div", { key: '9928d3209f3aa6128781c30d6b7a1c0d8065236d', class: "pt-1 mapped_row" }, h("p", { key: 'be400859962a784d27b63ba64e8f8c2b7f377b97', class: "mapped_item channel_name" }, channels_data.selectedChannel?.name), h("svg", { key: '660fe9c985350c500d0b4e82e9f7941944a36590', xmlns: "http://www.w3.org/2000/svg", height: "14", width: "12.25", viewBox: "0 0 448 512" }, h("path", { key: '6cb24b6b5c05ad1af90d4c61b5b6e0b8bd8c7084', d: "M438.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-160-160c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L338.8 224 32 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l306.7 0L233.4 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l160-160z" })), h("p", { key: 'd6966702726fb143f881d2f767d189ab05db461f', class: "pl-2 mapped_item channel_name" }, "igloorooms")), h("div", { key: 'cb3c1dd217fed168534e24ba54fa55a26392864f' }, channels_data.selectedChannel?.property?.room_types?.map(room_type => {
...
const IrSwitch = class {
    ...
    render() {
        return (h(Host, { key: '4304660c42b16349c546fef5688ec145f2054d59' }, h("button", { key: '28c7b9a74e5593a98f3a87ed7c68f2498e749318', disabled: this.disabled, ref: el => (this.switchRoot = el), type: "button", id: this.switchId || this._id, onClick: this.handleCheckChange.bind(this), role: "switch", "data-state": this.checked ? 'checked' : 'unchecked', value: 'on', class: "SwitchRoot" }, h("span", { key: '594f6281edc40caa781e115d5f655fa6e8511534', class: "SwitchThumb", "data-state": this.checked ? 'checked' : 'unchecked' })), h("input", { key: '4690d7eef821780e4db1ccadbe8a84a4e5ffaf6a', type: "checkbox", checked: this.checked, "aria-hidden": "true", tabIndex: -1, value: 'on', class: "hidden-input" })));
    }
};
