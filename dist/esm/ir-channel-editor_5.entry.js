...
const IrChannelEditor = class {
    ...
    render() {
        return (h(Host, { key: 'c93fbad12f75439fdd06a4963c87fa37591fc561', class: " d-flex flex-column h-100" }, h("nav", { key: 'fa7d8d16545d57c8ecc2d01648d7cd210a601aea', class: "position-sticky sticky-top pb-1 top-0 bg-white " }, h("div", { key: '9e3573f2a288dfe4e8e8c25a6ab0ea89c8d184a3', class: "d-flex align-items-center px-1 py-1  justify-content-between" }, h("h3", { key: 'd6ebc7041b9c411f667d346f02bc7ffff9245a50', class: "text-left font-medium-2  py-0 my-0" }, this.channel_status === 'create' ? locales.entries?.Lcz_CreateChannel : locales.entries?.Lcz_EditChannel), h("ir-icon", { key: '1bbfe5b4e04961290c9f2f0a39a01dc13e795d52', class: 'm-0 p-0 close', onIconClickHandler: () => {
                this.closeSideBar.emit(null);
            } }, h("svg", { key: 'ffb050814dfad59d319c52b1873b0de870edcf0e', slot: "icon", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 384 512", height: 20, width: 20 }, h("path", { key: 'd5f906fba37083b7f44e002a16c7002636acef06', d: "M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z" })))), h("ir-channel-header", { key: 'eb42a43abe98f12ee10ee835e8beb4a17488fc68', class: "mt-1 px-0", headerTitles: this.headerTitles })), h("section", { key: '474346cd3067abed72e79101286afa1c4f98283a', class: "flex-fill tab-container px-1" }, this.renderTabScreen()), h("ir-button", { key: 'c9d4973e9c33012b5f6a23b23bb6048f4d37a4de', isLoading: this.isLoading, onClickHandler: () => {
...
const IrChannelHeader = class {
    ...
    render() {
        return (h(Host, { key: '398b93bda17f06190aecf05733c9300874b1ef42' }, h("ul", { key: '2dded63c7bba96254a89e93222c3cd8ed1c0246b', class: "px-1" }, this.headerTitles.map((title, index) => (h("li", { class: `tab ${title.disabled ? 'text-light' : ''}`, key: title.id, onClick: () => {
                if (!title.disabled)
                    this.handleTabSelection(index);
            }, "data-disabled": title.disabled, "data-state": this.selectedIndex === index ? 'selected' : '' }, title.name)))), h("span", { key: '27e549446480ec8c774a8d0bad3772aa4f9ffcf9', class: "active-indicator", ref: el => (this.activeIndicator = el) })));
    }
};
...
const IrChannelMapping = class {
    ...
    render() {
        return (h(Host, { key: '2cd615b382e47e536490316bfbf0dfc205f815f6', class: "py-md-2 px-md-2" }, h("div", { key: '041bf42ebf31d074f51e0060a9ac1fe35769fe5a', class: "d-flex p-0 m-0 w-100 justify-content-end" }, h("button", { key: '2d0e6579120acee0439cfadaa5e84ad96f53ae11', onClick: () => {
                setMappedChannel();
            }, class: "btn refresh-btn" }, locales.entries?.Lcz_Refresh)), h("section", { key: '8487ec740d5bc050c2929cf06768f756e59967fd', class: "w-100" }, h("div", { key: 'ed5b7896fea55b2aa69960e64a7b1f4d33d025ab', class: "pt-1 mapped_row" }, h("p", { key: '799871adc49e242f65b3f6e919a9eccde85e131b', class: "mapped_item channel_name" }, channels_data.selectedChannel?.name), h("svg", { key: '8f5721871bbdae0911124a9e5552f32f630f38da', xmlns: "http://www.w3.org/2000/svg", height: "14", width: "12.25", viewBox: "0 0 448 512" }, h("path", { key: '9cc8628c0f2dc9c6605705f4afa77950c33edf05', d: "M438.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-160-160c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L338.8 224 32 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l306.7 0L233.4 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l160-160z" })), h("p", { key: 'db8a6e773fec8ca07edb00766117fe1530c79b94', class: "pl-2 mapped_item channel_name" }, "igloorooms")), h("div", { key: 'fea51ef134f8a39ddc5194d008f3abe8cedd8ed5' }, channels_data.selectedChannel?.property?.room_types?.map(room_type => {
