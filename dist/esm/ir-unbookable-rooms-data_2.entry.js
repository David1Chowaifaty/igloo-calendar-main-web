...
const IrUnbookableRoomsFilters = class {
    ...
    render() {
        ...
        return (h(Host, { key: 'ef208b439a81342da3fa1e0fbe45983ccae0eb1e' }, h("wa-card", { key: 'd7cd5cb251da5ae60154d1f4de4ec18000cd5a80', class: "unbookable-rooms__filters-card", appearance: "outlined" }, h("div", { key: '1f81a37b983b5c7a96c71814b571b14d560902c4', slot: "header", class: "filters-card__header" }, h("wa-icon", { key: 'bd504ce8386bb0a855057c63804c161ae3886314', name: "filter" }), h("h4", { key: 'ca8132e0e130e0af48356e87a5e8777c50263e85', class: "filters-card__title m-0" }, "Filters")), h("wa-select", { key: '575be66abfc626c8566ab3be6c19a55803f56b5d', label: "Look ahead", size: "small", value: this.filters.period_to_check?.toString(), defaultValue: this.filters.period_to_check?.toString(), onchange: e => {
                this.handlePeriodChange(e.target.value);
            } }, Array.from({ length: 5 }, (_, i) => i + 2).map(value => (h("wa-option", { value: value.toString() }, value, " month", value > 1 ? 's' : '')))), h("ir-input", { key: '70671f2ff818aaaef70395866d3e0a5f29b87f63', type: "number", label: "Minimum consecutive nights", min: "7", hint: "Period where room types are closed for booking.", max: "60", value: this.filters.consecutive_period?.toString(), "onText-change": e => {
                this.handleConsecutiveChange(e.detail);
            } }), this.mode === 'mpo' && sortedCountries.length > 1 && (h("wa-select", { key: 'b758879151bff4e9b50942bed8a73b8f5e83e640', label: "Country", size: "small", value: this.filters.country?.toString(), defaultValue: "all", onchange: e => {
                this.handleCountryChange(e.target.value);
            } }, h("wa-option", { key: '64527b9aedd72a3599c56eb7954fac2ef26bc567', value: "all" }, "Show all"), sortedCountries.map(([id, name]) => (h("wa-option", { value: id.toString() }, name))))), h("div", { key: '21dbf4f293b667488055689c449d7c21dabfaaef', class: "filter__actions" }, h("ir-custom-button", { key: 'eb25495e219d5dfe03c4c0a9087b3d8cc265f5d5', onClickHandler: () => this.filtersReset.emit(), variant: "neutral", appearance: "filled" }, "Reset"), h("ir-custom-button", { key: '4fffff160133d73c92a3b1e29e48ea04d113de75', loading: this.isLoading, onClickHandler: () => this.filtersSave.emit(), variant: "brand" }, "Save")))));
    }
};
