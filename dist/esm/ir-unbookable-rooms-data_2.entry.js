...
const IrUnbookableRoomsFilters = class {
    ...
    render() {
        ...
        return (h(Host, { key: '45a685a8641ab23f7203e0e2d5e282fee5f1e258' }, h("wa-card", { key: 'b55228879a0d5f857a759570e1fc075c2ecf4763', class: "unbookable-rooms__filters-card", appearance: "outlined" }, h("div", { key: '438c52568bd3b5c4a120bad179ab5ad936ee2b89', slot: "header", class: "filters-card__header" }, h("wa-icon", { key: '313c33f341ad82d13b742121b095f1cde8e3ca9d', name: "filter" }), h("h4", { key: '4705ba2f160d2544c650298085100abeb37c0119', class: "filters-card__title m-0" }, "Filters")), h("wa-select", { key: '9a598e9df12b495624532350602ad2210538d5ed', label: "Look ahead", size: "s", value: this.filters.period_to_check?.toString(), defaultValue: this.filters.period_to_check?.toString(), onchange: e => {
                this.handlePeriodChange(e.target.value);
            } }, Array.from({ length: 5 }, (_, i) => i + 2).map(value => (h("wa-option", { value: value.toString() }, value, " month", value > 1 ? 's' : '')))), h("ir-input", { key: 'd93a807425b57e9bc597c47ca3de5e72126d61f9', type: "number", label: "Minimum consecutive nights", min: "7", hint: "Period where room types are closed for booking.", max: "60", value: this.filters.consecutive_period?.toString(), "onText-change": e => {
                this.handleConsecutiveChange(e.detail);
            } }), this.mode === 'mpo' && sortedCountries.length > 1 && (h("wa-select", { key: '8fb33994b25959899c8f1ef0093ed3aea27e46dc', label: "Country", size: "s", value: this.filters.country?.toString(), defaultValue: "all", onchange: e => {
                this.handleCountryChange(e.target.value);
            } }, h("wa-option", { key: 'efb19a0f9d36b87a799ef71c3415dca63d8e7c02', value: "all" }, "Show all"), sortedCountries.map(([id, name]) => (h("wa-option", { value: id.toString() }, name))))), h("div", { key: 'd3899e8da10b16391ba2f6733c901f375c5f1fbd', class: "filter__actions" }, h("ir-custom-button", { key: 'ec336ece6f6af289b570485f9753f1b80f79007d', onClickHandler: () => this.filtersReset.emit(), variant: "neutral", appearance: "filled" }, "Reset"), h("ir-custom-button", { key: '12c07272afc147b8c8029557b2ff457b8f3444d4', loading: this.isLoading, onClickHandler: () => this.filtersSave.emit(), variant: "brand" }, "Save")))));
    }
};
