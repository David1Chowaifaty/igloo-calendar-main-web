...
import { C as CityLedgerService, m as mapClTxToFolioRow, b as buildPaymentTypes } from './utils-4b305bfd.js';
...
import { l as buildSplitIndex, c as calculateDaysBetweenDates, k as getPrivateNote, f as formatName } from './booking-bf2d7cb1.js';
...
import { t as taxationModes } from './property.service-2641b60e.js';
...
const IrArrivalTimeDialog = class {
    ...
    render() {
        return (h("ir-dialog", { key: '02fbf4b59d50a6b6fb694d61bf49883092df7e9d', label: "Edit Arrival Time", open: this.open, onIrDialogHide: () => {
                this.open = false;
            } }, h("wa-select", { key: 'c910c0c32670f107f7486efb1151d5318ca4c239', size: "small", value: this.selectedArrivalTime, defaultValue: this.selectedArrivalTime, onchange: e => this.updateArrivalTime(e.target.value) }, this.arrivalTime.map(time => (h("wa-option", { value: time.CODE_NAME, selected: time.CODE_NAME === this.selectedArrivalTime }, time.CODE_VALUE_EN)))), h("div", { key: '220372d8af4e2a2538aaf826b2ca23394f9269f5', slot: "footer", class: "ir-dialog__footer" }, h("ir-custom-button", { key: 'ac64565bfbdf00f51dc8894feef2e1c0b68048b3', size: "medium", appearance: "filled", variant: "neutral", onClickHandler: () => this.closeDialog() }, locales.entries.Lcz_Cancel), h("ir-custom-button", { key: '8b4abc268a6fd15acce0471377ca842c4d504176', size: "medium", variant: "brand", onClickHandler: () => this.saveArrivalTime(), loading: this.isLoading }, locales.entries.Lcz_Save))));
    }
};
...
const IrBillingDrawer = class {
    ...
    billingClose;
    render() {
        return (h("ir-drawer", { key: '112c9ffd1f129faa108d489e3d48fa988eb9ef8e', style: {
            ...
            }, open: this.open, label: "Billing" }, this.open && h("ir-billing", { key: 'f3900d941a0c2c9154f491e6184ed3100a78a234', booking: this.booking })));
    }
};
...
const IrBookingStatusTag = class {
    ...
    render() {
        return (h("wa-badge", { key: '5d0c0e30c125d97ec916222031af9ca9d5300638', style: { padding: '0.375em 0.625em', letterSpacing: '0.03rem' }, variant: this.badgeVariant[this.isRequestToCancel ? '003' : this.status.code] }, this.status.description.toUpperCase()));
    }
};
...
const IrPaymentFolioForm = class {
    ...
    render() {
        // const isNewPayment = this.folioData?.payment_type?.code === '001' && this.folioData.id === -1;
        return (h("form", { key: 'ff0e5bec27afe63f0e51ffe3ae93fed797e6b09c', onSubmit: e => {
                e.preventDefault();
                const submitter = e.submitter;
                if (submitter?.value === 'save') {
                    this.savePayment();
                }
            }, class: "payment-folio__form", id: this.formId }, h("ir-custom-date-picker", { key: 'eef807e42a2c1d1fe0e98feb6dd5f88004e234db', id: this.controlIds.date, label: "Date", "aria-invalid": this.errors?.date && !this.folioData?.date ? 'true' : 'false', "data-testid": "pickup_date", onDateChanged: evt => {
                this.updateFolioData({ date: evt.detail.start?.format(DATE_FORMAT) });
            }, minDate: hooks().add(-2, 'months').format('YYYY-MM-DD'), emitEmptyDate: true, maxDate: this.today, date: this.folioData?.date }), h("ir-validator", { key: '1148a4667119f134f8d4610a95d5e1b882d001af', value: this.folioData?.payment_type?.code, autovalidate: this.autoValidate, schema: paymentTypeSchema.shape.code, valueEvent: "change wa-change select-change", blurEvent: "wa-hide" }, h("wa-select", { key: '9e824d685dfa008098b2e44c86ec7eb1383d18ef', id: this.controlIds.transactionType, size: "small", "onwa-hide": event => this.stopEventPropagation(event), "onwa-show": event => this.stopEventPropagation(event), placeholder: "Select...", label: "Transaction type", defaultValue: this.folioData?.payment_type?.code, value: this.folioData?.payment_type?.code, disabled: this.mode === 'payment-action', onchange: event => {
                this.stopEventPropagation(event);
                this.handleDropdownChange(event.target.value);
            } }, h("wa-option", { key: '1d3fcfecf07d68ebdf15af1f28e8435d7026a400', value: "" }, "Select..."), this.renderDropdownItems())), this.requiresPaymentMethod(this.folioData?.payment_type?.code) && (h("ir-validator", { key: 'c574ef3104ab11c3712375a1606fd5a507df4a51', value: this.folioData?.payment_method?.code ?? '', autovalidate: this.autoValidate, schema: paymentMethodSchema.shape.code, valueEvent: "change wa-change select-change", blurEvent: "wa-hide" }, h("wa-select", { key: '1216b6400d90adf9b38db66ef70d1dc57c9485c5', id: this.controlIds.paymentMethod, size: "small", label: `${this.folioData.payment_type?.code === '001' ? 'Payment' : 'Refund'} method`, "onwa-show": event => this.stopEventPropagation(event), "onwa-hide": event => this.stopEventPropagation(event), defaultValue: this.folioData?.payment_method?.code, value: this.folioData?.payment_method?.code ?? '', onchange: event => {
                this.stopEventPropagation(event);
                this.handlePaymentMethodDropdownChange(event.target.value);
            } }, h("wa-option", { key: '4c9ad3e57cd624a9b61bde026d52ea2be9f6521d', value: "" }, "Select..."), this.paymentEntries?.methods?.map(pt => {
            return (h("wa-option", { key: pt.CODE_NAME, label: pt.CODE_VALUE_EN, value: pt.CODE_NAME }, pt.CODE_VALUE_EN));
        })))), h("ir-validator", { key: '6bf7998a1926d1b6e6c388eee6051b331687e059', value: this.folioData?.amount?.toString() ?? undefined, autovalidate: this.autoValidate, schema: folioBaseSchema.shape.amount, valueEvent: "text-change input input-change", blurEvent: "input-blur" }, h("ir-input", { key: '3f8f8741e278d826eeac93104a0d79f3d44e1f55', id: this.controlIds.amount, "aria-invalid": String(!!this.errors?.amount), value: this.folioData?.amount?.toString() ?? '', label: "Amount", mask: "price", min: 0, "onText-change": e => this.updateFolioData({ amount: !e.detail ? undefined : Number(e.detail) }) }, h("span", { key: '8df6e937150e29593062bc28caf4fcf3df8a2ca6', slot: "start" }, calendar_data.currency.symbol))), h("ir-validator", { key: '2dc2f7728b9643863ca9c1ef5181c0110ace27d5', value: this.folioData?.reference ?? '', autovalidate: this.autoValidate, schema: folioBaseSchema.shape.reference, valueEvent: "text-change input input-change", blurEvent: "input-blur" }, h("ir-input", { key: '450a2d8da48408e3511437345a9d1640d8dd8080', id: this.controlIds.reference, value: this.folioData?.reference ?? '', label: "Reference", maxlength: 50, "onText-change": e => this.updateFolioData({ reference: e.detail ?? '' }) })), isAgentMode(this.booking) && this.hasUnassignedItems() && h("ir-service-assignee-select", { key: 'dd7dd388042ad224f534d279a6cebbeac4e5da47', agent: this.booking.agent })));
    }
    static get watchers() { return {
