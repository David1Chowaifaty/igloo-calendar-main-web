...
import { C as CityLedgerService, m as mapClTxToFolioRow, b as buildPaymentTypes } from './utils-f1649df9.js';
...
import { l as buildSplitIndex, c as calculateDaysBetweenDates, k as getPrivateNote, f as formatName } from './booking-60a231c5.js';
...
import { t as taxationModes } from './property.service-7ef2a08e.js';
...
const IglRoomType = class {
    ...
    render() {
        ...
        return (h(Host, { key: 'cf2bff2462da8412636ee8974b755e439255c6ab' }, isValidBookingType && this.roomType.rateplans?.length > 0 && h("h5", { key: '14f30a282a18129c928f50f8b826fb9c9482a1e5', class: "roomtype__name" }, this.roomType.name), this.roomType.rateplans?.map(ratePlan => {
...
const IrArrivalTimeDialog = class {
    ...
    render() {
        return (h("ir-dialog", { key: '8a11e43a1c1db4c6caff980f95c1cdf0020463ee', label: "Edit Arrival Time", open: this.open, onIrDialogHide: () => {
                this.open = false;
            } }, h("wa-select", { key: '83d3cea91ccaa57227cef605748a6153c23008c1', size: "small", value: this.selectedArrivalTime, defaultValue: this.selectedArrivalTime, onchange: e => this.updateArrivalTime(e.target.value) }, this.arrivalTime.map(time => (h("wa-option", { value: time.CODE_NAME, selected: time.CODE_NAME === this.selectedArrivalTime }, time.CODE_VALUE_EN)))), h("div", { key: 'f9eae96e2e537270a31e35523fc755f894f8d146', slot: "footer", class: "ir-dialog__footer" }, h("ir-custom-button", { key: '3c6c7f610874adc8fdf0b9cab7f682979b2f7387', size: "medium", appearance: "filled", variant: "neutral", onClickHandler: () => this.closeDialog() }, locales.entries.Lcz_Cancel), h("ir-custom-button", { key: '661db7e9254ce9ecacb0859521fdba8983c99f07', size: "medium", variant: "brand", onClickHandler: () => this.saveArrivalTime(), loading: this.isLoading }, locales.entries.Lcz_Save))));
    }
};
...
const IrBillingDrawer = class {
    ...
    billingClose;
    render() {
        return (h("ir-drawer", { key: 'c6306cde658197e4f4ae3a11daf337d56376a3c4', style: {
            ...
            }, open: this.open, label: "Billing" }, this.open && h("ir-billing", { key: '7324b430dbda3da28bdb5c6da1e00cf9ac7fad27', booking: this.booking })));
    }
};
...
const IrBookingCityLedger = class {
    ...
    renderTable() {
        ...
        return (h("div", { class: "table--container booking-city-ledger__table-wrap" }, h("table", { class: "table data-table" }, h("thead", null, h("tr", null, h("th", null, "Status"), h("th", null, "Date"), h("th", null, "Description"), h("th", { class: "text-right" }, "Debit"), h("th", { class: "text-right" }, "Credit"))), h("tbody", null, this.folioRows.map(row => (h("tr", { key: row._rowId, class: "ir-table-row" }, h("td", null, h("wa-tag", { size: "small", variant: row.status.variant }, row.status.label, row.status.id === 'billed' && h("wa-icon", { name: "lock" }))), h("td", { class: "booking-city-ledger__cell-date" }, hooks(row.serviceDate).format('MMM DD, YYYY')), h("td", null, row.description || '—'), h("td", { class: "text-right is-debit" }, row.debit ? this.formatAmount(row.debit) : '—'), h("td", { class: "text-right is-credit" }, row.credit ? this.formatAmount(row.credit) : '—'))))))));
    }
    render() {
        if (!this.booking?.agent) {
            return h(Host, null);
...
const IrBookingStatusTag = class {
    ...
    render() {
        return (h("wa-badge", { key: 'e70d6bcbed7798c156d64dd2610a73ebe18459de', style: { padding: '0.375em 0.625em', letterSpacing: '0.03rem' }, variant: this.badgeVariant[this.isRequestToCancel ? '003' : this.status.code] }, this.status.description.toUpperCase()));
    }
};
...
const IrPaymentFolioForm = class {
    ...
    render() {
        // const isNewPayment = this.folioData?.payment_type?.code === '001' && this.folioData.id === -1;
        return (h("form", { key: '5a1aed51b855afc94745b1b9754b0d771de0095b', onSubmit: e => {
                e.preventDefault();
                const submitter = e.submitter;
                if (submitter?.value === 'save') {
                    this.savePayment();
                }
            }, class: "payment-folio__form", id: this.formId }, h("ir-custom-date-picker", { key: '22bb31aa5100b313316ab6d9161237611724a133', id: this.controlIds.date, label: "Date", "aria-invalid": this.errors?.date && !this.folioData?.date ? 'true' : 'false', "data-testid": "pickup_date", onDateChanged: evt => {
                this.updateFolioData({ date: evt.detail.start?.format(DATE_FORMAT) });
            }, minDate: hooks().add(-2, 'months').format('YYYY-MM-DD'), emitEmptyDate: true, maxDate: this.today, date: this.folioData?.date }), h("ir-validator", { key: 'd49e4458d79959eceb2a44e63c5d935cfa5ce1a9', value: this.folioData?.payment_type?.code, autovalidate: this.autoValidate, schema: paymentTypeSchema.shape.code, valueEvent: "change wa-change select-change", blurEvent: "wa-hide" }, h("wa-select", { key: '7d8ba8242ad493348958c28a66b585c0d438ddfd', id: this.controlIds.transactionType, size: "small", "onwa-hide": event => this.stopEventPropagation(event), "onwa-show": event => this.stopEventPropagation(event), placeholder: "Select...", label: "Transaction type", defaultValue: this.folioData?.payment_type?.code, value: this.folioData?.payment_type?.code, disabled: this.mode === 'payment-action', onchange: event => {
                this.stopEventPropagation(event);
                this.handleDropdownChange(event.target.value);
            } }, h("wa-option", { key: '3ad79168f992e441adf01cb3d6d80eaeab2821bd', value: "" }, "Select..."), this.renderDropdownItems())), this.requiresPaymentMethod(this.folioData?.payment_type?.code) && (h("ir-validator", { key: 'f76866c72d2cc58b37b9799b3b8fd5e8f3071b22', value: this.folioData?.payment_method?.code ?? '', autovalidate: this.autoValidate, schema: paymentMethodSchema.shape.code, valueEvent: "change wa-change select-change", blurEvent: "wa-hide" }, h("wa-select", { key: 'b4307665bb3d7b51beed8284b7e3f346c3924b5d', id: this.controlIds.paymentMethod, size: "small", label: `${this.folioData.payment_type?.code === '001' ? 'Payment' : 'Refund'} method`, "onwa-show": event => this.stopEventPropagation(event), "onwa-hide": event => this.stopEventPropagation(event), defaultValue: this.folioData?.payment_method?.code, value: this.folioData?.payment_method?.code ?? '', onchange: event => {
                this.stopEventPropagation(event);
                this.handlePaymentMethodDropdownChange(event.target.value);
            } }, h("wa-option", { key: '1b5481a7e250908310a02d97e30866474db666ce', value: "" }, "Select..."), this.paymentEntries?.methods?.map(pt => {
            return (h("wa-option", { key: pt.CODE_NAME, label: pt.CODE_VALUE_EN, value: pt.CODE_NAME }, pt.CODE_VALUE_EN));
        })))), h("ir-validator", { key: 'a54f9aa354bed0a6df74b879753da314fb50870d', value: this.folioData?.amount?.toString() ?? undefined, autovalidate: this.autoValidate, schema: folioBaseSchema.shape.amount, valueEvent: "text-change input input-change", blurEvent: "input-blur" }, h("ir-input", { key: 'ff15234787bb5e8150c5e8be3bdff097e7780a27', id: this.controlIds.amount, "aria-invalid": String(!!this.errors?.amount), value: this.folioData?.amount?.toString() ?? '', label: "Amount", mask: "price", min: 0, "onText-change": e => this.updateFolioData({ amount: !e.detail ? undefined : Number(e.detail) }) }, h("span", { key: '744578c152e64f4e1b50d544fdb31e8ffef0dc6b', slot: "start" }, calendar_data.currency.symbol))), h("ir-validator", { key: '5b867c59f67df4403be0de25ba665555f6ef8534', value: this.folioData?.reference ?? '', autovalidate: this.autoValidate, schema: folioBaseSchema.shape.reference, valueEvent: "text-change input input-change", blurEvent: "input-blur" }, h("ir-input", { key: '64566c56fd1aa4aefc371a9594256360066ce881', id: this.controlIds.reference, value: this.folioData?.reference ?? '', label: "Reference", maxlength: 50, "onText-change": e => this.updateFolioData({ reference: e.detail ?? '' }) })), isAgentMode(this.booking) && this.hasUnassignedItems() && h("ir-service-assignee-select", { key: '5a24cfb52ac80f11fbc5bb27e9393d12d0b34e73', agent: this.booking.agent })));
    }
    static get watchers() { return {
