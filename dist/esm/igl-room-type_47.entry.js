...
import { l as buildSplitIndex, c as calculateDaysBetweenDates, k as getPrivateNote, f as formatName } from './booking-80bcb1b1.js';
...
const IrBillingDrawer = class {
    ...
    billingClose;
    render() {
        return (h("ir-drawer", { key: '55e3d29c598c6f5efe166d52a3c49442481d6255', style: {
            ...
            }, open: this.open, label: "Billing" }, this.open && h("ir-billing", { key: '7b56647f64a3ac15cd1521f9c934d700231669ca', booking: this.booking })));
    }
};
...
const IrBookingStatusTag = class {
    ...
    render() {
        return (h("wa-badge", { key: '4dc4790f6f03a0fcff0c613eec9b103c7c0c28ab', style: { padding: '0.375em 0.625em', letterSpacing: '0.03rem' }, variant: this.badgeVariant[this.isRequestToCancel ? '003' : this.status.code] }, this.status.description.toUpperCase()));
    }
};
...
const IrPaymentFolioForm = class {
    ...
    render() {
        // const isNewPayment = this.folioData?.payment_type?.code === '001' && this.folioData.id === -1;
        return (h("form", { key: '5ccf1affa2d6a5587ff723c8a5c77eb71acd5896', onSubmit: e => {
                e.preventDefault();
                const submitter = e.submitter;
                if (submitter?.value === 'save') {
                    this.savePayment();
                }
            }, class: "payment-folio__form", id: this.formId }, h("ir-custom-date-picker", { key: '83c950a4c6c64dddd75b9234f400648941df916f', id: this.controlIds.date, label: "Date", "aria-invalid": this.errors?.date && !this.folioData?.date ? 'true' : 'false', "data-testid": "pickup_date", onDateChanged: evt => {
                this.updateFolioData({ date: evt.detail.start?.format(DATE_FORMAT) });
            }, minDate: hooks().add(-2, 'months').format('YYYY-MM-DD'), emitEmptyDate: true, maxDate: this.today, date: this.folioData?.date }), h("ir-validator", { key: '44e8421d6ba8cdc539f0e2c4ec80bcbedc92f046', value: this.folioData?.payment_type?.code, autovalidate: this.autoValidate, schema: paymentTypeSchema.shape.code, valueEvent: "change wa-change select-change", blurEvent: "wa-hide" }, h("wa-select", { key: '1770416f1ef8a7ddddf1b0b1d812481111fd8455', id: this.controlIds.transactionType, size: "small", "onwa-hide": event => this.stopEventPropagation(event), "onwa-show": event => this.stopEventPropagation(event), placeholder: "Select...", label: "Transaction type", defaultValue: this.folioData?.payment_type?.code, value: this.folioData?.payment_type?.code, disabled: this.mode === 'payment-action', onchange: event => {
                this.stopEventPropagation(event);
                this.handleDropdownChange(event.target.value);
            } }, h("wa-option", { key: 'acc9b6159bddf0905b3a990fd76cf678b9680be0', value: "" }, "Select..."), this.renderDropdownItems())), this.requiresPaymentMethod(this.folioData?.payment_type?.code) && (h("ir-validator", { key: '636b577738e19eef83dbf5016cd95a2ce7dedf31', value: this.folioData?.payment_method?.code ?? '', autovalidate: this.autoValidate, schema: paymentMethodSchema.shape.code, valueEvent: "change wa-change select-change", blurEvent: "wa-hide" }, h("wa-select", { key: '3dc39a11d102304043d9677720d43e2c88967fc1', id: this.controlIds.paymentMethod, size: "small", label: `${this.folioData.payment_type?.code === '001' ? 'Payment' : 'Refund'} method`, "onwa-show": event => this.stopEventPropagation(event), "onwa-hide": event => this.stopEventPropagation(event), defaultValue: this.folioData?.payment_method?.code, value: this.folioData?.payment_method?.code ?? '', onchange: event => {
                this.stopEventPropagation(event);
                this.handlePaymentMethodDropdownChange(event.target.value);
            } }, h("wa-option", { key: '6cbe9159f6e014e1336a0f9e1a3f6b3f55d9462b', value: "" }, "Select..."), this.paymentEntries?.methods?.map(pt => {
            return (h("wa-option", { key: pt.CODE_NAME, label: pt.CODE_VALUE_EN, value: pt.CODE_NAME }, pt.CODE_VALUE_EN));
        })))), h("ir-validator", { key: '6d0e062d5851f34e0cc8c22746d82abe741775b7', value: this.folioData?.amount?.toString() ?? undefined, autovalidate: this.autoValidate, schema: folioBaseSchema.shape.amount, valueEvent: "text-change input input-change", blurEvent: "input-blur" }, h("ir-input", { key: 'f3abc58c6a5b414c8cefe37607b9a1eb5616c132', id: this.controlIds.amount, "aria-invalid": String(!!this.errors?.amount), value: this.folioData?.amount?.toString() ?? '', label: "Amount", mask: "price", min: 0, "onText-change": e => this.updateFolioData({ amount: !e.detail ? undefined : Number(e.detail) }) }, h("span", { key: '171a8fd173589722fa54cc1f560e3a2148b8498c', slot: "start" }, calendar_data.currency.symbol))), h("ir-validator", { key: 'e7065daed236880d95d666a43b09fd4dd6a6f526', value: this.folioData?.reference ?? '', autovalidate: this.autoValidate, schema: folioBaseSchema.shape.reference, valueEvent: "text-change input input-change", blurEvent: "input-blur" }, h("ir-input", { key: 'b09af0fe8f9f922c7e950e87309a801a7a05ce5f', id: this.controlIds.reference, value: this.folioData?.reference ?? '', label: "Reference", maxlength: 50, "onText-change": e => this.updateFolioData({ reference: e.detail ?? '' }) }))));
    }
    static get watchers() { return {
...
const IrPrintingExtraService = class {
    ...
    invocableKeys;
    render() {
        return (h("section", { key: '8f12a4481e8778e82b69ec4f95e306f357b0bfdf', class: "ir-print-extra-services" }, h("h3", { key: 'b5c35f7cf36d57e27f62c25ad6dca8ca9e8a5042', class: "ir-print-extra-services__title" }, "Extra services"), h("div", { key: 'eaa548f429b24edb2105344ffd1aa0d67beb6153', class: "ir-print-extra-services__list" }, this.extraServices?.map(service => {
