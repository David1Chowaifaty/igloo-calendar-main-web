...
const IrTranslationsManager = class {
    ...
    refresh$ = new cjsExports.Subject();
    subscription;
    componentWillLoad() {
        const debouncedSearch$ = this.search$.pipe(cjsExports.debounceTime(600), cjsExports.map(value => value.trim()), 
        // A single character matches too much to be worth a round trip.
        cjsExports.map(value => (value.length >= 2 ? value : '')), cjsExports.distinctUntilChanged(), cjsExports.tap(value => (this.appliedSearchQuery = value)));
        this.subscription = cjsExports.merge(debouncedSearch$, this.refresh$)
            .pipe(cjsExports.tap(() => (this.isLoadingCrossTable = this.isCrossTableMode)), 
        // switchMap drops the response of any query a newer one has already superseded.
        cjsExports.switchMap(() => cjsExports.from(this.fetchCrossTableEntries()).pipe(cjsExports.catchError(() => cjsExports.of([])))))
            .subscribe(entries => {
