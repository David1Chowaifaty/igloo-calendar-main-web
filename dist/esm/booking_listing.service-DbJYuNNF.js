...
import { h as isPrivilegedUser, j as extras } from './utils-D9jFBfUm.js';
import { a as axios } from './axios-CleaxLzD.js';

const ymdDate = libExports.z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Expected date in YYYY-MM-DD format');
libExports.z.object({
    channel: libExports.z.string(),
    // These are null in your initialState, so allow nulls
    property_id: libExports.z.number().int().nullable(),
    balance_filter: libExports.z.string().nullable(),
    filter_type: libExports.z.union([libExports.z.number(), libExports.z.string()]).nullable(),
    from: ymdDate,
    to: ymdDate,
    name: libExports.z.string(),
    book_nbr: libExports.z.string(),
    booking_status: libExports.z.string(),
    userTypeCode: libExports.z.number().optional(),
    // In the interface these were literal 0/false, but you treat them like values.
    affiliate_id: libExports.z.number().int().default(0),
    is_mpo_managed: libExports.z.boolean().default(false),
    is_mpo_used: libExports.z.boolean().default(false),
    is_for_mobile: libExports.z.boolean().default(false),
    is_combined_view: libExports.z.boolean().default(false),
    start_row: libExports.z.number().int(),
    end_row: libExports.z.number().int(),
    total_count: libExports.z.number().int(),
    is_to_export: libExports.z.boolean(),
    property_ids: libExports.z.array(libExports.z.number().int()).optional(),
});
const initialState = {
    channels: [],
    settlement_methods: [],
