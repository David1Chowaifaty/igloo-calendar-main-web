...
import { B as BookingService } from './booking.service-729d0777.js';
import { t as transformNewBooking, r as isBlockUnit, a as calculateDaysBetweenDates, B as compareTime, C as createDateWithOffsetAndHour, l as calendar_dates, j as dateToFormattedString, D as getDaysArray, E as convertDatePrice, F as formatDate } from './utils-0f5c07aa.js';
import { h as hooks } from './moment-ab846cee.js';
import { E as EventsService } from './events.service-89589cfa.js';
...
import { T as ToBeAssignedService } from './toBeAssigned.service-df90d87a.js';
...
const IglBookingEvent = class {
    ...
    render() {
        ...
        return (h(Host, { key: '556f4962a3a7f26dedc749957f2059f789445c5d', class: `bookingEvent  ${this.isNewEvent() || this.isHighlightEventType() ? 'newEvent' : ''} ${legend.clsName} `, style: this.getPosition(), id: 'event_' + this.getBookingId() }, h("div", { key: '83953ab68de3069ccb3d9acb52a748b8a5d666af', class: `bookingEventBase  ${!this.isNewEvent() && hooks(new Date(this.bookingEvent.defaultDates.from_date)).isBefore(new Date(this.bookingEvent.FROM_DATE)) ? 'skewedLeft' : ''}
          ...
          ${!this.bookingEvent.is_direct &&
                ...
                'border border-dark ota-booking-event'}  ${this.isSplitBooking() ? 'splitBooking' : ''}`, style: { 'backgroundColor': legend.color, '--ir-event-bg': legend.color }, onTouchStart: event => this.startDragging(event, 'move'), onMouseDown: event => this.startDragging(event, 'move') }), noteNode ? h("div", { class: "legend_circle noteIcon", style: { backgroundColor: noteNode.color } }) : null, balanceNode ? h("div", { class: "legend_circle balanceIcon", style: { backgroundColor: balanceNode.color } }) : null, h("div", { key: 'cf56911beba90d58078e4b2d4962f556e6a89fdb', class: "bookingEventTitle", onTouchStart: event => this.startDragging(event, 'move'), onMouseDown: event => this.startDragging(event, 'move') }, this.getBookedBy(), this.renderEventBookingNumber()), (this.bookingEvent.is_direct || isBlockUnit(this.bookingEvent.STATUS_CODE)) && (h(Fragment, { key: '8419a082893a2ffa4634017c4e59e1cfce473737' }, h("div", { key: 'c4fd688e6c2883c2bd3698a26d4c1f3b0d864b8e', class: `bookingEventDragHandle leftSide ${!this.isNewEvent() && hooks(new Date(this.bookingEvent.defaultDates.from_date)).isBefore(new Date(this.bookingEvent.FROM_DATE)) ? 'skewedLeft' : ''}
            ${!this.isNewEvent() && hooks(new Date(this.bookingEvent.defaultDates.to_date)).isAfter(new Date(this.bookingEvent.TO_DATE)) ? 'skewedRight' : ''}`, onTouchStart: event => this.startDragging(event, 'leftSide'), onMouseDown: event => this.startDragging(event, 'leftSide') }), h("div", { key: 'bbde5c270c4100fbdbab3b5c242ec417da3ca490', class: `bookingEventDragHandle rightSide ${!this.isNewEvent() && hooks(new Date(this.bookingEvent.defaultDates.from_date)).isBefore(new Date(this.bookingEvent.FROM_DATE)) ? 'skewedLeft' : ''}
...
const IglCalBody = class {
    ...
    getGeneralRoomDayColumns(roomId, roomCategory, roomName) {
        ...
        return this.calendarData.days.map(dayInfo => {
            return (h("div", { class: `cellData ${'room_' + roomId + '_' + dayInfo.day} ${dayInfo.day === this.today || dayInfo.day === this.highlightedDate ? 'currentDay' : ''} ${this.dragOverElement === roomId + '_' + dayInfo.day ? 'dragOverHighlight' : ''} ${this.selectedRooms.hasOwnProperty(this.getSelectedCellRefName(roomId, dayInfo)) ? 'selectedDay' : ''}`, onClick: () => this.clickCell(roomId, dayInfo, roomCategory), "data-date": hooks(dayInfo.currentDate).format('YYYY-MM-DD'), "data-room-name": roomName }));
        });
    }
    ...
    render() {
        ...
        return (h(Host, { key: 'cf355d15dd2c31ec28bcdefdc63e1a5c2256171d' }, h("div", { key: '4d52c06e85aa1d65057f2f2c4959e60006d7eb76', class: "bodyContainer" }, this.getRoomRows(), h("div", { key: '06c915eec85062f729470ab4fe1ce31b22284291', class: "bookingEventsContainer preventPageScroll" }, (_a = this.getBookingData()) === null || _a === void 0 ? void 0 : _a.map(bookingEvent => {
            var _a, _b, _c;
...
const IglCalHeader = class {
    ...
    render() {
        return (h(Host, { key: 'a13c170b958c41aae04d845f1c220a19e986ddd5' }, h("div", { key: 'b14a4d8ae9e79e643c66c94778aeedb6a04fa463', class: "stickyCell align-items-center topLeftCell preventPageScroll" }, h("div", { key: '685d42d2a1c4dd573a623177946dc20b43e0f45b', class: "row justify-content-around no-gutters" }, !this.calendarData.is_vacation_rental && (h("ir-button", { key: '28d9d2bf2a961a1a1fd416bcac74ef8859c5c204', variant: "icon", icon_name: "server", style: Object.assign(Object.assign({}, colorVariants.secondary), { '--icon-size': '1.5rem' }), "data-toggle": "tooltip", "data-placement": "bottom", title: locales.entries.Lcz_UnassignedUnitsTooltip, onClickHandler: () => this.handleOptionEvent('showAssigned'), btn_styles: "caledarBtns", visibleBackgroundOnHover: true })), h("ir-date-picker", { key: 'fe44aa88c8d10d68c9ae30e3b660074e1971e8c2', minDate: hooks().add(-2, 'months').startOf('month').format('YYYY-MM-DD'),
            // autoApply
            // singleDatePicker
            onDateChanged: evt => {
                console.log('evt', evt);
...
const IglTbaCategoryView = class {
    ...
    render() {
        var _a;
        return (h(Host, { key: '8c6890085c588548169d4f66d7785b4a68c3c895' }, h("div", { key: '02b66a2968cbfd05372ea7867cd4ceb30d590346', class: "sectionContainer" }, h("div", { key: 'bf762a8bbb352688b323efb5db19053dd34370f2', class: "font-weight-bold mt-1 font-small-3" }, (_a = this.categoriesData[this.categoryId]) === null || _a === void 0 ? void 0 : _a.name), this.getEventView(this.categoryId, this.eventDatas))));
    }
};
...
const IrInteractiveTitle = class {
    ...
    render() {
        return (h(Host, { key: '9372eafdf4b1365472f10dec81209e6372d2b7d0', style: { '--ir-popover-left': this.irPopoverLeft } }, h("p", { key: 'e07106e17bdaf467cd0ea2988628926f87ec328d', class: "popover-title", style: {
                whiteSpace: 'nowrap',
                overflow: 'hidden',
                textOverflow: 'ellipsis',
            } }, h("span", { key: 'f35179336b0cabe44569d1bd61274111d72a011e', ref: el => (this.croppedTitleEl = el), class: "croppedTitle" }, this.croppedTitle), this.hkStatus && (h("div", { key: '2b76e38970902ffb2795efee564ae11f6d6a6d0d', title: "This unit is dirty", class: `hk-dot` }, h("svg", { key: '75f493179b3ebd916f29c17a9923150e9f950930', xmlns: "http://www.w3.org/2000/svg", height: "12", width: "13.5", viewBox: "0 0 576 512" }, h("path", { key: '93d16f68cf6d1b76807b6f405d72582dd770e76b', fill: "currentColor", d: "M566.6 54.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0l-192 192-34.7-34.7c-4.2-4.2-10-6.6-16-6.6c-12.5 0-22.6 10.1-22.6 22.6l0 29.1L364.3 320l29.1 0c12.5 0 22.6-10.1 22.6-22.6c0-6-2.4-11.8-6.6-16l-34.7-34.7 192-192zM341.1 353.4L222.6 234.9c-42.7-3.7-85.2 11.7-115.8 42.3l-8 8C76.5 307.5 64 337.7 64 369.2c0 6.8 7.1 11.2 13.2 8.2l51.1-25.5c5-2.5 9.5 4.1 5.4 7.9L7.3 473.4C2.7 477.6 0 483.6 0 489.9C0 502.1 9.9 512 22.1 512l173.3 0c38.8 0 75.9-15.4 103.4-42.8c30.6-30.6 45.9-73.1 42.3-115.8z" })))))));
    }
