...
import { c as calculateDaysBetweenDates } from './booking-887zeuHr.js';
...
const IrProformaInvoicePreview = class {
    ...
    renderPropertyCompanyHeader() {
        ...
        return (h("div", { class: "invoice-company", "aria-label": t('Lcz_IssuingCompanyDetailsAria', { fallback: 'Issuing company details' }) }, company.name && h("p", { class: "invoice-company__name" }, company.name), company.address && h("p", { class: "invoice-company__address" }, company.address), this.CompanyLocation, company.phone && (h("ir-printing-label", { class: "proforma-invoice__company-details", label: `${t('Lcz_Phone', { fallback: 'Phone' })}:`, content: `${company.country?.phone_prefix ?? ''} ${company.phone}`.trim() })), company.tax_nbr && h("ir-printing-label", { class: "proforma-invoice__company-details", label: `${t('Lcz_TaxId', { fallback: 'Tax ID' })}:`, content: company.tax_nbr })));
    }
    renderPropertyInfo() {
        ...
        return (h("section", { class: "property-overview", "aria-label": t('Lcz_PropertyOverviewAria', { fallback: 'Property overview' }) }, h("div", { class: "property-overview__text" }, h("p", { class: "property-overview__name" }, this.property.name), h("p", { class: "property-overview__location" }, propertyLocation)), propertyLogo && h("img", { src: propertyLogo, alt: `${this.property.name} logo`, class: "property-logo" })));
    }
    ...
    renderBillToSection() {
        ...
        if (target?.code === '002') {
            return (h("div", { class: "bill-to", "aria-label": t('Lcz_BillToCompanyAria', { fallback: 'Bill to company' }) }, company_name && h("p", { class: "bill-to__name" }, company_name), company_tax_nbr && h("p", { class: "bill-to__id" }, company_tax_nbr)));
        }
        return (h("div", { class: "bill-to", "aria-label": t('Lcz_BillToGuestAria', { fallback: 'Bill to guest' }) }, billed_to_name && h("p", null, billed_to_name), h("p", { class: "bill-to__name" }, ' ', billed_to_name && h("b", { class: "bill-to__connector" }, t('Lcz_For', { fallback: 'For' })), " ", [guest.first_name ?? '', guest.last_name ?? ''].join(' ').trim()), guest.email && h("p", { class: "bill-to__contact" }, guest.email), this.guestPhoneNumber && h("p", { class: "bill-to__contact" }, this.guestPhoneNumber)));
    }
    renderCancellationPenalty() {
        ...
        return (h("section", { class: "proforma-payment__section" }, h("div", { class: "ir-proforma-invoice__service" }, h("div", { class: 'ir-proforma-invoice__cancellation-info' }, h("p", null, t('Lcz_CancellationPenalty', { fallback: 'Cancellation penalty' })), h("p", { class: 'ir-proforma-invoice__cancellation-date' }, "( ", this.formatDisplayDate(cancellationPenalty.date), " )")), h("span", { class: "ir-proforma-invoice__checkbox-price" }, formatAmount(this.booking.currency.symbol, cancellationPenalty.amount)))));
    }
    render() {
        ...
        return (h(Host, null, h("article", { class: "invoice", "aria-label": t('Lcz_ProformaInvoice', { fallback: 'Proforma invoice' }) }, h("header", { class: "invoice__header" }, h("h3", { class: "invoice__title" }, t('Lcz_ProformaInvoice', { fallback: 'Proforma Invoice' })), h("section", { class: "invoice__layout", "aria-label": t('Lcz_InvoiceSummaryAria', { fallback: 'Invoice summary' }) }, h("div", { class: "invoice__column invoice__column--details" }, h("div", { class: "invoice__details" }, h("ir-printing-label", { label: t('Lcz_DateOfIssueLabel', { fallback: 'Date of issue:' }), content: this.issueDate }), h("ir-printing-label", { label: t('Lcz_BookingNoLabel', { fallback: 'Booking no:' }), content: this.bookingNumber })), billToContent && (h("section", { class: "bill-to-section", "aria-label": t('Lcz_BillTo', { fallback: 'Bill to' }) }, h("h4", { class: "section-heading" }, t('Lcz_BillTo', { fallback: 'Bill To' })), billToContent))), h("div", { class: "invoice__column invoice__column--property" }, companyDetails && (h("section", { class: "issuer-section", "aria-label": t('Lcz_IssuerAria', { fallback: 'Issuer' }) }, companyDetails)), propertyOverview))), h("main", null, existInvocableRoom && (h("section", { style: { marginTop: '2.5rem' } }, h("div", { class: "proforma__accommodation-container" }, h("p", { class: "proforma__accommodation-title" }, "ACCOMMODATION"), h("p", { class: "booking-dates" }, this.formatBookingDates(this.booking?.from_date)), h("p", { class: "booking-dates" }, this.formatBookingDates(this.booking?.to_date)), h("p", { class: "number-of-nights" }, totalNights, " ", totalNights === 1 ? t('Lcz_Night', { fallback: 'night' }) : t('Lcz_Nights', { fallback: 'nights' })), h("p", { class: "vat-exclusion" }, h("i", null, this.property?.tax_statement))), h("div", null, invocableRoom.map((room, idx) => {
        ...
        })))), existInvocablePickup && h("ir-printing-pickup", { pickup: this.booking.pickup_info }), existInvocableExtraService && (h("ir-printing-extra-service", { invocableKeys: this.invocableKeys, extraServices: this.booking.extra_services, currency: this.booking.currency })), this.renderCancellationPenalty(), h("section", { class: "proforma-payment__section" }, h("ir-printing-label", { label: t('Lcz_Balance', { fallback: 'Balance' }) + ":", content: formatAmount(this.booking.currency.symbol, this.booking.financial.due_amount) }), h("ir-printing-label", { label: t('Lcz_CollectedLabel', { fallback: 'Collected (payments - refunds):' }), content: formatAmount(this.booking.currency.symbol, this.booking.financial.collected + this.booking.financial.refunds) }))), this.footerNote && (h("footer", { class: "invoice__footer" }, h("p", { class: "invoice__footer-note" }, this.footerNote))))));
    }
...
export { IrProformaInvoicePreview as ir_proforma_invoice_preview };
