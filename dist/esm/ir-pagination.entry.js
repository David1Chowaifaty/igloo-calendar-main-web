...
const IrPagination = class {
    ...
    render() {
        ...
        return (h("div", { class: {
            ...
            }, role: "navigation", "aria-label": "Pagination Navigation" }, this.showTotalRecords && (h("p", { class: "pagination-info", "aria-live": "polite" }, this.renderItemRange())), h("div", { class: "buttons-container" }, this.allowPageSizeChange && this.pageSizes && (h("wa-select", { value: String(this.pageSize), class: "pagination__current-page-select", onchange: e => this.handlePageSizeChange(+e.target.value), "aria-label": `Current page ${this.currentPage} of ${this.pages}`, disabled: this.disabled, size: "s", defaultValue: String(this.pageSize) }, this.pageSizes.map(size => (h("wa-option", { value: size.toString(), key: `page-size-${this.recordLabel}-${size}` }, size))))), this.pages > 1 && (h(Fragment, null, h("ir-custom-button", { onClickHandler: () => this.handlePageChange(this.currentPage - 1, 'previous'), variant: "neutral", appearance: "plain", disabled: isFirstPage || this.disabled, "aria-label": "Go to previous page" }, h("wa-icon", { class: "ir-flip-rtl", name: "angle-left", label: "Go to previous page" })), h("wa-select", { value: this.currentPage?.toString(), class: "pagination__current-page-select", onchange: e => this.handlePageChange(+e.target.value, 'direct'), "aria-label": `Current page ${this.currentPage} of ${this.pages}`, disabled: this.disabled, size: "s", defaultValue: this.currentPage ? this.currentPage?.toString() : '1' }, Array.from(Array(this.pages), (_, i) => i + 1).map(i => (h("wa-option", { value: i.toString(), key: `${this.recordLabel}-${i}` }, i)))), h("ir-custom-button", { "aria-label": "Go to next page", onClickHandler: () => this.handlePageChange(this.currentPage + 1, 'next'), disabled: isLastPage || this.disabled, variant: "neutral", appearance: "plain" }, h("wa-icon", { class: "ir-flip-rtl", name: "angle-right", label: "Go to next page" })))))));
    }
