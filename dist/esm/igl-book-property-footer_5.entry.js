...
import { B as BookingService, b as booking_store, s as setBookingDraft, m as modifyBookingStore, f as setBookedByGuestManualEditState } from './booking.service-1d91dd42.js';
import { z } from './index-87419685.js';
import { f as formatAmount, B as validateEmail } from './utils-3655fb87.js';
import { I as IMask } from './index-e2caf943.js';
import { B as BookingGuestSchema } from './types-f16787f2.js';
import { v as v4 } from './v4-964634d6.js';
import './index-f100e9d2.js';
import './axios-aa1335b8.js';
import './booking-b2adb086.js';
...
const IglBookPropertyHeader = class {
    ...
    render() {
        ...
        return (h(Host, { key: '905f10314a3d4520ea47f06e78353ffea82a8b40' }, this.isEventType('SPLIT_BOOKING') && this.getSplitBookingList(), h("div", { key: '65ce5bbca0e55b311964fcb45be1938ba269d3f1', class: `fd-book-property__header-container` }, showSourceNode && this.getSourceNode(), h("ir-date-range", { key: 'dad8da916138bdeaed29bc879f4abfb0db4b3869', "data-testid": "date_picker", variant: "booking", dateLabel: locales.entries.Lcz_Dates, maxDate: this.getMaxDate(), minDate: this.getMinDate(), disabled: (this.isEventType('BAR_BOOKING') && !this.wasBlockedUnit) || this.isEventType('SPLIT_BOOKING'), defaultData: this.bookingDataDefaultDateRange }), !this.isEventType('EDIT_BOOKING') && this.getAdultChildConstraints(), h("ir-custom-button", { key: 'cf194db1e13e591859517b4a3c85a77a95af15c0', loading: isRequestPending('/Check_Availability'), variant: "brand", onClickHandler: () => this.handleButtonClicked() }, locales.entries.Lcz_Check)), h("p", { key: '8c915e378c1986a9b1063a12360c7cf36e448b15', class: "text-right message-label" }, calendar_data.tax_statement)));
    }
};
...
const IglBookingForm = class {
    ...
    render() {
        return (h("form", { key: 'df2f6e58f58d546ec04e5b8b33058d86a64971a5', class: "d-flex flex-column h-100", id: "new_booking_form", autoComplete: "off", onSubmit: e => {
                e.preventDefault();
            } }, h("div", { key: '1bb502676ed8273bd8340d88c7d4ff1ca38e1066', class: "d-flex flex-wrap" }, h("ir-date-view", { key: '6ccee41b1af34d7c5b710ccb76f3fecc016a970f', class: "mr-1 flex-fill font-weight-bold font-medium-1", from_date: new Date(this.dateRangeData.fromDate), to_date: new Date(this.dateRangeData.toDate), dateOption: "DD MMM YYYY" }), this.guestData.length > 1 && (h("div", { key: 'df9ebe814c861e0139c54d025032eeecb02f5a4d', class: "mt-1 mt-md-0 text-right" }, locales.entries.Lcz_TotalPrice, " ", h("span", { key: '143cb9bc2eeab011af882a468c97a089c11dfa6b', class: "font-weight-bold font-medium-1" }, formatAmount(this.currency.symbol, this.bookingData.TOTAL_PRICE || '0'))))), Object.values(booking_store.ratePlanSelections).map(val => Object.values(val).map(ratePlan => {
            const rp = ratePlan;
            if (rp.reserved === 0) {
                return null;
            }
...
const IglBookingOverviewPage = class {
    ...
    render() {
        return (h(Host, { key: 'bd0dadf307e12c819280bc8586a4769dd52818e4' }, h("igl-book-property-header", { key: 'd7ebaf53e44358770c27305d1b5a6459bdafc8cd', wasBlockedUnit: this.wasBlockedUnit, bookedByInfoData: this.bookedByInfoData, defaultDaterange: this.defaultDaterange, dateRangeData: this.dateRangeData, minDate: this.setMinDate(),
            // minDate={this.isEventType('ADD_ROOM') || this.isEventType('SPLIT_BOOKING') ? this.bookedByInfoData.from_date || this.bookingData.FROM_DATE : undefined}
            splitBookingId: this.showSplitBookingOption, bookingData: this.bookingData, message: this.message, bookingDataDefaultDateRange: this.bookingData.defaultDateRange, showSplitBookingOption: this.showSplitBookingOption, adultChildConstraints: this.adultChildConstraints, splitBookings: this.getSplitBookings(), propertyId: this.propertyId }), h("div", { key: '86eeeabcfd3caf78ea815078fd4986ad5911a1ad', class: " text-left" }, isRequestPending('/Check_Availability') && this.isEventType('EDIT_BOOKING') ? (h("div", { class: "loading-container" }, h("div", { class: "loader" }))) : (h(Fragment, null, booking_store.roomTypes?.map(roomType => (h("igl-room-type", {
...
const IglPropertyBookedBy = class {
    ...
    render() {
        return (h(Host, { key: '5c3d63707393020ce59d9bc101aa279b4f24083d' }, h("div", { key: '25d8bf824dd2c12ce7f8194e2febe7de319397d9', class: "text-left mt-3" }, h("div", { key: '626698c98d449a386bcbd749b6473d95430d51d1', class: "d-flex", style: { alignItems: 'flex-end', gap: '0.5rem' } }, h("ir-picker", { key: '4d4dc0430e89016ae5dd66e445bed8e6bf063363', class: "bookedByEmailContainer m-0 p-0", label: locales.entries.Lcz_BookedBy, value: this.bookedByData.email, ref: el => (this.pickerRef = el), "aria-invalid": String(Boolean(this.isButtonPressed && this.bookedByData.email !== '' && validateEmail(this.bookedByData.email))), withClear: true, "onText-change": event => this.fetchGuests(event.detail), debounce: 300, "onInput-picker-blurred": e => {
        ...
        })), h("div", { key: 'b8d32aa7ee0fd7a03ab3fe5c03542ec326a6d030', style: { paddingBottom: '0.5rem' } }, h("wa-tooltip", { key: 'cbaab605c3b7ab55d0778fd3b95d15373de8915d', for: `main_guest-search-tooltip` }, "Leave empty if email is unavailable"), h("wa-icon", { key: '472949f41ec36042b7ef2a0427528fe6e4d33e5e', name: "circle-info", id: `main_guest-search-tooltip` })))), h("div", { key: '9093f57307de6dc1a36321a50bc22b89e28a989a', class: "bookedDetailsForm text-left mt-2 font-small-3 " }, h("div", { key: '27494081b8b993cdcf79a8c420f33e71f1535392', class: "d-flex flex-column flex-md-row  justify-content-md-between ", style: { gap: '1rem' } }, h("div", { key: '3d86a2e76fc8ace0cca42d57872d088f2ab08a75', class: "fd-property-booked-by__guest-form " }, h("ir-validator", { key: '6d65cd7def0f6c6af3fe1b8b784f7f657bec1d51', value: this.bookedByData.firstName, schema: BookingGuestSchema.shape.first_name }, h("ir-input", { key: 'a55d8afa7da7e4de09f362d42272ab1c3226aef8', "onText-change": event => {
                this.updateGuest({ first_name: event.detail });
                this.handleDataChange('firstName', { target: { value: event.detail.trim() } });
            }, defaultValue: this.bookedByData.firstName, value: this.bookedByData.firstName, label: locales.entries.Lcz_FirstName, placeholder: locales.entries.Lcz_FirstName, required: true, name: "last_name_custom", autocomplete: "family-name" })), h("ir-validator", { key: '21513167bad0610dae1678ef4fae8bdf4861aa75', value: this.bookedByData.lastName, schema: BookingGuestSchema.shape.last_name }, h("ir-input", { key: '55686eb27119f6e73b2be800798eba13a089b8a4', "onText-change": event => {
                this.updateGuest({ last_name: event.detail });
                this.handleDataChange('lastName', { target: { value: event.detail.trim() } });
            }, name: "first_name_custom", autocomplete: "given-name", defaultValue: this.bookedByData.lastName, value: this.bookedByData.lastName, label: locales.entries.Lcz_LastName, placeholder: locales.entries.Lcz_LastName, required: true })), h("ir-country-picker", { key: '58bb9bfae35ba5803b95926b550d19f8316da8e4', label: locales.entries.Lcz_Country, variant: "modern", testId: "main_guest_country", class: "flex-grow-1 m-0", onCountryChange: e => this.handleCountryChange(e.detail.id), countries: this.countries, country: this.countries.find(c => c.id === this.bookedByData.countryId) }), h("ir-mobile-input", { key: '343faeb136332991f634d9af17e4c7700f8dc176', size: "small", "onMobile-input-change": e => {
            ...
            // required
            countryCode: this.countries.find(c => c.phone_prefix === this.bookedByData.isdCode)?.code, countries: this.countries }), h("wa-select", { key: 'e35e51a9704e21badbcfca2e203a78c2580b3fe6', size: "small", label: locales.entries.Lcz_YourArrivalTime, "data-testid": "arrival_time", "aria-disabled": String(Boolean(this.isButtonPressed && this.bookedByData.selectedArrivalTime.code === '')), id: v4(), defaultValue: this.arrivalTimeList[0].CODE_NAME, value: this.bookedByData.selectedArrivalTime.code, onchange: event => this.handleDataChange('selectedArrivalTime', event) }, this.arrivalTimeList.map(time => (h("wa-option", { value: time.CODE_NAME, selected: this.bookedByData.selectedArrivalTime.code === time.CODE_NAME }, time.CODE_VALUE_EN))))), h("div", { key: 'b55d8eda14b008754fd94b5a621ff29967375f41', class: "p-0 flex-fill  ml-md-3 d-flex flex-column", style: { gap: '0.5rem' } }, h("wa-textarea", { key: '21b3a0ad7c7bb8330acd7b49545c71358cc3112c', onchange: event => this.handleDataChange('message', event), size: "small", value: this.bookedByData.message, defaultValue: this.bookedByData.message, label: locales.entries.Lcz_AnyMessageForUs, rows: 4 }), this.paymentMethods.length > 1 && (h("wa-select", { key: '62f9270a7a4b6ce7f3e13790891bec55402d6c2a', label: 'Payment Method', size: "small", value: booking_store?.selectedPaymentMethod?.code, onchange: e => modifyBookingStore('selectedPaymentMethod', {
                code: e.target.value,
            }) }, this.paymentMethods.map(p => (h("wa-option", { value: p.code }, p.description))))), booking_store.selectedPaymentMethod?.code === '001' && (h(Fragment, { key: 'ed7032f7d841485dcf753759c14a132d1e8e2666' }, h("ir-input", { key: '70ca1fd6652e5e7e8948f27cef93debe1d00f8ab', value: this.bookedByData.cardNumber, defaultValue: this.bookedByData.cardNumber, "onText-change": e => this.handleCreditCardDataChange('cardNumber', e.detail.trim()), label: locales.entries.Lcz_CardNumber }), h("ir-input", { key: '23cfc20a2f1db4a8aa08733500e5d328cde6c639', value: this.bookedByData.cardHolderName, defaultValue: this.bookedByData.cardHolderName, "onText-change": e => this.handleCreditCardDataChange('cardHolderName', e.detail.trim()), label: locales.entries.Lcz_CardHolderName }), h("ir-input", { key: 'f708e61768dffa0f3ad56cc34aef104399f85485', "onText-change": e => {
            ...
            }, label: locales.entries.Lcz_ExpiryDate }))), booking_store.selectedPaymentMethod?.code === '005' && (h("div", { key: '1b782ebb44dbe90390005b496a6bfa1cd93629d0', class: "form-group mt-md-1 mt-1 p-0 d-flex flex-column flex-md-row align-items-md-center" }, h("label", { key: '2a3804e6532a63bf7ad4b1068dae14b6881dbe94', class: "p-0 m-0 margin3" }), h("div", { key: '130f72e4a478bb96c8e842ae1d1784a366fb3d11', class: "p-0 m-0  controlContainer flex-fill" }, h("div", { key: '1362157a27dd002fd17e7de2e91687ce41721e32', class: "property-booked-by__money-transfer-description", innerHTML: this.paymentMethods.find(p => p.code === '005')?.localizables.find(l => l.language.code.toLowerCase() === 'en')?.description })))), h("wa-checkbox", { key: '024284561a932a8247604311d9481a9a49eecf37', checked: this.bookedByData.emailGuest, onchange: event => this.handleDataChange('emailGuest', event) }, locales.entries.Lcz_EmailTheGuest))))));
    }
};
