...
import { B as BookingService, b as booking_store, s as setBookingDraft, m as modifyBookingStore, o as setBookedByGuestManualEditState } from './booking.store-c071bb5f.js';
import { z } from './index-87419685.js';
import { s as showToast, f as formatAmount, H as validateEmail } from './utils-3c91939f.js';
...
const IglBookPropertyHeader = class {
    ...
    render() {
        ...
        return (h(Host, { key: '99b387f86ee4d8d8b3dc9c749c6ff06024f1c5dc' }, this.isEventType('SPLIT_BOOKING') && this.getSplitBookingList(), h("div", { key: '9d6f22414a575f153136e24f14d390f01d410dc3', class: `fd-book-property__header-container` }, showSourceNode && this.getSourceNode(), h("ir-date-range", { key: '05a54d59b62a093e88eb053493ce9fb3f367bde2', "data-testid": "date_picker", variant: "booking", dateLabel: locales.entries.Lcz_Dates, maxDate: this.getMaxDate(), minDate: this.getMinDate(), disabled: (this.isEventType('BAR_BOOKING') && !this.wasBlockedUnit) || this.isEventType('SPLIT_BOOKING'), defaultData: this.bookingDataDefaultDateRange }), !this.isEventType('EDIT_BOOKING') && this.getAdultChildConstraints(), h("ir-custom-button", { key: '4f4cdb205cc34387aea8da5e24a66a1d693fd934', loading: isRequestPending('/Check_Availability'), variant: "brand", onClickHandler: () => this.handleButtonClicked() }, locales.entries.Lcz_Check)), h("p", { key: 'ecb972351503e62e24abc884a166f96d71be4fa3', class: "text-right message-label" }, calendar_data.tax_statement)));
    }
};
...
const IglBookingForm = class {
    ...
    render() {
        return (h("form", { key: 'a6880df3abf7ac9d381799f7ac1c2e84da86a11b', class: "d-flex flex-column h-100", id: "new_booking_form", autoComplete: "off", onSubmit: e => {
                e.preventDefault();
            } }, h("div", { key: '735350ab616062e31cfd504940d95d4009711394', class: "d-flex flex-wrap" }, h("ir-date-view", { key: '1d5fa55583592c866240613e022359562a214fed', class: "mr-1 flex-fill font-weight-bold font-medium-1", from_date: new Date(this.dateRangeData.fromDate), to_date: new Date(this.dateRangeData.toDate), dateOption: "DD MMM YYYY" }), this.guestData.length > 1 && (h("div", { key: '65476e43c597bf3ebcb8cf76f553ea7946b94665', class: "mt-1 mt-md-0 text-right" }, locales.entries.Lcz_TotalPrice, " ", h("span", { key: '2f0b44610d3d289e8a3e0051c4f7b15f56af7581', class: "font-weight-bold font-medium-1" }, formatAmount(this.currency.symbol, this.bookingData.TOTAL_PRICE || '0'))))), Object.values(booking_store.ratePlanSelections).map(val => Object.values(val).map(ratePlan => {
            const rp = ratePlan;
            if (rp.reserved === 0) {
                return null;
            }
...
const IglBookingOverviewPage = class {
    ...
    render() {
        return (h(Host, { key: '2ddde187831dc3b56555299b8138da8c20acc82a' }, h("igl-book-property-header", { key: 'a65d28d573998ec6fe294c38ba1912be5d9f8aa4', wasBlockedUnit: this.wasBlockedUnit, bookedByInfoData: this.bookedByInfoData, defaultDaterange: this.defaultDaterange, dateRangeData: this.dateRangeData, minDate: this.setMinDate(),
            // minDate={this.isEventType('ADD_ROOM') || this.isEventType('SPLIT_BOOKING') ? this.bookedByInfoData.from_date || this.bookingData.FROM_DATE : undefined}
            splitBookingId: this.showSplitBookingOption, bookingData: this.bookingData, message: this.message, bookingDataDefaultDateRange: this.bookingData.defaultDateRange, showSplitBookingOption: this.showSplitBookingOption, adultChildConstraints: this.adultChildConstraints, splitBookings: this.getSplitBookings(), propertyId: this.propertyId }), h("div", { key: '6642a04c71efb012539fd445088ee4b94d4a6c0e', class: " text-left" }, isRequestPending('/Check_Availability') && this.isEventType('EDIT_BOOKING') ? (h("div", { class: "loading-container" }, h("div", { class: "loader" }))) : (h(Fragment, null, booking_store.roomTypes?.map(roomType => (h("igl-room-type", {
...
const IglPropertyBookedBy = class {
    ...
    render() {
        return (h(Host, { key: 'be0adf20e6ed8b37f94f94fb40f026d024253c1d' }, h("div", { key: '7b22901fd254a66482937226ef4b84aba1af2ac5', class: "text-left mt-3" }, h("div", { key: 'f9de00647e1cc2e75fd458f954cae482e190998a', class: "d-flex", style: { alignItems: 'flex-end', gap: '0.5rem' } }, h("ir-picker", { key: '4b817e18b0a7bcc33b4d57c510ee1c74cc3d7642', class: "bookedByEmailContainer m-0 p-0", label: locales.entries.Lcz_BookedBy, value: this.bookedByData.email, ref: el => (this.pickerRef = el), "aria-invalid": String(Boolean(this.isButtonPressed && this.bookedByData.email !== '' && validateEmail(this.bookedByData.email))), withClear: true, "onText-change": event => this.fetchGuests(event.detail), debounce: 300, "onInput-picker-blurred": e => {
        ...
        })), h("div", { key: 'f942aa7ea4d20305a7f9bab25038d494a2809474', style: { paddingBottom: '0.5rem' } }, h("wa-tooltip", { key: 'bfca76480919605c30fe9d11b4cbe896088f9cac', for: `main_guest-search-tooltip` }, "Leave empty if email is unavailable"), h("wa-icon", { key: '5de26e7d28a9b0cb4823efb37f4ecfe4adb8b6ca', name: "circle-info", id: `main_guest-search-tooltip` })))), h("div", { key: '5f8fef06a132f96207bee4e06bdb9f26d09aaebb', class: "bookedDetailsForm text-left mt-2 font-small-3 " }, h("div", { key: '3b1afb5c333fe92a6adcbaaeb61f31b8cbad5515', class: "d-flex flex-column flex-md-row  justify-content-md-between ", style: { gap: '1rem' } }, h("div", { key: '1cfb22373ef6a7cb6a4a776986552ee7de419064', class: "fd-property-booked-by__guest-form " }, h("ir-validator", { key: '6f660180f7660bd2ea69f81dbdf7157a6c84cf22', value: this.bookedByData.firstName, schema: BookingGuestSchema.shape.first_name }, h("ir-input", { key: 'ed75932db2b2328f14c6c540ce36f4f8f33e2887', "onText-change": event => {
                this.updateGuest({ first_name: event.detail });
                this.handleDataChange('firstName', { target: { value: event.detail.trim() } });
            }, defaultValue: this.bookedByData.firstName, value: this.bookedByData.firstName, label: locales.entries.Lcz_FirstName, placeholder: locales.entries.Lcz_FirstName, required: true, name: "last_name_custom", autocomplete: "family-name" })), h("ir-validator", { key: '3500fbc02cf4370d04da5714265a7c0f30235e4f', value: this.bookedByData.lastName, schema: BookingGuestSchema.shape.last_name }, h("ir-input", { key: '5baca3a9e25855b41cc8507c9300b16c4669c1b3', "onText-change": event => {
                this.updateGuest({ last_name: event.detail });
                this.handleDataChange('lastName', { target: { value: event.detail.trim() } });
            }, name: "first_name_custom", autocomplete: "given-name", defaultValue: this.bookedByData.lastName, value: this.bookedByData.lastName, label: locales.entries.Lcz_LastName, placeholder: locales.entries.Lcz_LastName, required: true })), h("ir-country-picker", { key: 'aa427d759c0a281263fd1d8a82a6576a61f909e5', label: locales.entries.Lcz_Country, variant: "modern", testId: "main_guest_country", class: "flex-grow-1 m-0", onCountryChange: e => this.handleCountryChange(e.detail.id), countries: this.countries, country: this.countries.find(c => c.id === this.bookedByData.countryId) }), h("ir-mobile-input", { key: '38ec261346135e17c7352d88fef28d1882b0b686', size: "small", "onMobile-input-change": e => {
            ...
            // required
            countryCode: this.countries.find(c => c.phone_prefix === this.bookedByData.isdCode)?.code, countries: this.countries }), h("wa-select", { key: 'c04dc670b325ffd440db03da3b1ae35cf3beae82', size: "small", label: locales.entries.Lcz_YourArrivalTime, "data-testid": "arrival_time", "aria-disabled": String(Boolean(this.isButtonPressed && this.bookedByData.selectedArrivalTime.code === '')), id: v4(), defaultValue: this.arrivalTimeList[0].CODE_NAME, value: this.bookedByData.selectedArrivalTime.code, onchange: event => this.handleDataChange('selectedArrivalTime', event) }, this.arrivalTimeList.map(time => (h("wa-option", { value: time.CODE_NAME, selected: this.bookedByData.selectedArrivalTime.code === time.CODE_NAME }, time.CODE_VALUE_EN))))), h("div", { key: 'f1a5756acbca645f04f9bdba12dd546336465940', class: "p-0 flex-fill  ml-md-3 d-flex flex-column", style: { gap: '0.5rem' } }, h("wa-textarea", { key: 'c5d5a2fbdad80ebb9006497dfca8263ce88fc7d4', onchange: event => this.handleDataChange('message', event), size: "small", value: this.bookedByData.message, defaultValue: this.bookedByData.message, label: locales.entries.Lcz_AnyMessageForUs, rows: 4 }), this.paymentMethods.length > 1 && (h("wa-select", { key: 'd534fea268d303f0019d5df88036da30a150a22c', label: 'Payment Method', size: "small", value: booking_store?.selectedPaymentMethod?.code, onchange: e => modifyBookingStore('selectedPaymentMethod', {
                code: e.target.value,
            }) }, this.paymentMethods.map(p => (h("wa-option", { value: p.code }, p.description))))), booking_store.selectedPaymentMethod?.code === '001' && (h(Fragment, { key: 'af67b350b5bf0aaa4b27c38e530a5def6b2e1f50' }, h("ir-input", { key: '4889d7c54002ea26374ac50062a98621d34a3d71', value: this.bookedByData.cardNumber, defaultValue: this.bookedByData.cardNumber, "onText-change": e => this.handleCreditCardDataChange('cardNumber', e.detail.trim()), label: locales.entries.Lcz_CardNumber }), h("ir-input", { key: '723b46c0fa7661a342ed8fca5fd59c2c9ca4ab54', value: this.bookedByData.cardHolderName, defaultValue: this.bookedByData.cardHolderName, "onText-change": e => this.handleCreditCardDataChange('cardHolderName', e.detail.trim()), label: locales.entries.Lcz_CardHolderName }), h("ir-input", { key: '57e4ff4de90ca559b9af0d61fc336d10cf5674ba', "onText-change": e => {
            ...
            }, label: locales.entries.Lcz_ExpiryDate }))), booking_store.selectedPaymentMethod?.code === '005' && (h("div", { key: 'b737be96cf192da13736cfd69b9c1292bcbcce67', class: "form-group mt-md-1 mt-1 p-0 d-flex flex-column flex-md-row align-items-md-center" }, h("label", { key: '464f477d913201572202f04e3fffededa7f6ec6f', class: "p-0 m-0 margin3" }), h("div", { key: 'e68244d31dc77782387c6dd088a69ef2751bb690', class: "p-0 m-0  controlContainer flex-fill" }, h("div", { key: '2056c7a1198fbf604db96c5d4a0bef1fcef3709c', class: "property-booked-by__money-transfer-description", innerHTML: this.paymentMethods.find(p => p.code === '005')?.localizables.find(l => l.language.code.toLowerCase() === 'en')?.description })))), h("wa-checkbox", { key: '0e1ca2761db9740c2f0a2ea3c4343626ec54790a', checked: this.bookedByData.emailGuest, onchange: event => this.handleDataChange('emailGuest', event) }, locales.entries.Lcz_EmailTheGuest))))));
    }
};
