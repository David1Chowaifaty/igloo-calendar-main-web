...
import { B as BookingService, b as booking_store, s as setBookingDraft, m as modifyBookingStore, f as setBookedByGuestManualEditState } from './booking.service-a4a3b5df.js';
import { z } from './index-87419685.js';
import { f as formatAmount, B as validateEmail } from './utils-c81c2ca7.js';
import { I as IMask } from './index-e2caf943.js';
import { B as BookingGuestSchema } from './types-f16787f2.js';
import { v as v4 } from './v4-964634d6.js';
import './index-f100e9d2.js';
import './axios-aa1335b8.js';
import './booking-e5a61425.js';
...
const IglBookPropertyHeader = class {
    ...
    render() {
        ...
        return (h(Host, { key: 'afaf54a2e22b5e0249603c425ace567362f888f0' }, this.isEventType('SPLIT_BOOKING') && this.getSplitBookingList(), h("div", { key: 'ce5e6151a27a2cb9655f1f78046212eb7ca37e67', class: `fd-book-property__header-container` }, showSourceNode && this.getSourceNode(), h("ir-date-range", { key: '7af80eb2401f30baa094d00df3c28b0501ba977c', "data-testid": "date_picker", variant: "booking", dateLabel: locales.entries.Lcz_Dates, maxDate: this.getMaxDate(), minDate: this.getMinDate(), disabled: (this.isEventType('BAR_BOOKING') && !this.wasBlockedUnit) || this.isEventType('SPLIT_BOOKING'), defaultData: this.bookingDataDefaultDateRange }), !this.isEventType('EDIT_BOOKING') && this.getAdultChildConstraints(), h("ir-custom-button", { key: '8cda4b210631e47e04dcc57a4e64af88a70b88d6', loading: isRequestPending('/Check_Availability'), variant: "brand", onClickHandler: () => this.handleButtonClicked() }, locales.entries.Lcz_Check)), h("p", { key: '1d136e0ae218f2aec704d08fff36e6d584513f63', class: "text-right message-label" }, calendar_data.tax_statement)));
    }
};
...
const IglBookingForm = class {
    ...
    render() {
        return (h("form", { key: '5e0ec56d2642e2e3ab3319ace3379a5ed1ed534e', class: "d-flex flex-column h-100", id: "new_booking_form", autoComplete: "off", onSubmit: e => {
                e.preventDefault();
            } }, h("div", { key: 'b81bad83c179107c680e26c00d075a7b5786e6c5', class: "d-flex flex-wrap" }, h("ir-date-view", { key: 'c99812bb19a437fa64380de5797270b241a517f7', class: "mr-1 flex-fill font-weight-bold font-medium-1", from_date: new Date(this.dateRangeData.fromDate), to_date: new Date(this.dateRangeData.toDate), dateOption: "DD MMM YYYY" }), this.guestData.length > 1 && (h("div", { key: '3464d142625a8e41a22fee5796f072f07016c932', class: "mt-1 mt-md-0 text-right" }, locales.entries.Lcz_TotalPrice, " ", h("span", { key: 'c0a80696d1ad83f9e1b4dd9cf3ed7fd67270bfb4', class: "font-weight-bold font-medium-1" }, formatAmount(this.currency.symbol, this.bookingData.TOTAL_PRICE || '0'))))), Object.values(booking_store.ratePlanSelections).map(val => Object.values(val).map(ratePlan => {
            const rp = ratePlan;
            if (rp.reserved === 0) {
                return null;
            }
...
const IglBookingOverviewPage = class {
    ...
    render() {
        return (h(Host, { key: 'b6a51f4ff478f0930490866a8c96d0844c9788e2' }, h("igl-book-property-header", { key: '32041967a903c57b68c00e44c2f7eef9c53e016e', wasBlockedUnit: this.wasBlockedUnit, bookedByInfoData: this.bookedByInfoData, defaultDaterange: this.defaultDaterange, dateRangeData: this.dateRangeData, minDate: this.setMinDate(),
            // minDate={this.isEventType('ADD_ROOM') || this.isEventType('SPLIT_BOOKING') ? this.bookedByInfoData.from_date || this.bookingData.FROM_DATE : undefined}
            splitBookingId: this.showSplitBookingOption, bookingData: this.bookingData, message: this.message, bookingDataDefaultDateRange: this.bookingData.defaultDateRange, showSplitBookingOption: this.showSplitBookingOption, adultChildConstraints: this.adultChildConstraints, splitBookings: this.getSplitBookings(), propertyId: this.propertyId }), h("div", { key: 'da59802c520beaaeb40f5d6ee0860d1e2fb48c6b', class: " text-left" }, isRequestPending('/Check_Availability') && this.isEventType('EDIT_BOOKING') ? (h("div", { class: "loading-container" }, h("div", { class: "loader" }))) : (h(Fragment, null, booking_store.roomTypes?.map(roomType => (h("igl-room-type", {
...
const IglPropertyBookedBy = class {
    ...
    render() {
        return (h(Host, { key: 'b83cb3613b9bda4f5213cfbd6590e6bb7cf104ff' }, h("div", { key: '287a62eb60e16019ea404a8c4ec6f3c79b362aa5', class: "text-left mt-3" }, h("div", { key: 'ea60e08e5d101fa0b7c293b906b5c7228ba28bfd', class: "d-flex", style: { alignItems: 'flex-end', gap: '0.5rem' } }, h("ir-picker", { key: '989cf31470f307956ddf6177bab2441035c9baa8', class: "bookedByEmailContainer m-0 p-0", label: locales.entries.Lcz_BookedBy, value: this.bookedByData.email, ref: el => (this.pickerRef = el), "aria-invalid": String(Boolean(this.isButtonPressed && this.bookedByData.email !== '' && validateEmail(this.bookedByData.email))), withClear: true, "onText-change": event => this.fetchGuests(event.detail), debounce: 300, "onInput-picker-blurred": e => {
                e.stopPropagation();
                e.stopImmediatePropagation();
        ...
        })), h("div", { key: '245e20b6916543002a68ec33e7f71b12b8e0ec3d', style: { paddingBottom: '0.5rem' } }, h("wa-tooltip", { key: 'e69d237daf6305bd126dca89368963bc1b48e902', for: `main_guest-search-tooltip` }, "Leave empty if email is unavailable"), h("wa-icon", { key: 'bd7eeb9694f4c6e7b42143866d0f27e1955ec2f1', name: "circle-info", id: `main_guest-search-tooltip` })))), h("div", { key: 'e5519cee43dda9b8e90032aed8e087b812947615', class: "bookedDetailsForm text-left mt-2 font-small-3 " }, h("div", { key: '80cda4b52ca014fc526236b22814b14542a491b9', class: "d-flex flex-column flex-md-row  justify-content-md-between ", style: { gap: '1rem' } }, h("div", { key: '66a2e9bdf0b1327c8f14ed1774528d1195c21a81', class: "fd-property-booked-by__guest-form " }, h("ir-validator", { key: '617babe0181677ecec831b95e47380625ccb9460', value: this.bookedByData.firstName, schema: BookingGuestSchema.shape.first_name }, h("ir-input", { key: '13f7cc869841eb42af99cd947eff6822420280de', "onText-change": event => {
                this.updateGuest({ first_name: event.detail });
                this.handleDataChange('firstName', { target: { value: event.detail.trim() } });
            }, defaultValue: this.bookedByData.firstName, value: this.bookedByData.firstName, label: locales.entries.Lcz_FirstName, placeholder: locales.entries.Lcz_FirstName, required: true, name: "last_name_custom", autocomplete: "family-name" })), h("ir-validator", { key: '37aae99348a475c685dea5f7a84d2dd2dbb22374', value: this.bookedByData.lastName, schema: BookingGuestSchema.shape.last_name }, h("ir-input", { key: 'b33172a069de31fd243f8375b96071614b27015f', "onText-change": event => {
                this.updateGuest({ last_name: event.detail });
                this.handleDataChange('lastName', { target: { value: event.detail.trim() } });
            }, name: "first_name_custom", autocomplete: "given-name", defaultValue: this.bookedByData.lastName, value: this.bookedByData.lastName, label: locales.entries.Lcz_LastName, placeholder: locales.entries.Lcz_LastName, required: true })), h("ir-country-picker", { key: '5188af39c21af1a02e4d962f8b0cc6874bc9ec79', label: locales.entries.Lcz_Country, variant: "modern", testId: "main_guest_country", class: "flex-grow-1 m-0", onCountryChange: e => this.handleCountryChange(e.detail.id), countries: this.countries, country: this.countries.find(c => c.id === this.bookedByData.countryId) }), h("ir-mobile-input", { key: '263650fd69da8079d74c930e7914593cf0969580', size: "small", "onMobile-input-change": e => {
            ...
            // required
            countryCode: this.countries.find(c => c.phone_prefix === this.bookedByData.isdCode)?.code, countries: this.countries }), h("wa-select", { key: '9f15723012255cbf1275d776b175177cbd705895', size: "small", label: locales.entries.Lcz_YourArrivalTime, "data-testid": "arrival_time", "aria-disabled": String(Boolean(this.isButtonPressed && this.bookedByData.selectedArrivalTime.code === '')), id: v4(), defaultValue: this.arrivalTimeList[0].CODE_NAME, value: this.bookedByData.selectedArrivalTime.code, onchange: event => this.handleDataChange('selectedArrivalTime', event) }, this.arrivalTimeList.map(time => (h("wa-option", { value: time.CODE_NAME, selected: this.bookedByData.selectedArrivalTime.code === time.CODE_NAME }, time.CODE_VALUE_EN))))), h("div", { key: 'f417ce50d406106ab7913b9c1c8bf9d5cbfd3751', class: "p-0 flex-fill  ml-md-3 d-flex flex-column", style: { gap: '0.5rem' } }, h("wa-textarea", { key: '5c04f16d615179a24198fe615c6e9dc5e84b54ae', onchange: event => this.handleDataChange('message', event), size: "small", value: this.bookedByData.message, defaultValue: this.bookedByData.message, label: locales.entries.Lcz_AnyMessageForUs, rows: 4 }), this.paymentMethods.length > 1 && (h("wa-select", { key: 'a602313a2701a04c2c5f317e9681c3cde6901de8', label: 'Payment Method', size: "small", value: booking_store?.selectedPaymentMethod?.code, onchange: e => modifyBookingStore('selectedPaymentMethod', {
                code: e.target.value,
            }) }, this.paymentMethods.map(p => (h("wa-option", { value: p.code }, p.description))))), booking_store.selectedPaymentMethod?.code === '001' && (h(Fragment, { key: '2d8f2074190e00273046d09f916493074b5ceedd' }, h("ir-input", { key: '037da4dfd3f6cbec35d92c79333ca4a7e3a3bfca', value: this.bookedByData.cardNumber, defaultValue: this.bookedByData.cardNumber, "onText-change": e => this.handleCreditCardDataChange('cardNumber', e.detail.trim()), label: locales.entries.Lcz_CardNumber }), h("ir-input", { key: 'd13667be869d2d8333620caf6bf1a4326c6595e5', value: this.bookedByData.cardHolderName, defaultValue: this.bookedByData.cardHolderName, "onText-change": e => this.handleCreditCardDataChange('cardHolderName', e.detail.trim()), label: locales.entries.Lcz_CardHolderName }), h("ir-input", { key: '442b3e7d268187616d1b13fad62b85781560b1c9', "onText-change": e => {
            ...
            }, label: locales.entries.Lcz_ExpiryDate }))), booking_store.selectedPaymentMethod?.code === '005' && (h("div", { key: '8be5c72b3ae30926651569196dbb0cb71f3ccc10', class: "form-group mt-md-1 mt-1 p-0 d-flex flex-column flex-md-row align-items-md-center" }, h("label", { key: 'f4bd8e577cfee8e55039cd161a1b9453a492c19b', class: "p-0 m-0 margin3" }), h("div", { key: '8b0f1ed9ceafcad54037629e21d9ebbc6465c4bd', class: "p-0 m-0  controlContainer flex-fill" }, h("div", { key: 'ba36f07da87ceb6be0b1415a78f51f5e53e4d93d', class: "property-booked-by__money-transfer-description", innerHTML: this.paymentMethods.find(p => p.code === '005')?.localizables.find(l => l.language.code.toLowerCase() === 'en')?.description })))), h("wa-checkbox", { key: '2be0700a6d20ef90eb9fbcb943eac34e44ac189b', checked: this.bookedByData.emailGuest, onchange: event => this.handleDataChange('emailGuest', event) }, locales.entries.Lcz_EmailTheGuest))))));
    }
};
