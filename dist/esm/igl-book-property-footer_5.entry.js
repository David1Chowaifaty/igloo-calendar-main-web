...
import { B as BookingService, b as booking_store, s as setBookingDraft, m as modifyBookingStore, o as setBookedByGuestManualEditState } from './booking.store-8b688b9d.js';
import { z } from './index-87419685.js';
import { s as showToast, f as formatAmount, I as validateEmail } from './utils-91ae2576.js';
...
const IglBookPropertyHeader = class {
    ...
    render() {
        ...
        return (h(Host, { key: 'c7eaadef685b8e54a68c1cd78e308f6522963769' }, this.isEventType('SPLIT_BOOKING') && this.getSplitBookingList(), h("div", { key: '30037a7acfc9c74ebaae4aa095d20dedf2248da7', class: `fd-book-property__header-container` }, showSourceNode && this.getSourceNode(), h("ir-date-range", { key: '5220210daa467e3966b84b1c07ad3d3917636672', "data-testid": "date_picker", variant: "booking", dateLabel: locales.entries.Lcz_Dates, maxDate: this.getMaxDate(), minDate: this.getMinDate(), disabled: (this.isEventType('BAR_BOOKING') && !this.wasBlockedUnit) || this.isEventType('SPLIT_BOOKING'), defaultData: this.bookingDataDefaultDateRange }), !this.isEventType('EDIT_BOOKING') && this.getAdultChildConstraints(), h("ir-custom-button", { key: '8d391292afc37dceed09476010c15f4d20600c75', loading: isRequestPending('/Check_Availability'), variant: "brand", onClickHandler: () => this.handleButtonClicked() }, locales.entries.Lcz_Check)), h("p", { key: 'a50491698c27d12c372ff5218d5ec3e855e2dcb7', class: "text-right message-label" }, calendar_data.tax_statement)));
    }
};
...
const IglBookingForm = class {
    ...
    render() {
        return (h("form", { key: '24e88f7b2a0deccac16e4d587d05b684c7318532', class: "d-flex flex-column h-100", id: "new_booking_form", autoComplete: "off", onSubmit: e => {
                e.preventDefault();
            } }, h("div", { key: '06099321731c6d4403c482d95e67690efb40443d', class: "d-flex flex-wrap" }, h("ir-date-view", { key: 'c947c14eb7656af412c087ef0491bfa64cd90cfd', class: "mr-1 flex-fill font-weight-bold font-medium-1", from_date: new Date(this.dateRangeData.fromDate), to_date: new Date(this.dateRangeData.toDate), dateOption: "DD MMM YYYY" }), this.guestData.length > 1 && (h("div", { key: '50848e060e097e2b2e869a4ae0118df9cbcc5273', class: "mt-1 mt-md-0 text-right" }, locales.entries.Lcz_TotalPrice, " ", h("span", { key: 'b72ad13403a51417885f356e89c4d80cc160af8a', class: "font-weight-bold font-medium-1" }, formatAmount(this.currency.symbol, this.bookingData.TOTAL_PRICE || '0'))))), Object.values(booking_store.ratePlanSelections).map(val => Object.values(val).map(ratePlan => {
            const rp = ratePlan;
            if (rp.reserved === 0) {
                return null;
            }
...
const IglBookingOverviewPage = class {
    ...
    render() {
        return (h(Host, { key: '9023a166279b711fc4781f9e03472736cf42b35c' }, h("igl-book-property-header", { key: '003718bc1b388f0cd59b8cb06a931ae7d636d03e', wasBlockedUnit: this.wasBlockedUnit, bookedByInfoData: this.bookedByInfoData, defaultDaterange: this.defaultDaterange, dateRangeData: this.dateRangeData, minDate: this.setMinDate(),
            // minDate={this.isEventType('ADD_ROOM') || this.isEventType('SPLIT_BOOKING') ? this.bookedByInfoData.from_date || this.bookingData.FROM_DATE : undefined}
            splitBookingId: this.showSplitBookingOption, bookingData: this.bookingData, message: this.message, bookingDataDefaultDateRange: this.bookingData.defaultDateRange, showSplitBookingOption: this.showSplitBookingOption, adultChildConstraints: this.adultChildConstraints, splitBookings: this.getSplitBookings(), propertyId: this.propertyId }), h("div", { key: 'ead29dcd048b2c70f1dbe908d28187e47609164d', class: " text-left" }, isRequestPending('/Check_Availability') && this.isEventType('EDIT_BOOKING') ? (h("div", { class: "loading-container" }, h("div", { class: "loader" }))) : (h(Fragment, null, booking_store.roomTypes?.map(roomType => (h("igl-room-type", {
...
const IglPropertyBookedBy = class {
    ...
    render() {
        return (h(Host, { key: '8c0e06ecc17a1dc93112e60efb70951f9c39bae0' }, h("div", { key: '96f3fef9e3e6ff1f706d5d50cb076e6d28606414', class: "text-left mt-3" }, h("div", { key: '52e0d30590d555326b34d8d079445078a28bdc53', class: "d-flex", style: { alignItems: 'flex-end', gap: '0.5rem' } }, h("ir-picker", { key: 'c4ee33947250704266fcee14261e4919b76bea24', class: "bookedByEmailContainer m-0 p-0", label: locales.entries.Lcz_BookedBy, value: this.bookedByData.email, ref: el => (this.pickerRef = el), "aria-invalid": String(Boolean(this.isButtonPressed && this.bookedByData.email !== '' && validateEmail(this.bookedByData.email))), withClear: true, "onText-change": event => this.fetchGuests(event.detail), debounce: 300, "onInput-picker-blurred": e => {
                e.stopPropagation();
                e.stopImmediatePropagation();
        ...
        })), h("div", { key: 'b36b02b9d48d937bf833eff5ad90de9e0ca22213', style: { paddingBottom: '0.5rem' } }, h("wa-tooltip", { key: 'd3f1ab83eae4f5ee714609580313c135d293c03b', for: `main_guest-search-tooltip` }, "Leave empty if email is unavailable"), h("wa-icon", { key: '71414d9534725e3fc1815bd68645b09b58a8e388', name: "circle-info", id: `main_guest-search-tooltip` })))), h("div", { key: '9ad1aae113cca5c4eb79d459b7383d74d16f5185', class: "bookedDetailsForm text-left mt-2 font-small-3 " }, h("div", { key: '743179687f9a1bff86414db6e081b0a0b307345e', class: "d-flex flex-column flex-md-row  justify-content-md-between ", style: { gap: '1rem' } }, h("div", { key: '96467303901c1f6919ba8da8298a32dc9d41f55d', class: "fd-property-booked-by__guest-form " }, h("ir-validator", { key: '5b37e773ced3570112a24ede47ae22c9ffb4c3d8', value: this.bookedByData.firstName, schema: BookingGuestSchema.shape.first_name }, h("ir-input", { key: '38002a4bbae60ccad185c58b77d8932525ff6b9a', "onText-change": event => {
                this.updateGuest({ first_name: event.detail });
                this.handleDataChange('firstName', { target: { value: event.detail.trim() } });
            }, defaultValue: this.bookedByData.firstName, value: this.bookedByData.firstName, label: locales.entries.Lcz_FirstName, placeholder: locales.entries.Lcz_FirstName, required: true, name: "last_name_custom", autocomplete: "family-name" })), h("ir-validator", { key: '57d6e4133e016cde7e7decb14550ae2c3de75d2c', value: this.bookedByData.lastName, schema: BookingGuestSchema.shape.last_name }, h("ir-input", { key: 'ab516092ae4d2f4d09e180373775009a21129c73', "onText-change": event => {
                this.updateGuest({ last_name: event.detail });
                this.handleDataChange('lastName', { target: { value: event.detail.trim() } });
            }, name: "first_name_custom", autocomplete: "given-name", defaultValue: this.bookedByData.lastName, value: this.bookedByData.lastName, label: locales.entries.Lcz_LastName, placeholder: locales.entries.Lcz_LastName, required: true })), h("ir-country-picker", { key: '196beb183deb419b6c15d12dd620dbd44d6608c9', label: locales.entries.Lcz_Country, variant: "modern", testId: "main_guest_country", class: "flex-grow-1 m-0", onCountryChange: e => this.handleCountryChange(e.detail.id), countries: this.countries, country: this.countries.find(c => c.id === this.bookedByData.countryId) }), h("ir-mobile-input", { key: '9aa80153f7d175e91fe262a2f857215c783c9e8c', size: "small", "onMobile-input-change": e => {
            ...
            // required
            countryCode: this.countries.find(c => c.phone_prefix === this.bookedByData.isdCode)?.code, countries: this.countries }), h("wa-select", { key: '0c18a3987168e7d5a305107d8f9f6d6649257bee', size: "small", label: locales.entries.Lcz_YourArrivalTime, "data-testid": "arrival_time", "aria-disabled": String(Boolean(this.isButtonPressed && this.bookedByData.selectedArrivalTime.code === '')), id: v4(), defaultValue: this.arrivalTimeList[0].CODE_NAME, value: this.bookedByData.selectedArrivalTime.code, onchange: event => this.handleDataChange('selectedArrivalTime', event) }, this.arrivalTimeList.map(time => (h("wa-option", { value: time.CODE_NAME, selected: this.bookedByData.selectedArrivalTime.code === time.CODE_NAME }, time.CODE_VALUE_EN))))), h("div", { key: '0390af944a838f94d0a6e3cbffa996f2b3c917b7', class: "p-0 flex-fill  ml-md-3 d-flex flex-column", style: { gap: '0.5rem' } }, h("wa-textarea", { key: 'c48e2b7297068d32c62394df0151817fa0762230', onchange: event => this.handleDataChange('message', event), size: "small", value: this.bookedByData.message, defaultValue: this.bookedByData.message, label: locales.entries.Lcz_AnyMessageForUs, rows: 4 }), this.paymentMethods.length > 1 && (h("wa-select", { key: '2fde1c5ed0fcba8de92fd94ced5ee54f7ffec641', label: 'Payment Method', size: "small", value: booking_store?.selectedPaymentMethod?.code, onchange: e => modifyBookingStore('selectedPaymentMethod', {
                code: e.target.value,
            }) }, this.paymentMethods.map(p => (h("wa-option", { value: p.code }, p.description))))), booking_store.selectedPaymentMethod?.code === '001' && (h(Fragment, { key: '7500ac3d62329fe260f3209f034007db380714a5' }, h("ir-input", { key: 'deb71e1da1719fc8edb80a24be5344a857ed2bce', value: this.bookedByData.cardNumber, defaultValue: this.bookedByData.cardNumber, "onText-change": e => this.handleCreditCardDataChange('cardNumber', e.detail.trim()), label: locales.entries.Lcz_CardNumber }), h("ir-input", { key: 'fd8c6d6659b6248426e9972cd611d61f43c8e713', value: this.bookedByData.cardHolderName, defaultValue: this.bookedByData.cardHolderName, "onText-change": e => this.handleCreditCardDataChange('cardHolderName', e.detail.trim()), label: locales.entries.Lcz_CardHolderName }), h("ir-input", { key: 'ac2f1463f78f3418e49a1369224edbd8a6b91576', "onText-change": e => {
            ...
            }, label: locales.entries.Lcz_ExpiryDate }))), booking_store.selectedPaymentMethod?.code === '005' && (h("div", { key: 'f1c27ef269050844ffea67d6e7e8206fb5e1e410', class: "form-group mt-md-1 mt-1 p-0 d-flex flex-column flex-md-row align-items-md-center" }, h("label", { key: '55aef53befca31f76a770124df9c5bdfa7c8874d', class: "p-0 m-0 margin3" }), h("div", { key: 'fd9fa80a4f59dc43a774e9fb5bb83a7642587470', class: "p-0 m-0  controlContainer flex-fill" }, h("div", { key: '24f825469b1004e21492ee1a10f4f2ebcad390e3', class: "property-booked-by__money-transfer-description", innerHTML: this.paymentMethods.find(p => p.code === '005')?.localizables.find(l => l.language.code.toLowerCase() === 'en')?.description })))), h("wa-checkbox", { key: 'bbf83ff95484f7ec1396982b2bd17466e0018f13', checked: this.bookedByData.emailGuest, onchange: event => this.handleDataChange('emailGuest', event) }, locales.entries.Lcz_EmailTheGuest))))));
    }
};
