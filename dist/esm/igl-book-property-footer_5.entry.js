...
import { B as BookingService, b as booking_store, s as setBookingDraft, m as modifyBookingStore, o as setBookedByGuestManualEditState } from './booking.store-baf751f1.js';
import { z } from './index-87419685.js';
import { f as formatAmount, F as validateEmail } from './utils-123be455.js';
import { I as IMask } from './index-e2caf943.js';
import { B as BookingGuestSchema } from './types-f16787f2.js';
import { v as v4 } from './v4-964634d6.js';
import './index-f100e9d2.js';
import './axios-aa1335b8.js';
import './booking-971ac81d.js';
...
const IglBookingForm = class {
    ...
    render() {
        return (h("form", { key: '2345cdb98680c3fd4343b225af1cfc9954a979d8', class: "d-flex flex-column h-100", id: "new_booking_form", autoComplete: "off", onSubmit: e => {
                e.preventDefault();
            } }, h("div", { key: '649559790261e3ac9a214007038199832e09a52c', class: "d-flex flex-wrap" }, h("ir-date-view", { key: '063a31514a3ba2b867dd1b1fc08f45b5d09e27a2', class: "mr-1 flex-fill font-weight-bold font-medium-1", from_date: new Date(this.dateRangeData.fromDate), to_date: new Date(this.dateRangeData.toDate), dateOption: "DD MMM YYYY" }), this.guestData.length > 1 && (h("div", { key: '578477d84cd7a72205a1eb4c97c814d7579d85ec', class: "mt-1 mt-md-0 text-right" }, locales.entries.Lcz_TotalPrice, " ", h("span", { key: '8c3e29fe3bf056293bd342b477a983d4ea764b55', class: "font-weight-bold font-medium-1" }, formatAmount(this.currency.symbol, this.bookingData.TOTAL_PRICE || '0'))))), Object.values(booking_store.ratePlanSelections).map(val => Object.values(val).map(ratePlan => {
            const rp = ratePlan;
            if (rp.reserved === 0) {
                return null;
            }
...
const IglBookingOverviewPage = class {
    ...
    render() {
        return (h(Host, { key: 'ad4b33c029c660bc3f62613aad0d804f11b63115' }, h("igl-book-property-header", { key: '721d1d7feffd8896ee14083eb270a630980cafeb', wasBlockedUnit: this.wasBlockedUnit, bookedByInfoData: this.bookedByInfoData, defaultDaterange: this.defaultDaterange, dateRangeData: this.dateRangeData, minDate: this.setMinDate(),
            // minDate={this.isEventType('ADD_ROOM') || this.isEventType('SPLIT_BOOKING') ? this.bookedByInfoData.from_date || this.bookingData.FROM_DATE : undefined}
            splitBookingId: this.showSplitBookingOption, bookingData: this.bookingData, message: this.message, bookingDataDefaultDateRange: this.bookingData.defaultDateRange, showSplitBookingOption: this.showSplitBookingOption, adultChildConstraints: this.adultChildConstraints, splitBookings: this.getSplitBookings(), propertyId: this.propertyId }), h("div", { key: '6dcba8fcdef4eb9233dc177044a2fe2a8bb77783', class: " text-left" }, isRequestPending('/Check_Availability') && this.isEventType('EDIT_BOOKING') ? (h("div", { class: "loading-container" }, h("div", { class: "loader" }))) : (h(Fragment, null, booking_store.roomTypes?.map(roomType => (h("igl-room-type", {
...
const IglPropertyBookedBy = class {
    ...
    render() {
        return (h(Host, { key: '8bbc665339271efef38f792a704810adbe78bbbe' }, h("div", { key: 'f6f74f72add59655bc852b462ba6c97312919907', class: "text-left mt-3" }, h("div", { key: '28537869641b6b15d407097ffea81a55a6aca42d', class: "d-flex", style: { alignItems: 'flex-end', gap: '0.5rem' } }, h("ir-picker", { key: 'ee9d6444d47f253e7d0b87400d1912eb6421ee30', class: "bookedByEmailContainer m-0 p-0", label: locales.entries.Lcz_BookedBy, value: this.bookedByData.email, ref: el => (this.pickerRef = el), "aria-invalid": String(Boolean(this.isButtonPressed && this.bookedByData.email !== '' && validateEmail(this.bookedByData.email))), withClear: true, "onText-change": event => this.fetchGuests(event.detail), debounce: 300, "onInput-picker-blurred": e => {
                e.stopPropagation();
                e.stopImmediatePropagation();
        ...
        })), h("div", { key: 'a92965e4a9873e826043e7c500b3df40c405bd61', style: { paddingBottom: '0.5rem' } }, h("wa-tooltip", { key: 'a55a8f97ace22f069d14d610eea9dff168c0c115', for: `main_guest-search-tooltip` }, "Leave empty if email is unavailable"), h("wa-icon", { key: 'ef5c3ec891d243b6782f093617174fa65f13cd3b', name: "circle-info", id: `main_guest-search-tooltip` })))), h("div", { key: '1b6ce6792982c52b15288ce8d37f9e91eed74f41', class: "bookedDetailsForm text-left mt-2 font-small-3 " }, h("div", { key: '5e47db21adf3d87693800aafca19740b2281a4f9', class: "d-flex flex-column flex-md-row  justify-content-md-between ", style: { gap: '1rem' } }, h("div", { key: '5e9fd8b3c6a969329faf0a0365d05c59c641f74b', class: "fd-property-booked-by__guest-form " }, h("ir-validator", { key: 'ed0a4d92f7f4b8edc9170ddb32b66ab4a735618e', value: this.bookedByData.firstName, schema: BookingGuestSchema.shape.first_name }, h("ir-input", { key: 'a97b038686b7cdb2446d74b3008577491e1deafb', "onText-change": event => {
                this.updateGuest({ first_name: event.detail });
                this.handleDataChange('firstName', { target: { value: event.detail.trim() } });
            }, defaultValue: this.bookedByData.firstName, value: this.bookedByData.firstName, label: locales.entries.Lcz_FirstName, placeholder: locales.entries.Lcz_FirstName, required: true, name: "last_name_custom", autocomplete: "family-name" })), h("ir-validator", { key: 'b35aaf249cca8be96ed0ebd6022399f3ec3bac15', value: this.bookedByData.lastName, schema: BookingGuestSchema.shape.last_name }, h("ir-input", { key: 'c938b2af22db1f1eeaeb8c260c85ae97c5d68393', "onText-change": event => {
                this.updateGuest({ last_name: event.detail });
                this.handleDataChange('lastName', { target: { value: event.detail.trim() } });
            }, name: "first_name_custom", autocomplete: "given-name", defaultValue: this.bookedByData.lastName, value: this.bookedByData.lastName, label: locales.entries.Lcz_LastName, placeholder: locales.entries.Lcz_LastName, required: true })), h("ir-country-picker", { key: '53769be8e1691052e35c8b8a1b4da3dd0c1b77b9', label: locales.entries.Lcz_Country, variant: "modern", testId: "main_guest_country", class: "flex-grow-1 m-0", onCountryChange: e => this.handleCountryChange(e.detail.id), countries: this.countries, country: this.countries.find(c => c.id === this.bookedByData.countryId) }), h("ir-mobile-input", { key: '5a0b17f2e9bda4e360043a6cf1ed1c41ef3a9b0b', size: "small", "onMobile-input-change": e => {
            ...
            // required
            countryCode: this.countries.find(c => c.phone_prefix === this.bookedByData.isdCode)?.code, countries: this.countries }), h("wa-select", { key: '098f2d65e8e700d8b2f6382c20ebbe4549a0a77d', size: "small", label: locales.entries.Lcz_YourArrivalTime, "data-testid": "arrival_time", "aria-disabled": String(Boolean(this.isButtonPressed && this.bookedByData.selectedArrivalTime.code === '')), id: v4(), defaultValue: this.arrivalTimeList[0].CODE_NAME, value: this.bookedByData.selectedArrivalTime.code, onchange: event => this.handleDataChange('selectedArrivalTime', event) }, this.arrivalTimeList.map(time => (h("wa-option", { value: time.CODE_NAME, selected: this.bookedByData.selectedArrivalTime.code === time.CODE_NAME }, time.CODE_VALUE_EN))))), h("div", { key: 'f353643631e0be1dfe1ef70f9c82f45520acf339', class: "p-0 flex-fill  ml-md-3 d-flex flex-column", style: { gap: '0.5rem' } }, h("wa-textarea", { key: '31f15ec69c03e1b44aed41aad9ff739df8f1d50f', onchange: event => this.handleDataChange('message', event), size: "small", value: this.bookedByData.message, defaultValue: this.bookedByData.message, label: locales.entries.Lcz_AnyMessageForUs, rows: 4 }), this.paymentMethods.length > 1 && (h("wa-select", { key: '7228bfb68ac7c94c081ab017db22ace73743def8', label: 'Payment Method', size: "small", value: booking_store?.selectedPaymentMethod?.code, onchange: e => modifyBookingStore('selectedPaymentMethod', {
                code: e.target.value,
            }) }, this.paymentMethods.map(p => (h("wa-option", { value: p.code }, p.description))))), booking_store.selectedPaymentMethod?.code === '001' && (h(Fragment, { key: 'ee29195e4ed909f9c2dbadb92d3ad4a06bf6855f' }, h("ir-input", { key: '23346746fa924dc7685cf28001ee628fd95dd9d8', value: this.bookedByData.cardNumber, defaultValue: this.bookedByData.cardNumber, "onText-change": e => this.handleCreditCardDataChange('cardNumber', e.detail.trim()), label: locales.entries.Lcz_CardNumber }), h("ir-input", { key: 'c15b291c40ee91fee1ebb9142a5cd7f914eee3a2', value: this.bookedByData.cardHolderName, defaultValue: this.bookedByData.cardHolderName, "onText-change": e => this.handleCreditCardDataChange('cardHolderName', e.detail.trim()), label: locales.entries.Lcz_CardHolderName }), h("ir-input", { key: 'b9d24fe15ce7c7724caafee5718fa8085d605c0a', "onText-change": e => {
            ...
            }, label: locales.entries.Lcz_ExpiryDate }))), booking_store.selectedPaymentMethod?.code === '005' && (h("div", { key: '718eedf86614d6c613e8fc94be8322bdcfd544f9', class: "form-group mt-md-1 mt-1 p-0 d-flex flex-column flex-md-row align-items-md-center" }, h("label", { key: 'ec2876476a5ee20bdb91d61c7627ea9e75a6c96f', class: "p-0 m-0 margin3" }), h("div", { key: '90410733d16a9e70f3cdf566a1fd723ec4f7b314', class: "p-0 m-0  controlContainer flex-fill" }, h("div", { key: '823ae9b62f3fe20c2b3e8ca264f31728882ce164', class: "property-booked-by__money-transfer-description", innerHTML: this.paymentMethods.find(p => p.code === '005')?.localizables.find(l => l.language.code.toLowerCase() === 'en')?.description })))), h("wa-checkbox", { key: 'e5f2186cc3245edeb12a843a1a415d68f629fefa', checked: this.bookedByData.emailGuest, onchange: event => this.handleDataChange('emailGuest', event) }, locales.entries.Lcz_EmailTheGuest))))));
    }
};
