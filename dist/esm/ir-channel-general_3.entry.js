...
const IrChannelHeader = class {
    ...
    render() {
        return (h(Host, { key: '0b4f3104dbaa895f78ff95c14765a9a8f9c0e506' }, h("ul", { key: 'aaece44446c7fa2dc1e6376e12407ae4d7097e38', class: "px-1" }, this.headerTitles.map((title, index) => (h("li", { class: `tab ${title.disabled ? 'text-light' : ''}`, key: title.id, onClick: () => {
                if (!title.disabled)
                    this.handleTabSelection(index);
            }, "data-disabled": title.disabled, "data-state": this.selectedIndex === index ? 'selected' : '' }, title.name)))), h("span", { key: '2be7c224c3d8d1903fae6a13be8655655333b805', class: "active-indicator", ref: el => (this.activeIndicator = el) })));
    }
};
...
const IrChannelMapping = class {
    ...
    render() {
        return (h(Host, { key: '1b655216009735b7134a933460ce1f3db1f477ab', class: "py-md-2 px-md-2" }, h("div", { key: 'eacb27a0646e0837d85afc9bf3564b9f40214655', class: "d-flex p-0 m-0 w-100 justify-content-end" }, h("button", { key: 'c76bc281509f9828075b2cb0e8bd93cd04a9e638', onClick: () => {
                setMappedChannel();
            }, class: "btn refresh-btn" }, locales.entries?.Lcz_Refresh)), h("section", { key: '07f79526e0a587dbab4a8ce8ce3a89e302acb4cf', class: "w-100" }, h("div", { key: 'c9130d5b6b85509cc68d9a5f92bfc62a8d02d150', class: "pt-1 mapped_row" }, h("p", { key: '8a9f6a71ad753896651e18cfe1a964bc710f7d9a', class: "mapped_item channel_name" }, channels_data.selectedChannel?.name), h("svg", { key: '7dcfcc534aa73bf63c30fda96b4ffd14bb7f11c7', xmlns: "http://www.w3.org/2000/svg", height: "14", width: "12.25", viewBox: "0 0 448 512" }, h("path", { key: '948c067c9e600dab185bf17eeb107e2deb0973ad', d: "M438.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-160-160c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L338.8 224 32 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l306.7 0L233.4 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l160-160z" })), h("p", { key: '397dca634767f19061e06a0194588445fed1f00b', class: "pl-2 mapped_item channel_name" }, "igloorooms")), h("div", { key: '90b81a7d07309961063761603bd6ee50800860e3' }, channels_data.selectedChannel?.property?.room_types?.map(room_type => {
