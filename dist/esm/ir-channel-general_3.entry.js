...
const IrChannelHeader = class {
    ...
    render() {
        return (h(Host, { key: '54dff9ba2cd6c9a690ebe689f277e86234e2ba79' }, h("ul", { key: '76c3e0e90ff7a1f8fda53a6221c74c023ad2261f', class: "px-1" }, this.headerTitles.map((title, index) => (h("li", { class: `tab ${title.disabled ? 'text-light' : ''}`, key: title.id, onClick: () => {
                if (!title.disabled)
                    this.handleTabSelection(index);
            }, "data-disabled": title.disabled, "data-state": this.selectedIndex === index ? 'selected' : '' }, title.name)))), h("span", { key: '65875c3c3d52ee93d31ffd5f573e728b0dcc1351', class: "active-indicator", ref: el => (this.activeIndicator = el) })));
    }
...
const IrChannelMapping = class {
    ...
    render() {
        var _a, _b, _c, _d, _e;
        return (h(Host, { key: 'bff1df4e3c98cde56a0eaa18c7bf315973d934a9', class: "py-md-1 px-md-2" }, h("div", { key: 'fe8aba1e57595e42df4f4e55b5cf42a13aacf196', class: "d-flex p-0 m-0 w-100 justify-content-end" }, h("button", { key: '072cd2a82ca78b5b2fa7c8fda83eef09d8572813', onClick: () => {
                setMappedChannel();
            }, class: "btn refresh-btn" }, (_a = locales.entries) === null || _a === void 0 ? void 0 : _a.Lcz_Refresh)), h("section", { key: 'c3ca85fe017884b5fa937f0e8266e713e6535c2a', class: "w-100" }, h("div", { key: '01ef9d31d11a087b4aeb76a34af4e8122886eb77', class: "py-1 mapped_row" }, h("p", { key: '25f231a51aa41aee5aefbb4308e318fac9b7c094', class: "mapped_item channel_name" }, (_b = channels_data.selectedChannel) === null || _b === void 0 ? void 0 : _b.name), h("svg", { key: 'b77f1ef8b9b0d0d82ef2eb5bbe9984069cf53910', xmlns: "http://www.w3.org/2000/svg", height: "14", width: "12.25", viewBox: "0 0 448 512" }, h("path", { key: 'caeb0a2dd3f63e34623bc76ead88edf829209b9e', d: "M438.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-160-160c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L338.8 224 32 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l306.7 0L233.4 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l160-160z" })), h("p", { key: '56cba5535e1fedaf673dfa598493956f86f067cd', class: "pl-2 mapped_item channel_name" }, "Igloorooms")), h("div", { key: 'f6bff4543b692634981873eda1e5882e02b6a0ea' }, (_e = (_d = (_c = channels_data.selectedChannel) === null || _c === void 0 ? void 0 : _c.property) === null || _d === void 0 ? void 0 : _d.room_types) === null || _e === void 0 ? void 0 : _e.map(room_type => {
            const mappedRoomType = this.mappingService.checkMappingExists(room_type.id, true);
            return (h(Fragment, null, h("div", { key: room_type.id, class: "mb-1 mapped_row" }, h("p", { class: "mapped_item" }, room_type.name), h("svg", { xmlns: "http://www.w3.org/2000/svg", height: "14", width: "12.25", viewBox: "0 0 448 512" }, h("path", { d: "M438.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-160-160c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L338.8 224 32 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l306.7 0L233.4 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l160-160z" })), this.renderMappingStatus(mappedRoomType, room_type.id, true)), room_type.rate_plans.map(rate_plan => {
                const mappedRatePlan = this.mappingService.checkMappingExists(rate_plan.id, false, room_type.id);
                // console.log(mappedRatePlan);
                return (h("div", { key: rate_plan.id, class: " mapped_row mb-1" }, h("p", { class: "pl-1 submap-text mapped_item" }, rate_plan.name), !mappedRatePlan.hide && (h("svg", { xmlns: "http://www.w3.org/2000/svg", height: "14", width: "12.25", viewBox: "0 0 448 512" }, h("path", { fill: "currentColor", d: "M438.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-160-160c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L338.8 224 32 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l306.7 0L233.4 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l160-160z" }))), this.renderMappingStatus(mappedRatePlan, rate_plan.id, false, room_type.id)));
            })));
        })))));
    }
};
