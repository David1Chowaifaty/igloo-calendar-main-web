...
import { f as downloadFile } from './utils-COglgzDo.js';
import { a as axios } from './axios-B50ozOIF.js';
import { h as hooks } from './moment-Mki5YqAR.js';
import { P as PropertyIdSchema, D as DateSchema, B as BookingNumberSchema, T as TaxTypesSchema } from './commonSchemas-ByEkDTMV.js';
...
const ChannelReportBaseSchema = libExports.z.object({
    ...
    SOURCE_ICON: libExports.z.string(),
});
/**
...
const FetchUnBookableRoomsSchema = libExports.z.object({
    property_ids: libExports.z.array(PropertyIdSchema),
...
const TaxCategorySchema = libExports.z.object({
    ...
    pct: libExports.z.number(),
    default_price: libExports.z.number().nullable().optional(),
...
const HandleExposedPropertyTaxCategoriesParamsSchema = libExports.z.object({
    ...
    SERVICE_CHARGE_INCLUDED_CODE: libExports.z.string().optional(),
    BABY_COT_PRICING_MODEL: libExports.z.string().optional(),
    SERVICE_CHARGE_PCT: libExports.z.number().optional(),
    tax_categories: libExports.z.array(TaxCategorySchema),
    TAXATION_STRATEGY: libExports.z.string(),
    DAY_USE_BLOCK: libExports.z
        .union([libExports.z.literal('0'), libExports.z.literal('1')])
        .nullable()
        .optional()
        .default(null),
});
const SetPropertyGapConfigParamsSchema = libExports.z.object({
    property_id: PropertyIdSchema,
    gap_rule_code: libExports.z.string(),
    gap_lookahead_days: libExports.z.number(),
});
const GetUnifiedFolioParamsSchema = libExports.z.object({
    property_id: PropertyIdSchema,
    from_date: DateSchema.nullable(),
    to_date: DateSchema.nullable(),
    target_type: libExports.z.string().nullable(),
    ...
    o_Total_Rows: libExports.z.number().int().nullable(),
    is_export_to_excel: libExports.z.boolean(),
    Link_excel: libExports.z.string(),
});
// A unified folio row can be tied either to an agent account or to a guest.
const UnifiedFolioTargetTypeSchema = libExports.z.enum(['AGENT', 'GUEST']);
const UnifiedFolioRecordSchema = libExports.z.object({
    TARGET_TYPE: UnifiedFolioTargetTypeSchema,
    AGENT_ID: libExports.z.number().nullable().optional(),
    AGENT_NAME: libExports.z.string().nullable().optional(),
    GUEST_ID: libExports.z.number().nullable().optional(),
    GUEST_NAME: libExports.z.string().nullable().optional(),
    GUEST_EMAIL: libExports.z.string().nullable().optional(),
    BOOKING_ID: libExports.z.number().nullable().optional(),
    BOOKING_NUMBER: libExports.z.string().nullable().optional(),
    DOC_ID: libExports.z.number().nullable().optional(),
    DOC_NUMBER: libExports.z.string().nullable().optional(),
    DOC_DATE: libExports.z.string().nullable().optional(),
    DOC_TYPE: libExports.z.string().nullable().optional(),
    DOC_HOUR: libExports.z.number().nullable().optional(),
    DOC_MINUTE: libExports.z.number().nullable().optional(),
...
const PrintGuestFolioDocParamsSchema = libExports.z.object({
    property_id: PropertyIdSchema,
    booking_nbr: BookingNumberSchema,
    mode: libExports.z.string(),
    reference: libExports.z.string(),
    extras: libExports.z.string().optional(),
});
const GetExposedBookingsByInvoicedStatusParamsSchema = libExports.z.object({
    property_id: PropertyIdSchema,
    booking_nbr: BookingNumberSchema,
    from_date: DateSchema,
    to_date: DateSchema,
    source: libExports.z.string().optional(),
    is_totally_invoiced: libExports.z.boolean().optional().default(false),
    start_row: libExports.z.number().default(0),
    end_row: libExports.z.number(),
});
const GetDayUseBookingsForCalendarParamsSchema = libExports.z.object({
    property_id: PropertyIdSchema,
    from_date: DateSchema,
    to_date: DateSchema,
});
const CalculateNetAmountParamsSchema = libExports.z.object({
    property_id: PropertyIdSchema,
    amount: libExports.z.number(),
