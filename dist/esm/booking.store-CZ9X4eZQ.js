...
import { i as isBlockUnit, f as dateDifference, e as extras, c as convertDateToTime, a as convertDateToCustomFormat, d as dateToFormattedString, b as calculateDaysBetweenDates } from './booking-SXprRnHy.js';
...
import { c as calendar_data$1 } from './calendar-data-DIuSgTRk.js';
...
function resolveAgentFromBookingSource(source) {
    const matchedSource = calendar_data$1.property?.allowed_booking_sources?.find(s => s.id.toString() === source.id.toString());
    if (matchedSource?.type === 'TRAVEL_AGENCY') {
        const agent = calendar_data$1.property.agents?.find(a => a.id.toString() === matchedSource.tag);
        return agent ?? null;
    }
    return null;
}
...
async function getRatePlanDisplayAmount({ rateplanSelection, totalNights, index, bookingService, variationService, }) {
    if (rateplanSelection.is_amount_modified) {
        ...
        const tax = await bookingService.calculateExclusiveTax({
            property_id: calendar_data$1.property.id,
            amount: net,
        });
        return net + tax;
    }
...
export { AckExposedRevisionPropsSchema as A, BookingInvoiceInfoSchema as B, CalculateExclusiveTaxPropsSchema as C, GetRoomsToCheckInPropsSchema as G, IssueInvoicePropsSchema as I, PrintInvoicePropsSchema as P, SetDepartureTimePropsSchema as S, VoidInvoicePropsSchema as V, ZIEntrySchema as Z, GetRoomsToCheckOutPropsSchema as a, booking_store as b, GetBookingInvoiceInfoPropsSchema as c, BookingService as d, VariationService as e, calculateTotalRooms as f, getMyBookings as g, resetReserved as h, setBookingSelectOptions as i, reserveRooms as j, setBookedByGuestManualEditState as k, updateBookedByGuest as l, modifyBookingStore as m, updateRoomParams as n, getVisibleInventory as o, getReservedRooms as p, hasAtLeastOneRoomSelected as q, resetBookingStore as r, setBookingDraft as s, getBookingTotalPrice as t, updateRoomGuest as u, bookedByGuestBaseData as v, resetAvailability as w };
