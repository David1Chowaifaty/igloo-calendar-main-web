...
import { U as UserService } from './user.service-4e26c1fc.js';
...
const IrUserFormPanel = class {
    ...
    render() {
        return (h("form", { key: '3eb18adac29fdd02a04003e1c5c590cd7b637435', class: "sheet-container", onSubmit: async (e) => {
                e.preventDefault();
                await this.createOrUpdateUser();
            } }, h("ir-title", { key: 'c6bf16327c505605062cc7d3b5b7f83709ebd8fd', class: "px-1 sheet-header", displayContext: "sidebar", label: this.isEdit ? this.user.username : 'Create New User' }), h("section", { key: '6633c519ffd6e349bb0a8d8eff7094643fcc112f', class: "px-1 sheet-body" }, h("ir-input-text", { key: '0da97fe8a732f86dc746f5513ac6c0bb7b839c2f', testId: "email", zod: this.userSchema.pick({ email: true }), wrapKey: "email", autoValidate: this.autoValidate, error: this.errors?.email, label: locales.entries.Lcz_Email, placeholder: "", onTextChange: e => this.updateUserField('email', e.detail), value: this.userInfo.email, onInputBlur: this.handleBlur.bind(this), maxLength: 40, errorMessage: this.emailErrorMessage }), h("ir-input-text", { key: 'e3d2747ced6389729d3d83a608c8d02873d89291', testId: "mobile", disabled: this.disableFields, zod: this.userSchema.pick({ mobile: true }), wrapKey: "mobile", error: this.errors?.mobile, asyncParse: true, autoValidate: this.user ? (this.userInfo?.mobile !== this.user.mobile ? true : false) : this.autoValidate, label: locales.entries.Lcz_Mobile, mask: this.mobileMask, placeholder: '', value: this.userInfo.mobile, onTextChange: e => this.updateUserField('mobile', e.detail) }), (this.user && this.user?.type?.toString() === this.superAdminId) || this.isPropertyAdmin ? null : (h("div", { class: "mb-1" }, h("ir-select", { testId: "user_type", error: this.errors?.type && !this.userInfo.type, disabled: this.disableFields, label: "Role", data: this.allowedUsersTypes.map(t => ({
                text: t.value,
                value: t.code,
            })), firstOption: locales.entries.Lcz_Select, selectedValue: this.userInfo.type?.toString(), onSelectChange: e => this.updateUserField('type', e.detail) }))), this.user?.type?.toString() !== '5' && (h(Fragment, { key: 'acde35a5d990abe4770b5a626453c8231f0e6347' }, h("input", { key: '005b70f474922a9ae54b336d9a8b0f1605007628', type: "text", name: "dummy", style: { display: 'none' } }), h("ir-input-text", { key: '6483e34a80fe42720934774e85c6fc064a4d86af', testId: "username", zod: this.userSchema.pick({ username: true }), wrapKey: "username", autoValidate: this.autoValidate, error: this.errors?.username, label: locales.entries.Lcz_Username, disabled: this.disableFields, placeholder: "", onTextChange: e => this.updateUserField('username', e.detail), value: this.userInfo.username,
        ...
        // ))
        ), this.user?.sign_ins?.length > 0 && (h("section", { key: '8ee99478462ea2d7507273a2f2fff0389695efa3', class: "logins-history-section mt-2" }, h("div", { key: 'e67ada42f169f278254184eee6b71df63023a4a4', class: "d-flex align-items-center logins-history-title-container justify-content-between" }, h("h4", { key: '18d5102cc554647f3a10cce39aa3b47c7914496d', class: "logins-history-title m-0 p-0" }, "Recent sign-ins"), this.user.sign_ins.length > 5 && (h("ir-button", { key: '3fe1fe746ed97e9c0e35067fe6439548f2471207', btn_styles: 'pr-0', text: !this.showFullHistory ? locales.entries.Lcz_ViewAll : locales.entries.Lcz_ViewLess, btn_color: "link", size: "sm", onClickHandler: () => (this.showFullHistory = !this.showFullHistory) }))), h("ul", { key: '01110e7587aaaec4af93c8d941394d77a1fb1e95', class: "logins-history-list" }, this.user.sign_ins.slice(0, this.showFullHistory ? this.user.sign_ins.length : 5).map((s, i) => {
        ...
        })))), h("ir-sidebar", { key: 'b69921976a50fffc0306ef887e7f7c66ef92f00f', open: this.isOpen, showCloseButton: false, style: {
                '--sidebar-block-padding': '0',
            }, onIrSidebarToggle: e => {
                e.stopImmediatePropagation();
                e.stopPropagation();
                this.isOpen = false;
            } }, this.isOpen && (h("ir-reset-password", { key: '9b130e65b6c50cc435a35da414926d745204ae5c', ticket: this.token.getToken(), skip2Fa: true, username: this.user.username, onCloseSideBar: e => {
                e.stopImmediatePropagation();
                e.stopPropagation();
                this.isOpen = false;
            }, slot: "sidebar-body" })))), h("div", { key: 'b34731eedf0fd920c20861513ff3c46c86e1042e', class: "sheet-footer" }, h("ir-button", { key: 'a0f89210a5b1bb91723eb84bcc0d721e73573c46', "data-testid": "cancel", onClickHandler: () => this.closeSideBar.emit(null), class: "flex-fill", btn_styles: "w-100 justify-content-center align-items-center", btn_color: "secondary", text: locales.entries.Lcz_Cancel }), h("ir-button", { key: '6e653d0bca52f0af8d415128d0797176ce1d9d88', "data-testid": "save", isLoading: this.isLoading, class: "flex-fill", btn_type: "submit", btn_styles: "w-100 justify-content-center align-items-center", text: locales.entries.Lcz_Save }))));
    }
};
...
const IrUserManagementTable = class {
    ...
    render() {
        return (h(Host, { key: 'ce3c5821c637c67f46938afa4f7015baeb811d93' }, h("section", { key: 'fe52e309c9e68b8b9d04cb6814917d4a366dcd95', class: "table-container h-100 p-1 w-100 m-0 table-responsive" }, h("table", { key: 'b0cb607b57b7b8ec4d9d3e85d6fa0c8b66705494', class: "table" }, h("thead", { key: '831ad9cdd4fc38421540c3a85c2d3cf8f5300288' }, h("tr", { key: '3839828382c09c0246be59057de42d8996e815df' }, h("th", { key: 'ccdd8d92a320ba9c00fe5df8dda60be4e5c3f00c', class: "text-left" }, locales.entries.Lcz_Username ?? 'Username'), h("th", { key: '253da85324533b0ae0b074f56ea7370f1c3838a1', class: "text-left" }, locales.entries.Lcz_Email), h("th", { key: '55bd6f4a6597b486ab7e12360859ea9bec090374', class: "text-left" }, locales.entries.Lcz_Mobile ?? 'Mobile'), h("th", { key: 'c5654686d3fbc94e93ab83d0d8045a93b7e89bf6', class: "text-left" }, locales.entries.Lcz_Role), h("th", { key: '86f4cf0d40f20b2b90c361855bdb4f74dff5437a', class: "text-left small", style: { fontWeight: 'bold' } }, h("p", { key: '76fa30625ba756603763d3e7ee8a9a8ab77b6ae2', class: "m-0 p-0 " }, locales.entries.Lcz_CreatedAt), h("p", { key: '933035b9ee3d8d1f68d0245d2343208eb5b0afbc', class: "m-0 p-0" }, locales.entries.Lcz_LastSignedIn)), this.haveAdminPrivileges && h("th", { key: 'e67d68952bf063b6ce8e4e0ce54d9172a307fb74' }, locales.entries.Lcz_Active), h("th", { key: '883707f8d7235a21cc307d59ef55b8dec22ace36', class: 'action-row' }, this.canCreate && (h("ir-icon", { key: '9e4d2893556a9ec13b8faf49c245cc37288cd9f5', style: { paddingLeft: '0.875rem' }, "data-testid": "new_user", title: locales.entries.Lcz_CreateUser, onIconClickHandler: () => {
                this.verifyAdminAction({
                    type: 'user',
                    mode: 'create',
                    user: null,
                });
            } }, h("svg", { key: 'a286d78d14bb07e2381d1566aae6d144ebd51c8b', slot: "icon", xmlns: "http://www.w3.org/2000/svg", height: "20", width: "17.5", viewBox: "0 0 448 512" }, h("path", { key: '737e837cc1d3bccbd2ebf2d45f6a128c28d0648b', fill: "currentColor", d: "M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32V224H48c-17.7 0-32 14.3-32 32s14.3 32 32 32H192V432c0 17.7 14.3 32 32 32s32-14.3 32-32V288H400c17.7 0 32-14.3 32-32s-14.3-32-32-32H256V80z" }))))))), h("tbody", { key: '11a7135636abb52f8105539325d60186cf357b56' }, this.users.map(user => {
        ...
        })))), h("ir-sidebar", { key: 'b1a88a3909ec1d3356653b09584a4f3b6ab51d73', open: this.currentTrigger !== null && this.currentTrigger?.type !== 'delete', onIrSidebarToggle: () => (this.currentTrigger = null), showCloseButton: false, style: {
                '--sidebar-block-padding': '0',
                '--sidebar-width': this.currentTrigger ? (this.currentTrigger?.type === 'unassigned_units' ? 'max-content' : '40rem') : 'max-content',
            } }, this.renderCurrentTrigger()), h("ir-modal", { key: '549d38d0e2c8a18b443222fc33e15a6b81198b6b', autoClose: false, modalBody: this.modalType === 'delete'
