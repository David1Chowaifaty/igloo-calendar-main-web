...
import { U as UserService } from './user.service-f6f537e1.js';
...
const IrUserFormPanel = class {
    ...
    render() {
        return (h("form", { key: '953ad65f3ce126bf1cd66fb5422c2e3b3cbb84c1', class: "sheet-container", onSubmit: async (e) => {
                e.preventDefault();
                await this.createOrUpdateUser();
            } }, h("ir-title", { key: 'e6ad0a31929b1b202498c26ef79928078de405bd', class: "px-1 sheet-header", displayContext: "sidebar", label: this.isEdit ? this.user.username : 'Create New User' }), h("section", { key: 'fd113fe6434da69cfef440ff81e0acff531967a1', class: "px-1 sheet-body" }, h("ir-input-text", { key: 'e53656887df2e0d1d588c29d5e333f406cf9baea', testId: "email", zod: this.userSchema.pick({ email: true }), wrapKey: "email", autoValidate: this.autoValidate, error: this.errors?.email, label: locales.entries.Lcz_Email, placeholder: "", onTextChange: e => this.updateUserField('email', e.detail), value: this.userInfo.email, onInputBlur: this.handleBlur.bind(this), maxLength: 40, errorMessage: this.emailErrorMessage }), h("ir-input-text", { key: 'cbfcedd440a24a699e4777c93c67a142617e648f', testId: "mobile", disabled: this.disableFields, zod: this.userSchema.pick({ mobile: true }), wrapKey: "mobile", error: this.errors?.mobile, asyncParse: true, autoValidate: this.user ? (this.userInfo?.mobile !== this.user.mobile ? true : false) : this.autoValidate, label: locales.entries.Lcz_Mobile, mask: this.mobileMask, placeholder: '', value: this.userInfo.mobile, onTextChange: e => this.updateUserField('mobile', e.detail) }), (this.user && this.user?.type?.toString() === this.superAdminId) || this.isPropertyAdmin ? null : (h("div", { class: "mb-1" }, h("ir-select", { testId: "user_type", error: this.errors?.type && !this.userInfo.type, disabled: this.disableFields, label: "Role", data: this.allowedUsersTypes.map(t => ({
                text: t.value,
                value: t.code,
            })), firstOption: locales.entries.Lcz_Select, selectedValue: this.userInfo.type?.toString(), onSelectChange: e => this.updateUserField('type', e.detail) }))), this.user?.type?.toString() !== '5' && (h(Fragment, { key: '5e7bac147a2e5f1552e9e4cc4a6ba6829def8c10' }, h("input", { key: '483a652d5efd9d640a2ce24b5d68360f48687b4a', type: "text", name: "dummy", style: { display: 'none' } }), h("ir-input-text", { key: 'bbe71e997b91864f3307dfaa5cb8f9a5adf30249', testId: "username", zod: this.userSchema.pick({ username: true }), wrapKey: "username", autoValidate: this.autoValidate, error: this.errors?.username, label: locales.entries.Lcz_Username, disabled: this.disableFields, placeholder: "", onTextChange: e => this.updateUserField('username', e.detail), value: this.userInfo.username,
        ...
        // ))
        ), this.user?.sign_ins?.length > 0 && (h("section", { key: 'ef7c63ff1e9b5cdb980cb438a2e0c284e4550eec', class: "logins-history-section mt-2" }, h("div", { key: '8adb0f7dc4baafca8528cd585acacb50335e7e2e', class: "d-flex align-items-center logins-history-title-container justify-content-between" }, h("h4", { key: '43b6694e8ccefda3e015d46feb4d6d94ff0220ee', class: "logins-history-title m-0 p-0" }, "Recent sign-ins"), this.user.sign_ins.length > 5 && (h("ir-button", { key: '489a180b2a1462dbdc5c4ba345cccbd6699046e8', btn_styles: 'pr-0', text: !this.showFullHistory ? locales.entries.Lcz_ViewAll : locales.entries.Lcz_ViewLess, btn_color: "link", size: "sm", onClickHandler: () => (this.showFullHistory = !this.showFullHistory) }))), h("ul", { key: '95a9db705dc8341aa90dfaf7b8ccf86008281b9b', class: "logins-history-list" }, this.user.sign_ins.slice(0, this.showFullHistory ? this.user.sign_ins.length : 5).map((s, i) => {
        ...
        })))), h("ir-sidebar", { key: 'd3e5e1ce18473efb613242cd16aeb7dbb6784eb3', open: this.isOpen, showCloseButton: false, style: {
                '--sidebar-block-padding': '0',
            }, onIrSidebarToggle: e => {
                e.stopImmediatePropagation();
                e.stopPropagation();
                this.isOpen = false;
            } }, this.isOpen && (h("ir-reset-password", { key: 'f44aa03aa5953868323db0bead5f51f3300ef195', ticket: this.token.getToken(), skip2Fa: true, username: this.user.username, onCloseSideBar: e => {
                e.stopImmediatePropagation();
                e.stopPropagation();
                this.isOpen = false;
            }, slot: "sidebar-body" })))), h("div", { key: '398017a1c5a3fedd6d135a24b80a202212e9ed13', class: "sheet-footer" }, h("ir-button", { key: '29ba7ad7acd75fcbacd35990b862f642a1934746', "data-testid": "cancel", onClickHandler: () => this.closeSideBar.emit(null), class: "flex-fill", btn_styles: "w-100 justify-content-center align-items-center", btn_color: "secondary", text: locales.entries.Lcz_Cancel }), h("ir-button", { key: '7f09372ca738ea68bb117968f3757f79751cc986', "data-testid": "save", isLoading: this.isLoading, class: "flex-fill", btn_type: "submit", btn_styles: "w-100 justify-content-center align-items-center", text: locales.entries.Lcz_Save }))));
    }
};
...
const IrUserManagementTable = class {
    ...
    render() {
        return (h(Host, { key: '8a2b1638e4e30565ad957a2fd17663e2d82dc6a9' }, h("section", { key: '779b7bfd75422bd4c0fd2c3f353e6c5cdfbc7b68', class: "table-container h-100 p-1 w-100 m-0 table-responsive" }, h("table", { key: 'dce8b36c67ec45cf92163190a6dc0c10c603db33', class: "table" }, h("thead", { key: '36350ac1fba72b186b80e1896ea5ba979975cc01' }, h("tr", { key: 'ab07a09e0165c0c5a05fd9501ba246fe6dcccdc7' }, h("th", { key: '740b767f223752feac830dd3de2d1ed0811f276b', class: "text-left" }, locales.entries.Lcz_Username ?? 'Username'), h("th", { key: '1b48885cd71d08d5cc7c205f783fdeca15c4f641', class: "text-left" }, locales.entries.Lcz_Email), h("th", { key: '77e5c19f143dea8653f9d74613889713d8a71d4a', class: "text-left" }, locales.entries.Lcz_Mobile ?? 'Mobile'), h("th", { key: '78e117103d430271f08e167cfeec04a91389958d', class: "text-left" }, locales.entries.Lcz_Role), h("th", { key: '0d96e1dd695d2eab4a5d3c37930af650226706da', class: "text-left small", style: { fontWeight: 'bold' } }, h("p", { key: 'ddddf8ecabc0b797f103d2142ce8cb7fe6488392', class: "m-0 p-0 " }, locales.entries.Lcz_CreatedAt), h("p", { key: 'bec62e572cabd60ffae76994856de783df1cc853', class: "m-0 p-0" }, locales.entries.Lcz_LastSignedIn)), this.haveAdminPrivileges && h("th", { key: '7bb9f563f4f7ecdd64d527bed57badd2ca114f59' }, locales.entries.Lcz_Active), h("th", { key: '7395e0015c91cb6d8e456df1be36b296e53e1d06', class: 'action-row' }, this.canCreate && (h("ir-icon", { key: '8dd5a8377b713d6863e3f07f52f3743eafaae37a', style: { paddingLeft: '0.875rem' }, "data-testid": "new_user", title: locales.entries.Lcz_CreateUser, onIconClickHandler: () => {
                this.verifyAdminAction({
                    type: 'user',
                    mode: 'create',
                    user: null,
                });
            } }, h("svg", { key: 'de30ce9c50734b94e8d73c769d3b92dc7d8182cb', slot: "icon", xmlns: "http://www.w3.org/2000/svg", height: "20", width: "17.5", viewBox: "0 0 448 512" }, h("path", { key: 'e9ada08afaaee857bdfc765e3ce32ed8f5c7c0ba', fill: "currentColor", d: "M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32V224H48c-17.7 0-32 14.3-32 32s14.3 32 32 32H192V432c0 17.7 14.3 32 32 32s32-14.3 32-32V288H400c17.7 0 32-14.3 32-32s-14.3-32-32-32H256V80z" }))))))), h("tbody", { key: '826796148803813378e27d685ed2af88cf25804c' }, this.users.map(user => {
        ...
        })))), h("ir-sidebar", { key: '1620a6c15d5e29caf3494c18671da2ae928008d8', open: this.currentTrigger !== null && this.currentTrigger?.type !== 'delete', onIrSidebarToggle: () => (this.currentTrigger = null), showCloseButton: false, style: {
                '--sidebar-block-padding': '0',
                '--sidebar-width': this.currentTrigger ? (this.currentTrigger?.type === 'unassigned_units' ? 'max-content' : '40rem') : 'max-content',
            } }, this.renderCurrentTrigger()), h("ir-modal", { key: '9e0a0b5b01c168e65b5355028fae4d9ff296b1a0', autoClose: false, modalBody: this.modalType === 'delete'
