...
import { U as UserService } from './user.service-037b73c1.js';
...
const IrUserFormPanel = class {
    ...
    render() {
        return (h("form", { key: 'b2570b291dbec86e8fa1c480b803f1170aaaafeb', class: "sheet-container", onSubmit: async (e) => {
                e.preventDefault();
                await this.createOrUpdateUser();
            } }, h("ir-title", { key: '57859e9a1f48e146cc12d2916f85a69e5f4c7dcc', class: "px-1 sheet-header", displayContext: "sidebar", label: this.isEdit ? this.user.username : 'Create New User' }), h("section", { key: 'c716ae87db02a6162376621aae5d65c0770f5a6a', class: "px-1 sheet-body" }, h("ir-input-text", { key: '13a58f1c9caf4f222314fbd70e3f903ec9d60996', testId: "email", zod: this.userSchema.pick({ email: true }), wrapKey: "email", autoValidate: this.autoValidate, error: this.errors?.email, label: locales.entries.Lcz_Email, placeholder: "", onTextChange: e => this.updateUserField('email', e.detail), value: this.userInfo.email, onInputBlur: this.handleBlur.bind(this), maxLength: 40, errorMessage: this.emailErrorMessage }), h("ir-input-text", { key: 'efffd062030766494639968f8439bf55f2dab3a7', testId: "mobile", disabled: this.disableFields, zod: this.userSchema.pick({ mobile: true }), wrapKey: "mobile", error: this.errors?.mobile, asyncParse: true, autoValidate: this.user ? (this.userInfo?.mobile !== this.user.mobile ? true : false) : this.autoValidate, label: locales.entries.Lcz_Mobile, mask: this.mobileMask, placeholder: '', value: this.userInfo.mobile, onTextChange: e => this.updateUserField('mobile', e.detail) }), (this.user && this.user?.type?.toString() === this.superAdminId) || this.isPropertyAdmin ? null : (h("div", { class: "mb-1" }, h("ir-select", { testId: "user_type", error: this.errors?.type && !this.userInfo.type, disabled: this.disableFields, label: "Role", data: this.allowedUsersTypes.map(t => ({
                text: t.value,
                value: t.code,
            })), firstOption: locales.entries.Lcz_Select, selectedValue: this.userInfo.type?.toString(), onSelectChange: e => this.updateUserField('type', e.detail) }))), this.user?.type?.toString() !== '5' && (h(Fragment, { key: '65011d45af53dc288219edb45381dece31f92c55' }, h("input", { key: '2dc382dec9066d79c95e9871811cd25bf16e10ed', type: "text", name: "dummy", style: { display: 'none' } }), h("ir-input-text", { key: '70ce14d17dc81a672db4917b9b548d69d7467389', testId: "username", zod: this.userSchema.pick({ username: true }), wrapKey: "username", autoValidate: this.autoValidate, error: this.errors?.username, label: locales.entries.Lcz_Username, disabled: this.disableFields, placeholder: "", onTextChange: e => this.updateUserField('username', e.detail), value: this.userInfo.username,
        ...
        // ))
        ), this.user?.sign_ins?.length > 0 && (h("section", { key: '2872a38f6a96e0d131580f05af15281ddab9ef1f', class: "logins-history-section mt-2" }, h("div", { key: '462593681683146cd26bd8b70b69830443efd489', class: "d-flex align-items-center logins-history-title-container justify-content-between" }, h("h4", { key: '5ad96be6e7b69cf76a59db1873e13560200a5046', class: "logins-history-title m-0 p-0" }, "Recent sign-ins"), this.user.sign_ins.length > 5 && (h("ir-button", { key: '6ed1397aba39e4e2abfa77b74908f0d7360b98a7', btn_styles: 'pr-0', text: !this.showFullHistory ? locales.entries.Lcz_ViewAll : locales.entries.Lcz_ViewLess, btn_color: "link", size: "sm", onClickHandler: () => (this.showFullHistory = !this.showFullHistory) }))), h("ul", { key: '7f5d0cc0cf8f576b5ed47116bd5741249ef82a4c', class: "logins-history-list" }, this.user.sign_ins.slice(0, this.showFullHistory ? this.user.sign_ins.length : 5).map((s, i) => {
        ...
        })))), h("ir-sidebar", { key: '775eb35281e79b09d4f75fda64bf5a9e66751199', open: this.isOpen, showCloseButton: false, style: {
                '--sidebar-block-padding': '0',
            }, onIrSidebarToggle: e => {
                e.stopImmediatePropagation();
                e.stopPropagation();
                this.isOpen = false;
            } }, this.isOpen && (h("ir-reset-password", { key: 'c3a1439609c9157e06bc1238f1eb0399df348811', ticket: this.token.getToken(), skip2Fa: true, username: this.user.username, onCloseSideBar: e => {
                e.stopImmediatePropagation();
                e.stopPropagation();
                this.isOpen = false;
            }, slot: "sidebar-body" })))), h("div", { key: '8cbf3c38c617c3be1ce7739820b471e7b8f3d8ab', class: "sheet-footer" }, h("ir-button", { key: '258c32de9e48d86a1423a2f79bbae199a75cd5a2', "data-testid": "cancel", onClickHandler: () => this.closeSideBar.emit(null), class: "flex-fill", btn_styles: "w-100 justify-content-center align-items-center", btn_color: "secondary", text: locales.entries.Lcz_Cancel }), h("ir-button", { key: 'b0bc5678b0061a44b967afcb2a1c43b42469d957', "data-testid": "save", isLoading: this.isLoading, class: "flex-fill", btn_type: "submit", btn_styles: "w-100 justify-content-center align-items-center", text: locales.entries.Lcz_Save }))));
    }
};
...
const IrUserManagementTable = class {
    ...
    render() {
        return (h(Host, { key: 'cb28683ee5ce2706d831e045ac73b85f0a5d44a5' }, h("section", { key: '1568b96759aba71dd3e47edf28ebc356af1cad84', class: "table-container h-100 p-1 w-100 m-0 table-responsive" }, h("table", { key: 'a5b29c9947d80eface31504faaca912c02681d66', class: "table" }, h("thead", { key: 'a25ba33d0a9d332c87e421f219bdb1e1f27b6aaa' }, h("tr", { key: '736dac4f53c6ef8672f3f6b04f499648af9262ff' }, h("th", { key: '230d62f0289c7c4f5869f2c2dd656702404f97b5', class: "text-left" }, locales.entries.Lcz_Username ?? 'Username'), h("th", { key: 'b3b940dc40f5b3ed0381b8131fe6d70bda40fae0', class: "text-left" }, locales.entries.Lcz_Email), h("th", { key: '43a497f30145be642a10004319e078d106740314', class: "text-left" }, locales.entries.Lcz_Mobile ?? 'Mobile'), h("th", { key: 'c396f31cfa20f69f7edbdf02edb5a7f836ea6f52', class: "text-left" }, locales.entries.Lcz_Role), h("th", { key: '414dba8bbccb74d2c6b4bbda4e5a3a1daf51465e', class: "text-left small", style: { fontWeight: 'bold' } }, h("p", { key: '4ea2c35f670fe13127f19a4a2f7949acdaf6faf3', class: "m-0 p-0 " }, locales.entries.Lcz_CreatedAt), h("p", { key: '7edb1284e44e3a47ef1730960e9fdcdb6c4fa00f', class: "m-0 p-0" }, locales.entries.Lcz_LastSignedIn)), this.haveAdminPrivileges && h("th", { key: 'd7daa97d4512f990f48039d96a158fc75647cbde' }, locales.entries.Lcz_Active), h("th", { key: '00cf3ce2e55cf661eb039ab14eb7e3affb6cc455', class: 'action-row' }, this.canCreate && (h("ir-icon", { key: '734ff244c1008e8a7c3700d1179a5fac4202985d', style: { paddingLeft: '0.875rem' }, "data-testid": "new_user", title: locales.entries.Lcz_CreateUser, onIconClickHandler: () => {
                this.verifyAdminAction({
                    type: 'user',
                    mode: 'create',
                    user: null,
                });
            } }, h("svg", { key: 'd221c2f4af7fffe4c836cf9b17dc582afec1bba9', slot: "icon", xmlns: "http://www.w3.org/2000/svg", height: "20", width: "17.5", viewBox: "0 0 448 512" }, h("path", { key: 'b4589b38adcf6c0bd32578e67fcae42bfece74d8', fill: "currentColor", d: "M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32V224H48c-17.7 0-32 14.3-32 32s14.3 32 32 32H192V432c0 17.7 14.3 32 32 32s32-14.3 32-32V288H400c17.7 0 32-14.3 32-32s-14.3-32-32-32H256V80z" }))))))), h("tbody", { key: '30a8f816c95c2a20df528eea3ce975c6f1c273f3' }, this.users.map(user => {
        ...
        })))), h("ir-sidebar", { key: 'ddaf416aea0adc1dd38c30a62a7a586527709c19', open: this.currentTrigger !== null && this.currentTrigger?.type !== 'delete', onIrSidebarToggle: () => (this.currentTrigger = null), showCloseButton: false, style: {
                '--sidebar-block-padding': '0',
                '--sidebar-width': this.currentTrigger ? (this.currentTrigger?.type === 'unassigned_units' ? 'max-content' : '40rem') : 'max-content',
            } }, this.renderCurrentTrigger()), h("ir-modal", { key: 'bc4abe677f3744400feac755e6be1164a2cc9d2b', autoClose: false, modalBody: this.modalType === 'delete'
