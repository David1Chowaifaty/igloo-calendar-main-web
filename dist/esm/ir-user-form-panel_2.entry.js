...
import { U as UserService } from './user.service-055d8d92.js';
...
const IrUserFormPanel = class {
    ...
    render() {
        return (h("form", { key: '51aec8b8613c1a8c804a3b188d8ab0d6726b9569', class: "sheet-container", onSubmit: async (e) => {
                e.preventDefault();
                await this.createOrUpdateUser();
            } }, h("ir-title", { key: 'd8f1a9e98e564ddbdb46e5ce9477f9c0ceb352e4', class: "px-1 sheet-header", displayContext: "sidebar", label: this.isEdit ? this.user.username : 'Create New User' }), h("section", { key: 'c9a55cb439346f7a20343525f78268c2e55aa448', class: "px-1 sheet-body" }, h("ir-input-text", { key: 'b530c8c30a46067cb70a31293b5087a065bd6bd1', testId: "email", zod: this.userSchema.pick({ email: true }), wrapKey: "email", autoValidate: this.autoValidate, error: this.errors?.email, label: locales.entries.Lcz_Email, placeholder: "", onTextChange: e => this.updateUserField('email', e.detail), value: this.userInfo.email, onInputBlur: this.handleBlur.bind(this), maxLength: 40, errorMessage: this.emailErrorMessage }), h("ir-input-text", { key: '8949ce1a808d212da996590f0c42c5d01685b7a8', testId: "mobile", disabled: this.disableFields, zod: this.userSchema.pick({ mobile: true }), wrapKey: "mobile", error: this.errors?.mobile, asyncParse: true, autoValidate: this.user ? (this.userInfo?.mobile !== this.user.mobile ? true : false) : this.autoValidate, label: locales.entries.Lcz_Mobile, mask: this.mobileMask, placeholder: '', value: this.userInfo.mobile, onTextChange: e => this.updateUserField('mobile', e.detail) }), (this.user && this.user?.type?.toString() === this.superAdminId) || this.isPropertyAdmin ? null : (h("div", { class: "mb-1" }, h("ir-select", { testId: "user_type", error: this.errors?.type && !this.userInfo.type, disabled: this.disableFields, label: "Role", data: this.allowedUsersTypes.map(t => ({
                text: t.value,
                value: t.code,
            })), firstOption: locales.entries.Lcz_Select, selectedValue: this.userInfo.type?.toString(), onSelectChange: e => this.updateUserField('type', e.detail) }))), this.user?.type?.toString() !== '5' && (h(Fragment, { key: 'c37552c4146b01f50be666bac8c97bd415d25294' }, h("input", { key: '211e820edc3568a975d646e30e47a212426a4f4d', type: "text", name: "dummy", style: { display: 'none' } }), h("ir-input-text", { key: 'fe8b2b0bb7b583e0245d5444cb5b95b24d5363f6', testId: "username", zod: this.userSchema.pick({ username: true }), wrapKey: "username", autoValidate: this.autoValidate, error: this.errors?.username, label: locales.entries.Lcz_Username, disabled: this.disableFields, placeholder: "", onTextChange: e => this.updateUserField('username', e.detail), value: this.userInfo.username,
        ...
        // ))
        ), this.user?.sign_ins?.length > 0 && (h("section", { key: 'bc8da02be8309b40d35780e8d6a8f3c3ffaaa186', class: "logins-history-section mt-2" }, h("div", { key: '3417193e7492b5518ef750c220d4b6fe30dd2410', class: "d-flex align-items-center logins-history-title-container justify-content-between" }, h("h4", { key: '1d35cb501051218ec9ea3e08b9b63f29222fd353', class: "logins-history-title m-0 p-0" }, "Recent sign-ins"), this.user.sign_ins.length > 5 && (h("ir-button", { key: '77b37f143a466fa1a1a3867b6fce8a8ecb0b2acb', btn_styles: 'pr-0', text: !this.showFullHistory ? locales.entries.Lcz_ViewAll : locales.entries.Lcz_ViewLess, btn_color: "link", size: "sm", onClickHandler: () => (this.showFullHistory = !this.showFullHistory) }))), h("ul", { key: 'cb0b9cace3733d4e9edd4053dad36a1b5ed7c124', class: "logins-history-list" }, this.user.sign_ins.slice(0, this.showFullHistory ? this.user.sign_ins.length : 5).map((s, i) => {
        ...
        })))), h("ir-sidebar", { key: 'd6b61b3643f2f2a252f281a315e330f7c5c37f06', open: this.isOpen, showCloseButton: false, style: {
                '--sidebar-block-padding': '0',
            }, onIrSidebarToggle: e => {
                e.stopImmediatePropagation();
                e.stopPropagation();
                this.isOpen = false;
            } }, this.isOpen && (h("ir-reset-password", { key: '581ee7ee31715fcf407b24529d70963719f4450b', ticket: this.token.getToken(), skip2Fa: true, username: this.user.username, onCloseSideBar: e => {
                e.stopImmediatePropagation();
                e.stopPropagation();
                this.isOpen = false;
            }, slot: "sidebar-body" })))), h("div", { key: '6a4c6ea1a644ab6934024a0c282f8dca8da159db', class: "sheet-footer" }, h("ir-button", { key: '6b732a3f2e614efd6b136bf3876bfb57c42e6dac', "data-testid": "cancel", onClickHandler: () => this.closeSideBar.emit(null), class: "flex-fill", btn_styles: "w-100 justify-content-center align-items-center", btn_color: "secondary", text: locales.entries.Lcz_Cancel }), h("ir-button", { key: 'b3e2aad559094d47484227b06785e1b885ae1e96', "data-testid": "save", isLoading: this.isLoading, class: "flex-fill", btn_type: "submit", btn_styles: "w-100 justify-content-center align-items-center", text: locales.entries.Lcz_Save }))));
    }
};
...
const IrUserManagementTable = class {
    ...
    render() {
        return (h(Host, { key: '055a30e5a8c93b89d1b4fdf26823354806366905' }, h("section", { key: '7e16c97fbc9ebf7ae2a881ab8a985c0e6231c987', class: "table-container h-100 p-1 w-100 m-0 table-responsive" }, h("table", { key: 'a3417b242eb5e3e874e157a2583bda9f002a81f9', class: "table" }, h("thead", { key: '31bdfd2885a379d4a52979a0c8ca8d3d4b77323e' }, h("tr", { key: '1bbaf4dca4c4f30a1449a4acbe6a865d922de35e' }, h("th", { key: 'a56a6cce41fdd33c6165ee8ba30032a1c79150d1', class: "text-left" }, locales.entries.Lcz_Username ?? 'Username'), h("th", { key: '65af842bc0189584703a3ae08129dc2ef1279df8', class: "text-left" }, locales.entries.Lcz_Email), h("th", { key: '9502554254986b9250cffb1cbf8b03146e0222b6', class: "text-left" }, locales.entries.Lcz_Mobile ?? 'Mobile'), h("th", { key: '1a0e5de352627f49fdb248565fc89c2e0e364191', class: "text-left" }, locales.entries.Lcz_Role), h("th", { key: 'c5d9fb6c5fb351ad3b822f714b00f3865f97bf08', class: "text-left small", style: { fontWeight: 'bold' } }, h("p", { key: 'c29103ba266fe92974e59721a1f3e1877907d76d', class: "m-0 p-0 " }, locales.entries.Lcz_CreatedAt), h("p", { key: '1b37465c0078b841135f521b33a31552d554ffda', class: "m-0 p-0" }, locales.entries.Lcz_LastSignedIn)), this.haveAdminPrivileges && h("th", { key: 'a0274a6da0427a37517c2c98c857d72bbc61004d' }, locales.entries.Lcz_Active), h("th", { key: '46690533ce2a0dec11cb7cd7071ce00a3e407c64', class: 'action-row' }, this.canCreate && (h("ir-icon", { key: '078ff792381104f9b8dc890d24c941b97f18d555', style: { paddingLeft: '0.875rem' }, "data-testid": "new_user", title: locales.entries.Lcz_CreateUser, onIconClickHandler: () => {
                this.verifyAdminAction({
                    type: 'user',
                    mode: 'create',
                    user: null,
                });
            } }, h("svg", { key: '0e1a5b57808d96fff03e00a9e455314dcd7b027b', slot: "icon", xmlns: "http://www.w3.org/2000/svg", height: "20", width: "17.5", viewBox: "0 0 448 512" }, h("path", { key: 'ad191d957ac6f2d66490bea73994c93219888c94', fill: "currentColor", d: "M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32V224H48c-17.7 0-32 14.3-32 32s14.3 32 32 32H192V432c0 17.7 14.3 32 32 32s32-14.3 32-32V288H400c17.7 0 32-14.3 32-32s-14.3-32-32-32H256V80z" }))))))), h("tbody", { key: '31962507f95912ae7252c397dfa72c8277121374' }, this.users.map(user => {
        ...
        })))), h("ir-sidebar", { key: 'f4d802ed06960119ba44ac244dd328156c552c7b', open: this.currentTrigger !== null && this.currentTrigger?.type !== 'delete', onIrSidebarToggle: () => (this.currentTrigger = null), showCloseButton: false, style: {
                '--sidebar-block-padding': '0',
                '--sidebar-width': this.currentTrigger ? (this.currentTrigger?.type === 'unassigned_units' ? 'max-content' : '40rem') : 'max-content',
            } }, this.renderCurrentTrigger()), h("ir-modal", { key: 'd5262cdaf30ee663693065a2904f929bf98255b7', autoClose: false, modalBody: this.modalType === 'delete'
