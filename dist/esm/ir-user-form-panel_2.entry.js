...
import { U as UserService } from './user.service-f2b5cd7b.js';
...
const IrUserFormPanel = class {
    ...
    render() {
        return (h("form", { key: '238d0e4c38b0c93dd8a8da732bfb1e8b7e945f8f', class: "sheet-container", onSubmit: async (e) => {
                e.preventDefault();
                await this.createOrUpdateUser();
            } }, h("ir-title", { key: '46e6a4118b0d166cdd0eb9af3e5b37a453e1da0a', class: "px-1 sheet-header", displayContext: "sidebar", label: this.isEdit ? this.user.username : 'Create New User' }), h("section", { key: 'f25bcb8a11b65fe7d84c004f9154e834128178b5', class: "px-1 sheet-body" }, h("ir-input-text", { key: '9c82c0467fcc29f6cdc619133a3c480cc2234a54', testId: "email", zod: this.userSchema.pick({ email: true }), wrapKey: "email", autoValidate: this.autoValidate, error: this.errors?.email, label: locales.entries.Lcz_Email, placeholder: "", onTextChange: e => this.updateUserField('email', e.detail), value: this.userInfo.email, onInputBlur: this.handleBlur.bind(this), maxLength: 40, errorMessage: this.emailErrorMessage }), h("ir-input-text", { key: 'a078a001e71f0731649c75ecc36586a70d8377d1', testId: "mobile", disabled: this.disableFields, zod: this.userSchema.pick({ mobile: true }), wrapKey: "mobile", error: this.errors?.mobile, asyncParse: true, autoValidate: this.user ? (this.userInfo?.mobile !== this.user.mobile ? true : false) : this.autoValidate, label: locales.entries.Lcz_Mobile, mask: this.mobileMask, placeholder: '', value: this.userInfo.mobile, onTextChange: e => this.updateUserField('mobile', e.detail) }), (this.user && this.user?.type?.toString() === this.superAdminId) || this.isPropertyAdmin ? null : (h("div", { class: "mb-1" }, h("ir-select", { testId: "user_type", error: this.errors?.type && !this.userInfo.type, disabled: this.disableFields, label: "Role", data: this.allowedUsersTypes.map(t => ({
                text: t.value,
                value: t.code,
            })), firstOption: locales.entries.Lcz_Select, selectedValue: this.userInfo.type?.toString(), onSelectChange: e => this.updateUserField('type', e.detail) }))), this.user?.type?.toString() !== '5' && (h(Fragment, { key: '14dba493649a00747695265d17a120bb0eb7ce89' }, h("input", { key: '0a0ce96c24985d50e9173541ad658c55cfadb214', type: "text", name: "dummy", style: { display: 'none' } }), h("ir-input-text", { key: '2c057f6b300b2387805bced99315a4a09e7e1395', testId: "username", zod: this.userSchema.pick({ username: true }), wrapKey: "username", autoValidate: this.autoValidate, error: this.errors?.username, label: locales.entries.Lcz_Username, disabled: this.disableFields, placeholder: "", onTextChange: e => this.updateUserField('username', e.detail), value: this.userInfo.username,
        ...
        // ))
        ), this.user?.sign_ins?.length > 0 && (h("section", { key: '215144a06a45edb8f121f26b7dfbcfe4377665d8', class: "logins-history-section mt-2" }, h("div", { key: '6ca9890271cc49a8c6b262eda3df383cf82b60cf', class: "d-flex align-items-center logins-history-title-container justify-content-between" }, h("h4", { key: 'f647ee68d1f6aa729d1f07949c0f5bb559e8ade4', class: "logins-history-title m-0 p-0" }, "Recent sign-ins"), this.user.sign_ins.length > 5 && (h("ir-button", { key: 'c00c360e428c3a4a751049e9142c16b8c76afff1', btn_styles: 'pr-0', text: !this.showFullHistory ? locales.entries.Lcz_ViewAll : locales.entries.Lcz_ViewLess, btn_color: "link", size: "sm", onClickHandler: () => (this.showFullHistory = !this.showFullHistory) }))), h("ul", { key: '14195d824852b945b7748b5383458883b3b74455', class: "logins-history-list" }, this.user.sign_ins.slice(0, this.showFullHistory ? this.user.sign_ins.length : 5).map((s, i) => {
        ...
        })))), h("ir-sidebar", { key: 'bbb2fc7e35bcd0b1bc0c3119c2ee7eb032ffa13b', open: this.isOpen, showCloseButton: false, style: {
                '--sidebar-block-padding': '0',
            }, onIrSidebarToggle: e => {
                e.stopImmediatePropagation();
                e.stopPropagation();
                this.isOpen = false;
            } }, this.isOpen && (h("ir-reset-password", { key: '8aed4aca4c9e6a6394e621d38daa67e21964f654', ticket: this.token.getToken(), skip2Fa: true, username: this.user.username, onCloseSideBar: e => {
                e.stopImmediatePropagation();
                e.stopPropagation();
                this.isOpen = false;
            }, slot: "sidebar-body" })))), h("div", { key: 'f8f978d8a3ff20d93751c1613b0f7b2451fbc6b5', class: "sheet-footer" }, h("ir-button", { key: '6ec0565cb4755b489d853d9a987a3bb7c2ed7d5f', "data-testid": "cancel", onClickHandler: () => this.closeSideBar.emit(null), class: "flex-fill", btn_styles: "w-100 justify-content-center align-items-center", btn_color: "secondary", text: locales.entries.Lcz_Cancel }), h("ir-button", { key: '72fd00e302c4b788fcd6c0993ea736298e5d1615', "data-testid": "save", isLoading: this.isLoading, class: "flex-fill", btn_type: "submit", btn_styles: "w-100 justify-content-center align-items-center", text: locales.entries.Lcz_Save }))));
    }
};
...
const IrUserManagementTable = class {
    ...
    render() {
        return (h(Host, { key: '89b857b85212d4f175c93e2c3f0b5ce863bbe51f' }, h("section", { key: '8dad827341964edf6ca0702137e02114889e0343', class: "table-container h-100 p-1 w-100 m-0 table-responsive" }, h("table", { key: '4abd0c82595596b67d671898fefa5da7edf121e1', class: "table" }, h("thead", { key: 'f54e9bde2441320432b9017fc6d61e6675922aab' }, h("tr", { key: '3ae143d1b6957ab89d2148c65f45b7a16bcb8f58' }, h("th", { key: '0d7285d4b1dd95b2eb6c57e994e9df9e4a9747d2', class: "text-left" }, locales.entries.Lcz_Username ?? 'Username'), h("th", { key: 'fac99d91e37e00c5fc6911afa8c92556fc9b5591', class: "text-left" }, locales.entries.Lcz_Email), h("th", { key: '2d7b9bf979afcdba019f32052dc1a363eae9de3c', class: "text-left" }, locales.entries.Lcz_Mobile ?? 'Mobile'), h("th", { key: '0e9cf5cbdbf41aee767f6ff431d721164075e73e', class: "text-left" }, locales.entries.Lcz_Role), h("th", { key: 'd4ba92888c96e1e2794fb0aa483c60e307a64985', class: "text-left small", style: { fontWeight: 'bold' } }, h("p", { key: '8aba04cc1cd6c93a13ede58a1e1efc9030c4593d', class: "m-0 p-0 " }, locales.entries.Lcz_CreatedAt), h("p", { key: '065c8c1c175d66e8cd54f66c4fa8aa59d9c9a9f8', class: "m-0 p-0" }, locales.entries.Lcz_LastSignedIn)), this.haveAdminPrivileges && h("th", { key: '88082dea69b52da696f9e563448cd4288031a62b' }, locales.entries.Lcz_Active), h("th", { key: 'e5c56d15b2a581ed605207e0c08494bb08f4c5c2', class: 'action-row' }, this.canCreate && (h("ir-icon", { key: 'f80ce4ee3c47426a3cf03c5b8e46e5aa87a31b0b', style: { paddingLeft: '0.875rem' }, "data-testid": "new_user", title: locales.entries.Lcz_CreateUser, onIconClickHandler: () => {
                this.verifyAdminAction({
                    type: 'user',
                    mode: 'create',
                    user: null,
                });
            } }, h("svg", { key: 'b8bfbd8258eafa9d4c0fc378198eb0a05666372e', slot: "icon", xmlns: "http://www.w3.org/2000/svg", height: "20", width: "17.5", viewBox: "0 0 448 512" }, h("path", { key: 'd9feadf72116d0d58ce9fa75e6609a78ee7bde01', fill: "currentColor", d: "M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32V224H48c-17.7 0-32 14.3-32 32s14.3 32 32 32H192V432c0 17.7 14.3 32 32 32s32-14.3 32-32V288H400c17.7 0 32-14.3 32-32s-14.3-32-32-32H256V80z" }))))))), h("tbody", { key: '43e47edab20ba194b774056ff7dc0938bea5a677' }, this.users.map(user => {
        ...
        })))), h("ir-sidebar", { key: '8ae714ea34c28a62120cdf69ccb3e1dbca6efed8', open: this.currentTrigger !== null && this.currentTrigger?.type !== 'delete', onIrSidebarToggle: () => (this.currentTrigger = null), showCloseButton: false, style: {
                '--sidebar-block-padding': '0',
                '--sidebar-width': this.currentTrigger ? (this.currentTrigger?.type === 'unassigned_units' ? 'max-content' : '40rem') : 'max-content',
            } }, this.renderCurrentTrigger()), h("ir-modal", { key: '1c19a8cafd56c7a51308eb96984b38ae0658dac7', autoClose: false, modalBody: this.modalType === 'delete'
