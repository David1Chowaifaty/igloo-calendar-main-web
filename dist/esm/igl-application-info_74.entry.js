...
import { b as booking_store, m as modifyBookingStore, B as BookingService, c as calculateTotalRooms, r as resetReserved, a as resetBookingStore, d as reserveRooms, u as updateRoomParams, g as getVisibleInventory, e as buildPaymentTypes } from './booking.service-a752ab0c.js';
import { l as locales } from './locales.store-f4150353.js';
import { i as isSingleUnit, c as calendar_data } from './calendar-data-8a36a1b2.js';
import { f as formatAmount, e as extras, h as handleBodyOverflow, g as getReleaseHoursString, d as dateToFormattedString, c as calculateDaysBetweenDates, v as validateEmail, b as buildSplitIndex, a as getPrivateNote, t as toFloat, r as renderTime, i as formatName } from './utils-5036fcd3.js';
import { v as v4 } from './v4-964634d6.js';
import { h as hooks } from './moment-ab846cee.js';
import { i as isRequestPending, a as interceptor_requests } from './ir-interceptor.store-ebb6c559.js';
import { z, Z as ZodError, a as ZodIssueCode } from './index-d55e923c.js';
...
const IrBookingStatusTag = class {
    ...
    render() {
        return (h("wa-badge", { key: '2c4519904450250f27944496b49da8620e151866', style: { padding: '0.375em 0.625em', letterSpacing: '0.03rem' }, variant: this.badgeVariant[this.isRequestToCancel ? '003' : this.status.code] }, this.status.description.toUpperCase()));
    }
};
...
const IrGuestInfoForm = class {
    ...
    render() {
        return (h(Host, { key: '87e2a9f7947ae1dfc7d2da3c3831504d54bcaa58' }, h("ir-validator", { key: '516b6730a7900e645bf28f27bd19528a87b5a670', schema: guestInfoFormSchema.shape.first_name, value: this.guest?.first_name ?? '', autovalidate: this.autoValidate, valueEvent: "text-change input input-change", blurEvent: "input-blur blur" }, h("ir-custom-input", { key: '259352da688ca404d6bbb22a99cb3ed761cb2646', id: 'firstName', value: this.guest?.first_name, required: true, "onText-change": e => this.handleInputChange({ first_name: e.detail }), label: locales.entries?.Lcz_FirstName })), h("ir-validator", { key: 'f8beec9403c0247b247d29de059d5506bd57b199', schema: guestInfoFormSchema.shape.last_name, value: this.guest?.last_name ?? '', autovalidate: this.autoValidate, valueEvent: "text-change input input-change", blurEvent: "input-blur blur" }, h("ir-custom-input", { key: '27e9f811f0514f2794f95e594c16bb0ca3653433', value: this.guest?.last_name, required: true, id: "lastName", "onText-change": e => this.handleInputChange({ last_name: e.detail }), label: locales.entries?.Lcz_LastName })), h("ir-validator", { key: '4a20221a1a1e361eb27e87aa2409846abc02e118', schema: guestInfoFormSchema.shape.email, value: this.guest?.email ?? '', autovalidate: this.autoValidate, valueEvent: "text-change input input-change", blurEvent: "input-blur blur" }, h("ir-custom-input", { key: '78cd087ef6b84ad495d6443e4d6f6d2b4c9358fb', label: locales.entries?.Lcz_Email, id: "email", value: this.guest?.email, required: true, "onText-change": e => this.handleInputChange({ email: e.detail }) })), h("ir-validator", { key: 'f2ee9bab5fefa2c8cbba3ebfbfc41870494947c9', schema: guestInfoFormSchema.shape.alternative_email, value: this.guest?.alternative_email ?? '', autovalidate: this.autoValidate, valueEvent: "text-change input input-change", blurEvent: "input-blur blur" }, h("ir-custom-input", { key: '2fcc21b6e15dd49e4cc06c50639b92ff9d98304a', label: locales.entries?.Lcz_AlternativeEmail, id: "altEmail", value: this.guest?.alternative_email, "onText-change": e => this.handleInputChange({ alternative_email: e.detail }) })), h("ir-validator", { key: '08aa384f362db773d15e97eeeee663bf127dcd43', schema: guestInfoFormSchema.shape.country_id, value: this.guest?.country_id ?? undefined, autovalidate: this.autoValidate, valueEvent: "countryChange" }, h("ir-country-picker", { key: '37898640a0912bf9beb87fcec23bf538da620d72', size: "small", variant: "modern", country: this.countries.find(c => c.id === this.guest?.country_id), label: locales.entries?.Lcz_Country, onCountryChange: e => this.handleInputChange({ country_id: e.detail.id }), countries: this.countries })), h("ir-validator", { key: 'cb1facbacf438e2e060b62ecaf8bf15bd23ccb2d', schema: guestInfoFormSchema.shape.mobile, value: this.guest?.mobile ?? '', autovalidate: this.autoValidate, valueEvent: "mobile-input-change" }, h("ir-mobile-input", { key: '9ae4181ffd76e92541a36943e65e0c5f0e90f94a', size: "small", "onMobile-input-change": e => {
                this.handleInputChange({ mobile: e.detail.formattedValue });
            }, "onMobile-input-country-change": e => this.handleInputChange({ country_phone_prefix: e.detail.phone_prefix }), value: this.guest?.mobile ?? '', required: true, countryCode: this.countries.find(c => c.phone_prefix === this.guest?.country_phone_prefix)?.code, countries: this.countries })), h("ir-validator", { key: '1c305592e04398593ad2c4bc0aa2f134db967bdd', schema: guestInfoFormSchema.shape.notes, value: this.guest?.notes ?? '', autovalidate: this.autoValidate, valueEvent: "wa-change change input", blurEvent: "wa-blur blur" }, h("wa-textarea", { key: '8a9d67a965e0058cc238d51fb8cae028ee495a70', size: "small", onchange: e => this.handleInputChange({ notes: e.target.value }), value: this.guest?.notes ?? '', label: locales.entries?.Lcz_PrivateNote }))));
    }
};
...
const IrIcon = class {
    ...
    iconClickHandler;
    render() {
        return (h("button", { key: '214bd9c7d97492f00755bfbe2a3900f9d9a1fe1a', type: this.type, class: "icon-button", onClick: () => this.iconClickHandler.emit() }, h("slot", { key: '58b4bf419331068eebf42562999fc6c1ff95d041', name: "icon" })));
    }
};
...
const IrPaymentFolioForm = class {
    ...
    render() {
        // const isNewPayment = this.folioData?.payment_type?.code === '001' && this.folioData.id === -1;
        return (h("form", { key: '21941ef0c7d2512f4255c44457a1460e2386b41d', onSubmit: e => {
                e.preventDefault();
                const submitter = e.submitter;
                if (submitter?.value === 'save') {
                    this.savePayment();
                }
            }, class: "payment-folio__form", id: this.formId }, h("ir-custom-date-picker", { key: '34e4411a6f5a9abd127a2bedbcc5ca304099ef9d', id: this.controlIds.date, label: "Date", "aria-invalid": this.errors?.date && !this.folioData?.date ? 'true' : 'false', "data-testid": "pickup_date", onDateChanged: evt => {
                this.updateFolioData({ date: evt.detail.start?.format(DATE_FORMAT) });
            }, minDate: hooks().add(-2, 'months').format('YYYY-MM-DD'), emitEmptyDate: true, maxDate: this.today, date: this.folioData?.date }), h("ir-validator", { key: '2fc5987ccb9a4da3b23a163550cdcec4bd372a67', value: this.folioData?.payment_type?.code, autovalidate: this.autoValidate, schema: paymentTypeSchema.shape.code, valueEvent: "change wa-change select-change", blurEvent: "wa-hide" }, h("wa-select", { key: '8137fe6d31a52e3b77dc562b0ba5c651c86cb08d', id: this.controlIds.transactionType, size: "small", "onwa-hide": event => this.stopEventPropagation(event), "onwa-show": event => this.stopEventPropagation(event), placeholder: "Select...", label: "Transaction type", defaultValue: this.folioData?.payment_type?.code, value: this.folioData?.payment_type?.code, disabled: this.mode === 'payment-action', onchange: event => {
                this.stopEventPropagation(event);
                this.handleDropdownChange(event.target.value);
            } }, h("wa-option", { key: 'e1d6129f383dfec5cf40f7266994c5834ae0a26b', value: "" }, "Select..."), this.renderDropdownItems())), this.requiresPaymentMethod(this.folioData?.payment_type?.code) && (h("ir-validator", { key: 'e5fe7eea72bd930da3e88e31fda68cf1902017ad', value: this.folioData?.payment_method?.code ?? '', autovalidate: this.autoValidate, schema: paymentMethodSchema.shape.code, valueEvent: "change wa-change select-change", blurEvent: "wa-hide" }, h("wa-select", { key: '70afce4396dcf78727e77599a2ca5b259d110680', id: this.controlIds.paymentMethod, size: "small", label: `${this.folioData.payment_type?.code === '001' ? 'Payment' : 'Refund'} method`, "onwa-show": event => this.stopEventPropagation(event), "onwa-hide": event => this.stopEventPropagation(event), defaultValue: this.folioData?.payment_method?.code, value: this.folioData?.payment_method?.code ?? '', onchange: event => {
                this.stopEventPropagation(event);
                this.handlePaymentMethodDropdownChange(event.target.value);
            } }, h("wa-option", { key: '64d6d4df9817497f190af8cf0c52d563ca09c4dc', value: "" }, "Select..."), this.paymentEntries?.methods?.map(pt => {
            return (h("wa-option", { key: pt.CODE_NAME, label: pt.CODE_VALUE_EN, value: pt.CODE_NAME }, pt.CODE_VALUE_EN));
        })))), h("ir-validator", { key: '01aab8e531782919b1b49eec9167695af0571e76', value: this.folioData?.amount?.toString() ?? undefined, autovalidate: this.autoValidate, schema: folioBaseSchema.shape.amount, valueEvent: "text-change input input-change", blurEvent: "input-blur" }, h("ir-custom-input", { key: 'da027977981b905994290a387c63f16b55b3f840', id: this.controlIds.amount, "aria-invalid": String(!!this.errors?.amount), value: this.folioData?.amount?.toString() ?? '', label: "Amount", mask: "price", min: 0, "onText-change": e => this.updateFolioData({ amount: !e.detail ? undefined : Number(e.detail) }) }, h("span", { key: '4ab425bbc708afe4112125d50426f6541499ad01', slot: "start" }, calendar_data.currency.symbol))), h("ir-validator", { key: '38a5f272d39779152eb292372ea77a443e65abf6', value: this.folioData?.reference ?? '', autovalidate: this.autoValidate, schema: folioBaseSchema.shape.reference, valueEvent: "text-change input input-change", blurEvent: "input-blur" }, h("ir-custom-input", { key: '7f5b9ee992f5c637ace0a552bf7e327af5ae6238', id: this.controlIds.reference, value: this.folioData?.reference ?? '', label: "Reference", maxlength: 50, "onText-change": e => this.updateFolioData({ reference: e.detail ?? '' }) }))));
    }
    static get watchers() { return {
