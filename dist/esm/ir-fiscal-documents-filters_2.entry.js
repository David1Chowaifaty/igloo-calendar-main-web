...
import { F as FdTypes, a as FdStatus } from './enums-Dv3168hx.js';
...
const IrFiscalDocumentsFilters = class {
    ...
    render() {
        return (h("form", { key: '401fc98d42b6fc618d7cac871dadd0bca7bf3b6e', onSubmit: e => {
                e.preventDefault();
                this.applyFilters.emit(this.filters);
            } }, h("div", { key: '20fefb3c4c94152c1a3caa9f40cfbdeb938a4f43', class: "filters-bar" }, h("ir-validator", { key: '616ff4eadc100e56c93e06fe982bbd52d8818412', value: this.filters?.fromDate || this.filters?.toDate, schema: libExports.z.string().nonempty(), class: "filters-bar__dates" }, h("ir-date-range-filter", { key: '1ee052456de39e4b298f1f795c59342a3fb10784', maxDate: today.format('YYYY-MM-DD'), class: "filters-bar__date_picker", fromDate: this.filters.fromDate, toDate: this.filters.toDate, onDatesChanged: e => this.updateFilters({ fromDate: e.detail.from, toDate: e.detail.to }) })), h("div", { key: 'bf7d2f26123f4ec83ddcdb3edfbe77afc9f6f2c0', class: "filters-bar__search-group" }, h("div", { key: '2b7816235ffc5d2470e168a6b52047b2d371ec40', class: "filters-bar__type-group" }, h("wa-select", { key: '0aac54155e02207734e2314e112691a219d9ef75', class: "filters-bar__status-select", value: this.filters.type, defaultValue: this.filters.type, onchange: e => this.updateFilters({ type: e.target.value }), size: "s", placeholder: "Document Type" }, this.typeOptions.map(option => (h("wa-option", { value: option.value, key: option.value }, option.label)))), h("wa-select", { key: '577364d9567ec961250337ceca532ac65f04e865', class: "filters-bar__status-select", value: this.filters.folioType, defaultValue: this.filters.folioType, onchange: e => this.handleFolioTypeChange(e.target.value), size: "s", placeholder: "Folios" }, this.folioOptions.map(option => (h("wa-option", { value: option.value, key: option.value }, option.label)))), h("wa-switch", { key: '2a4bd4f3c38b6f2946745c7327aaa75ed6e2d8f4', class: "filters-bar__tax-switch", checked: this.filters.taxableOnly, onchange: e => this.updateFilters({ taxableOnly: e.target.checked }) }, "Taxes")), this.filters.folioType === 'agent' && (h("ir-autocomplete", { key: 'ea9246e6b67dee75dfc8883510925c06754548a1', class: "filters-bar__folio-select", size: "s", placeholder: "Select agent", value: this.filters.agentId ? (this.agents.find(a => a.id === this.filters.agentId)?.name ?? '') : ALL_AGENTS_LABEL, "onText-change": (e) => {
                this.agentSearch = e.detail ?? '';
            }, "onCombobox-change": (e) => {
                this.agentSearch = '';
                this.updateFilters({ agentId: e.detail && e.detail !== ALL_AGENTS_VALUE ? Number(e.detail) : null });
            } }, h("ir-autocomplete-option", { key: '7e1bd0f094b464d4e7acf3865d4ad9ab2034bb54', label: ALL_AGENTS_LABEL, value: ALL_AGENTS_VALUE }, ALL_AGENTS_LABEL), this.filteredAgents.map(agent => (h("ir-autocomplete-option", { key: agent.id, label: agent.name, value: String(agent.id) }, agent.name))))), this.filters.folioType === 'guest' && (h("ir-picker", { key: '2a55b2453340c7d9e7ab226083d1d0272e3b1b0b', class: "filters-bar__folio-select", size: "s", placeholder: "Search customer by email or name", withClear: true, mode: "select-async", debounce: 500, loading: isRequestPending('/Fetch_Exposed_Guests'), "onText-change": event => this.fetchGuests(event.detail), "onCombobox-select": this.handleGuestSelect.bind(this), "onCombobox-clear": () => this.updateFilters({ guestId: null }) }, this.guests?.map(guest => {
            const label = `${guest.email} - ${guest.first_name} ${guest.last_name}`;
            return (h("ir-picker-item", { label: label, value: guest.id?.toString(), key: guest.id }, label));
        }))), h("div", { key: '1d59ec0532d69b0cd4aa78a940ab2b7c0adc9805', class: "filters-bar__search-actions" }, h("ir-input", { key: '160b721a4db1be0590e23f7dc70d3693d61770cb', class: "filters-bar__search-input", placeholder: this.searchPlaceholder, value: this.docNumber, "onText-change": e => {
                this.docNumber = e.detail;
                this.emitSearchDebounced(e.detail);
            }, withClear: true }, h("wa-icon", { key: 'd616f7675fff98dc0d6ce6f7152f9dec9e02278c', name: "magnifying-glass", slot: "start", class: "filters-bar__search-icon" })), h("ir-custom-button", { key: 'c5a714f0961ba6b11b0398a70c8cf88728b78d8c', class: "filters-bar__search-submit", variant: "neutral", appearance: "outlined", type: "submit" }, h("wa-icon", { key: '305b3bc8f8fab87f4e7d3fa1a6d3f9276bbb7088', name: "magnifying-glass" })))))));
    }
