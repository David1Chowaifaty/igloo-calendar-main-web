...
import { o as getEntryValue, i as formatAmount } from './utils-B2NKY4In.js';
...
const IrFiscalDocumentsFilters = class {
    ...
    render() {
        return (h("form", { key: 'c1f626756657a805599f856ef7c381827f2a8987', onSubmit: e => {
                e.preventDefault();
                const submitter = e.submitter;
                this.applyFilters.emit({ ...this.draft, export: submitter?.value === 'export' });
            } }, h("div", { key: '83657c2815515c30539f414de76925381a438adc', class: "filters-bar" }, h("ir-validator", { key: '9a14561606463a293cee15e2e1075fd64ad48d0f', value: this.draft?.fromDate || this.draft?.toDate, schema: libExports.z.string().nonempty(), class: "filters-bar__dates" }, h("ir-date-range-filter", { key: '19ff361cb6ac4555367e51265b3f93d74d486fdd', maxDate: today.format('YYYY-MM-DD'), class: "filters-bar__date_picker", fromDate: this.draft.fromDate, toDate: this.draft.toDate, onDatesChanged: e => {
            ...
            } })), h("div", { key: '9ea8b3e30abf6fe5d1b9af9127efea4eb271058e', class: "filters-bar__search-group" }, h("div", { key: 'e16f1d09b8a140c80cf16c629370edb36ee41eb0', class: "filters-bar__type-group" }, h("wa-select", { key: 'f7985225190e4a8d0ae303ba2dc36265e5ba278f', class: "filters-bar__status-select", value: this.draft.type, defaultValue: this.draft.type, onchange: e => this.updateDraft({ type: e.target.value }), size: "s", placeholder: "Document Type" }, this.typeOptions.map(option => (h("wa-option", { value: option.value, key: option.value }, option.label)))), h("wa-select", { key: 'bbf1b7bdeae0f428caba5f3454e838573cb709eb', class: "filters-bar__status-select", value: this.draft.folioType, defaultValue: this.draft.folioType, onchange: e => this.handleFolioTypeChange(e.target.value), size: "s", placeholder: "Folios" }, this.folioOptions.map(option => (h("wa-option", { value: option.value, key: option.value }, option.label))))), this.draft.folioType === 'agent' && (h("ir-autocomplete", { key: '4c857a5edc9110d834fb060eb09436e3bc87c466', class: "filters-bar__folio-select", size: "s", placeholder: "Select agent", withExpandIcon: true, value: this.draft.agentId ? (this.agents.find(a => a.id === this.draft.agentId)?.name ?? '') : ALL_AGENTS_LABEL, "onText-change": (e) => {
                this.agentSearch = e.detail ?? '';
            }, "onCombobox-change": (e) => {
                this.agentSearch = '';
                const value = e.detail;
                this.updateDraft({ agentId: value && value !== ALL_AGENTS_VALUE ? Number(value) : null });
            } }, h("ir-autocomplete-option", { key: '90419283aacc474c4782b762ef64b600d35862ce', label: ALL_AGENTS_LABEL, value: ALL_AGENTS_VALUE }, ALL_AGENTS_LABEL), this.filteredAgents.map(agent => (h("ir-autocomplete-option", { key: agent.id, label: agent.name, value: String(agent.id) }, agent.name))))), this.draft.folioType === 'guest' && (h("ir-picker", { key: '1513a025b335d0fb115451511c73bd55b25ad64e', class: "filters-bar__folio-select", size: "s", placeholder: "Customer email or name", withClear: true, mode: "select-async", debounce: 500, loading: isRequestPending('/Fetch_Exposed_Guests'), "onText-change": event => this.fetchGuests(event.detail), "onCombobox-select": this.handleGuestSelect.bind(this), "onCombobox-clear": () => {
                this.updateDraft({ guestId: null });
                this.applyFilters.emit(this.draft);
            } }, this.guests?.map(guest => {
            const label = `${guest.email} - ${guest.first_name} ${guest.last_name}`;
            return (h("ir-picker-item", { label: label, value: guest.id?.toString(), key: guest.id }, label));
        }))), h("div", { key: 'c2245bafd75a5569fd77e13f4a82807114d1615c', class: `filters-bar__search-actions${this.draft.folioType === 'guest' ? ' filters-bar__search-actions--wide' : ''}` }, h("div", { key: 'aa69d763de6f1ae2d6752b8c61316fd25acbe515', class: "filters-bar__search-combo" }, h("ir-input", { key: '7eff0d36ada02cec7fd94c1f3a917a55f0a480a8', class: `filters-bar__search-input${this.draft.folioType === 'guest' ? ' filters-bar__combo-input' : ''}`, placeholder: this.searchPlaceholder, value: this.draft.docNumber, "onText-change": e => {
            ...
            } }, h("wa-icon", { key: 'bd974f22fe90f2d10bf40cd39ae8071803b3b3bb', name: "magnifying-glass", slot: "start", class: "filters-bar__search-icon" })), this.draft.folioType === 'guest' && (h("wa-select", { key: 'cf8f3eb17a449d80dca246d00b7456619f69daaa', class: "filters-bar__combo-select", size: "s", value: this.draft.searchBy, defaultValue: this.draft.searchBy, onchange: e => this.updateDraft({ searchBy: e.target.value }) }, h("wa-option", { key: '73c78090f0a6bf2ebed5bec497dd604db2da437c', value: "doc_nbr" }, "Document number"), h("wa-option", { key: '5599d8aae8aeff710f63a043851438ffb3757c69', value: "booking_nbr" }, "Booking number")))), h("wa-tooltip", { key: 'd42fdfeb0d4a1206172799f59d69ec6edbb5abd0', for: "search-btn" }, "Search"), h("ir-custom-button", { key: 'fccbcb02828ff2bc9a6b6077ca8545d3a48422cb', id: "search-btn", loading: this.loading === 'search', class: "filters-bar__search-submit", value: "search", variant: "neutral", appearance: "outlined", type: "submit" }, h("wa-icon", { key: 'a58a6854a0c4566ebf2d5b5bd0ac4fdc24f8d7a1', name: "magnifying-glass" })), h("wa-tooltip", { key: '654c5d319a176e9bf26ea35bd546d903743ac1b0', for: "excel-btn" }, 'Export to excel'), h("ir-custom-button", { key: '2bb97beaef6c4d719118d1ae74ed62776536e7b0', disabled: !(this.draft?.fromDate || this.draft?.toDate), id: "excel-btn", variant: "neutral", loading: this.loading === 'export', appearance: "outlined", type: "submit", value: "export" }, h("wa-icon", { key: '665bd0ba6367ec9affb57cbee12cb8ac60aa2892', name: "file-excel", variant: "regular" })))))));
    }
        ...
        const base = [
            this.columnHelper.accessor('DOC_DATE', {
                ...
                cell: info => {
                    ...
                    const date = hooks(info.getValue(), 'YYYY-MM-DD').format('MMM DD, YYYY');
