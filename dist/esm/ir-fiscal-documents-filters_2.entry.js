...
import { n as getEntryValue, f as formatAmount } from './utils-xLaRr6Y5.js';
...
const IrFiscalDocumentsFilters = class {
    ...
    render() {
        return (h("form", { key: 'b98155b702f688cc8570536e5523e365b0cd56e5', onSubmit: e => {
                e.preventDefault();
                const submitter = e.submitter;
                this.applyFilters.emit({ ...this.draft, export: submitter?.value === 'export' });
            } }, h("div", { key: 'af1007b3819edba74d3137b5abb91228b741f6fa', class: "filters-bar" }, h("ir-validator", { key: 'dad75f58949967f5f80b0237c1b5248932772515', value: this.draft?.fromDate || this.draft?.toDate, schema: libExports.z.string().nonempty(), class: "filters-bar__dates" }, h("ir-date-range-filter", { key: 'a93dbe6d6c5a3116118ac3f8b629e135d5229f9b', maxDate: today.format('YYYY-MM-DD'), class: "filters-bar__date_picker", fromDate: this.draft.fromDate, toDate: this.draft.toDate, onDatesChanged: e => {
            ...
            } })), h("div", { key: '1c26560aea0647dd8a6836c1954516f64b1ce312', class: "filters-bar__search-group" }, h("div", { key: '04c206b06c1fd422087473ddbe35540ab0c5e9ee', class: "filters-bar__type-group" }, h("wa-select", { key: '9825988aa3d4fe8d9b9e87370d54ed3ed26d8d23', class: "filters-bar__status-select", value: this.draft.type, defaultValue: this.draft.type, onchange: e => this.updateDraft({ type: e.target.value }), size: "s", placeholder: "Document Type" }, this.typeOptions.map(option => (h("wa-option", { value: option.value, key: option.value }, option.label)))), h("wa-select", { key: '37d533d4647a4258b93f551ed3b3d05cf12a1904', class: "filters-bar__status-select", value: this.draft.folioType, defaultValue: this.draft.folioType, onchange: e => this.handleFolioTypeChange(e.target.value), size: "s", placeholder: "Folios" }, this.folioOptions.map(option => (h("wa-option", { value: option.value, key: option.value }, option.label))))), this.draft.folioType === 'agent' && (h("ir-autocomplete", { key: 'df171edbdfafffd40a46cb6471d30eca9e052f8e', class: "filters-bar__folio-select", size: "s", placeholder: "Select agent", withExpandIcon: true, value: this.draft.agentId ? (this.agents.find(a => a.id === this.draft.agentId)?.name ?? '') : ALL_AGENTS_LABEL, "onText-change": (e) => {
                this.agentSearch = e.detail ?? '';
            }, "onCombobox-change": (e) => {
                this.agentSearch = '';
                const value = e.detail;
                this.updateDraft({ agentId: value && value !== ALL_AGENTS_VALUE ? Number(value) : null });
            } }, h("ir-autocomplete-option", { key: 'fa50026d99b80e2b8d14d6d54dc21bc3d3e7b925', label: ALL_AGENTS_LABEL, value: ALL_AGENTS_VALUE }, ALL_AGENTS_LABEL), this.filteredAgents.map(agent => (h("ir-autocomplete-option", { key: agent.id, label: agent.name, value: String(agent.id) }, agent.name))))), this.draft.folioType === 'guest' && (h("ir-picker", { key: '18e69ce65420698789c086c340eb8ec9357f1f67', class: "filters-bar__folio-select", size: "s", placeholder: "Customer email or name", withClear: true, mode: "select-async", debounce: 500, loading: isRequestPending('/Fetch_Exposed_Guests'), "onText-change": event => this.fetchGuests(event.detail), "onCombobox-select": this.handleGuestSelect.bind(this), "onCombobox-clear": () => {
                this.updateDraft({ guestId: null });
                this.applyFilters.emit(this.draft);
            } }, this.guests?.map(guest => {
            const label = `${guest.email} - ${guest.first_name} ${guest.last_name}`;
            return (h("ir-picker-item", { label: label, value: guest.id?.toString(), key: guest.id }, label));
        }))), h("div", { key: '54463d1455aaa2cda990c1a78ad8deb60cc20686', class: `filters-bar__search-actions${this.draft.folioType === 'guest' ? ' filters-bar__search-actions--wide' : ''}` }, h("div", { key: '218108ad133a237ea31d98867621adcf0f8e4713', class: "filters-bar__search-combo" }, h("ir-input", { key: 'cd32c606d03e09037e4e256e8f3ea2d0bd384041', class: `filters-bar__search-input${this.draft.folioType === 'guest' ? ' filters-bar__combo-input' : ''}`, placeholder: this.searchPlaceholder, value: this.draft.docNumber, "onText-change": e => {
            ...
            } }, h("wa-icon", { key: '43c9f8b8806418252fd9bd821084134fe98fc22f', name: "magnifying-glass", slot: "start", class: "filters-bar__search-icon" })), this.draft.folioType === 'guest' && (h("wa-select", { key: 'd0de8349d06c0b533d6d7314798e32084e027c24', class: "filters-bar__combo-select", size: "s", value: this.draft.searchBy, defaultValue: this.draft.searchBy, onchange: e => this.updateDraft({ searchBy: e.target.value }) }, h("wa-option", { key: 'da791b54e27c332e4998a49ce14c0c539badd970', value: "doc_nbr" }, "Document number"), h("wa-option", { key: '174a3c04dacfe71f592b0411a9d34a7abdcbf2e7', value: "booking_nbr" }, "Booking number")))), h("wa-tooltip", { key: '1e36195591944fb649c537742e7d20c1840d8329', for: "search-btn" }, "Search"), h("ir-custom-button", { key: 'cbf0321e501b35356cbb92acec48dccc801d51ed', id: "search-btn", loading: this.loading === 'search', class: "filters-bar__search-submit", value: "search", variant: "neutral", appearance: "outlined", type: "submit" }, h("wa-icon", { key: '3253fa9ffa1cd87e238af729126ced856958344a', name: "magnifying-glass" })), h("wa-tooltip", { key: '43f44c9bc4995dfd4c30eadf8dd7cad391803fb1', for: "excel-btn" }, 'Export to excel'), h("ir-custom-button", { key: 'f8b85bca5a9ab22cce1bbe30f09970501300e649', disabled: !(this.draft?.fromDate || this.draft?.toDate), id: "excel-btn", variant: "neutral", loading: this.loading === 'export', appearance: "outlined", type: "submit", value: "export" }, h("wa-icon", { key: '4c323757128822bb7ad8729710d5db01ee031311', name: "file-excel", variant: "regular" })))))));
    }
