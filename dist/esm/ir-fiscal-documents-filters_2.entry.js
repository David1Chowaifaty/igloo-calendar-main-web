...
import { j as getEntryValue, f as formatAmount } from './utils-D7g9MYlv.js';
...
const IrFiscalDocumentsFilters = class {
    ...
    render() {
        return (h("form", { key: '77ca13e38471f8e860d098e6e8586b4d55946ada', onSubmit: e => {
                e.preventDefault();
                const submitter = e.submitter;
                this.applyFilters.emit({ ...this.draft, export: submitter?.value === 'export' });
            } }, h("div", { key: 'c002b2f9dad5e9927f999df460fb256f18cd0603', class: "filters-bar" }, h("ir-validator", { key: 'c77e93a993150fe53beab088ffdc7657f15b8327', value: this.draft?.fromDate || this.draft?.toDate, schema: libExports.z.string().nonempty(), class: "filters-bar__dates" }, h("ir-date-range-filter", { key: '5417c155cbc48c666ce0473b708f5219c36e0040', maxDate: today.format('YYYY-MM-DD'), class: "filters-bar__date_picker", fromDate: this.draft.fromDate, toDate: this.draft.toDate, onDatesChanged: e => {
            ...
            } })), h("div", { key: 'fcd1f40e7a91a90d00261e7fbbb4b7b69b800468', class: "filters-bar__search-group" }, h("div", { key: '0258baa17641c0edc8ea55663c5b7451e1aac436', class: "filters-bar__type-group" }, h("wa-select", { key: 'ed959e9de614d001f906f95f9df4aae49ec64d78', class: "filters-bar__status-select", value: this.draft.type, defaultValue: this.draft.type, onchange: e => this.updateDraft({ type: e.target.value }), size: "s", placeholder: "Document Type" }, this.typeOptions.map(option => (h("wa-option", { value: option.value, key: option.value }, option.label)))), h("wa-select", { key: '641955c3c0168d04edd5d177053f7907ef45286a', class: "filters-bar__status-select", value: this.draft.folioType, defaultValue: this.draft.folioType, onchange: e => this.handleFolioTypeChange(e.target.value), size: "s", placeholder: "Folios" }, this.folioOptions.map(option => (h("wa-option", { value: option.value, key: option.value }, option.label))))), this.draft.folioType === 'agent' && (h("ir-autocomplete", { key: '32b8b11068a8c7d381c0d218f91cf5ebf870ee2a', class: "filters-bar__folio-select", size: "s", placeholder: "Select agent", withExpandIcon: true, value: this.draft.agentId ? (this.agents.find(a => a.id === this.draft.agentId)?.name ?? '') : ALL_AGENTS_LABEL, "onText-change": (e) => {
                this.agentSearch = e.detail ?? '';
            }, "onCombobox-change": (e) => {
                this.agentSearch = '';
                const value = e.detail;
                this.updateDraft({ agentId: value && value !== ALL_AGENTS_VALUE ? Number(value) : null });
            } }, h("ir-autocomplete-option", { key: 'ceffca5473a72884471ed8921b7712ea445c7a89', label: ALL_AGENTS_LABEL, value: ALL_AGENTS_VALUE }, ALL_AGENTS_LABEL), this.filteredAgents.map(agent => (h("ir-autocomplete-option", { key: agent.id, label: agent.name, value: String(agent.id) }, agent.name))))), this.draft.folioType === 'guest' && (h("ir-picker", { key: 'b1778574b1a88c6395158b1476937af76dff474a', class: "filters-bar__folio-select", size: "s", placeholder: "Customer email or name", withClear: true, mode: "select-async", debounce: 500, loading: isRequestPending('/Fetch_Exposed_Guests'), "onText-change": event => this.fetchGuests(event.detail), "onCombobox-select": this.handleGuestSelect.bind(this), "onCombobox-clear": () => {
                this.updateDraft({ guestId: null });
                this.applyFilters.emit(this.draft);
            } }, this.guests?.map(guest => {
            const label = `${guest.email} - ${guest.first_name} ${guest.last_name}`;
            return (h("ir-picker-item", { label: label, value: guest.id?.toString(), key: guest.id }, label));
        }))), h("div", { key: '4fda5b3c864b38bf3579cf6da19359065d411710', class: `filters-bar__search-actions${this.draft.folioType === 'guest' ? ' filters-bar__search-actions--wide' : ''}` }, h("div", { key: 'cd52bf3d7a345086eb80f13e20b1ad63348252b7', class: "filters-bar__search-combo" }, h("ir-input", { key: 'd0dc95198a9dba42ff980d18cad40b40a996b218', class: `filters-bar__search-input${this.draft.folioType === 'guest' ? ' filters-bar__combo-input' : ''}`, placeholder: this.searchPlaceholder, value: this.draft.docNumber, "onText-change": e => {
            ...
            } }, h("wa-icon", { key: 'fe990caf5178db69325796684993cbab2bfb3011', name: "magnifying-glass", slot: "start", class: "filters-bar__search-icon" })), this.draft.folioType === 'guest' && (h("wa-select", { key: '438a04318570c414aab3c717e872f8aa193a21dd', class: "filters-bar__combo-select", size: "s", value: this.draft.searchBy, defaultValue: this.draft.searchBy, onchange: e => this.updateDraft({ searchBy: e.target.value }) }, h("wa-option", { key: '97d0527590b7f9a7431359e48a2cf4eab48b4751', value: "doc_nbr" }, "Document number"), h("wa-option", { key: 'd5937e7d0feb3d54c1fe1f9cd8eeb518d6a44575', value: "booking_nbr" }, "Booking number")))), h("wa-tooltip", { key: 'fc5888a588cf54e913dd749ebde031eddc55e1f7', for: "search-btn" }, "Search"), h("ir-custom-button", { key: 'dac92d839bea2959437a9c835161fbe5f265a77a', id: "search-btn", loading: this.loading === 'search', class: "filters-bar__search-submit", value: "search", variant: "neutral", appearance: "outlined", type: "submit" }, h("wa-icon", { key: '49f7812cc2e7e087231241676a3320f4dddf8914', name: "magnifying-glass" })), h("wa-tooltip", { key: '0e12c871cd56dc3a7b698b3afecee045a72eb06d', for: "excel-btn" }, 'Export to excel'), h("ir-custom-button", { key: '70eec103a8c4898a5afd06836688c43c84da76fe', disabled: !(this.draft?.fromDate || this.draft?.toDate), id: "excel-btn", variant: "neutral", loading: this.loading === 'export', appearance: "outlined", type: "submit", value: "export" }, h("wa-icon", { key: 'b5762d34957437ac95422670fdb986050ac810b6', name: "file-excel", variant: "regular" })))))));
    }
