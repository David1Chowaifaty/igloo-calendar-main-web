...
import { B as BookingService, r as resetBookingStore } from './booking.service-BVIex3m4.js';
...
import { f as checkMealPlan } from './utils-DE70XlzV.js';
...
const IglSpiltBookingForm = class {
    ...
    render() {
        return (h("form", { key: '04ae910e8f3e32068e928f92103bd5b51b9208c3', id: "split-booking-form", onSubmit: e => {
                e.preventDefault();
                this.doReservation();
            }, class: "sheet-container" }, h("div", { key: 'ad9c75f482e19fa5bbac3ae0a5343570d52f022f', class: "split-header" }, h("div", { key: '6d9d4781ddc1b5a091ad0ecfc98103e8bf43562e', class: "split-header__summary" }, h("ir-date-view", { key: '889db03f0bdce3c8d99ffacbb9089adb640aeb11', from_date: this.room.from_date, to_date: this.room.to_date, showDateDifference: false }), h("div", { key: '20767bcf221ca0ecff11a1879887f23036239d1d', class: "split-header__tags" }, h("wa-tag", { key: '6edfd5439282908641f4cc86f4ed09f9b3f67b3c', size: "s", variant: "neutral" }, this.room.rateplan.short_name), this.room.rateplan.is_non_refundable && (h("wa-tag", { key: 'd10651d06dc9bd8acf01a564dc2bd7ffee0262d0', size: "s", variant: "warning" }, locales.entries.Lcz_NonRefundable)))), h("div", { key: 'c21aacc3ebdb31a361b5b9f244118cb9ce8cb036', class: "split-header__controls" }, h("label", { key: 'd9615cdcf7bd7d848f92fd3129c2d1e7620d7e89', class: "split-header__label", htmlFor: "split-from-date" }, "Split from"), h("ir-date-picker", { key: '329f909388af4bde9dbf8b583aa1accfa4a87cfa', class: "split-header__date", "data-testid": "pickup_arrival_date", date: this.selectedDates?.from_date?.format('YYYY-MM-DD'), maxDate: this.defaultDates?.to_date.format('YYYY-MM-DD'), minDate: this.defaultDates?.from_date.format('YYYY-MM-DD'), emitEmptyDate: true, onDateChanged: evt => {
                this.selectedDates = {
                    ...this.selectedDates,
                    from_date: evt.detail.start,
                };
            } }, h("wa-input", { key: '994470afc8dec486300580a5bd9afe161e6ee551', id: "split-from-date", slot: "trigger", size: "s", readonly: true, class: "date-trigger", value: this.selectedDates.from_date ? this.selectedDates.from_date.format('MMM DD, YYYY') : null }, h("wa-icon", { key: '3b902abc285732bdf0641c0848dadb460137a5c5', slot: "start", name: "calendar" }))), h("wa-button", { key: '66af6bfd667da2ad7c7a9a1f9d98d5d0262c64be', class: "split-header__check", size: "s", variant: "brand", loading: isRequestPending('/Check_Availability'), onClick: () => this.checkBookingAvailability() }, h("wa-icon", { key: '32bfce0d21ed4dcb594ff04637b19a3daa7e6a49', slot: "start", name: "magnifying-glass" }), "Check"))), this.errors?.roomtype_id && h("p", { key: '56e5e91089555489b487b9f853913f7947a6b664', class: "error-message" }, "Please select a room"), h("wa-radio-group", { key: 'ae7260ce88c5e4bdbb2c081a76ffe51373fa5ccc', class: "room-type-list", name: "unit", onchange: e => {
