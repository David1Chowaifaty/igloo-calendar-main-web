...
import { k as extras, u as convertDateToTime, v as convertDateToCustomFormat, t as dateToFormattedString } from './utils-1xMAwYd7.js';
import { g as getMyBookings, c as calculateDaysBetweenDates } from './booking-CB1XnRO0.js';
import { c as createStore } from './index-BJS0kaeV.js';
import { h as hooks } from './moment-Mki5YqAR.js';
import { c as calendar_data } from './calendar-data-Cd3WjpQE.js';
import { l as locales } from './locales.store-C-PbJt6i.js';
import { T as TaxTypesSchema, P as PropertyIdSchema } from './commonSchemas-ByEkDTMV.js';

const ZIEntrySchema = libExports.z.object({
...
const NumberOrStringSchema = libExports.union([libExports.number(), libExports.string().optional()]);
const CurrencySchema$1 = libExports.object({
    id: libExports.number(),
});
const CurrencyWithCodeSchema = CurrencySchema$1.extend({
    code: libExports.string().optional(),
});
const ItemSchema$1 = libExports.object({
    amount: libExports.number(),
    type: libExports.string().optional(),
    key: libExports.union([libExports.number(), libExports.string().optional()]),
    description: libExports.string().optional().optional().default(''),
});
const TargetSchema = libExports.object({
    code: libExports.string().optional(),
    description: libExports.string().optional(),
});
libExports.object({
    unit_id: libExports.number(),
    from_date: libExports.string().optional(),
    to_date: libExports.string().optional(),
});
libExports.object({
    starter: libExports.string().optional(),
});
libExports.object({
    booking_nbr: libExports.string().optional(),
    currency_id: libExports.number(),
    language: libExports.string().optional().optional(),
    rate_plan_id: libExports.number(),
    room_type_id: libExports.number(),
    property_id: libExports.number(),
    is_preserve_history: libExports.boolean().optional(),
    room_identifier: libExports.string().optional().optional(),
});
libExports.object({
    booking_nbr: libExports.string().optional(),
    room_identifier: libExports.string().optional(),
    status: libExports.string().optional(),
});
libExports.object({
    booking_nbr: libExports.string().optional(),
    currency_id: libExports.number(),
    language: libExports.string().optional(),
});
const RestrictionSchema = libExports.object({
    room_type_id: NumberOrStringSchema,
    night: libExports.string().optional(),
});
libExports.object({
    is_closed: libExports.boolean(),
    restrictions: libExports.array(RestrictionSchema),
    operation_type: libExports.string().optional().optional(),
});
libExports.object({
    book_nbr: libExports.string().optional(),
    status: libExports.string().optional(),
});
const AdultChildCountSchema = libExports.object({
    adult: libExports.number(),
    child: libExports.number(),
});
libExports.object({
    from_date: libExports.string().optional(),
    to_date: libExports.string().optional(),
    propertyid: libExports.number(),
    adultChildCount: AdultChildCountSchema,
    language: libExports.string().optional(),
    room_type_ids: libExports.array(libExports.number()),
    room_type_ids_to_update: libExports.array(libExports.number()).optional(),
    rate_plan_ids: libExports.array(libExports.number()).optional(),
    currency: CurrencyWithCodeSchema,
    is_in_agent_mode: libExports.boolean().optional(),
    agent_id: NumberOrStringSchema.optional(),
});
const AvailabilityBracketSchema = libExports.object({
    from_date: libExports.string().optional(),
    to_date: libExports.string().optional(),
});
libExports.object({
    unit_id: libExports.number(),
    block_status_code: libExports.enum(['003', '004', '002']).optional(),
    description: libExports.string().optional().optional(),
    property_id: libExports.number(),
    brackets: libExports.array(AvailabilityBracketSchema),
});
const SetDepartureTimePropsSchema = libExports.object({
    property_id: libExports.number(),
    room_identifier: libExports.string().optional(),
    code: libExports.string().optional(),
});
libExports.object({
    service: libExports.custom(),
    booking_nbr: NumberOrStringSchema,
    is_remove: libExports.boolean(),
});
/*Arrivals */
const GetRoomsToCheckInPropsSchema = libExports.object({
    property_id: libExports.string(),
    check_in_date: libExports.string(),
    page_index: libExports.number().default(1),
    page_size: libExports.number().default(10),
});
/*Departures */
const GetRoomsToCheckOutPropsSchema = libExports.object({
    property_id: libExports.string(),
    check_out_date: libExports.string(),
    page_index: libExports.number().default(1),
    page_size: libExports.number().default(10),
});
/* INVOICE TYPES */
const GetBookingInvoiceInfoPropsSchema = libExports.object({
    booking_nbr: libExports.string().optional(),
});
const VoidInvoicePropsSchema = libExports.object({
    invoice_nbr: libExports.string().optional(),
    reason: libExports.string().optional(),
    property_id: libExports.number(),
});
const InvoiceSchema$1 = libExports.object({
    booking_nbr: libExports.string().optional(),
    currency: CurrencySchema$1,
    target: TargetSchema,
    Date: libExports.string().optional(),
    nbr: libExports.string().optional(),
    remark: libExports.string().optional(),
    billed_to_name: libExports.string().optional(),
    billed_to_tax: libExports.string().optional(),
    items: libExports.array(ItemSchema$1),
});
const IssueInvoicePropsSchema = libExports.object({
    is_proforma: libExports.boolean().optional().default(false),
    property_id: libExports.number(),
    invoice: InvoiceSchema$1,
});
const PrintInvoicePropsSchema = libExports.object({
    invoice_nbr: libExports.string().optional(),
    property_id: libExports.number(),
    mode: libExports.enum(['invoice', 'creditnote', 'proforma']),
    invoice: InvoiceSchema$1.optional(),
});
const ExposedGuestSchema = libExports.object({
    address: libExports.null(),
    alternative_email: libExports.null(),
    cci: libExports.null(),
    city: libExports.null(),
    country: libExports.null(),
    country_id: libExports.number(),
    country_phone_prefix: libExports.string(),
    dob: libExports.null(),
    email: libExports.string(),
    first_name: libExports.string(),
    id: libExports.number(),
    id_info: libExports.null(),
    is_main: libExports.boolean(),
    last_name: libExports.string(),
    mobile: libExports.string(),
    mobile_without_prefix: libExports.string(),
    nbr_confirmed_bookings: libExports.number(),
    notes: libExports.null(),
    password: libExports.null(),
    subscribe_to_news_letter: libExports.null(),
});
libExports.array(ExposedGuestSchema);
const SetHbPreferencePropsSchema = libExports.object({
    property_id: libExports.number(),
    room_identifier: libExports.string(),
    code: libExports.union([libExports.literal('001'), libExports.literal('002')]),
});
const CalculateExclusiveTaxPropsSchema = libExports.object({
    property_id: libExports.number().min(1),
    amount: libExports.number(),
    taxes_to_include: TaxTypesSchema,
});
const AckExposedRevisionPropsSchema = libExports.object({
    revision_id: libExports.number(),
});
const VoidPaymentPropsSchema = libExports.object({
    receipt_nbr: libExports.string(),
    booking_nbr: libExports.string(),
});
const CalculateOptimBaseGrossAmountParamsSchema = libExports.object({
    booking_nbr: libExports.string(),
});
const SimulateDirectBookingParamsSchema = libExports.object({
    booking_nbr: libExports.string(),
});
const DoDayUseParamsSchema = libExports.object({
    language: libExports.string().min(1),
    is_to_block: libExports.boolean().optional().default(false),
    booking: libExports.object({
        property: libExports.object({
            id: libExports.number(),
        }),
        occupancy: libExports.object({
            adult_nbr: libExports.number().nullable(),
            children_nbr: libExports.number().nullable(),
            infant_nbr: libExports.number().nullable(),
        })
            .optional()
            .nullable(),
        currency: libExports.object({
            id: libExports.number(),
        }),
        source: libExports.object({
            code: libExports.string().min(1),
        }),
        guest: libExports.object({
            first_name: libExports.string().min(1),
            last_name: libExports.string().min(1),
            email: libExports.union([libExports.string().trim().email(), libExports.literal('')]),
            mobile: libExports.string(),
        }),
        from_date: libExports.string().min(1),
        to_date: libExports.string().min(1),
        status: libExports.object({
            code: libExports.string().min(1),
        }),
        remark: libExports.string(),
    }),
    extra_service: libExports.object({
        pr_id: libExports.number(),
        category: libExports.object({
            code: libExports.string().min(1),
        }),
        description: libExports.string().optional().default(''),
        start_date: libExports.string().min(1),
        end_date: libExports.string().min(1),
        from_time: libExports.string().min(1),
        to_time: libExports.string().min(1),
        net_amount: libExports.number(),
        tax_amount: libExports.number(),
        gross_amount: libExports.number(),
        currency_id: libExports.number(),
        price: libExports.number(),
    }),
});
const SetArrivalTimePropsSchema = libExports.object({
    property_id: PropertyIdSchema,
    room_identifier: libExports.string(),
    code: libExports.string(),
});

const CurrencySchema = libExports.object({
    code: libExports.string(),
    id: libExports.number(),
    symbol: libExports.string(),
});
const StatusSchema = libExports.object({
    code: libExports.string(),
    description: libExports.any(),
});
const ItemSchema = libExports.object({
    amount: libExports.number(),
    booking_nbr: libExports.string(),
    currency: CurrencySchema,
    description: libExports.any(),
    invoice_nbr: libExports.string(),
    is_invoiceable: libExports.boolean(),
    key: libExports.number(),
    status: StatusSchema,
    system_id: libExports.number(),
    type: libExports.string(),
});
const CreditNoteSchema = libExports.object({
    date: libExports.string(),
    nbr: libExports.string(),
    reason: libExports.string(),
    system_id: libExports.string().nullable(),
    user: libExports.string().nullable(),
});
const InvoiceSchema = libExports.object({
    billed_to_name: libExports.any(),
    billed_to_tax: libExports.any(),
    booking_nbr: libExports.string(),
    credit_note: CreditNoteSchema.nullable(),
    currency: CurrencySchema,
    date: libExports.string(),
    items: libExports.array(ItemSchema),
    nbr: libExports.string(),
    pdf_url: libExports.any(),
    remark: libExports.string(),
    status: StatusSchema,
    system_id: libExports.number(),
    target: libExports.any(),
    user: libExports.string().nullable(),
    total_amount: libExports.any(),
});
const InvoiceableItemReasonSchema = libExports.object({
    code: libExports.enum(['001', '002', '003']),
    description: libExports.string().nullable(),
});
const InvoiceableItemSchema = libExports.object({
    amount: libExports.number(),
    booking_nbr: libExports.string(),
    currency: CurrencySchema,
    invoice_nbr: libExports.string().nullable(),
    is_invoiceable: libExports.boolean(),
    key: libExports.number(),
    status: libExports.any(),
    system_id: libExports.any(),
    reason: InvoiceableItemReasonSchema.nullable(),
    type: libExports.enum(['BSA', 'BSP', 'BSE', 'PAYMENT']),
});
const BookingInvoiceInfoSchema = libExports.object({
    invoiceable_items: libExports.array(InvoiceableItemSchema),
    invoices: libExports.array(InvoiceSchema).nullable(),
});
...
export { BookingService as B, VariationService as V, ZIEntrySchema as Z, resetReserved as a, booking_store as b, calculateTotalRooms as c, setBookingSelectOptions as d, reserveRooms as e, setBookedByGuestManualEditState as f, updateBookedByGuest as g, updateRoomParams as h, getVisibleInventory as i, setDayUseSelection as j, fillMissingReservedGuestNames as k, getReservedRooms as l, modifyBookingStore as m, resetAvailability as n, hasAtLeastOneRoomSelected as o, getBookingTotalPrice as p, syncFirstRoomGuestName as q, resetBookingStore as r, setBookingDraft as s, bookedByGuestBaseData as t, updateRoomGuest as u };
