...
import { R as ROOM_IN_OUT } from './booking.dto-xX-uaIxb.js';
...
function convertDateToCustomFormat(dayWithWeekday, monthWithYear, format = 'D_M_YYYY') {
    ...
    const date = hooks(dateStr, 'DD MMM YYYY', 'en');
    if (!date.isValid()) {
...
function convertDateToTime(dayWithWeekday, monthWithYear) {
    ...
    const date = hooks(dayWithWeekday + ' ' + monthWithYear, 'ddd DD MMM YYYY', 'en').toDate();
...
function validateEmail(email) {
    ...
    const parsedEmailResults = stringType().email().safeParse(email);
...
function canCheckIn({ from_date, to_date, isCheckedIn }) {
    if (!calendar_data.checkin_enabled || calendar_data.property.is_automatic_check_in_out || !calendar_data.property.is_frontdesk_enabled) {
        return false;
    }
    if (isCheckedIn) {
        return false;
    ...
    const now = hooks();
    if ((now.isSameOrAfter(hooks(from_date, 'YYYY-MM-DD'), 'date') && now.isBefore(hooks(to_date, 'YYYY-MM-DD'), 'date')) ||
        now.isSame(hooks(to_date, 'YYYY-MM-DD'), 'date')
...
export { getNextDay as A, addTwoMonthToDate as B, computeEndDate as C, toFloat as D, renderTime as E, canCheckout as a, checkUserAuthState as b, canCheckIn as c, showToast as d, dateDifference as e, downloadFile as f, isPrivilegedUser as g, extras as h, isBlockUnit as i, handleBodyOverflow as j, getDaysArray as k, convertDatePrice as l, manageAnchorSession as m, getReleaseHoursString as n, dateToFormattedString as o, convertDateToTime as p, convertDateToCustomFormat as q, getFormSubmitter as r, sleep as s, calculateTrend as t, checkMealPlan as u, validateEmail as v, findCountry as w, isWeekend as x, convertDMYToISO as y, formatLegendColors as z };
