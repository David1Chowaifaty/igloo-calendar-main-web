...
import { j as downloadFile } from './utils-1CCVam5W.js';
...
const IrTasksCard = class {
    ...
    render() {
        return (h("wa-card", { key: '40f01d9168a9470e82c9fd64ecb926a81cf941d3', class: "task-card" }, h("div", { key: '9a699f33a20f5fcc6ccb75ae3a3a09356295cb1d', class: "task-card__body" }, h("div", { key: 'ba49090b1aad8863fb0ec29eddeec57cd1fcac8d', class: "task-card__unit" }, h("span", { key: 'f50b6b63b2ec99705efa540c83bba3a0958eff71', class: "task-card__unit-name" }, this.task.unit.name), h("div", { key: 'd01c7de39e36fd903b31666b97ae82ad27d189d0', class: "task-card__meta" }, h("span", { key: 'ff5d075fb0b771320ac7ca15e04c4cbab55dd3b5', class: "task-card__status" }, this.task.status.description), this.task.hint && h("span", { key: '4fafc77db3abc1bf219343ee778ed5d8f870b0f3', class: "task-card__sep" }, "\u00B7"), this.task.hint && h("span", { key: 'f28c794a6554cc4fd860379777b5b9fe43bdf7a7', class: "task-card__hint" }, this.task.hint))), h("div", { key: '66eb5c6cbbd083fcd8808c2bf168527afe7819ac', class: "task-card__badges" }, this.taskTypeBadge(this.task.task_type?.code), this.task.extra_task?.map(et => this.taskTypeBadge(et.task_type?.code))), this.guests.length > 0 && (h("div", { key: '95849341360f6330274e4659a3c0d7af8b3e3b29', class: "task-card__guests" }, this.guests.map(g => (h("div", { class: "task-card__guest" }, h("wa-icon", { name: g.icon, class: "task-card__guest-icon", style: { fontSize: `${Math.min(0.75 + g.count * 0.15, 1.4)}rem` } }), h("span", { class: "task-card__guest-count" }, g.count)))))), h("div", { key: 'd5a9a498561805582acf5c678b11ec39f76d79a8', class: "task-card__assign" }, h("wa-select", { key: '752c19adc36fdf34ee45db463709c4905445b7b1', label: "Housekeeper", class: "task-card__hk-select", size: "s", placeholder: "Unassigned", value: this.task.hkm_id ? String(this.task.hkm_id) : '0', defaultValue: this.task.hkm_id ? String(this.task.hkm_id) : '0', onchange: e => {
                const hkm_id = Number(e.target.value);
                this.assignHousekeeper.emit({ task: this.task, hkm_id });
            } }, h("wa-option", { key: '50c5f69d6e1dac5f70b6b2433342503e97c29a3e', value: "0" }, locales.entries.Lcz_Unassigned), housekeeping_store.hk_criteria?.housekeepers
            .slice()
            .sort((a, b) => a.name.localeCompare(b.name))
            .map(hk => (h("wa-option", { key: hk.id, value: String(hk.id) }, hk.name))))), h("div", { key: 'ddbeab871fae511722598544e47d3a69c5961dd0', class: "task-card__actions" }, this.isSkippable && (h("ir-custom-button", { key: 'b9a73ec81ecb8207600eae113b411af61f3eb5d1', variant: "neutral", appearance: "outlined", onClickHandler: () => this.skipSelectedTask.emit(this.task) }, "Skip")), this.isCheckable && (h("div", { key: '7e90899014b97a49c0e692695cbe8aea0a6ff928', class: "task-card__clean-group" }, h("ir-custom-button", { key: 'b168d1472d091ff048be36e7daeab8bdf0bb19e3', variant: "brand", appearance: "filled", onClickHandler: () => {
                toggleTaskSelection(this.task);
                this.cleanSelectedTask.emit({ task: this.task, status: '004' });
            } }, "Clean & Inspect"), h("ir-custom-button", { key: 'a904860d1f0e64515fae94c4cc612cb2e488f569', variant: "brand", appearance: "accent", onClickHandler: () => {
                toggleTaskSelection(this.task);
...
const IrTasksFilters = class {
    ...
    render() {
        return (h("ir-filter-card", { key: '3488ebc0594fe3c3a38c560934684ceada14a6d9' }, h("fieldset", { key: '1e891abb478dd3932b3a6b9ce50c437d438ca556' }, h("wa-select", { key: '8153d3e8f0bd38c6d19c5ecce44790fd224c6f32', label: locales.entries.Lcz_Period, size: "s", "data-testid": "period", value: this.filters?.cleaning_periods?.code, defaultValue: this.filters?.cleaning_periods?.code, onchange: e => this.updateFilter({ cleaning_periods: { code: e.target.value } }) }, housekeeping_store?.hk_criteria?.cleaning_periods.map(v => (h("wa-option", { key: v.code, value: v.code }, v.description))))), housekeeping_store?.hk_criteria?.housekeepers.length > 1 && (h("fieldset", { key: 'acff02d06c31eef9fa16c302827f543e2cabf5af' }, h("wa-select", { key: 'd22cebcd863394869b0027586e8d2c11caaf67a4', label: locales.entries.Lcz_Housekeepers, size: "s", "data-testid": "housekeepers", value: this.filters?.housekeepers, defaultValue: this.filters?.housekeepers, onchange: e => this.updateFilter({ housekeepers: e.target.value }) }, h("wa-option", { key: '4dec9d46e44d5b742b7e2d2b3eda92d58c9e6286', value: "000" }, locales.entries.Lcz_Allhousekeepers), housekeeping_store?.hk_criteria?.housekeepers
            .slice()
            .sort((a, b) => a.name.toLowerCase().localeCompare(b.name.toLowerCase()))
            .map(v => (h("wa-option", { key: v.id, value: v.id.toString() }, v.name)))))), h("fieldset", { key: 'ae467074e12a7f22622185653658705104686432' }, h("wa-select", { key: '3eeb99898228975dd06609c575624de83c2f5280', label: "Include dusty units", size: "s", "data-testid": "dusty_units", value: this.filters?.dusty_units?.code, defaultValue: this.filters?.dusty_units?.code, onchange: e => this.updateFilter({ dusty_units: { code: e.target.value } }) }, housekeeping_store.hk_criteria?.dusty_periods?.map(v => (h("wa-option", { key: v.code, value: v.code }, v.description))))), h("fieldset", { key: '192d10a5c00f73d5ff3829ba0a02eddc536acfc4' }, h("wa-select", { key: 'ce93ec5b3230855aec335b19a26bc3ce9927f19f', label: locales.entries['Lcz_HighlightCheck-insFrom'], size: "s", "data-testid": "highlight_check_ins", value: this.filters?.highlight_check_ins?.code, defaultValue: this.filters?.highlight_check_ins?.code, onchange: e => this.updateFilter({ highlight_check_ins: { code: e.target.value } }) }, housekeeping_store.hk_criteria?.highlight_checkin_options?.map(v => (h("wa-option", { key: v.code, value: v.code }, v.description))))), h("div", { key: '15b1b8294f0c02c871b2f72df703767a55b7483b', slot: "footer" }, h("ir-custom-button", { key: '6023f6c370389cad87a1fdd66456f7027aecf0e1', variant: "neutral", appearance: "filled", "data-testid": "reset", onClickHandler: e => this.resetFilters(e) }, locales.entries.Lcz_Reset), h("ir-custom-button", { key: '718a0b68f9edfb5401c29663aa1a7b729e97380d', variant: "brand", "data-testid": "apply", loading: this.isLoading, onClickHandler: e => this.applyFiltersEvt(e) }, locales.entries.Lcz_Apply))));
    }
};
